/** JavaScript code for Reference Tooltips. ****************
 * Based on the gadget by the same name on wikipedia (see [[mediawikiwiki:Reference Tooltips]])
 * Modified by Tjcool007 (Moved settings to a page, instead of a jQuery dialog)
 */
...
window.pg || $(document).ready( function($) {
    ...
    if( skin == 'oasis' ) {$('#WikiHeader nav ul li:first-child ul.subnav-2').prepend('<li style="float:right"><a href="/wiki/Professor_Layton_Wiki:Reference Tooltips">Ref Tooltip Options</a></li>');}
    ...
    if ( wgCanonicalNamespace === '' || wgCanonicalNamespace === 'Project' || wgCanonicalNamespace === 'Help' ) {
        ...
        // GET CONFIGURATION
        var rtmode = getCookie('rtmode');
        if( rtmode == 'hover' )
        {
            try {
                var rtdelay = parseInt(getCookie('rtdelay'));
                if( rtdelay == null || rtdelay == '' )
                {
                    rtdelay = 200;
                }
            } catch(e) {
        ...
        if( rtmode != 'disabled' && rtmode != 'click' && rtmode != 'hover' )
        {
            rtmode = 'click';
        }
        ...
        if( rtmode != 'disabled' )
        {
            ...
            var isTouchscreen = (rtmode == 'click' ? true : false),
                timerLength = isTouchscreen ? 0 : rtdelay,
                settingsMenu;
