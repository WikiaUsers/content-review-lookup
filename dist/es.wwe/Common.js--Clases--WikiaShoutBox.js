/** WikiaShoutBox v1.7: Conector para el Widget ShoutBox de Wikia
...
var WikiaShoutBox = {
	...
	start: function() {
		...
		var cfg = $.cookies.set(WikiaShoutBox.cookietag, '1');
	...
	stop: function() {
		...
		var cfg = $.cookies.del(WikiaShoutBox.cookietag);
	},
	expand: function() {
	...
	sendMsg: function(wId) {
		...
		var message = $.trim(messageBox.attr('value')).replace(new RegExp('\\{\\{', 'g'), '{&#123;');
		...
		if (message.charAt(0) == ':') {
			...
			if (message.indexOf('/me') == 0) {
				if ($.trim(message.substr(3)).length == 0) {
					WikiaShoutBox.reload();
					return;
				}
			} else {
		...
		WikiaShoutBox.action('configure', {message: encodeURIComponent( message ) });
	},
