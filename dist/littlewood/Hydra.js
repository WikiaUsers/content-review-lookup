...
var fp = fp || {
  ...
  adjustSectionBoxHeights : function ( heightToAdd, sections ) {
    ...
    var boxes, actualHeightToAdd;
    sections.each( function() {
      boxes = $( this ).find( '.fpbox' );

      boxes.each( function() {
        actualHeightToAdd = Math.round( decimalPortion + avgHeightToAdd ); /* should iron out rounding error */
        decimalPortion += avgHeightToAdd - actualHeightToAdd;
        $( this ).height( $( this ).height() + actualHeightToAdd );
      } );
    } );
  }
};
