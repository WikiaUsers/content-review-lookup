...
var descriptionString = new String("本列表含有%d个项目"); //%d is where the number of items is inserted
	...
	while(si) {
		...
		if (tn == "ul" || tn == "ol") {
			if (depth > 1) {
				...
				si.parentNode.insertBefore(button("显示全部", allonexec, "listexpand"), si);
				si.parentNode.insertBefore(document.createTextNode(' • '), si);
				si.parentNode.insertBefore(button("隐藏全部", alloffexec, "listexpand"), si);
				si.parentNode.insertBefore(document.createTextNode(')'), si);
			}
		}
		total += subtotal;
