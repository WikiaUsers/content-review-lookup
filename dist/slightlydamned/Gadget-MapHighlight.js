(function(root, factory) {
	...
	$.fn.maphilight = function(opts) {
		...
		return this.each(function() {
			var img, wrap, options, map, canvas, canvas_always, highlighted_shape, imagemap, imgSrc;
			img = $(this);
			...
			imagemap = img.get(0).getAttribute('imagemap');

			if (!imagemap) {
				return;
			}

			map = $('map[name="'+imagemap.substr(1)+'"]');

			if(!(img.is('img,input[type="image"]') && imagemap && map.length > 0)) {
				return;
			}
