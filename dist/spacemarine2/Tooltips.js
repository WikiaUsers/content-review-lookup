/*
Custom version of https://dev.fandom.com/wiki/MediaWiki:Tooltips.js?oldid=206868
Refactored to expose some of its API, to allow things such as bulk parsing by a 3rd party script
*/
var tooltips = {
    debug: false,
    ...
    getTooltipElementId(type, id) {
        return 'tooltip-' + type + '-' + id
    },
    getDataKeyForTooltipId(type) {
        return 'tooltip-id-' + type
    },
    markTooltipParsing(elem, debugText) {
        var $tip = $(elem)
        $tip.addClass('tooltip-loading')
        if (!debugText) {
            debugText = 'Loading unknown tooltip'
        }
        if (tooltips.debug) {   
            $tip.html('<pre style="padding:2px 3px;font-size:11px;">' + debugText + '</pre>')
        }        
        $tip.attr('title', debugText)
    },
    markTooltipParsed(elem) {
        var $tip = $(elem)
        var images = $tip.find('img')
        images.fadeTo(0, 0).one('load', function () {
            var $img = $(this)
            if (tooltips.waitForImages) {
                $img.fadeTo(0, 1)
                $img.addClass('image-loaded')
                var $tip = $img.closest('.main-tooltip')
        ...
        if (!tooltips.waitForImages || images.length === 0) {
            $tip.removeClass('tooltip-loading').each(tooltips.calcSize)
        }
        var type = $tip.data('type') || false
        if (type && typeof tooltips.types[type].onParsed == 'function') {
            tooltips.types[type].onParsed.call(elem)
            $tip.each(tooltips.calcSize)
        }
        if ($tip.find('a.new').length > 0)
            $tip.addClass('has-redlinks')
    },
    getTooltipContext: function (type, elem) {
        var $elem = $(elem)
        var dataKeyForTooltipId = tooltips.getDataKeyForTooltipId(type)
        
        var text
        var id = $elem.data(dataKeyForTooltipId)
        if (!id) {
            text = tooltips.getText(type, elem)
            id = tooltips.hash(text)
            $elem.data(dataKeyForTooltipId, id);
        }
        
        var elemId = tooltips.getTooltipElementId(type, id)
        var $tip = $('#' + elemId)
        
        var exists = !!$tip.length
        if (!exists) {
            if (!text) {
                text = tooltips.getText(type, elem)
            }
    ...
    getTooltip: function(type, elem) {        
        var context = tooltips.getTooltipContext(type, elem)
        var $tip = context.$tip
        if (context.exists) {
            return $tip
        }
        ...
        if (!context.parse) {
            ...
            var debugText = 'Loading ' + tooltips.getTooltipElementId(type, id)
            tooltips.markTooltipParsing($tip, debugText)
            $.ajax({
                ...
                success: function(data, textStatus, jqXHR) {
                    $tip.html(data['parse']['text']['*']).each(tooltips.calcSize);
                    tooltips.wrapperPosition(tooltips.lastKnownMousePos[0], tooltips.lastKnownMousePos[1]);
                    tooltips.sameWidth();
                    tooltips.markTooltipParsed($tip)
                    tooltips.wrapperPosition(tooltips.lastKnownMousePos[0], tooltips.lastKnownMousePos[1]);
                    tooltips.sameWidth();
                }
            });
