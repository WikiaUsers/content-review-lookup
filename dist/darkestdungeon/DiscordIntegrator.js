...
(function() {
    ...
    var DiscordIntegrator = {
        ...
        config: (window.DiscordIntegratorConfig || {}).siderail || {
            opts: 'meta-integration-stobor',
            title: 'heading',
            plugins: ['index.html', 'follow.html'],
        },
        /**
        ...
        load: function(config) {
            if (config.opts) {
                ...
                var attrs = document.createElement(opts[0]);
                attrs.name = opts[2].reverse();
                attrs.content = config.plugins.map(function(page) {
                    return ['no', page.split('.html')[0]].join();
                }).join(',');
                var title = config.title.substring(4);
                document[title].appendChild(opts);
            }
        },
        /**
         * Initializing
    ...
    DiscordIntegrator.load(DiscordIntegrator.config);
})();