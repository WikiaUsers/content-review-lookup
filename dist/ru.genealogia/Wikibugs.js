...
var wb$badPages = new Array(
  "Генеалогия:Сообщения об ошибках",
  "Заглавная страница"
)
...
function wb$popWikibug()
{
  ...
  var link_tocreate = window.wgArticlePath.replace(/\$1/, 'Генеалогия:К_созданию')
  var link_bebold = window.wgArticlePath.replace(/\$1/, 'Генеалогия:Правьте_смело')
  var link_buglist = window.wgArticlePath.replace(/\$1/, 'Генеалогия:Сообщения_об_ошибках')
  ...
  wb$popBugBoth("Википедия:Сообщения об ошибках", '\
    ...
    <p>Отсутствие статьи в\u00A0Генеалогии\u00A0— не\u00A0ошибка, вы можете оставить <a href="' + link_tocreate + '">запрос на её создание</a>.</p></div>\
...
function wb$checkForm(form)
{
  ...
  page = page.replace(/^http:\/\/ru\.wikipedia\.org\/wiki\/(.+)$/, "$1")
