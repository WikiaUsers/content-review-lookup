// Author(s) : Pecoes (plus help from Lunary)
// __NOWYSIWYG__ <source lang="javascript">
/*jshint browser:true jquery:true laxbreak:true curly:false */
/*global mediaWiki */
// Don't touch the page if it's an Edit page. This is critical as the Source Editor will die otherwise.
// NOTE: Message Walls and new Forums (based on Walls) are _always_ wgAction=edit so they need to be dealt
//   with specially.
...
(function (window, $) {
   ...
   var footerHr = '<hr style="clear: both; border-bottom:1px solid #C3B599; margin-top:20px">',
       footerSearch = '<table align="center"><tr><td><div style="width:330px"><form method="get" action="index.php?title=Special:Search" class="WikiaSearch" id="WikiaSearch2"><input name="search" autocomplete="off" placeholder="Rechercher sur le Wiki Harry Potter" type="text"><input type="hidden" name="fulltext" value="0"><input type="submit"><button class="secondary" style="height:300px"><img src="data:image/gif;base64,R0lGODlhAQABAIABAAAAAP///yH5BAEAAAEALAAAAAABAAEAQAICTAEAOw%3D%3D" class="sprite search" height="17px" width="21"></button></form></div></td></tr></table>';
    
    $.get('http://fr.harrypotter.wikia.com/wiki/MediaWiki:Bottom_nav?action=render')
    .done(function (data) {
        $(function ($) {
