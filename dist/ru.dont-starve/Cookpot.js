function timer() {
    for (var i = 4; i > 1; i = i - 1) {
        ...
        $('<span style="position: relative;"><img class="hiddeningredientcookpot cookpot-result-cell" id="result' + i + '"></span>').prependTo('#cookpotWorkSpace');
    }

    $('<span style="font-weight:bold;" id="chance1"></span>').prependTo('#cookpotWorkSpace');
    $('<span style="position: relative;"><img class="ingredientcookpot cookpot-result-cell" id="result1"></span>').prependTo('#cookpotWorkSpace');
    ...
    $(".cookpot .inglist a").each(function() {
        ...
        var name = $(this).children('img').attr('data-image-name').replace(".png", "");
        $(this)
        	.attr('title', name)
        	.attr('onclick', 'cookpotAdd("' + title + '","' + src + '")')
        	.removeAttr('href')
        	.removeClass('image-thumbnail image');
    });
    ...
    if ((cookpotResult !== undefined) && (cookpotResult[0] !== undefined)) {
        ...
        for (var i = 0; i <= 15; i = i + 5) {
            if (cookpotResult[i] !== '0') {
                ...
                $('#result' + id).attr('href', cookpotResult[i + 1]);
                $("#name" + id).attr('href', cookpotResult[i + 1]);
                if (i !== 0) {
...
(function addClickEventsOnResult() {
    $(document).on('click', ".cookpot-result-cell", function() {
    ...
    $(document).on('click', ".cookpot-name-label", function() {
                var href = $(this).attr('href');
                window.open(href, '_blank');
    });
})();