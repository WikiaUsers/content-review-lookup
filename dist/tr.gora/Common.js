...
function substUsernameTOC() {
	...
	{
		$(this).text($(this).text().replace('<isim>', username));
	});
}
...
function verifySummary(){
	...
	// Check for licensing
	if ( wpLicense.value == "" ){
		alert('Lisanslama tamamlanmış olmalı.');
		return false;
	}

	// Check for source
	if ( document.getElementById('sourceBox').value == "" ){
		alert('Kaynak tamamlanmış olmalı.');
		return false;
	}
	...
	if ( wpDestFile.value.match(/(JPG|PNG|GIF|SVG|jpg\.jpg|png\.png|gif\.gif|svg\.svg)$/)) {
		alert('Lütfen dosya adında büyük harf ya da kopya uzantı kullanmayın.');
		return false;
	}
	...
	strBuilder += '|eser sahibi=' + document.getElementById('artistBox').value + '\r\n';
...
function verifyName(){
	...
	if ( wpDestFile.value.match(/(\(|\)|!|\?|,|\+|\'|\’)/)) {
		alert('Lütfen dosya adında parantez, eğik çizgi, noktalama işareti ya da alfanumerik olmayan karakterler kullanmayın.');
		return false;
	}
	return true;
}
