...
function loadGSList(){
	if ($("#gslist").length>0) {
		...
		var pArr = new Array();
		$.getJSON("http://vanilla-wow.wikia.com/api.php?action=query&generator=categorymembers&gcmlimit=500&gcmsort=timestamp&gcmdir=desc&gcmtitle=Category:Guild_stubs&prop=revisions&rvprop=timestamp&format=json&callback=?", function(data) {
			if (data.query) {
...
$(".firstHeading").append('&nbsp;<span style="font-size: xx-small; border-bottom: 1px dotted; cursor:help;" title="Enable auto-refreshing page loads">AJAX:</span><input type="checkbox" id="ajaxToggle"><span style="display:none;" id="ajaxLoadProgress"><img src="http://www.wikia.com/skins/common/progress-wheel.gif" border="0" alt="AJAX operation in progress" /></span>');
...
$(function () { 
if (ajaxPages.indexOf(wgPageName)!=-1) preloadAJAXRL();
});