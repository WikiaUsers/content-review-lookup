//********************************************//
// A compilation of scripts for local AV use  //
//********************************************//
...
/* a bunch of stuff... (WHAM!)
 / written by Joey Ahmadi (except for the jQueryUI obviously)
	...
	if(ug.indexOf('staff') + ug.indexOf('helper') + ug.indexOf('vstf') + ug.indexOf('sysop') > -4) {
		if(wgCanonicalSpecialPageName == "Contributions") {
			...
			* Copyright (c) 2012 AUTHORS.txt; Licensed MIT, GPL */
			...
			$('head').append("<link href='http://vstf.wikia.com/index.php?title=MediaWiki:Uijquery.css&action=raw&ctype=text/css' rel='stylesheet' />");	
			$('#content,.WikiaArticle').prepend('<div class="dialog-full" title="WHAM!"></div>');
 
 
			$('#contentSub').append('&nbsp;<a id="quicktools" href="#" title="Super Tool" style="color:red;font-weight:120%">Quick Tools</a>&nbsp;');
			$('#quicktools').click(function(event) {
				event.preventDefault();
				$('.dialog-full').dialog('open');
			});
			...
			$(".dialog-full").dialog({
				...
				buttons: {
					"Delete all pages": function() {
						$( this ).dialog( "close" ).click(delAll());
 
					},
					"Rollback all edits": function() {
						$( this ).dialog( "close" ).click(rollbackAll());
					},
					"Quick block": function() {
						$( this ).dialog( "close" ).click(blockFast());
					},
					"All of the above": function() {
						$( this ).dialog( "close" ).click(wham());;
					},
					"UnBot me": function() {
						$( this ).dialog( "close" ).click(quickBotFlag('remove'));
					},
					Cancel: function() {
						$( this ).dialog( "close" );
					}	
			...
			/* DEL ALL */
			function delAll() {
				var title = encodeURIComponent($('li .newpage ~ a').html());
				var url =  wgServer+'/api.php?action=query&prop=info&intoken=delete&titles='+title+'&format=json';
				...
				$.getJSON(url, function(data) {
					...
					$('li .newpage ~ a').each(function() {
						...
						var url = wgServer+'/api.php?action=delete&title='+title+'&reason='+deleteReason+'&format=json&token='+ET;
						$.post(url,function() {
							$('li .newpage ~ a').css({'color':'grey','text-decoration':'line-through'}).removeAttr('href');
						});
					});
				});
			}
			...
			function blockFast() {
				if( skin == 'monobook' || $('input[name="target"]').val() != '' ) {
					...
					if($('#WikiaUserPagesHeader h1 small').text() != '') {
						var delAllUser = $('#WikiaUserPagesHeader h1 small').text();
					} else {
						var delAllUser = $('#WikiaUserPagesHeader h1').text();
					}
				}
				var blockExpiry = prompt('Please state the block duration','2 weeks');
				var blockReason = prompt('Please state the block reason','vandalism');
				...
				$.getJSON(url, function(data) {
					...
					var ET = ET.slice(0, -2);
					var ET = ET+'%2B\\';
					var url = wgServer+'/api.php?action=block&user='+delAllUser+'&expiry='+blockExpiry+'&reason='+blockReason+'&nocreate&autoblock&noemail&format=json&token='+ET;
					$.post(url, function() {
			...
			function quickBotFlag(aorr) {
				var qUserName = encodeURIComponent(wgUserName);
				if( aorr == 'add' ) {
					var reason = prompt('Please state the reason','cleanup');
				}
				var url = wgServer + wgScriptPath + '/api.php?action=query&list=users&ususers=' + qUserName + '&ustoken=userrights&format=json';
				$.getJSON( url, function (data) {
					if ( !reason ) {
						reason = '';
					}
					...
					if( aorr == 'add' ) {
						var url = wgServer + wgScriptPath + '/api.php?action=userrights&user=' + qUserName + '&add=bot&reason=' + encodeURIComponent(reason) + '&format=json&token=' + encodeURIComponent(token);
						$.post( url, function () {
							//Makes rollback links +bot when adding bot flag
							$('.mw-rollback-link a').each(function() {
								$(this).attr('href', $(this).attr('href') + '&bot=1') ;
							} );
							alert('User has been ninja\'d!');
						});
					} else if( aorr == 'remove' ) {
						var url = wgServer + wgScriptPath + '/api.php?action=userrights&user=' + qUserName + '&remove=bot&format=json&token=' + encodeURIComponent(token);
						$.post( url, function () {
							alert('User has been de-ninja\'d!');
						});
					}
				});
			}
...
( function ( $, mw, window ) {
	...
	var	ajaxIndicator = window.ajaxIndicator || 'https://images.wikia.nocookie.net/dev/images/8/82/Facebook_throbber.gif',
		...
		ajPages = window.ajaxPages || [ 'Special:RecentChanges','Special:WikiActivity' ];
...
function ajaxPatrolLinks() {
	...
	$( '.patrollink' ).click( function ( e ) {
		...
		$patrolLink.html('<img src="https://images.wikia.nocookie.net/dev/images/8/82/Facebook_throbber.gif" style="vertical-align: baseline;" border="0" alt="Marking as patrolled..." />');
