...
var chatOptions = {
	...
	modules: {
		chatHacks: {
			...
			load: function () {
				if ($("#pingspan").length > 0 || this.loaded)
					return;
				importScriptPage('MediaWiki:Chat.js/options.js/options.js', 'creepypasta');
				this.loaded = true;
			}
		},
		tabComplete: {
			...
			loaded: false,
			load: function () {
				importScriptPage("MediaWiki:Benjamin/1.js");
				this.loaded = true;
			}
		},
		multiKick: {
		...
		ignoreURL: {
			...
			loaded: false,
			load: function () {
				importScriptPage('MediaWiki:Benjamin/secret.js');
				this.loaded = true;
			}
		},
		stopSideScroll: {
