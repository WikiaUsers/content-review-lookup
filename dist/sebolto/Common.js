...
;(function (window, mw, $) {
  ...
  /**
   * @description This function is used to hide the backlink to the parent page
   * when viewing a subpage. The function should only be called on the mainspace
   * when viewing a page existing at either "/Legends" or "/Canon". It works in
   * tandem with Module:Top. This function was adapted from code found in
   * Wookieepedia's MediaWiki:Common.js file & previously written by
   * User:Grunny.
   *
   * @author Grunny <sw.wikia.com/w/User talk:Grunny>
   * @author Sebolto <swf.wikia.com/w/User talk:Sebolto>
   ...
   * @returns {undefined}
   */
  this.hideContentSub = function () {
    var $subtitle, $subpage;

    $subpage = $(".page-header__page-subtitle .subpages");
    $subtitle = $(".page-header__page-subtitle");

    $subpage.remove();
    $subtitle .text(function (a, b) {
      return b.replace(" |", "");
    });
  };
  ...
  this.init = function () {
    ...
    // Hide backlink when viewing mainspace pages that are also subpages
    if (config.wgNamespaceNumber === 0 || $("#hideContentSub").length) {
      this.hideContentSub();
    }

    // Load tools restricted to Administrators
    if (new RegExp("sysop").test(config.wgUserGroups.join(" "))) {
      this.loadAdminTools();
    }
  };
