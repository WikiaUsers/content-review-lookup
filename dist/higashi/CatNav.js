...
$(function() {
    ...
    catnav.data = {
        ...
        selectedPage: null,
        storage: {
            url: "http://sonako.wikia.com", // url to the wiki that hosts your imports
    ...
    catnav.data.markup.html = '<nav id="catnav">\n' +
        ...
        '\t\t\t</h3>\n' +
        '\t\t\t<h4>\n' +
        '\t\t\t\tĐể không mất công tạo lại danh sách yêu thích trên nhiều thiết bị, bạn hãy tạo trang Lưu thể loại yêu thích <b>(yêu cầu phải đăng nhập trên Higashi)</b>: <a target="_blank" title="Với những bạn chưa tạo trang lưu (kèm hướng dẫn)." href="http://higashi.wikia.com/wiki/Special:MyPage/catnav.css?action=edit&preload=Template:CatNav">Click tại đây (dành cho tạo mới)</a> hoặc <a target="_blank" title="Với những ai đã tạo trang lưu trước đó." href="http://higashi.wikia.com/wiki/Special:MyPage/catnav.css?action=edit">Click tại đây để chinh sửa</a>' +
        '\t\t\t</h4>\n' +
        '\t\t\t<h4>\n' +
        '\t\t\t\tNhập/Xuất dữ liệu thể loại yêu thích từ trang <a target="_blank" title="Trang lưu dữ liệu thể loại ưu thích." href="http://higashi.wikia.com/wiki/Special:MyPage/catnav.css">Lưu cài đặt: </a>' +
        ...
        '\t\t\t\t<option value="Idle Projects" title="Tập hợp các Project trên Higashi đã không được cập nhật trong 3 tháng qua.">Idle Projects</option>\n' +
        '\t\t\t\t<option value="Stalled Projects" title="Tập hợp các Project trên Higashi đã không được cập nhật trong 6 tháng qua.">Stalled Projects</option>\n' +
        '\t\t\t\t<option value="Inactive Projects" title="Tập hợp các Project trên Higashi đã không được cập nhật trong hơn 12 tháng qua.">Inactive Projects</option>\n' +
        ...
        '\t\t\t\t<option value="Full Text" title="Tập hợp các trang Toàn tập (dịch/viết trọn vẹn 1 tập) của các bộ Light Novel trên Higashi.">Full Text</option>\n' +
        '\t\t\t\t<option value="Minh họa" title="Tập hợp các trang minh họa trên Higashi Light Novel Wiki.">Minh họa</option>\n' +
        '\t\t</select>\n' +
    ...
    catnav.fn.catMembers = function(cat, ns, arr, cont, cb) {
        ...
        */
        ...
        req.open("get", mw.config.get("wgScriptPath") + "/api.php?action=query&format=json&list=categorymembers&cmtitle=Category:" + encodeURIComponent(cat) + "&cmnamespace=" + ns + "&cmcontinue=" + encodeURIComponent(cont) + "&cmlimit=max&cb=" + new Date().getTime(), true);
        req.onload = function() {
    ...
    catnav.fn.queryPages = function(titles, cb) {
        ...
        req.open("get", "/api/v1/Articles/Details?&abstract=0&width=140&height=140&titles=" + encodeURIComponent(missingTitles.join(",")), true);
        req.onload = function() {
    ...
    catnav.fn.updateMarkup = function(titles) {
        ...
        for (var i in titles) {
            var a = catnav.data.details[titles[i]],
                item = $('<div class="catnav-item' + (a.img ? "" : " catnav-item-noimage") + '"><a href="' + a.url + '" title="' + a.title.replace(/["'&<>]/g, function(m) {
    ...
    catnav.fn.insertFavorite = function(category) {
        var item = $('<div class="catnav-commoncats-item"></div>')
            ...
            .text(category)
            .prepend('<img src="http://i.imgur.com/toc40DW.png" class="catnav-commoncats-item-delete" />', '&nbsp;&nbsp;');
        ...
        $.ajax({
            ...
            success: function(data) {
                var pages = data.query.pages,
                    ...
                    cities = {},
                    currCity,
                    wiki = location.hostname.replace(/\.wikia\.com$/i, ""),
                    storage;
                for (pageid in pages) {
                    if (pageid == -1) {
                        alert("Không tìm thấy dữ liệu. Hãy đảm bảo là trang 'http://higashi/wiki/User:" + user + "/catnav.css' có tồn tại");
                    } else {
    ...
    catnav.fn.getExhibitionMembers = function(category, arr, page, cb) {
        ...
        var req = new XMLHttpRequest();
        req.open("GET", "/index.php?action=ajax&articleId=" + encodeURIComponent(category) + "&method=axGetArticlesPage&rs=CategoryExhibitionAjax&page=" + page + "&cb=" + new Date().getTime(), true);
        req.onload = function() {
            if (req.status == 200) {
                var data = JSON.parse(req.responseText),
    ...
    catnav.fn.exportGlobalFavorites = function() {
        var wiki = location.hostname.replace(/\.wikia\.com$/i, ""),
    ...
    // set contrast color     higashi
    catnav.fn.contrastfy = function(hex) {
        var color = hex.replace(/[^a-f0-9]/gi, "").match(/[a-f0-9]{2}/gi),
            rgb = color.map(function(val) {
                return parseInt(val, 16);
            }),
            avg = (rgb[0] + rgb[1] + rgb[2]) / 3,
            contrast = 128 + avg - (avg < 128 ? 0 : 255);
    ...
    if (mw.config.get("wgNamespaceNumber") === -1 && mw.config.get("wgTitle") == "CatNav") {
        ...
        $("body").append(
            ...
            '\t\t</h3>\n' +
            '\t\t<p>\n' +
            '\t\t\tSao chép lại thông tin phía dưới và lưu vào cuối trang <a target="_blank" href="http://higashi.wikia.com/wiki/Special:MyPage/catnav.css?action=edit">Dữ liệu cài đặt</a> của bạn.<br />\n' +
            '\t\t\tTrong trường hợp bạn đã tạo trang Lưu thể loại yêu thích của mình trước đó, bạn có thể bỏ qua dòng chữ @higashi phía dưới, chỉ cần sao chép lại những dòng thể loại muốn lưu ở sau.\n' +
            '\t\t</p>\n' +
        ...
        // update titles
        $("#WikiaPageHeader h1, h1#firstHeading").html("Tìm Kiếm Nâng Cao");
        $("head title").html("Tìm Kiếm Nâng Cao | Higashi Light Novel Wiki");
