...
dev.StarRatings.ui.simple = function($this, callbacks/*, conf*/) {
	...
	return {
		set: function(data) {
			...
			$status.text('(' + data.total + ' vote(s), ' + data.mean.toFixed(2) + ' average, the most popular vote(s) are ' + (data.mode + '' || '?') + ')');
			return $this;
		},
		onAjaxFail: function() {
...
dev.StarRatings.ui.pecoesSvg = function($this, callbacks) {
	...
	if (!$.fn.rateable) {
		ready = dev.StarRatings.load('u:higashi:Mediawiki:StarRatings/ui.js');
	}
...
(function(module, window, $, mw) {
	...
	if (mw.config.get('wgNamespaceNumber') === -1 && mw.config.get('wgTitle') === 'StarRatings') {
		//return module.load('u:higashi:Mediawiki:StarRatings/special.js');
	}

	// Shows the in-page UI for viewing the stats for the clicked poll
	function showStats(poll) {
		if (!module.showStats) {
			module.load('u:higashi:Mediawiki:StarRatings/stats.js').done(function() {
				showStats(poll);
			});
			return;
		}
