...
$( function() {
	$( '.mw-rollback-link > a' ).after( $( '<span>' ).addClass( 'edit-rollback' ).prop( 'title', '巻き戻しの要約' ) );
	$( '#mw-content-text' ).on( 'click', '.edit-rollback', function() {
		...
		if ( $( this ).parent().is( $rollback.parent() ) ) {
			...
			$rollback = $( '<div id="rollback-summary">' ).append(
				...
				'[[Special:Contribs/' + name + '|' + name + ']] ([[User talk:' + name + '|トーク]]) による編集を差し戻しました: '
				),
				$( '<input type="button">' ).addClass( 'mw-ui-button mw-ui-constructive rollback-submit-button' ).val( '巻き戻す' )
			).insertAfter( this );
		}
