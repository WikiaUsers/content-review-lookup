mw.loader.using(['site']).then(function() {
	i18n = {
		multiupload: "複数ファイルのアップロード:",
		yes: "する",
		no: "しない",
		sourcefiles: "アップロード元のファイル名:",
		uploadfiles: "ファイルをアップロード",
		nofiles: "アップロードするファイルを1つ以上選択してください。",
		nolicense: "適切なライセンスを選択してください。",
		summary: "概要",
		license: "ライセンス",
	...
	$("#multiFileSubmit").click(function() {
		...
		function gNF() {
			...
			$.ajax({url:'/ja/api.php',data:{action:'query',meta:'tokens',format:'json'},dataType:'json'}).done(function(data) {
				fd = new FormData();
