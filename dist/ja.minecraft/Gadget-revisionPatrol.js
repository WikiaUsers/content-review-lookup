...
$( function() {
	var i18n = {
		autopatrolled: '自動巡回済み',
		// $1 is the user, $2 is the timestamp
		patrolled: '$1 が巡回済み ($2)',
		unpatrollable: '巡回不可',
		unpatrolled: '未巡回',
		autopatrolledLegend: '自動巡回済みの編集',
		patrolledLegend: '巡回済みの編集',
		unpatrollableLegend: '巡回不可の編集',
		unpatrolledLegend: '未巡回の編集',
		error: '巡回情報を取得できませんでした',
		rightsError: '自動巡回の権限情報が取得できませんでした',
	...
	if ( mode === 'history' ) {
		...
		Array.prototype.forEach.call( revElems, function( li, i ) {
			...
			var user = li.querySelector( '.mw-userlink' ).textContent;
			users[user] = true;
		...
		if ( newestPage === false ) {
			...
			var user = newestRev.querySelector( '.mw-userlink' ).textContent;
			users[user] = true;
		...
		if ( findMovedRev( oldestRev ) === false ) {
			...
			var user = oldestRev.querySelector( '.mw-userlink' ).textContent;
			users[user] = true;
				...
				rightsReq.then( function( apUsers ) {
					...
					Object.keys( ids ).forEach( function( revId ) {
						...
						if (
							...
							apUsers[revElem.querySelector( '.mw-userlink' ).textContent]
						) {
