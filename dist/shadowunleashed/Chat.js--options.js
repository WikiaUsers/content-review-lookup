...
var chatOptions = {
	...
	modules: {
		...
		multiKick: {
			...
			loaded: false,
			load: function () {
				if ((!wgUserGroups.indexOf("chatmoderator") && !wgUserGroups.indexOf("sysop") && !wgUserGroups.indexOf("staff") && !wgUserGroups.indexOf("helper") && !wgUserGroups.indexOf("user")) || $("multiKickerButton").length) {
					return; // Do not load
				}
		...
		ignoreURL: {
			...
			loaded: false,
			load: function () {
				$('head').append('<style type="text/css">li[data-user="CPwikiCHATlogger"] {display:none;}</style>');
				this.loaded = true;
			}
		},
		stopSideScroll: {
...
function openOptions() {
	...
	if (wgServer !== "http://creepypasta.wikia.com")
...
if (!$("#chatOptionsButton").length) {
	$('.Rail').prepend('<div id="chatOptionsButton" onclick="openOptions();" style="margin:auto; cursor: pointer; font-size:150%; bottom:4px; padding-bottom:2px; border-bottom: 1px solid #CCCCCC; width:90%;" align="center"><img src="' + chatOptions.look.modalIcon + '" width="18px"/>&nbsp;HAXORZ</div>'); // Prevent multiple buttons from being appended
}
