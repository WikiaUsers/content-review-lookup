...
'<form action="javascript:addItem(\'popup\')" id="chartPropertiespopup"><label>Promedio: <input type="number" min="1" id="averagepopup"> días</label><div class="moreitems">Agregar más objeto(s): <input type="text" id="extraItempopup">&nbsp;<b>Nota:</b> Si agregas más objetos, los promedios no se mostrarán.</div><button>Cargar</button><button type="reset">Reiniciar<span class="resetallfields"> todos los campos</span></button></form><a class="GEPermLink" id="GEPermLinkpopup" title="Enlace permanente a las configuraciones y objetos del gráfico actual. Haz click derecho para copiar la URL." href="javascript:void(0)">Enlace permanente</a><div id="addedItemspopup"></div>'+
...
function chartPermLinkUrl() {
	...
	urlHash += '#i='+items;
	var url = 'http://codesah.wikia.com/wiki/CDSH:GEMW/G'+urlHash;
	return url;
}
...
function addItem(i,it) {
	...
	if (item&&item.length) {
		if ($('#addedItems'+i+' [data-item], #GEdataprices'+i+'[data-item]').map(function() { return $(this).attr('data-item').toLowerCase(); }).get().indexOf(item.toLowerCase())!=-1 || _GEC.AIQueue.indexOf(item.toLowerCase())!=-1) {
			if (!it) alert(item+' ya está en el gráfico.')
		...
		$.get('/api.php?action=parse&format=json&page=Exchange:'+encodeURIComponent(item)+'/Data', function(response) {
			if (response.error) {
				var item = decodeURIComponent((this.url.match(/Exchange:([^\/]*)\/Data/)||[])[1]);
				if ($('#extraItem'+_GEC.chartid).val().length) {
					if (response.error.code == 'missingtitle') {
						alert("El objeto "+item+" no existe en nuestra base de datos del Gran Mercado.");
					} else {
						alert("Un error ha ocurrido al cargar "+item+".");
					}
				}
			...
			var item = response.parse.title.match(/Exchange:([^\/]*)\/Data/)[1]
...
function loadChartsQueueComplete(c) {
	if (getSeriesIndex(_GEC.chartid,'Volume')!=-1) {
		...
		chartdata[1].title.text = 'Historial de precios';
		reloadChart(c,{
			series: chartdata[0],
			yAxis: chartdata[1]
		});
	...
	for (var i=0;i<data.length;i++) {
		...
		if (!($('#addedItems'+c).html()||'').replace(/\s+/,'').length) {
			$('#addedItems'+c).html('Remover objetos de la gráfica: <a href="javascript:removeGraphItem(\''+iname+'\',\''+c+'\')" data-item="'+iname+'">'+iname+'</a>');
			itemhash = '#i='+iname;
		} else {
	...
	if ((wgNamespaceNumber == 116 && wgTitle.split('/')[1] == 'Data'||wgPageName == 'CoDeSaH Wiki:Grand_Exchange_Market_Watch/Gráfico') && itemhash.replace('#i=','').length) location.hash = itemhash
	_GEC.AIQueue = [];
	_GEC.AILoaded = [];
	_GEC.AIData = [];
...
function setChartRange(c) {
	...
	var hasT = wgNamespaceNumber == 116 && wgTitle.split('/')[1] == 'Data'||wgPageName == 'CoDeSaH Wiki:Grand_Exchange_Market_Watch/Gráfico';
	...
	if (dates[0] != 'Fecha inválida' && dates[1] == 'Fecha inválida' && typeof(zoom) == 'number') {
	...
	if (dates[0] != 'Fecha inválida' && dates[1] != 'Fecha inválida') {
...
function setChartExtremes(i) {
	...
	if (extremes.dataMin * 0.95 != extremes.userMin || extremes.dataMax * 1.05 != extremes.userMax) {
		...
		if (_GEC['chart'+i].yAxis[2]) { 
			...
			_GEC['chart'+i].yAxis[1].setExtremes(0, extremes.dataMax * 1.05);
		}
	}
...
function addPopupPriceInfo(c) {
	...
	'<caption onclick="$(this).next(\'tbody\').toggle();$(this).children().toggle()"><span style="display:none;white-space:nowrap">Información del precio</span><span>Cerrar</span></caption><tbody>'+
	'<tr><th>Precio</th><td>'+toKMB(curprice)+'</td></tr>';
	for (var i=0;i<priceDiffs.length;i++) {
		itemStats += '<tr><th>'+dayDiffs[i]+' días</th><td style="color:'+rg(priceDiffs[i])+'">'+(priceDiffs[i]>0?'+':'')+toKMB(priceDiffs[i])+' ('+percentDiffs[i]+'%)</td></tr>';
	}
	...
	itemStats += $('#GEdataprices'+c).attr('data-limit').length?'<tr><th><a href="/wiki/Gran_Mercado" target="_blank" title="Límite de compra en el Gran Mercado">Límite de compra</a></th><td>'+toKMB($('#GEdataprices'+c).attr('data-limit'))+'</td></tr>':'';
	itemStats += '<tr><th colspan="2"><a href="/wiki/Exchange:'+encodeURI($('#GEdataprices'+c).attr('data-item'))+'" target="_blank" title="'+$('#GEdataprices'+c).attr('data-item')+'">Más información del precio</a></th></tr>';
...
function getData(c,isSmall,avginput) {
	...
	itemName = itemName || wgPageName.replace(/_/g,' ').split('/')[0].replace('Exchange:','');
	var prices = $('#GEdataprices'+c).html().split(/,\s/);
	var data = [];
	var thisprice;
	var volumes = [];
	...
	if (volumes.length >= 10&&!isSmall) {
		yAxis = [{
			title: {
				text: isIndexChart?'Historial del índice':'Historial de precios',
				offset: 60,
				style: {
					color: 'black',
					fontSize: '12px',
		...
		yAxis = {
			title: {
				text: isSmall?null:(isIndexChart?'Historial del índice':'Historial de precios'),
				offset: 60,
				style: {
					color: 'black',
					fontSize: '12px',
...
Highcharts.setOptions({
	lang: {
		resetZoom: '<span style="font-size:11px;">reiniciar tamaño</span>',
...
if (!isSmall) {
	$('#GEdatachart'+c).before('<form action="javascript:addItem('+c+')" id="chartProperties'+c+'"><label>Promedio: <input type="number" min="1" id="average'+c+'" value="'+_GEC.average+'"> días</label>'+(isIndexChart?'':'<br/>Agregar más objeto(s): <input type="text" id="extraItem'+c+'">&nbsp;<b>Nota:</b> Si agregas más objetos, los promedios no se mostrarán.')+'<br/><button>Cargar</button><input type="reset" value="Reiniciar'+(isIndexChart?'':' todos los campos')+'"/></form>'+(wgNamespaceNumber == 116 && wgTitle.split('/')[1] == 'Data'||wgPageName == 'CoDeSaH Wiki:Grand_Exchange_Market_Watch/Gráfico' ? '<a class="GEPermLink" id="GEPermLink'+c+'" title="Enlace permanente a las configuraciones y objetos del gráfico actual. Haz click derecho para copiar la URL." href="javascript:void(0)">Enlace permanente</a>':'')+'<div id="addedItems'+c+'"></div>');
}
...
var enlarge = '<a href="javascript:popupChart('+c+');" style="text-decoration:underline;">Aumentar gráfico</a>';
...
_GEC['chart'+c] = new Highcharts.StockChart({
	...
	title: {
		text: isSmall?(isInfobox?enlarge:itemName):'CoDeSaH Wiki:Grand Exchange Market Watch',
		style: {
			color: 'black',
		...
		series: {
			...
			dataGrouping: {
				dateTimeLabelFormats: {
					...
					week: ['Semana desde %A, %e %B %Y', '%A, %e %B', '-%A, %e %B %Y'],
					month: ['%B %Y', '%B', '-%B %Y'],
					year: ['%Y', '%Y', '-%Y']
				}
			}
...
if ((wgNamespaceNumber == 116 && wgTitle.split('/')[1] == 'Data'||wgPageName == 'CoDeSaH Wiki:Grand_Exchange_Market_Watch/Gráfico') && location.hash.match('#i=')) {
	var hash = location.hash;
