...
if (navigator.info[0] == "Chrome") {
	...
	$('#ChatHeader .public').append('<a href="/wiki/Lego_Message_Boards_Wiki:Chat/Party_Mode" target="_blank" id="partyHelp">Help</a><a id="partyInfo">Tech</a><div id="partyTech" style="display: none;"><ul><li>Browser: ' + navigator.info[0] + '</li><li>Version: ' + navigator.info[1] + '</li><li>OS: ' + navigator.platform + '</li></ul></div>');
...
$('#partyTrigger').toggle(function() {
	...
	$('#option-1:not(:checked)').on('click',function() {
	...
	$('#option-2:not(:checked)').on('click',function() {
		$('#partyMode source').remove();
		if (navigator.info[0] == "MSIE") {
			$('#partyMode audio').append('<source src="' + partyLinkIE2 + '" type="audio/mpeg">');
		}
		else {
			$('#partyMode audio').append('<source src="' + partyLink2 + '" type="audio/ogg">');
		} }
	);
	$('#option-3:not(:checked)').on('click',function() {
		$('#partyMode source').remove();
		if (navigator.info[0] == "MSIE") {
			$('#partyMode audio').append('<source src="' + partyLinkIE3 + '" type="audio/mpeg">');
		}
		else {
	...
	if (navigator.info[0] == "MSIE") {
		$.get('/index.php', {action: 'raw', title: 'User:' + wgUserName + '/Party-link-ie-1'}, function success(data) { window.customLink1 = data; $('#option-1-group').show(); });
		$.get('/index.php', {action: 'raw', title: 'User:' + wgUserName + '/Party-link-ie-2'}, function success(data) { window.customLink2 = data; $('#option-2-group').show(); });
		$.get('/index.php', {action: 'raw', title: 'User:' + wgUserName + '/Party-link-ie-3'}, function success(data) { window.customLink3 = data; $('#option-3-group').show(); });
	}
	else {
		$.get('/index.php', {action: 'raw', title: 'User:' + wgUserName + '/Party-link-1'}, function success(data) { window.customLink1 = data; $('#option-1-group').show(); });
		$.get('/index.php', {action: 'raw', title: 'User:' + wgUserName + '/Party-link-2'}, function success(data) { window.customLink2 = data; $('#option-2-group').show(); });
		$.get('/index.php', {action: 'raw', title: 'User:' + wgUserName + '/Party-link-3'}, function success(data) { window.customLink3 = data; $('#option-3-group').show(); });
	}
	...
	$('#option-4:not(:checked)').on('click',function() {
		$('#partyMode source').remove();
		if (navigator.info[0] == "MSIE") {
			$('#partyMode audio').append('<source src="' + customLinkIE1 + '" type="audio/mpeg">');
		}
		else {
			$('#partyMode audio').append('<source src="' + customLink1 + '" type="audio/ogg">');
		} }
	);
	$('#option-5:not(:checked)').on('click',function() {
		$('#partyMode source').remove();
		if (navigator.info[0] == "MSIE") {
			$('#partyMode audio').append('<source src="' + customLinkIE2 + '" type="audio/mpeg">');
		}
		else {
			$('#partyMode audio').append('<source src="' + customLink2 + '" type="audio/ogg">');
		} }
	);
	$('#option-6:not(:checked)').on('click',function() {
		$('#partyMode source').remove();
		if (navigator.info[0] == "MSIE") {
			$('#partyMode audio').append('<source src="' + customLinkIE3 + '" type="audio/mpeg">');
		}
		else {
			$('#partyMode audio').append('<source src="' + customLink3 + '" type="audio/ogg">');
		} }
	);
	$('#option-7:not(:checked)').on('click',function() {
		$('#partyMode source').remove();
		$('#partyMode audio').off().remove();
		$('#partyMode').append('<audio loop autoplay></audio>'); }
	);
