 ...
 * 
 * Thanks to Speedit for a reliable method of adding content
 * to the Wikia rail, with callbacks and double run protection.
 * 
...
require(["wikia.window", "jquery", "mw"], function (window, $, mw) {
    ...
    var $rail = $('#WikiaRail'); // Get wikia rail.
    var json;

    if ((window.dev || {}).discordintegrator) {
        return;
    }

    function DiscordIntegrator() {
        // GET widget JSON for the server.
        $.getJSON("https://discordapp.com/api/guilds/290889126771818496/widget.json", $.proxy(this.call, this));
    }

    DiscordIntegrator.prototype.call = function (d) {
        json = d;
        this.execute();
    };

    DiscordIntegrator.prototype.execute = function () {
        // Check if the widget has been created.
        // If yes, then add to rail on load.
        // Else, create widget.
        if (!$rail.hasClass('loaded')) {
            $rail.on('afterLoad.rail', $.proxy(this.createWidget, this));
        } else {
            this.createWidget();
        };
    }

    DiscordIntegrator.prototype.createWidget = function () {
        ...
        $(widgetFooterText)
            ...
            .text("Free text and voice chat from Discord")
        ...
        (window.dev = window.dev || {}).discordintegrator = new DiscordIntegrator();
