...
if(wgCanonicalSpecialPageName == 'Chat') {
			...
			for(var i = 0; i < pings.length; i++) {
				if(text.toLowerCase().indexOf(pings[i].toLowerCase()) != -1 || this != mainRoom.viewDiscussion) {
					if(!window.hasFocus) { //Only annoy people if the window isn't focused
						document.getElementById('sound').innerHTML = '<audio src="https://images.wikia.nocookie.net/pvzcc/images/e/ed/Coin.ogg" autoplay=""></audio>';
	...
	window.commands = {
		...
		'numuser': function(com, text) { //Numberofusers
			$('#Write [name="message"]').val('The number of users, including me is ' + wgWikiaChatUsers.length + '.');
			return true;
		},
		'kick': 'kickban',
		'ban': 'kickban',
