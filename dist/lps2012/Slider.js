importArticle({
    type: "style",
    article: "MediaWiki:Style-minify.css"
});
...
function ws_photo(b, e, g) {
    ...
    var o = Math.max(t, e.length);
    for (var p = 0, m = 0; p < o; p++) {
        if (m >= e.length) {
            m -= e.length;
        }
        c(e[m]).clone().addClass("ws_photoItem").appendTo(a);
        if (b.images && p < e.length) {
            n.push(!b.images[m].noimage);
        }
        m++;
    }
    var q = a.children("img");
    c.support.transition = (function() {
        var i = document.body || document.documentElement,
            k = i.style;
        return k.transition !== undefined || k.WebkitTransition !== undefined || k.MozTransition !== undefined || k.MsTransition !== undefined || k.OTransition !== undefined;
    })();
    ...
    function s(C, w, i, v, k) {
        ...
        if (c.support.transition) {
            C.css({
                ...
                transition: "all " + k + "ms ease-in-out"
            });
        } else {
            C.animate({
                top: A + "%",
    ...
    q.each(function(i) {
        s(c(this), b.startSlide == i, false, true, 0);
    });
    this.go = function(i, v) {
        if (b.images && !n[i]) {
            n[i] = true;
            var k = i;
            while (true) {
                c(q[k]).attr("src", b.images[k % e.length].src);
                if (k > o) {
                    break;
                }
                k += e.length;
        ...
        if (window.XMLHttpRequest) {
            var u = b.duration * 0.5;
            q.each(function(w) {
                s(c(this), i == w, v == w, false, u);
            });
            if (c.support.transition) {
                l.css({
                    opacity: 0,
                    transition: "all " + u * 0.7 + "ms ease-in-out"
                });
            } else {
                l.fadeOut(u * 0.7);
            }
