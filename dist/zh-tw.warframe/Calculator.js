...
function io_armor() {
	return $('<table>', {
		append: [
            ...
            titleCell('Other'),
            ioRow('Arcane Guardian', [[ 0, 3, 'rank', 'Arcane rank' ], [ 1, 2, 'num', 'Number equipped', 1]]),
            ioRow('Arcane Tanker', [[ 0, 3, 'rank', 'Arcane rank' ], [ 1, 2, 'num', 'Number equipped', 1]]),
            ioRow('Arcane Ultimatum', [[ 0, 3, 'rank', 'Arcane rank' ], [ 1, 2, 'num', 'Number equipped', 1]]),
...
function update_table() {
    ...
    var otherMap = {
        arcaneguardian: [150 * (1 + reasonableInput('#in_arcaneguardian_rank', 3)) * (reasonableInput('#in_arcaneguardian_num', 2)), 'rank', 'num'],
        arcanetanker: [300 * (1 + reasonableInput('#in_arcanetanker_rank', 3)) * (reasonableInput('#in_arcanetanker_num', 2)), 'rank', 'num'],
        arcaneultimatum: [150 * (1 + reasonableInput('#in_arcaneultimatum_rank', 3)) * (reasonableInput('#in_arcaneultimatum_num', 2)), 'rank', 'num'],
    ...
    if ($('#calculator_armor').length) {
        ...
        var multipliers = 1 + (modMap.armoredagility[0] + modMap.mechapulse[0] + modMap.standunited[0] + modMap.steelfiber[0] + modMap.gladiatoraegis[0] + modMap.umbralfiber[0] + abilityMap.scarabswarm[0] + (1 + strengthMap.elementalwardstrength[0]/100)*abilityMap.elementalward[0] + (1 + strengthMap.metronomestrength[0]/100)*abilityMap.metronome[0] + (1 + strengthMap.vexarmorstrength[0]/100)*abilityMap.vexarmor[0] + (1 + strengthMap.warcrystrength[0]/100)*abilityMap.warcry[0] + otherMap.niduspassive[0])/100;
        var additions = modMap.healthconversion[0] + abilityMap.bastille[0] + defy_add + (1 + strengthMap.hallowedreckoningstrength[0]/100)*abilityMap.hallowedreckoning[0] + (1 + strengthMap.renewalstrength[0]/100)*abilityMap.renewal[0] + otherMap.arcaneguardian[0] + otherMap.arcanetanker[0] + otherMap.arcaneultimatum[0] + otherMap.justicesyndicateeffect[0] + otherMap.stoneskinfocus[0] + otherMap.atlaspassive[0] + otherMap.grendelpassive[0];
        var metamultipliers = 1 + ((1 + strengthMap.ironcladchargestrength[0]/100)*abilityMap.ironcladcharge[0])/100;
        var total_armor = (Armor * multipliers + additions) * metamultipliers;
