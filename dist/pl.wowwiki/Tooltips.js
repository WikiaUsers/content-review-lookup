...
dbSite = "http://www.pl.wow.wikia.com/wiki/";
}
...
newFieldBuffer += genExtDBOption("http://www.pl.wow.wikia.com/wiki/","WoWWiki (default)");
...
if (wgServer == "http://www.pl.wow.wikia.com") ttBgStyle = "background: transparent url(https://images.wikia.nocookie.net/wowwiki/images/thumb/c/c0/Ttbackground.svg/275px-Ttbackground.svg.png);";
...
tempName = ttLink.getAttribute("href").replace("http://","").replace("www.","").replace("pl.wow.wikia.com","").replace("/wiki","").replace("/index.php?title=", "").replace("&amp;action=edit", "").replace("/","").replace(/_/g," "); 
} else {
...
coAdjust=($(tip).attr("id")=="coordstfb")?50:0;
coAdjust+=($(tip).html().indexOf("achievementtitle")!=-1)?75:0;
var newTop = mousePos.y - (tip.clientHeight + 40);
if (skin == "monaco") {
skinAdjust[0] = ($(tip).attr("id")=="templatetfb")?(2.25 * tip.clientWidth):(tip.clientWidth / 1.5) - coAdjust;
skinAdjust[1] = (tip.clientWidth > 300)?(1.5 * tip.clientWidth):(2 * tip.clientWidth) - coAdjust;
} else {
skinAdjust[0] = (tip.getAttribute("id")=="templatetfb")?(1.75 * tip.clientWidth):(tip.clientWidth / 2) - coAdjust;
skinAdjust[1] = (tip.clientWidth > 300)?(1.35 * tip.clientWidth):(1.85 * tip.clientWidth) - coAdjust;
}
var newLeft = mousePos.x - skinAdjust[0];
if ((newTop < dbS[1]) || ($("#adSpace0").html() && (mousePos.y - tip.clientHeight - 200) < 0)) { 
newTop = mousePos.y + 1;
if (newTop + tip.clientHeight > winSize.y) newTop = dbS[1]; 
}
if (newLeft < 0) newLeft = 50;
if (dbS[0] > 0 || (newLeft >= winSize.x - (tip.clientWidth + 50))) newLeft = mousePos.x - skinAdjust[1];
...
itemname = ttLink.getAttribute("href").replace("http://","").replace("www.","").replace("pl.wow.wikia.com","").replace("wiki/","").replace("/index.php?title=", "").replace("&amp;action=edit", "").replace("/","").replace(/_/g," ");
getInfo(itemname, 1);
}
}
...
// quest tooltips
if (wgServer == "http://www.pl.wow.wikia.com") {
questStartIcon = "<img src='https://images.wikia.nocookie.net/wowwiki/images/4/46/AvailableQuestIcon.png'>";
questEndIcon = "<img src='https://images.wikia.nocookie.net/wowwiki/images/c/c0/ActiveQuestIcon.png'>";
}
else {
questStartIcon = "";
questEndIcon = "";
...
//achievement tooltips function showAchTip(i) { var Span = document.getElementById("att" + i); var ttLink = Span.parentNode; if (ttLink.getAttribute("class") != "selflink") { Span.setAttribute("title", ""); if (ttLink.getAttribute("title")) ttLink.setAttribute("title", ""); achname = ttLink.getAttribute("href").replace("http://","").replace("www.","").replace("pl.wow.wikia.com","").replace("wiki/","").replace("/index.php?title=", "").replace("&amp;action=edit", "").replace("/","").replace(/_/g," ");
...
return "https://images.wikia.nocookie.net/wowwiki/images/thumb/" + imageTitle;
}
...
tooltip = ttCoordStart + '<div class="ZoneMap" style="position:relative;width:300px;height:200px;"><img src="' + zoneImage + '" alt=""><div style="position:absolute;left:' + xPos + '%;top:' + yPos + '%;margin:-5px"><img src="https://images.wikia.nocookie.net/wowwiki/images/f/f4/Blip.png" alt=""></div></div></div>';
...
if (getCookie("extDB") != "http://www.pl.wow.wikia.com/wiki/") {
