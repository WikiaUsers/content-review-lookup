...
$( function() {
	$( '.mw-rollback-link > a' ).after( $( '<span>' ).addClass( 'edit-rollback' ).prop( 'title', 'Editar resumo da Reversão' ) );
		...
		if ( $( this ).parent().is( $rollback.parent() ) ) {
			...
			$rollback = $( '<div id="rollback-summary">' ).append(
				$( '<input type="text">' ).addClass( 'mw-ui-input rollback-text' ).prop( { maxlength: 250, spellcheck: true } ).val(
					'Revertendo edições de [[Especial:Contribuições/' + name + '|' + name + ']] ([[Usuário Discussão:' + name + '|discussão]])'
				),
