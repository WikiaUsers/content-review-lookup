// <nowiki>
// Importado de https://en.wikipedia.org/wiki/MediaWiki:Gadget-popups.js
// JS para gadget de pop-ups de navegação: pré-visualizações de artigos e funções de edição aparecem ao passar o mouse sobre os links
/* eslint-env browser  */
...
// Tentando enfiar o máximo possível no objeto pg (popups globais)
var pg = {
	re: {},               // regexps
	ns: {},               // espaços nominais
	string: {},           // textos traduzidos
	wiki: {},             // informação do site local
	user: {},             // informação do usuário atual
	misc: {},             // YUCK PHOOEY
	option: {},           // opções, veja newOption etc
	optionDefault: {},    // valores de opção padrão
	flag: {},             // misc flags
	cache: {},            // página e cache de imagem
	structures: {},       // estruturas de navlink
	timer: {},            // todos os tipos de temporizadores (muitos, malditos)
	counter: {},          // .. e todos os tipos de contadores
	current: {},          // informação do estado
	fn: {},               // funções
	endoflist: null
};
/* Pular se o gadget/script estiver sendo carregado duas vezes */
if( window.pg ) {
	return;
}
...
// elimine pop-ups do TOC
// Isso também mata qualquer coisa onclick que costumava acontecer no toc
...
/**
 * Obtém o item da lista de referências que o link de nota de rodapé fornecido direciona. este
 * normalmente é um elemento li dentro do elemento ol.references dentro da lista ref.
 * @param {Element} a - Um link de nota de rodapé.
 * @returns {Element|boolean} O elemento de destino ou falso se não for encontrado.
 */
function footnoteTarget(a) {
	var aTitle=Title.fromAnchor(a);
	// Queremos ".3A" em vez de "% 3A" ou "?" aqui, então use a propriedade anchor diretamente
...
// Verifica se o modificador correto pressionado/descompactado, se necessário
function isCorrectModifier(a,evt) {
	if (!getValueOf('popupModifier')) { return true; }
	// se popupModifierAction = habilitar, devemos abrir quando o modificador é pressionado
	// se popupModifierAction = desabilitar, devemos abrir a menos que o modificador seja pressionado
	var action = getValueOf('popupModifierAction');
...
function mouseOverWikiLink2(a, evt) {
	...
	// definir a variável global (ugh) para manter o artigo (página da wiki)
	...
	if (a.navpopup.pending === null || a.navpopup.pending !== 0) {
		// pop-ups novos ou com negócios inacabados são refeitos do zero
...
// simplePopupContent: o conteúdo que não requer download adicional
// (é mostrado mesmo quando simplePopups é verdadeiro)
...
// Devemos mostrar um contexto não simples?
// Se simplePopups é definido como verdadeiro, então não mostramos contexto não simples,
// mas se um "mostrar pré-visualização" for clicado, mostramos o contexto não simples
function shouldShowNonSimple(a) {
   return !getValueOf('simplePopups') || a.simpleNoMore; 
}

// Devemos mostrar um contexto não simples regido pela opção (por exemplo popupUserInfo)?
// Se o usuário solicitar explicitamente um contexto não simples, definindo a opção como true,
// então o mostramos mesmo no modo não simples.
...
// Tente imitar a forma como o mediawiki gera âncoras HTML a partir dos títulos das seções
function anchorize(d, anch) {
	...
	// agora tente lidar com == foo [[bar|baz]] boom == -> #foo_baz_boom
	var lines=d.split('\n');
...
/**
   Cria um novo objeto Drag. Isso é usado para tornar vários elementos DOM arrastáveis.
   @constructor
*/
function Drag () {
	/**
	   Condição para determinar se deve ou não arrastar. Esta função deve ter um parâmetro, um evento.
	   Para desativar isso, defina-o como <code>null</code>.
	   @type Function
	*/
...
function setUserInfo() {
    ...
    pg.user.canPatrol = false;
    if (getValueOf('popupPatrol')) {
		api.get(params).done(function(data){
			var rights = data.query.users[0].rights;
			pg.user.canPatrol = rights.indexOf('patrol') !== -1;
		});
    }
}
...
function loadDiff(article, oldid, diff, navpop) {
	...
	mw.loader.using( 'mediawiki.api' ).then( function() {
		...
		api.get( params ).then( function( data ) {
			...
			addPatrolLink(navpop, 'popupMiscTools');
...
// Put a "mark patrolled" link to an element target 
function addPatrolLink (navpop, target) {
	if (! pg.user.canPatrol) return;
	// If 'newRev' is older than 'oldRev' than it could be confusing, so we do not show the review link.
	if (navpop.diffData.newRev.revid <= navpop.diffData.oldRev.revid) return;
	if ( [ 'prev', 'diff' ].indexOf( navpop.parentAnchor.textContent ) === -1 ) return; // Only allow patrolling prev/diff links
	if ( !navpop.parentAnchor.closest( '.revisionpatrol-unpatrolled, .mw-changeslist-reviewstatus-unpatrolled' ) ) return;
	...
	a.onclick = function() {
		...
		api.postWithToken('patrol',params).done(function(){
			a.style.display = "none";
			// TODO: Update current page and other already constructed popups 
		} ).fail(function(){
			alert(popupString('Could not marked this edit as patrolled'));
...
pg.string = {
	...
	'weeks': 'semanas',
	'search': 'pesquisa',
	'SearchHint': 'Encontre artigos da Wikipédia em inglês contendo %s',
	'web': 'web',
	'global': 'global',
	'globalSearchHint': 'Pesquise na Wikipédia em diferentes idiomas para %s',
	'googleSearchHint': 'Google para %s',
	/////////////////////////////////////
	// article-related actions and info
	// (some actions also apply to user pages)
	/////////////////////////////////////
	'actions': 'ações',		 ///// view articles and view talk
	'popupsMenu': 'popups',
	'togglePreviewsHint': 'Alternar a geração de visualizações em pop-ups nesta página',
	'enable previews': 'ativar as pré-visualizações',
	'disable previews': 'desativar as pré-visualizações',
	'toggle previews': 'alternar as pré-visualizações',
	'show preview': 'mostrar pré-visualização',
	'reset': 'redefinir',
	'more...': 'mais...',
	'disable': 'desativar pop-ups',
	'disablePopupsHint': 'Desativar pop-ups nesta página. Recarregue a página para reativar.',
	'historyfeedHint': 'Feed RSS das mudanças recentes nesta página',
	'purgePopupsHint': 'Redefinir pop-ups, limpando todos os dados pop-up armazenados em cache.',
	'PopupsHint': 'Redefinir pop-ups, limpando todos os dados pop-up armazenados em cache.',
	'spacebar': 'espaço',
	'view': 'ver',
	...
	'this&nbsp;revision': 'essa&nbsp;revisão',
	'revision %s of %s': 'revisão %s de %s',
	'Revision %s of %s': 'Revisão %s de %s',
	'the revision prior to revision %s of %s': 'a revisão antes da revisão %s de %s',
	'Toggle image size': 'Clique para alternar o tamanho da imagem',
	'del': 'del',				 ///// delete, protect, move
	'delete': 'exluir',
	'deleteHint': 'Excluir %s',
	'undeleteShort': 'des',
	'UndeleteHint': 'Mostrar o histórico de exclusão para %s',
	'protect': 'proteger',
	'protectHint': 'Restringir direitos de edição a %s',
	'unprotectShort': 'des',
	'unprotectHint': 'Permitir que %s seja editado por qualquer pessoa novamente',
	'send thanks': 'enviar agradecimento',
	'ThanksHint': 'Enviar uma notificação de agradecimento a este usuário',
	'move': 'mover',
	'move page': 'mover página',
	'MovepageHint': 'Mudar o título de %s',
	'edit': 'editar',			   ///// edit articles and talk
	'edit article': 'editar artigo',
	'editHint': 'Alterar o conteúdo de %s',
	'edit talk': 'editar discussão',
	'new': 'new',
	'new topic': 'novo tópico',
	'newSectionHint': 'Comece uma nova seção em %s',
	'null edit': 'edição nula',
	'nullEditHint': 'Enviar uma edição para% s, sem fazer alterações ',
	'hist': 'hist',			   ///// history, diffs, editors, related
	'history': 'histórico',
	'historyHint': 'Listar as mudanças feitas em %s',
	'last': 'anterior', // For labelling the previous revision in history pages; the key is "last" for backwards compatibility
	'lastEdit': 'lastEdit',
	'mark patrolled': 'marcar como patrulhada',
	'markpatrolledHint': 'Marcar esta edição como patrulhada',
	'Could not marked this edit as patrolled': 'Não foi possível marcar esta edição como patrulhada',
	'show last edit': 'edição mais recente',
	'Show the last edit': 'Mostra os efeitos da mudança mais recente',
	'lastContrib': 'lastContrib',
	'last set of edits': 'últimas edições',
	'lastContribHint': 'Mostra o efeito líquido das mudanças feitas pelo último editor',
	'cur': 'atual',
	'diffCur': 'diffCur',
	'Show changes since revision %s': 'Mostrar mudanças desde a revisão %s',
	'%s old': '%s atrás', // as in 4 weeks old
	'oldEdit': 'oldEdit',
	'purge': 'atualizar',
	'purgeHint': 'Exija uma nova cópia de %s',
	'raw': 'fonte',
	'rawHint': 'Baixar a fonte de %s',
	'render': 'simple',
	'renderHint': 'Mostrar uma versão HTML simples de %s',
	'Show the edit made to get revision': 'Mostrar a edição feita para obter a revisão',
	'sinceMe': 'sinceMe',
	'changes since mine': 'diff minha edição',
	'sinceMeHint': 'Mostrar mudanças desde minha última edição',
	'Couldn\'t find an edit by %s\nin the last %s edits to\n%s': 'Couldn\'t find an edit by %s\nin the last %s edits to\n%s',
	'eds': 'eds',
	'editors': 'editores',
	'editorListHint': 'Listar os usuários que editaram %s',
	'related': 'related',
	'relatedChanges': 'relatedChanges',
	'related changes': 'alterações relacionadas',
	'RecentchangeslinkedHint': 'Mostrar alterações em artigos relacionados a %s',
	'editOld': 'editOld',		  ///// edit old version, or revert
	'rv': 'rv',
	'revert': 'reverter',
	'revertHint': 'reverter para %s',
	'defaultpopupReviewedSummary': 'Aceito ao revisar a [[Special:diff/%s/%s|diferença]] entre esta versão e a versão aceita anteriormente usando [[:en:Wikipedia:Tools/Navigation_popups|popups]]',
	'defaultpopupRedlinkSummary': 'Removendo link para página vazia [[%s]] usando [[:en:Wikipedia:Tools/Navigation_popups|popups]]',
	'defaultpopupFixDabsSummary': 'Desambiguar [[%s]] para [[%s]] usando [[:en:Wikipedia:Tools/Navigation_popups|popups]]',
	'defaultpopupFixRedirsSummary': 'Redirecionar desvio de [[%s]] para [[%s]] usando [[:en:Wikipedia:Tools/Navigation_popups|popups]]',
	'defaultpopupExtendedRevertSummary': 'Reverter para a revisão datada de %s por %s usando [[:en:Wikipedia:Tools/Navigation_popups|popups]]',
	'defaultpopupRevertToPreviousSummary': 'Reverter para a revisão antes da revisão %s usando [[:en:Wikipedia:Tools/Navigation_popups|popups]]',
	'defaultpopupRevertSummary': 'Reverter para revisão %s usando [[:en:Wikipedia:Tools/Navigation_popups|popups]]',
	'defaultpopupQueriedRevertToPreviousSummary': 'Reverter para a revisão anterior à revisão $1 datada de $2 por $3 usando [[:en:Wikipedia:Tools/Navigation_popups|popups]]',
	'defaultpopupQueriedRevertSummary': 'Reverter para revisão $1 datada $2 by $3 usando [[:en:Wikipedia:Tools/Navigation_popups|popups]]',
	'defaultpopupRmDabLinkSummary': 'Remover link para a página dab [[%s]] usando [[:en:Wikipedia:Tools/Navigation_popups|popups]]',
	'Redirects': 'Redirecionar', // as in Redirects to ...
	' to ': ' para ',		   // as in Redirects to ...
	'Bypass redirect': 'Ignorar redirecionamento',
	'Fix this redirect': 'Corrigir este redirecionamento',
	'disambig': 'desambiguar',		  ///// add or remove dab etc.
	'disambigHint': 'Remova a ambiguidade deste link para [[%s]]',
	'Click to disambiguate this link to:': 'Clique para remover a ambiguidade deste link para:',
	'remove this link': 'remover esse link',
	'remove all links to this page from this article': 'remover todos os links para esta página deste artigo',
	'remove all links to this disambig page from this article': 'remover todos os links para esta página de desambiguação deste artigo',
	'mainlink': 'link principal',		  ///// links, watch, unwatch
	'wikiLink': 'Link da wiki',
	'wikiLinks': 'Links da wiki',
	'links here': 'links aqui',
	'whatLinksHere': 'whatLinksHere',
	'what links here': 'Links afluentes',
	'WhatlinkshereHint': 'Listar as páginas com ligadas para %s',
	'unwatchShort': 'des',
	'watchThingy': 'vigiar',  // called watchThingy because {}.watch is a function
	'watchHint': 'Adicionado %s das minhas páginas vigiadas',
	'unwatchHint': 'Removido %s das minhas páginas vigiadas',
	'Only found one editor: %s made %s edits': 'Só encontrei um editor: %s feito %s edições',
	'%s seems to be the last editor to the page %s': '%s parece ser o último editor da página %s',
	'rss': 'rss',
	/////////////////////////////////////
	// diff previews
	/////////////////////////////////////
	'Diff truncated for performance reasons': 'Diferença truncada por motivos de desempenho',
	'Old revision': 'Revisão antiga',
	'New revision': 'Nova revisão',
	'Something went wrong :-(': 'Algo deu errado :-(',
	'Empty revision, maybe non-existent': 'Revisão vazia, talvez inexistente',
	'Unknown date': 'Data desconhecida',
	/////////////////////////////////////
	// other special previews
	/////////////////////////////////////
	'Empty category': 'Categoria vazia',
	'Category members (%s shown)': 'Membros da categoria (mostrando %s)',
	'No image links found': 'Nenhum link de imagem foi encontrado',
	'File links': 'Links de arquivos',
	'No image found': 'Nenhuma imagem encontrada',
	'Image from Commons': 'Image from Commons',
	'Description page': 'Página de descrição',
	'Alt text:': 'Alt text:',
	'revdel':'Revisão oculta',
	/////////////////////////////////////
	// ações e informações relacionadas ao usuário
	/////////////////////////////////////
	'user': 'usuário',			   ///// página do usuário, discusão, e-mail, espaço
	'user&nbsp;page': 'página&nbsp;do usuário',
	'user talk': 'Discussão do usuário',
	'edit user talk': 'editar discussão do usuário',
	'leave comment': 'deixar comentário',
	'email': 'e-mail',
	'email user': 'e-mail do usuário',
	'EmailuserHint': 'Enviar um e-mail para %s',
	'space': 'espaço', // short form for userSpace link
	'PrefixIndexHint': 'Mostrar páginas no espaço do usuário de %s',
	'count': 'contador',			 ///// contribuições, registros
	'edit counter': 'editar contador',
	'editCounterLinkHint': 'Contar as contribuições feitas por %s',
	'contribs': 'contribs',
	'contributions': 'contribuições',
	'deletedContribs': 'contribuições excluídas',
	'DeletedcontributionsHint': 'Listar edições excluídas feitas por %s',
	'ContributionsHint': 'Listar as contribuições feitas por %s',
	'log': 'registro',
	'user log': 'registro do usuário',
	'userLogHint': 'Mostra registro do usuário %s',
	'arin': 'Pesquisar ARIN',			 ///// ARIN lookup, block user or IP
	'Look up %s in ARIN whois database': 'Procure %s no banco de dados ARIN whois',
	'unblockShort': 'des',
	'block': 'bloquear',
	'block user': 'bloquear usuário',
	'IpblocklistHint': 'Desbloquear %s',
	'BlockipHint': 'Impedir que %s edite',
	'block log': 'registro de bloqueio',
	'blockLogHint': 'Mostrar o registro de bloqueio para %s',
	'protectLogHint': 'Mostrar o registro de proteção para %s',
	'pageLogHint': 'Mostrar o registro da página para %s',
	'deleteLogHint': 'Mostrar o registro de exclusão para %s',
	'Invalid %s %s': 'A opção %s é invalido: %s',
	'No backlinks found': 'Nenhum links back encontrado',
	' and more': ' e mais',
	'undo': 'desfazer',
	'undoHint': 'desfazer essa edição',
	'Download preview data': 'Baixar dados de pré-visualização',
	'Invalid or IP user': 'Usuário inválido ou IP',
	'Not a registered username': 'Não é um nome de usuário registrado',
	'BLOCKED': 'BLOQUEADO',
	' edits since: ': ' edições desde: ',
	'last edit on ': 'última edição em ',
	/////////////////////////////////////
	// Edição automática
	/////////////////////////////////////
	'Enter a non-empty edit summary or press cancel to abort': 'Insira um resumo de edição não vazio ou pressione cancelar para abortar',
	'Failed to get revision information, please edit manually.\n\n': 'Falha ao obter informações de revisão, edite manualmente.\n\n',
	'The %s button has been automatically clicked. Please wait for the next page to load.': 'O %s botão foi clicado automaticamente. Por favor espere a próxima página carregar.',
	'Could not find button %s. Please check the settings in your javascript file.': 'Não foi possível encontrar o botão %s. Por favor, verifique as configurações em seu arquivo javascript.',
	/////////////////////////////////////
...
// Por algum motivo, os pop-ups exigem uma página totalmente carregada jQuery.ready(...) causa problemas para alguns.
// O antigo addOnloadHook fez algo semelhante ao abaixo
...
// Suporte para visualização ao vivo do MediaWiki, salvamentos do Editor Visual e menu desdobrável do Echo.
( function () {
	var once = true;
	...
	// Este gancho também é acionado após o carregamento da página.
