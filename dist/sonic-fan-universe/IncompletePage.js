$(function () {
    ...
    window.IncompletePage = (function (my, console, Math) {
 
        my = $.extend({
            question: 'The page is currently incomplete, would you like to view it anyway?',
            yes: 'Yes, I'd like to view it.',
            no: 'No, when it is finished I will.',
            isIncomplete: function () {
                return (/^Incomplete\:/.test(document.title));
            },
            back: false
        }, my);
        ...
        function getBackgroundColor () {
            ...
            if ('transparent' !== color) return color;
            console.log('IncompletePage: Cannot determine color');
            return color;
        }
 
        // Use LocalStorage, it doesn't get sent to the server every HTTP request
        var ids = $.storage.get('IncompletePageJS');
        // Backwards compatibility. This block can be removed after a week or so
        if (!ids) {
            ids = $.cookies.get('incomplete');
            if (ids) { // Old cookie found, convert to local storage
                ids = ids.split(',');
                $.cookies.del('incomplete', {hoursToLive:0, path:'/', domain: location.host});
                $.storage.set('IncompletePageJS', ids);
            } else {
                ids = [];
            }
        }
        if (my.isIncomplete() && -1 === $.inArray(wgArticleId, ids)) {
    ...
    }) (window.IncompletePage, window.console || { log: $.noop }, Math);
 });