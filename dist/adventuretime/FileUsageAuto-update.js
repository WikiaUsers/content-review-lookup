		...
		**************************************/
		...
		initialize: function(){
			if (typeof LIR.supportedLanguages.indexOf(mediaWiki.config.get('wgContentLanguage')) == -1){
				...
				if (LIR.supportedLanguages.indexOf(mediaWiki.config.get('wgUserLanguage')) == -1){
					var userLang = "en";
				}else{
				...
				if (typeof localStorage[wgUserName + "_LIRNamespaceSelection"] === "undefined"){
				 ...
				 /* Get language pack! */
				$.ajax({
					url: "https://dev.fandom.com/wiki/MediaWiki:FileUsageAuto-update/" + mediaWiki.config.get('wgContentLanguage') + ".js?action=raw&ctype=text/javascript",
					dataType: "script",
					cache: true,
					success: function(){
