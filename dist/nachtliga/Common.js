...
/* Source: en.wikipedia.org/wiki/MediaWiki:Common.js */
...
// Scribblemap-Funktionalitäten
function wwScribbleMaps() {
  divs = document.getElementsByTagName('div');
  for (x = 0; x < divs.length; x++) {
  if (hasClass(divs[x], 'wwSM')) {
    var params = divs[x].className.split('-');
    if (params.length > 1)
      mapID = params[1];
    else
      mapID = '';
...
// Tippfehlerbeseitigung in Umfrage
// Derzeit nicht benötigt, aber aufgehoben für eventuellen späteren Gebrauch.
function correctMisspelling() {
  var div = document.getElementById('ajax-poll-D37F8D1A3B3C733A3AC7ABE7433E0857');
  if (div != null) {
    var text = div.firstChild.firstChild;
    text.data = 'Das Forscherliga-Wiki sucht das beste 5-Spieler-Dungeon. Teil 1: Niedrigstufige Classic-Instanzen.'
  }
}

//addOnloadHook(correctMisspelling);

// </pre>

// Lizenzinfo in Upload-Bildschirm erzwingen.
// Wir entfernen hier die "keine Auswahl"-Option und setzen die LU-Option auf Standard
...
function addFaceBookFanPage() {
  var headerName;
  for (var j = 0; j < 2; j++) {
    switch (j) { 
      case 0: 
        headerName = 'WikiaPageHeader';
        break;
      case 1: 
        headerName = 'WikiaUserPagesHeader';
        break;
    }
    var header = document.getElementById(headerName);
    if (header != null) {
      var elements = header.getElementsByTagName('li');
      for (var i = 0; i < elements.length; i++) {
        var liElement = elements[i];
        var attrNode = liElement.getAttributeNode('class');
        if (attrNode != null) {
          if (attrNode.value == 'likes') {
            //liElement.style.display = 'none';
            var fbElement = document.createElement("fb:like");
            fbElement.setAttribute('href', 'http://www.facebook.com/pages/Forscherliga-Wiki/193823804003037?ref=hnav');
            fbElement.setAttribute('send', 'false');
            fbElement.setAttribute('layout', 'button_count');
            fbElement.setAttribute('width', '50');
            fbElement.setAttribute('show_faces', 'false');
            liElement.removeChild(liElement.firstChild.nextSibling);
            liElement.appendChild(fbElement);
            //liElement.insertBefore(fbElement, liElement.firstChild);
          }
        }
      }
    }
...
// addOnloadHook(addFaceBookFanPage); /* momentan nicht mehr benötigt. */