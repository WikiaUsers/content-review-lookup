// скрипт таймера (Шаблон:Таймер)
// источник: https://warframe.fandom.com/ru/wiki/MediaWiki:Countdown.js
         ...
         if ((i === (loopLimit1 + 1)) && (nextDate.getTime() <= now.getTime())) {
             ...
             if ((Math.floor(diff2 / oneSecond) * oneSecond) < delayConv) {
                 ...
                 if (document.getElementById('delayCountDisplay_' + k).innerHTML === '') {
                     if (Y_Count !== 0) $('#years_' + k).html(Y_zeros2 + Math.floor(yearsleft2) + '<i>' + yearunit2 +'</i>' + sep);
                     if (M_Count !== 0) $('#months_' + k).html(M_zeros2 + Math.floor(monthsleft2) + '<i>' + monthunit2 +'</i>' + sep);
                     if (D_Count !== 0) $('#days_' + k).html(D_zeros2 + Math.floor(daysleft2) + '<i>' + dayunit2 +'</i>' + sep);
                     if (h_Count !== 0) $('#hours_' + k).html(h_zeros2 + Math.floor(hoursleft2) + '<i>' + hourunit2 +'</i>' + sep);
                     if (m_Count !== 0) $('#minutes_' + k).html(m_zeros2 + Math.floor(minutesleft2) + '<i>' + minuteunit2 +'</i>' + sep);
                     if (s_Count !== 0) $('#seconds_' + k).html(s_zeros2 + Math.floor(secondsleft2) + '<i>' + secondunit2 +'</i>');
                 } else {
                     $('#years_' + k).html('');
                     $('#months_' + k).html('');
                     $('#days_' + k).html('');
             ...
             // time string will result in "02h 00m 00s" thus far
                 ...
                 if (Y_Count !== 0) $('#years_' + k).html(Y_zeros + Math.floor(yearsleft) + '<i>' + yearunit + '</i>' + sep);
                 if (M_Count !== 0) $('#months_' + k).html(M_zeros + Math.floor(monthsleft) + '<i>' + monthunit + '</i>' + sep);
                 if (D_Count !== 0) $('#days_' + k).html(D_zeros + Math.floor(daysleft) + '<i>' + dayunit + '</i>' + sep);
                 if (h_Count !== 0) $('#hours_' + k).html(h_zeros + Math.floor(hoursleft) + '<i>' + hourunit + '</i>' + sep);
                 if (m_Count !== 0) $('#minutes_' + k).html(m_zeros + Math.floor(minutesleft) + '<i>' + minuteunit + '</i>' + sep);
                 if (s_Count !== 0) $('#seconds_' + k).html(s_zeros + Math.floor(secondsleft) + '<i>' + secondunit + '</i>');
                 if ($('.loopCount').length > 0) {
