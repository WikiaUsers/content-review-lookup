...
!(function() {
  ...
  if (conf.wgAction !== 'view' || !conf.wgArticleId) return
  if (
    conf.wgPageContentModel && // MediaWiki 1.19 don't has this conf
    !['wikitext', 'scribunto'].includes(conf.wgPageContentModel.toLowerCase())
  ) {
    return
  }
  ...
  mw.loader.using(['mediawiki.util'], function() {
    ...
    $subtitle.html(function() {
      ...
      html = html.replace(/\|\s+?<a.*?id="ca-varlang-[0-9]+".*?>.*?<\/a>/g, '')
      html = html.trim()
      return html
    })
    ...
    var variantLink = function(lang) {
      ...
      return $('<li>').append(
        $('<a>', {
          ...
          href: mw.util.getUrl(conf.wgPageName, { variant: lang })
        })
      )
    }
    ...
    var $variantMenu = $('<div>', {
      ...
      $('<div>', { class: 'wds-dropdown__toggle' }).append(
        $('<span>', {
          class: 'user-variant',
          text: variantName[userVariant] || variantName['zh']
        }),
        $triangle
      ),
      $('<div>', {
    ...
    // Append menu
    $('.page-header__contribution').prepend($variantMenu)
