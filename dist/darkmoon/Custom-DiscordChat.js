...
(function (window, $, mw) {
    ...
    window.DiscordChat = {
        ...
        messages: {},
        data: null,
        display: function(input) { // Helper to convert user input keywords to CSS classes
        ...
        serverStats: function(type) {
            ...
            // Parsing the pipe-separated display message
            var displayMode = (window.DiscordChat.messages['Custom-DiscordChat-display'] || 'bullet | label');
            var displayValue = displayMode.split('|');
            
            var rightRailDisplay = displayValue[0] || 'bullet';
            var mainPageDisplay = displayValue[1] || displayValue[0] || 'label'; // Fallback to first part if second is missing

            var activeClass = (type === 'rightRail') 
                ? window.DiscordChat.display(rightRailDisplay) 
                : window.DiscordChat.display(mainPageDisplay);
        ...
        rightRail: function() {
            ...
            var element = $('<section class="rail-module DiscordChat">' +
                                '<h2 class="rail-module__header has-icon">' +
                                    '<svg xmlns="http://www.w3.org/2000/svg" class="DiscordChat__icon" width="18" height="16" viewBox="0 0 28 20"><path d="' + window.DiscordChat.svg + '"/></svg>' +
                                    headerLabel +
                                '</h2>' +
                                '<div class="DiscordChat__wrapper">' +
                                    '<div class="DiscordChat__wrapper__statistics">' +
                                        avatarContainer.prop('outerHTML') +
                                        window.DiscordChat.serverStats('rightRail') +
                                    '</div>' +
                                    '<a href="' + window.DiscordChat.data.instant_invite + '" ' +
                                        'target="_blank" ' +
                                        'class="DiscordChat__button wds-button wds-is-secondary" ' +
                                        'data-label="' + joinLabel + '">' + 
                                        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 28 20"><path d="' + window.DiscordChat.svg + '"/></svg>' +
                                    '</a>' +
                                '</div>' +
                            '</section>');
            ...
            $('.rail-module.DiscordChat').siblings('.chat-module').css('display', 'none');
            mw.hook('DiscordChat.added').fire(element);
        },
        
        mainPage: function() {
            var $anchor = $('.DiscordChat__mainpage');
            ...
            $anchor.html(
                '<section class="DiscordChat is-mainpage">' +
                    '<div class="DiscordChat__wrapper">' +
                        '<div class="DiscordChat__wrapper__statistics">' + 
                            avatarStack.prop('outerHTML') + 
                            window.DiscordChat.serverStats('mainPage') +
                        '</div>' +
                        '<a href="' + window.DiscordChat.data.instant_invite + '" ' +
                            'target="_blank" ' +
                            'class="DiscordChat__button wds-button wds-is-secondary" ' +
    ...
    mw.loader.using(['mediawiki.api', 'mediawiki.util'], function() {
        importArticles({
            ...
            type: 'style',
            article: 'u:clodaghelm:MediaWiki:Custom-DiscordChat.css'
        });
