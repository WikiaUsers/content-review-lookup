if (wgPageName === "Wiki-Paten-Antrag") {
var _api = {
...
var kategorie_arr = ["Videospiele","TV","Filme","Anime","EI: Bücher","EI: Lifestyle","EI: Comics","EI: Musik"];
var kategorie_arr_options = "";
for (var i = 0; i < kategorie_arr.length; i++) {
   kategorie_arr_options += '<option>' + kategorie_arr[i] + '</option>';
  }
  
var hilfe_art_arr = ["Wiki-Paten-Antrag – Anfänger", "Wiki-Paten-Antrag – Fortgeschritten", "Wiki-Paten-Antrag – Release steht bevor"];
...
url = $("#wikiURL").val();
urlLang = $("#langpath").val();
if (url.length === 0) {
    alert('Die Wiki-URL fehlt!');
    window.location = "#wikiURL";
    return;
}
if (url.search(/(?:https?:?\/\/)|(?:\.fandom\.com)/) !== -1) {
    alert('Bitte gib nur den Teil der URL ein, der vor ".fandom.com" und hinter "https://" steht.');
...
_month = "0" + (_date_timeval.getMonth()+1);
_day = "0" + _date_timeval.getDate();
_date = _date_timeval.getFullYear() + "-" + _month.slice(-2) + "-" + _day.slice(-2);
destination = "Wiki-Paten-Antrag:" + url + "/" + _date;
...
$('.wiki-paten-hilfe-beantragen-fortgeschritten').click(function() { wiki_paten_bereich = "Wiki-Paten-Antrag – Fortgeschritten" });
...
+ '<br />'
+ 'https://' + '<input id="wikiURL" class="noFullWidth" style="width: 180px;" placeholder="community" />' + '.fandom.com/' + '<input id="langpath" class="noFullWidth" style="width: 15px;" maxlength="2" />'
+ '<br />'
+ '<i>Achtung: Bei den meisten deutschen Wikis befindet sich in der Adresse ein "de" hinter dem ".com/". Bei Adressen ohne "de" muss das Feld leer bleiben.</i>'
+ '<br />'
+ '<b>' + 'Wiki-Kategorie' + '</b>'
+ '<br />'
...
$("#ampform input:not(.noFullWidth), #ampform textarea").css("box-sizing", "border-box").css("width", "100%");
