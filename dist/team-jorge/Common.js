...
document.getElementById("MainPageHero").innerHTML+='<div class="financial-independence">FINANCIAL INDEPENDENCE</div>';
...
window.snowStorm = {
	...
	animationInterval: 40,		// Theoretical "miliseconds per frame" measurement. 20 = fast + smooth, but high CPU use. 50 = more conservative, but slower
	...
	snowColor: '#ffff00',		// Don't eat (or use?) yellow snow.
	...
	freezeOnBlur: false,		// Only snow when the window is in focus (foreground.) Saves CPU.
	...
	flakeWidth: 50,			// Max pixel width reserved for snow element
	flakeHeight: 50,			// Max pixel height reserved for snow element
...
snowStorm.SnowFlake = function(type, x, y) {
	...
	this.o.innerHTML = '<img src="http://i.imgur.com/3kV2Ce5.png" style="width: 50px; height: 50px;" />';
	this.o.style.color = '#'+(Math.random()*0xFFFFFF<<0).toString(16);
	this.o.style.position = (snowStorm.internal.fixedForEverything ? 'fixed' : 'absolute');
	this.o.style.width = '50px';
	this.o.style.height = '50px';
...
/*function replaceAction() {
    ...
    for (var i=0;i<aArray.length;i++) {
        //aArray[i].href = aArray[i].href.replace(/veaction/gi,"action");
        aArray[i].value.onclick = function() {
            aArray[i].value.setAttribute("href", aArray[i].href.replace(/veaction/gi,"action"));
            return false;
        }
        //aArray[i].setAttribute("href", "xyz.php");
    }

}

setTimeout(replaceAction,500);*/
