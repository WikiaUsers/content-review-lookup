...
mw.hook('wikipage.content').add(function($content) {
        ...
        const onMouseUp = () => {
            ...
            if (element.animate && options.animationFrames) {
                const anim = element.animate(
                    options.animationFrames(offsetX, offsetY),
    ...
    function initSwordOfCorruption() {
        if (mw.config.get('wgPageName') !== 'Sword_of_Corruption') return;
        ...
        const messages = [
            ...
            "Chaos, Chaos!",
            `This isn't real enough? I'm not real enough for you, <span class="InputUsername">${username}</span>? It was real enough for Chef.`,
            ...
            "There once was a ship that put to sea, the name of this ship was the Armada.",
        ...
        function spawnImageNotice() {
            ...
            setTimeout(() => {
                img.style.opacity = String(0.7 + Math.random() * 0.3);
            }, 50);
        ...
        function animate() {
            ...
            notices.forEach((n) => {
                ...
                if (n.x <= 0) {
                    n.x = 0;
                    n.vx *= -1;
                } else if (n.x + n.offsetWidth >= window.innerWidth) {
                    n.x = window.innerWidth - n.offsetWidth;
                    n.vx *= -1;
                }
                ...
                if (n.y <= 0) {
                    n.y = 0;
                    n.vy *= -1;
                } else if (n.y + n.offsetHeight >= window.innerHeight) {
                    n.y = window.innerHeight - n.offsetHeight;
                    n.vy *= -1;
                }
...
$(function() {
    ...
    const $placeholder = $('#userProfileLinkPlaceholder');
    
    if (!$placeholder.length) return;

    if (username) {
        const profileUrl = '/wiki/User:' + encodeURIComponent(username);
        $placeholder.html('<a href="' + profileUrl + '">You</a>');
    } else {
        $placeholder.html('Log in to see your profile link');
    }
});
