	...
	.then(() => {
		...
		$('[class*="rating-"]').click(function() {
			...
			fetch('/wiki/Backrooms_Indonesia_Wiki:Ratings.json?action=raw')
				...
				.then(() => {
					deleted = '';
					return Promise.all([...Object.keys(globalUp), ...Object.keys(globalDown)].join('|').match(/([^|]*\|){1,50}/g).map(chunk => fetch(`/api.php?action=query&titles=${chunk.slice(0, -1)}&format=json`)
						.then(response => response.json())
