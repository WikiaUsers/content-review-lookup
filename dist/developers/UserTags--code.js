...
var importList = (function(config, alwaysOnly) {
	...
	var moduleMap = {
		explode: { s: 'u:developers:User:Dorumin/module.implode.js' }
	};
	...
	for (module in config.modules) {
		...
		if (moduleMap.hasOwnProperty(module)) {
			...
			script = 'u:developers:User:Dorumin/module.' + module + '.js';
		} else {
			continue;
		}
		if (hash[script] !== 1) {
	...
	if (!alwaysOnly) {
		list[list.length] = 'u:developers:User:Dorumin/core.js';
	}
	return list;
})(config, !loaderData);
