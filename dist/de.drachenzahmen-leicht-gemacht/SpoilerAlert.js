/* Based on SpoilerAlert by pecoes & Gguigui1: http://dev.wikia.com/wiki/SpoilerAlert - updated by Καρδιά του Δράκου: http://jakwytresowacsmoka.wikia.com/wiki/MediaWiki:SpoilerAlert.js
*/
...
if ($.inArray("Spoiler", wgCategories) > -1) {
    ...
    $(function () {
        ...
        '<div id="spoiler-container" style="bottom: 0px; left: 0px; position: absolute; right: 0px; top: 0px; z-index: 2000000001;">' +
            '<table id="spoiler-alert" border="0" style="background-color: rgba(255, 255, 255, 0.5); border: 3px solid #900;  box-shadow: 3px 3px 10px; border-radius: 20px; font-size: 13px; line-height: 22px; margin: 150px auto 0; width: 430px;">' +
                '<tbody>' +
                    '<tr>' +
                        '<td rowspan="2" style="padding: 10px;">' +
                            '<img height="100" src="https://vignette.wikia.nocookie.net/drachenzahmen-leicht-gemacht/images/7/76/Vorlagenbild_Seitenspoiler.png/revision/latest?cb=20181106220301&path-prefix=de">' +
                        '</td>' +
                        '<td style="border-style: none; padding: 5px; text-align: center;" colspan="2"><strong style="font-size:18px;font-weight: bold;">Diese Seite enthält SPOILER.<br>Möchten Sie sie trotzdem ansehen?</strong></td>' +
                    '</tr>' +
                    '<tr>' +
                        '<td style="border-style: none; padding: 5px; text-align: center;">' +
                            '<button style="font-weight:bold;" id="yes">Aber ja doch</button>' +
                        '</td>' +
                        '<td style="border-style: none; padding: 5px; text-align: center;">' +
                            '<button style="font-weight:bold;" id="no">Nein, noch nicht</button>' +
                        '</td>' +
                    '</tr>' +
                '</tbody>' + 
            '</table>' +
