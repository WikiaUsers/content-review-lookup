...
function sortAndResize(table) {
   $(table).removeClass('jquery-tablesorter');
   mw.loader.using('jquery.tablesorter', function() { $(table).tablesorter(); });
      ...
      for (var i = 0; i < vals.length; i ++) {
         ...
         for (var j = 0; j < vals[i].length; j ++) {
            if (j < rowhead[row].length) {
               ...
               if (isNaN(sort[i][j]))
                  console.log('UnitComparator.js: isNaN - ' + sort[i][j]);
               else
...
function updateHTKComparator() {
   ...
   var boost    = (offense && rage > 0 ? spellInfo('rage', 'damage boost', rage) + 1 : 1);
   ...
   switch (a2.value) {
      ...
      case 'Lightning Spell':
         var spell = 'Lightning Spell';

         for (var i = 1; i <= spellInfo(spell, 'levels'); i ++) {
            troopname.push(spell);
            trooplvl.push(i);
            troopclass.push('Spell');
            trooptype.push('Spell');
            troopfavorite.push('None');
            troopmultiplier.push(1);
            troophp.push(0);
            troopdps.push(spellInfo(spell, 'total damage', i));
            troopattackspeed.push(1);
            colhead.push([spell]);
            colhead[colhead.length - 1].push(i);
         }
   ...
   for (var row = 0; row < rowhead.length; row ++) {
      ...
      for (var col = 0; col < colhead.length; col ++) {
         ...
         if (offense) {
            // Fixing rage spell effect
            var tmpboost = boost;

            // Only half as effective on heroes
            if (troopclass[col] === 'Hero')
               tmpboost = (boost - 1) / 2 + 1;
            // No rage effect at all on spells
            else if (troopclass[col] === 'Spell')
               tmpboost = 1;
