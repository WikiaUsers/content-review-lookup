...
window.pg || $(document).ready( function($) {
    ...
    if ( wgCanonicalNamespace === '' || wgCanonicalNamespace === 'Project' || wgCanonicalNamespace === 'Help' ) {
        /* Kita tidak perlu buat kerja dua kali, jadi import stylesheet daripada wiki:en */
        importStylesheetURI("//en.wikipedia.org/w/index.php?title=MediaWiki:Gadget-ReferenceTooltips.css&action=raw&ctype=text/css");
        ...
        if( settings[0] == 0 ) {
            ...
            footer.append($("<li>").append($("<a>").text("Hidupkan Reference Tooltips").attr("href","javascript:(function(){})()").click(function(){toggleRT(1)})));
            return;
        }
            ...
            function openSettingsMenu(){
                    if( settingsMenu ) {
                            ...
                            settingsMenu = $("<form>").append(
                                    $("<button>").css("width","100%").text("Lumpuhkan Reference Tooltips").button().click(function(){toggleRT(0)}),
                                    $("<br>"),
                                    $("<small>").text("Setelah dilumpuhkan, Reference Tooltips boleh dihidupkan semula menggunakan pautan di bawah laman."),
                                    $("<hr>"),
                                    $("<label>").text("Sela sebelum tooltip muncul (dalam milisaat): ").append($("<input>").attr({"type":"number","value":settings[1],step:50,min:0,max:5000})),
                                    $("<br>"),
                                    $("<span>").text("Tooltip diaktifkan dengan:"),
                                    $("<label>").append(
                            ...
                            ).submit(function(e){e.preventDefault()}).dialog({modal:true,width:500,title:"Opsyen Reference Tooltips",buttons:{"Simpan tetapan":function(){
