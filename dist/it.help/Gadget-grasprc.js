...
window.getNextWiki = function() {
	...
	var api = (d == "\/\/help-it.gamepedia.com")?new mw.Api():new mw.ForeignApi(d+"/api.php");
...
window.reloadWikiList = function() {
	...
	$.getJSON("https://www.pcj.us/gp/?since="+since+"&until="+until+"&callback=?").done(function(data){
		...
		for (wiki in rlWikis) {
			...
			else setWikiValue(wiki,getWikiValue(wiki).edits+rlWikis[wiki].edits,rlWikis[wiki].rctimestamp);
		}
...
mw.loader.using(['mediawiki.api','mediawiki.ForeignApi']).then(function() {
	...
	$("#grc-options").append('<label for="maxChanges">Max number of changes per wiki: </label> <input type="text" value="'+maxChanges+'" id="maxChanges" size="1" maxlength="3" />');
