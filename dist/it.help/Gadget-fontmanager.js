mw.loader.using(['mediawiki.api','mediawiki.ForeignApi']).then(function() {
	if (mw.config.get("wgNamespaceNumber") == -1 && mw.config.get("wgTitle") == "FontManager" && (mw.config.get("wgUserGroups").indexOf("sysop") != -1 || mw.config.get("wgUserGroups").indexOf("wiki_guardian") != -1)) {
		a = (mw.config.get("wgServerName") == "help-it.gamepedia.com")?new mw.Api():new mw.ForeignApi("//help-it.gamepedia.com/api.php");
		a.get({action:"query",titles:"MediaWiki:Gadget-fontmanager-data.json",prop:"revisions",rvlimit:1,rvprop:"content"}).done(function(data) {
			...
			for (f in fonts){
				...
				$f.children("div.font_name").text(font.name);
				...
				if (font.main != undefined) {
					$m = $("#"+font.main);
					$f.attr("data-font_child",1).addClass("mw-collapsible mw-collapsed").hide();
					if ($m.find("div.hiddenFamily").length==0) $m.append('<div class="hiddenFamily">Related fonts:<br /><ul class="hlist"></ul></div><br />');
					$m.find("div.hiddenFamily ul").append('<li><a href="javascript:;" data-family_member_reveal="'+font.id+'" class="revealFamily">'+font.name+'</a></li>');
					$f.detach().appendTo("#"+font.main);
				}
				else $f.detach().appendTo("#sortedFonts");
			}
			$("div.font_css").hide();
			...
			$("a.revealFamily").click(function () {
				$("#"+$(this).attr("data-family_member_reveal")).toggle("slow");
			});
			attribs.sort();
