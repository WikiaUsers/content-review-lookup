...
window.reloadWikis = function() {
	...
	for (wiki in subscribedWikis) {
		...
		h+= '<div id="rc-'+subscribedWikis[wiki].key+'"><h3><a href="\/\/'+subscribedWikis[wiki].domain+'/Special:RecentChanges" target="_blank">'+subscribedWikis[wiki].name+'</a> <img src="https://help-it.gamepedia.com/media/help-it.gamepedia.com/d/db/Icon_delete.png" style="cursor:pointer; width:60px; position:relative; top:-3px;" onClick="$(\'#rco-'+subscribedWikis[wiki].key+'\').prop(\'checked\',false).change();" /></h3><ul></ul></div>';
	}
	$("#grasprc").html(h);
	wiki_i = 0;
	getNextWiki();
...
window.rcSubscribe = function(key,domain,name) {
	if ($("#rco-"+key).prop("checked") && $.grep(subscribedWikis,function(w){return w.key==key;}).length==0) {
		...
		$("#grasprc").append('<div id="rc-'+key+'"><h3><a href="\/\/'+domain+'/Special:RecentChanges" target="_blank">'+name+'</a> <img src="https://help-it.gamepedia.com/media/help-it.gamepedia.com/d/db/Icon_delete.png" style="cursor:pointer; width:60px; position:relative; top:-3px;" onClick="$(\'#rco-'+key+'\').prop(\'checked\',false).change();" /></h3><ul></ul></div>');
...
window.grcMinTimer = function() {
	if ((new Date).getMinutes() == 10 && !grcDoneThisMinute) {
		...
		if (Math.abs(Date.parse($("#until").val())-(new Date()))<7200000) $("#until").val(new Date(new Date().setMinutes(10)).toLocaleString());
		reloadWikiList();
	}
	else {
...
window.reloadWikiList = function() {
	var since = ($("#since").val()=="")?getWikiValue("lastLoad"):Math.floor((new Date($("#since").val())).setMinutes(10)/1000);
	$("#since").val(new Date(new Date(since*1000).setMinutes(10)).toLocaleString());
	var until = ($("#until").val()=="")?Math.floor((new Date().setMinutes(10))/1000):Math.floor((new Date($("#until").val()).setMinutes(10))/1000);
	$("#until").val(new Date(new Date(until*1000).setMinutes(10)).toLocaleString());
...
mw.loader.using(['mediawiki.api','mediawiki.ForeignApi']).then(function() {
	...
	$("#grc-options").append('<br /><label for="since">Since: </label> <input type="text" id="since" /> <input type="button" id="sinceReset" value="Reset" />');
	$.datetimepicker.setDateFormatter({
	    ...
	    formatDate: function (date, format) {
	        ...
	        return new Date(new Date(date).setMinutes(10)).toLocaleString();
	    }
	});
	$("#since").datetimepicker({theme:'dark',onChangeDateTime:function(current_time,$input){reloadWikiList();},defaultTime:(new Date(new Date()-3600000).setMinutes(10))});
	$("#sinceReset").click(function() {
		$("#since").val((new Date(new Date().setMinutes(10)-7200000)).toLocaleString());
		reloadWikiList();
	});
	$("#grc-options").append('<br /><label for="until">Until: </label> <input type="text" id="until" /> <input type="button" id="untilReset" value="Reset" />');
	$("#untilReset").click(function() {
		$("#until").val(new Date(new Date().setMinutes(10)).toLocaleString());
		reloadWikiList();
	});
