...
(function ($) {
    ...
    mainRoom.model.chats.bind('afteradd', function (chat) {
        ...
        if ($.inArray('img', disable) === -1) {
            // Image
            if (t.match(/(?=.*\[img .*\])/i)) {
                $m = $m.replace(/<a.*?\>/gi, '').replace(/<\/a\>/gi, '').replace(/\[img (.*?)\]/gi, '<a href="$1"><img src="$1" style="max-height: 200px; max-width: 200px; border: 2px solid blue; border-radius: 10px; box-shadow: 0 0 20px blue inset" /></a>');
            }
        }
        ...
        if ($.inArray('sp', disable) === -1) {
            // Spoiler
            if (t.match(/(?=.*\[sp\])(?=.*\[\/sp\])/gi)) {
                $m = $m.replace(/\[sp\]/gi, '<button style="border: 2px solid blue" id="spoil">O</button>&nbsp;<span id="spoil2" style="display: none">').replace(/\[\/sp\]/gi, '</span>');
            }
        }
  ...
  if ($.inArray('xl', disable) === -1) {
            // XLink
            if (t.match(/(?=.*\[xl .*\])/i)) {
                $m = $m.replace(/\[xl (.*?) \]/gi, '<a href="$1">').replace(/\[\/xl\]/gi, '</a>');
            }
        }
