    ...
    var conf = mw.config.get( [
        ...
        self = {
                                ...
                                .append(
                                    ...
                                    $( '<div>' )
                                        ...
                                        .append(
                                            ...
                                            $( '<b>' )
                                                .text( 'Huom:' ),
                                            ' Jos lisäät tavaroita, keskiarvoa ei näytetä.'
                                        ),
                                    $( '<button>' )
                                        .text( 'Esitä' ),
                                    $( '<button>' )
            ...
            setupCharts: function () {
                ...
                $( 'div.GEdatachart' ).each( function ( c ) {
                    ...
                    if ( !isSmall ) {
                        $dataChart.before(
                            '<form action="javascript:addItem('+c+')" id="chartProperties'+c+'"><label>Average: <input type="number" min="1" id="average'+c+'" value="'+_GEC.average+'"> days</label>'+(isIndexChart?'':'<br/>Lisää tavaroita: <input type="text" id="extraItem'+c+'">&nbsp;<b>Huom:</b> Jos lisäät tavaroita, keskiarvoa ei näytetä.')+'<br/><button>Esitä</button><input type="reset" value="Reset'+(isIndexChart?'':' kaikki kentät')+'"/></form>'+(wgNamespaceNumber == 112 && wgTitle.split('/')[1] == 'Data'||wgPageName == 'Grand_Exchange_Market_Watch/Chart' ? '<a class="GEPermLink" id="GEPermLink'+c+'" title="Pysyvä linkki tämänhetkisiin tavaroihin ja asetuksiin." href="javascript:void(0)">Pysyvä linkki</a>':'')+'<div id="addedItems'+c+'"></div>'
                        );
                    }
                    ...
                    var enlarge = '<a href="javascript:popupChart(' + c + ');" style="text-decoration:underline;color:inherit;font-size:inherit;">Suurenna</a>';
                    // @todo this doesn't do anything on small charts
                    //       is it supposed to?
                    var zoomOut = '<a href="javascript:_GEC.chart' + c + '.zoomOut();" style="text-decoration:underline;color:inherit;font-size:inherit;">Pienennä</a>';
        ...
        chart = {
            ...
            addItem: function ( i, it ) {
                _GEC.chartid = i;
                var $extraItem = $( '#extraItem' + _GEC.chartid ),
                ...
                if ( item && item.length ) {
                    ...
                    if (
                        ...
                        _GEC.AIQueue.indexOf( item.toLowerCase() ) !== -1
                    ) {
                        if (!it) {
                    ...
                    $.get(
                        '/api.php',
                        {
                            ...
                            rvprop: 'content',
                            format: 'json',
                            titles: rs.luaExcg ? 'Module:Exchange/' + util.ucFirst( item ) + '/Data' :
                                'Exchange:' + item + '/Data'
                        },
                        ...
                        function ( resp ) {
                            ...
                            if ( pages[-1] ) {
                                ...
                                if (
                                    ...
                                    loadChartsQueueComplete( _GEC.chartid );
                                } else if ( !_GEC.AIData.length ) {
                                    setChartRange( _GEC.chartid );
                                }
                            ...
                            */
                            ...
                            if ( getSeriesIndex( _GEC.chartid, 'average' ) !== -1 ) {
                                _GEC['chart' + _GEC.chartid]
                                    .series[getSeriesIndex( _GEC.chartid, 'average' )]
                                    .remove();
                            }
                ...
                // @todo when does this happen
                    ...
                    id = i == 'popup' ? $( '#GEpopupchart' ).attr( 'data-chartid' ) : i;
             ...
             */
            loadQueueComplete: function ( c ) {
                var id,
                    chartdata,
                    isSmall = $( '#GEdatachart' + c ).hasClass( 'smallChart' ),
                    data = [_GEC.AIQueue.length],
                    i,
                    index,
                    itemhash,
                    $addedItems,
                ...
                if ( getSeriesIndex( _GEC.chartid, '7-päivän volyymi' ) !== -1 ) {
                    id = c === 'popup' ? $( '#GEchartpopup' ).attr( 'data-chartid' ) : c;
                    chartdata = getData( id, true );
                    chartdata[1].title.text = 'Hintahistoria';
                ...
                for ( i = 0; i < _GEC.AIData.length; i++ ) {
                    index = _GEC.AIQueue.indexOf( ( _GEC.AIData[i] || {name: ''} ).name.toLowerCase() );
                ...
                if ( isSmall && typeof _GEC.addedData[c] !== 'object' ) {
                    _GEC.addedData[c] = [];
                }

                for ( i = 0; i < data.length; i++ ) {
                    if ( data[i] ) {
                        _GEC['chart' + c].addSeries( data[i] );
                    }
...
function addPopupPriceInfo( c ) {
    ...
    '<caption onclick="$(this).next(\'tbody\').toggle();$(this).children().toggle()"><span style="display:none;white-space:nowrap">Hintatietoja</span><span>Sulje</span></caption><tbody>'+
    '<tr><th>Hinta</th><td>' + util.toKMB( curprice ) + '</td></tr>';
    for (i=0;i<priceDiffs.length;i++) {
        itemStats += '<tr><th>'+dayDiffs[i]+'-päivän</th><td style="color:'+rg(priceDiffs[i])+'">'+(priceDiffs[i]>0?'+':'') + util.toKMB( priceDiffs[i] ) + ' ('+percentDiffs[i]+'%)</td></tr>';
    }
    itemStats += $('#GEdataprices'+c).attr('data-value').length?'<tr><th><a href="/wiki/Value" target="_blank" title="Value">Arvo</a></th><td>'+util.toKMB($('#GEdataprices'+c).attr('data-value'))+'</td></tr>':'';
    itemStats += $('#GEdataprices'+c).attr('data-limit').length?'<tr><th><a href="/wiki/Grand_Exchange#Trade_Restrictions" target="_blank" title="Grand Exchange buying limit">Ostoraja</a></th><td>'+util.toKMB($('#GEdataprices'+c).attr('data-limit'))+'</td></tr>':'';
    itemStats += '<tr><th colspan="2"><a href="/wiki/Exchange:'+encodeURI($('#GEdataprices'+c).attr('data-item'))+'" target="_blank" title="'+$('#GEdataprices'+c).attr('data-item')+'">Lisää hintatietoja</a></th></tr>';
    ...
    function getData( c, isSmall, avginput ) {
        ...
        if ( !isSmall && ( itemName.toLowerCase() !== 'blank' || chartLoaded ) ) {
            ...
            dataList.push( {
                name: inputAvg + '-päivän keskiarvo',
        ...
        yAxis = {
            title: {
                text: isSmall ? null : ( isIndexChart ? 'Indeksihistoria' : 'Hintahistoria' ),
                offset: 60,
                rotation: 270,
                style: {
                    color: 'black',
