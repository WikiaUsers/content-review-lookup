...
addOnloadHook(spoilers);
function spoilers() {
    for(var i=0; spoil = document.getElementsByTagName('span')[i]; i++) {
        if ((spoil.getAttribute('class') == "spoiler-tip")) {
            spoil.innerHTML='<a href="javascript:togglespoilers()">(toggle spoilers)</a>';
        }
    }
    for(var i=0; spoil = document.getElementsByTagName('div')[i]; i++) {
        if ((spoil.getAttribute('class') == "spoiler-content")) {
            spoil.style.color='#000000';
