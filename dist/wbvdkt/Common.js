...
/* Sliding date box stuff */
$(function () {
	$('.sdb .sdb-table').appendTo('.sdb');
	$('.sdb > *:not(.sdb-table)').remove();
});
$(document).ready(function(){
  $('.sdb-first-right').click(function(){
    $('table.sdb-table').attr('style','width: 1690px; margin-left: -338px');
  });
  $('.sdb-before-left').click(function(){
    $('table.sdb-table').attr('style','width: 1690px; margin-left: 0px');
  });
  $('.sdb-before-right').click(function(){
    $('table.sdb-table').attr('style','width: 1690px; margin-left: -676px');
  });
  $('.sdb-present-left').click(function(){
    $('table.sdb-table').attr('style','width: 1690px; margin-left: -338px');
  });
  $('.sdb-present-right').click(function(){
    $('table.sdb-table').attr('style','width: 1690px; margin-left: -1014px');
  });
  $('.sdb-after-left').click(function(){
    $('table.sdb-table').attr('style','width: 1690px; margin-left: -676px');
  });
  $('.sdb-after-right').click(function(){
    $('table.sdb-table').attr('style','width: 1690px; margin-left: -1352px');
  });
  $('.sdb-last-left').click(function(){
    $('table.sdb-table').attr('style','width: 1690px; margin-left: -1014px');
  });
});

/* Deletion dropdown list */
$(function () {
   $('select#wpDeleteReasonList').html('<option value="As per [[WBVDKT Wiki:Deletion]] SD1: The page was spam">SD1: Spam</option><option value="As per [[WBVDKT Wiki:Deletion]] SD2: The page was created with the intention of abuse">SD2: Abuse</option><option value="As per [[WBVDKT Wiki:Deletion]] SD3: The page was irrelevant to this Wiki">SD3: Irrelevant</option><option value="As per [[WBVDKT Wiki:Deletion]] SD4: The page was accidently created">SD4: Accident</option><option value="As per [[WBVDKT Wiki:Deletion]] SD5: The page is identical or covers nearly exactly the same topic as another">SD5: Identical</option><option value="As per [[WBVDKT Wiki:Deletion]] SD6: The page is unused">SD6: Unused</option><option value="As per [[WBVDKT Wiki:Deletion]] SD7: The page is a TDE">SD7: TDE</option><option value="As per [[WBVDKT Wiki:Deletion]] SD8: The only editor/author requested it">SD8: Author request</option><option value="As per [[WBVDKT Wiki:Deletion]] ND1: The page involves a subject/topic not relevant enough to deserve an article">ND1: Relevance</option><option value="As per [[WBVDKT Wiki:Deletion]] ND2: The page involves a topic too similar to another article which already exists">ND2: Similar</option><option value="">Other</option>')
   /* Auto-blanking other reason on delete pages) */
   $('#wpDeleteReasonRow input').attr('value', '');
});
/* Edit button shortcut changed from Alt + E to Alt + S) */
$('a[data-id=edit]').attr('accesskey', 's');

/* Button for redlinking tables */
$('table.redlink-normal').before('<button class="toggle-redlink-normal">Toggle redlinks</button>');
$(document).ready(function(){
  $('button.toggle-redlink-normal').click(function(){
    $('table').toggleClass('redlink-normal');
  });
});

/* Quick options in recent changes */
$(function () { 
	$('fieldset.rcoptions').append('<hr><div id="mw-rc-qoptions">Quick options: <a href="/wiki/Special:RecentChanges">Normal</a> | <a href="/wiki/Special:RecentChanges?namespace=0">Articles only</a> | <a href="/wiki/Special:RecentChanges?namespace=1&invert=1">No talk pages/grades</a> | <a href="/wiki/Special:RecentChanges?namespace=8&invert=1">No MediaWiki/skins</a> | <a href="/wiki/Special:RecentChanges?namespace=10&invert=1">No templates</a><br><small>Note: These reset your current recent changes options back to the ones in <a href="/wiki/Special:Preferences">Special:Preferences</a></small></div>');
});

/* Archive styles */
$('.ns-112 div.chatlog').wrapInner('<pre></pre>');
$('.ns-112 div.chatlog img').attr('width','14','height','14');
$('.ns-112 div.chatlog pre pre').unwrap();
