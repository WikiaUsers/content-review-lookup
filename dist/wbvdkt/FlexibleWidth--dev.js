...
importCSS('http://wbvdkt.wikia.com/wiki/MediaWiki:FlexibleWidth/dev.css?action=raw&ctype=text/css');

/* **************************** */
/* *** Reorganizing buttons *** */
/* **************************** */
$(function () {
  /* Restoring normal page functions to editor */
  $('.editor section#WikiaPage').prepend('<header id="WikiHeader" class="WikiHeader"></header>');
  $('.editor article#WikiaMainContent').prepend('<header id="WikiaPageHeader" class="WikiaPageHeader"><h1></h1></header>');
  $('.editor header#EditPageHeader h1 a').html(''+ wgTitle +'').prependTo('header#WikiaPageHeader h1');
...
  $('.ns-1 #WikiaPageHeader h1').prepend('Talk:');
  $('.ns-2 #WikiaPageHeader h1').prepend('User:');
  $('.ns-3 #WikiaPageHeader h1').prepend('User talk:');
...
  $('.editor.ns-4 #WikiaPageHeader h1 a').prepend('WBVDKT Wiki:')
  $('.editor #WikiaPageHeader h1').prepend('Editing ');
});
...
function MoveEditButton() {
	$('#WikiHeader').prepend('<div id="wikia-buttons"></div>');
	$('#WikiaPageHeader .wikia-menu-button a:first').appendTo('#wikia-buttons').addClass('wikia-button');
	$('#WikiaPageHeader .comments').appendTo('#wikia-buttons').addClass('wikia-button');
		$('.comments .commentsbubble').prepend(' (').append(')').removeClass('commentsbubble');
	...
	$('.editor header#WikiaPageHeader #wikia-buttons').prepend('<a href="/wiki/' + wgPageName + '" class="wikia-button">Read</a>');
	$('.editor header#WikiHeader #wikia-buttons [data-id="history"]').after('<a href="/wiki/Special:MovePage/' + wgPageName + '" data-id="move" class="wikia-button">Move</a>');
}
...
  $('#WikiHeader > *:not(#mw-accountnavigation, #wikia-buttons)').remove();
...
/* *********************** */
$(function () {
	$('#editform').after('<div id="fw-editpreviewcontainer" style="min-height: 100px;"></div>');
	...
	/* Put categories in editor */
	$('#wpTextbox1').append($('#csWikitext').html());
	$('.module_categories').remove();
});

/* Button functions */
$(document).ready(function(){
	$('a#wpPreview').click(function(){
		$('div#fw-editpreviewcontainer').html('');
...
if (m0 == 13) {	var d0 = y0+"/01"  } 
else if (m0 < 10)  { var d0 = y+"/0"+m0 }
else { var d0 = y+"/"+m0 } 

if (m1 < 10) { var d1 = y+"/0"+m1 } 
else { var d1= y+"/"+m1 }

if (m2 == 0) { var d2 = y2+"/12"  }
else if (m2 < 10) { var d2 = y+"/0"+m2 }
else { var d2 = y+"/"+m2 }

if (m3 == 0)  { var d3 = y2+"/12"  }
else if (m3 == -1) { var d3 = y2+"/11"  }
else if (m3 < 10)  { var d3 = y+"/0"+m3 }
else { var d3 = y+"/"+m3 }

if (m4 == 0)  { var d4 = y2+"/12"  }
else if (m4 == -1) { var d4 = y2+"/11"  }
else if (m4 == -2) { var d4 = y2+"/10"  }
else if (m4 < 10)  { var d4 = y+"/0"+m4 }
else { var d4 = y+"/"+m4 }

$(function() {
	$('#month-d0').attr('href','/wiki/'+ d0).html(d0);
	$('#mw-sidebar-menuCEparent').attr('href','/wiki/'+ d1);
	$('#month-d2').attr('href','/wiki/'+ d2).html(d2);
	$('#month-d3').attr('href','/wiki/'+ d3).html(d3);
	$('#month-d4').attr('href','/wiki/'+ d4).html(d4);
});