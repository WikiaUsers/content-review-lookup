...
$.when(
	...
	var TooltipsEditor = window.TooltipsEditor = Object.assign(this, {
		...
		revertAction: function() {
			...
			mw.notify("for "+key, {title: "실행 취소", type:"info"});
			that.updateActions();
		...
		updateActions: function(keys) {
			...
			$log.append($("<li>", {text: "또한 편집을 저장하면 모든 툴팁이 글자 순으로 정렬됩니다.", "class": "actions-none"}));
			$("#TooltipsEditor-undoLog").append(ls[3]); //.prepend(ls[3])
			
			$("#TooltipsEditor-totalTooltips").html($("<p>", {
				html: [
					"툴팁 개수: "+Object.keys(this.json).length,
					$("<span>", (function() {
		...
		reset: function(confirm) {
			if (confirm) {
				...
				$("#TooltipsEditor header h3").text("툴팁 편집기");
			}
		},
			...
			this.editor.append(
				$("<div>", {
					...
					html: [
						...
						"<br>",
						$("<span>", { text: "검색 결과: ", id: "searchResultsMessage" }),
						$("<ul>", {
				...
				$("<div>", {
					...
					[
						...
						$("<fieldset>", {
							...
							html: [
								$("<legend>", { text: "도구 상자 (클릭하여 삽입할 수 있습니다)"}),
								$("<div>", {
									html: [
										$("<span>", { text: "텍스트 꾸미기" }),
						...
						$("<span>", { text: "툴팁 ID: ", "class": "TooltipsEditor-inputbox-label TooltipsEditor-label" }),
						$("<input>", { id: "TooltipsEditor-key", "class": "TooltipsEditor-inputbox" }),
						"<br>",
					],
					otherparams,
					[
						$("<span>", { text: "툴팁 제목: ", "class": "TooltipsEditor-label" }),
						$("<div>", {
							id: "TooltipsEditor-title-AceEditor"
						}),
						$("<span>", { text: "툴팁 텍스트: ", "class": "TooltipsEditor-label" }),
						$("<div>", {
