...
function updatetimer(i) {
  ...
  //var diff = Math.abs(Math.floor((then.getTime()-now.getTime())/1000));
  var tpm;
  ...
  if ((then.getTime()-now.getTime()) < 0)
    //timers[i].firstChild.nodeValue = tpm + left + ".";
    timers[i].firstChild.nodeValue = timers[i].firstChild.nodeValue;
  else 
    timers[i].firstChild.nodeValue = tpm + left + " to go until release.";
  }
