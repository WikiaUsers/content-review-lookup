...
trows = $("table.zebra > tbody > tr");
x=0;
while (trows.eq(x).children("th").length > 0) x++;
for (y=(x+1);y<trows.length;y+=2) {
if (trows.eq(y).children("th").length == 0) trows.eq(y).addClass("alt");
}
}
...
function noRCUA() {
if (wgPageName == "Special:RecentChanges") $("ul.special>li:contains('useravatar')").hide();
}
