...
/* Головна сторінка/Слайдер - початок*/
...
// *****************************************************
 ...
 //  </span>
 //  <span class="nocountdown">Javascript відключений.</span>
...
if ($tfb.html() == "") $tfb.html('<div class="tooltip-content"><b>Помилка</b><br />Підказки не існує<br />або її не повинно існувати.</div>');
...
function eLink(db,nm) {
	dbs = new Array("http://us.battle.net/wow/en/search?f=wowitem&q=","https://www.wowhead.com/database=");
...
function bindTT() {
	...
	if ($p.hasClass("selflink") == false) {
		...
		if ($p.hasClass("new")) {
			...
			for (x=y;x<z;x++) els += eLink(x,$t.data("tt").replace("Завдання:",""));
...
if ((wgPageName == "Special:Upload" || wgPageName == "Службова:Upload") && getParamValue("wpDestFile") == null) {
...
var pArr = new Array();
$.getJSON("http://uk.wow.wikia.com/w/api.php?action=query&generator=categorymembers&gcmlimit=500&gcmsort=timestamp&gcmdir=desc&gcmtitle=Категорія:Заготівки_гільдій&prop=revisions&rvprop=timestamp&format=json&callback=?", function(data) {
if (data.query) {
...
daysE = (daysE > 29)?'<span style="color:red;">('+daysE+' дня/днів)</span>':'('+daysE+' дня/днів)';
gslBuffer += '<li><a href="/wiki/'+guild+'" title="'+guild+'">'+guild+'</a> ' + daysE + ' - <a href="/wiki/'+guild+'?action=history">Історія</a> &bull; <a href="/wiki/'+guild+'?action=delete">Видалити</a></li>';
}
gslBuffer += "</ul>";
...
// AJAX RC
var ajaxPages = new Array("Special:RecentChanges","Службова:RecentChanges");
...
function ajaxRC() {
appTo = $(".firstHeading");
appTo.append('&nbsp;<span style="font-size: xx-small; border-bottom: 1px dotted; cursor:help;" title="Увімк./Вимк. режиму автоматичного оновлення сторінки">Автооновлення:</span><input type="checkbox" id="ajaxToggle"><span style="position:relative; top:5px; left:5px;" id="ajaxRCprogress"><img src="http://uk.wow.wikia.com/w/images/0/0e/Progressbar.gif" border="0" alt="AJAX виконує запит" /></span>');
...
$("#stl" + tableNum).html('[<a href="/wiki/'+tablePage+'?action=edit">правка</a>]&nbsp;[<a href="javascript:;" id="htl' + tableNum + '" onClick="hideTable(' + tableNum + ');">приховати</a>]');
ttMouseOver();
}
});
}
...
$("#stl" + i).html('[<a href="javascript:;" onClick="loadTableData(' + i + ')">показати дані</a>]');
}
});
}
...
output = "";
url = "http://uk.wow.wikia.com/w/api.php?action=query&generator=allimages&prop=duplicatefiles&gailimit=500&format=json";
...
clsesru = new Array("лицар","друїд","мисливець","маг","чернець","паладин","жрець","разбійник","шаман","чорнокнижник","воїн");
for (x=0;x<11;x++) {
...
/* + посилання «правити» для нульової секції. */
...
if (wgNamespaceNumber==14 && wgAction=="view") addPortletLink('p-views', 'javascript:createPageInCategory();', "Створити", 'ca-create-category-page', "Створити сторінку в цій категорії", '',document.getElementById("ca-history"));
if (getParamValue("category") && wgAction=="edit") $("#wpTextbox1").val("\n\n[" + "[Категорія:"+getParamValue("category")+"]]");
ttMouseOver();
addAjaxDisplayLink();
...
$(".mw-mpt-link").html("<a href='/Службова:WhatLinksHere/"+$(".firstHeading").text().replace("Move ","").replace(/'/g,"%27")+"'>Посилання на стару назву сторінки</a>");
});
