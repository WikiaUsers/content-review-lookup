...
/***************************************/
/*
* Muestra imágenes al pasar el mouse sobre un enlace que apunte a la imagen a mostrar
*
...
ImageSwitcher.prototype = {
	...
	eEnter: function(e) {
		...
		if ($UT.hasClass(linkcheck, k+'episodio')) {
			...
			url = getWikiImage('EP'+EPCode+(parseInt(EPCode, 10) > window.wgEPLastGif ? '.png' : '.gif'));
		} else {
			if ($UT.hasClass(link, 'new') || link.title.toString().indexOf(fileNS) != 0) {
				return;
			}
			url = getWikiImage(link.title.replace(fileNS, ''));
		}
	...
	setupTarget: function(imgTarget, imgWrap) {
		...
		this.targetimgs[index] = {
			...
			wrapper:imgWrap,
			preserve:preserve,
			size:[imgTarget.width,imgTarget.height],
...
//if ((wgAction == 'view' || wgAction == 'edit' || wgAction == 'submit') && wgNamespaceNumber != -1) {
