...
(function () { // Local scope to avoid polluting global namespace with declarations
  ...
  // If we're loaded through the resource loader, we may sometimes not have importScript
  // or importScriptURI. Try to work around this.

  function load (uri) {
    if (window.mediaWiki) { // Resource loader present?
      window.mediaWiki.loader.load (uri);
    } else if (typeof (window.importScriptURI) == 'function') {
      window.importScriptURI (uri);
    } else {
...
})(); // End local scope, invoke function. We use a separate scope to keep localizations separate from the rest.
...
(function () {
  ...
  function run () {
    ...
    HotCat.started = true;
    if (window.is_safari) {
      // Safari/Mac calls initialize before the load of /local_defaults (and /wgUserLang) are done. Let's wait a bit.
      // (This is a hack.)
