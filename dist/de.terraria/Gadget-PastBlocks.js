...
var tip = "Öffne das Sperr-Logbuch dieses Benutzerkontos";

$('#bodyContent a.mw-anonuserlink').each(function(){
  var $this = $(this);
  var user = $this.attr('href').replace(/\/(Special:Contributions|Spezial:Beiträge)\//, '');
  $.getJSON(urlPre + '/api.php?format=json&action=query&list=logevents&leaction=block/block&leprop=user&letitle=User:' + user, function(data){
    ...
    if (numBlocks > 0){
      var plural = numBlocks > 1 ? 'n' : '';
      var logLink = urlPre + '/index.php?title=Special:Log/block&page=User:' + user;
      $this.after('<small> (<b>' + numBlocks + '</b> <a href="' + logLink + '" title="'+ tip +'">Sperre' + plural + '</a>)</small>');
    }
  });
});
...
function markBlocked( container ) {
	...
	var userTitleRX = new RegExp( '^(' + userNS.join( '|' ) + '|(Special:Contributions|Spezial:Beiträge)\\/)([^\\/#]+)$', 'i' );
