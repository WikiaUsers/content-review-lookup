...
var descriptionString = new String("Ta lista zawiera %d obiektów"); //%d oznacza liczbę obiektów
	...
	while(si) {
		...
		if (tn == "ul" || tn == "ol") {
			if (depth > 1) {
				...
				si.parentNode.firstChild.appendChild(button("pokaż wszystkie", allonexec, "listAllButton"), si);
				si.parentNode.firstChild.appendChild(document.createTextNode(')'), si);
				$("#listAllButton").click(function () {
				    if ( $("#listAllButton").html() === "pokaż wszystkie") {
				        $("#listAllButton").html("ukryj wszystkie");
				        $("#listAllButton").attr("href","javascript:" + allonexec);
				    }
				    else {
				        $("#listAllButton").html("pokaż wszystkie");
