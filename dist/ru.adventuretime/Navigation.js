...
var NavigationTitleHide = 'скрыть контент';
var NavigationTitleShow = 'показать контент';
 
// adds show/hide-button to collapsible navigation
function collapsible_navigation()
{
	$('div.NavFrames').each( function() {
		...
		$that.children('.NavHeads').each( function() {
			$(this).append( '<a class="NavToggle">' + which + '</a>' ).click( function() {
				...
				if ( which ) {
					$this.attr( 'title', NavigationTitleShow )
						.siblings( ':not(.NavHeads)' ).stop( true, true ).fadeOut();
				} else {
					$this.attr( 'title', NavigationTitleHide )
						.siblings( ':not(.NavHeads)' ).stop( true, true ).fadeIn();
				}
			}).click();
		});
	});
