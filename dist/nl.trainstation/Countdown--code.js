...
;(function (module, mw, $, undefined) {
	...
	function output (i, diff) {
		...
		delta = diff % 60;
		result = ' ' + i18n[delta === 1 ? 'seconde' : 'seconden'];
		if (countdowns[i].opts & SHORT_FORMAT) result = result.charAt(1);
		parts.unshift(delta + result);
		diff = Math.floor(diff / 60);
		delta = diff % 60;
		result = ' ' + i18n[delta === 1 ? 'minuut' : 'minuten'];
		if (countdowns[i].opts & SHORT_FORMAT) result = result.charAt(1);
		parts.unshift(delta + result);
		diff = Math.floor(diff / 60);
		delta = diff % 24;
		result = ' ' + i18n[delta === 1 ? 'uur'   : 'uren'  ];
		if (countdowns[i].opts & SHORT_FORMAT) result = result.charAt(1);
		parts.unshift(delta + result);
		diff = Math.floor(diff / 24);
		result = ' ' + i18n[diff  === 1 ? 'dag'    : 'dagen'   ];
		if (countdowns[i].opts & SHORT_FORMAT) result = result.charAt(1);
		...
		if (parts.length) {
			if (countdowns[i].opts & SHORT_FORMAT) {
				...
				result = parts.join(', ') + ' ' + i18n.en + ' ' + result;
			}
		}
