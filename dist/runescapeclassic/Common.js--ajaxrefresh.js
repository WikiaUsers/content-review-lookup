 ...
 */
function arSetCookie( c_name, value, expiredays ) {
	var exdate = new Date();
...
function preloadAJAXRL() {
	var	ajaxRLCookie = ( arGetCookie( "ajaxload-" + wgPageName ) == "on" ) ? true : false,
	...
	if ( arGetCookie( "ajaxload-" + wgPageName ) == "on" ) {
		loadPageData();
	}
}
...
function toggleAjaxReload() {
	if ( $( '#ajaxToggle' ).prop( 'checked' ) === true ) {
		arSetCookie( "ajaxload-" + wgPageName, "on", 30 );
		doRefresh = true;
		loadPageData();
	} else {
		arSetCookie( "ajaxload-" + wgPageName, "off", 30 );
		doRefresh = false;
