...
(function (module, mw, $, window) {
    ...
    var translations = {
        be: {
            contents: 'Змест',
            toc: 'Вярнуцца да зместу',
            open: 'Разгарнуць',
            close: 'Зачыніць',
            back: 'У пачатак старонкі',
            reset: 'Абнавіць'
        },
        ca: {
            contents: 'Continguts',
    ...
    function prepareScript(table) {
        if (table.querySelector('ol').children.length == 0) {
            // TOC is loaded but its list has not been populated yet
            setTimeout(function(){
                prepareScript(table);
            }, 500);
        } else {
    ...
    $(function () {
        var table = window.document.getElementById('toc');
        if (table != null) {
            prepareScript(table);
        }
    });
