...
function connectionCheckTimeout(testStr) {
	if (!chatOptions.options.timeoutRetry) chatOptions.options.timeoutRetry = 30;
	if (!chatOptions.options.timeoutRetries) chatOptions.options.timeoutRetries = 2;
	if (!chatOptions.options.timeoutTest) chatOptions.options.timeoutTest = 60;
	if (chatOptions.options.timeoutRetries < 2) chatOptions.options.timeoutRetries = 2;
	if (chatOptions.options.timeoutTest < 15) chatOptions.options.timeoutRetries = 15;
	if (!chatOptions.options.autosave) chatOptions.options.autosave = 5;
	...
	if (typeof testStr != "undefined") {
		...
		if (!window.DisconnectionCheck.reconnecting && typeof ChatRefresher == "undefined" && (window.DisconnectionCheck.retry >= chatOptions.options.timeoutRetries*1) || (!$("#WikiChatList #user-"+wgUserName).length)) {
		...
		if (!window.DisconnectionCheck.reconnecting && window.DisconnectionCheck.retry && window.DisconnectionCheck.retry < chatOptions.options.timeoutRetries*1) {
	...
	window.DisconnectionCheck.Timer = setInterval('connectionCheckTimeout()', chatOptions.options.timeoutRetry * 5000);
...
debug452("reset", "debugreset");
	...
	if (new Date().getTime() - window.lastSaved > chatOptions.options.autosave*60000) {
	    saveHistory();
...
function reconnectPopup() {
	...
	+"function RefresherCheck() {"
	  +"if (window.opener != null) {"
	    ...
	    +"};"
	  +"};"
	  +"if (RefresherChecks++ >= "+chatOptions.options.timeoutTest+") {"
