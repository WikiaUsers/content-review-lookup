...
// Add inline IRC capability for Street Fighter Fight Night
GetIrc = function() {
  var irc_div = document.getElementById('irc');
  if (irc_div) {
    var irc_channel = irc_div.innerHTML;
    if (wgUserName) {
      var irc_nickname = wgUserName.replace(/[^a-zA-Z 0-9]+/g,'');
    } else { irc_nickname = 'WikiaGuest' + (Math.random() * 100); }

    irc_div.innerHTML = "<iframe id='ircframe' frameborder='0' vspace='0' hspace='0' marginwidth='0' marginheight='0' width='100%' scrolling=yes height='500px'></iframe>";

    var ircFrame = document.getElementById("ircframe");
    var doc = ircFrame.contentDocument;
    doc.open();
    doc.write("<form method='post' action='http://irc.wikia.com/irc.cgi' name='loginform'><input type='hidden' name='interface' value='nonjs'><input type='hidden' name='Nickname' value='" + irc_nickname + "'><input type='hidden' name='Channel' value='" + irc_channel + "'></form>");
    doc.close();
    doc.loginform.submit();
  }
}

YAHOO.util.Event.onDOMReady(GetIrc);
...
function ouvreChat()
{
  ...
  for(var i = 0; i < span.length; i++)
  {
    ...
    a.onclick = function() {
   window.open('/wiki/Special:Chat?useskin=wikia', 'fr-lostpedia', 'width=600,height=600,menubar=no,status=no,location=no,toolbar=no,scrollbars=no,resizable=yes');
   return false;
    };
  }
}
