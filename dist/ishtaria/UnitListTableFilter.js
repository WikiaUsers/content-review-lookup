function filterTable() {
	$("table.filterable").each(function() {
		...
		$(this).find("tr:first-child th, tr:first-child td").each(function() {
			if (!$(this).hasClass("unfilterable")) {
				...
				$(this).append($('<div class="filterMenu" style="position:absolute;top:'+$(this).height()+'px;left:0;width:'+(25+l*8)+'px;text-align:left;padding:3px;border:1px #000 solid;background:#ccc;z-index:1;display:none"></div>'));
				for (j=0; j<cols.length; j++) {
					$(this).find(".filterMenu").append('<div><input type="checkbox" value="'+cols[j]+'" col="'+(i+1)+'" class="filterOption" checked>'+ucFirst(cols[j])+'</div>')
				}
			}
			i++;
		});
	...
	$(".filterOption").click(function() {
		...
		$(this).closest("table").find("tr:nth-child(n+1)").each(function() {
			if ($(this).find("td:nth-child("+col+")").hasClass(val) || $(this).find("td:nth-child("+col+")").text()==val) {
...
function arrayUnique(a) {
    return a.sort(customCompare).reduce(function(p, c) {
