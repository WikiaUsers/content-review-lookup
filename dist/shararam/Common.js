...
// ! Штука для особо важных сообщений, которую лучше не трогать без необходимости.
// Необходимостью я считаю выборы. И всё.
// $("#WikiHeader").html($("#WikiHeader").html() + '<table style="padding: 5px; width: 100%; border: 2px solid orange; background-color: yellow;"><tr><td><div align="left" style="text-align: justify; font-face: Tahoma; font-weight: bold;">У НАС ВЫБОРЫ!<br>Это тестовое оповещение<br>На ШараВики проходят выборы консулов. Проголосовать можно <a href="http://ru.shararam-smeshi.wikia.com/wiki/Участник:Enetrel_SP">здесь</a>.</div></td></tr></table>');
// Оповещение на случай анонимного дня
// $("#WikiHeader").html($("#WikiHeader").html() + '<table style="padding: 5px; width: 100%; border: 3px solid orange; background-color: #FFC938;"><tr><td><div align="left" style="text-align: justify; font-face: Soup; font-weight: bold;">Сегодня Анонимный день!<br>Это значит, что любой человек может без регистрации редактировать разрешённые для этого статьи (большинство), комментировать, общаться на форуме и вообще пользоваться частью возможностей вики без регистрации!</div></td></tr></table>');

/* Импорт скриптов */
/* Всё "импортное" прописывать тут */
importArticles({
    type: "script",
    articles: [
...
/* Все плашки-титулы теперь правятся в MediaWiki:ProfileTags */
