var tokentravdialogues = document.querySelectorAll('.tokentravdialogue');
var options = {};
var tokentravNPCName = {};
var dialogueText = {};
var textSounds = {};
var endSounds = {};
var startSounds = {};
...
function append_button(v, dialoguename) {
	...
	btn.classList.add('tokentravdialogueoption');
	btn.innerHTML = '<div class="tokentravdialogueoptiontext">' + getKeywordsFor(v.option) + '</div>';
	if (v.btnstyle) {
	    var btnText = btn.querySelector('.tokentravdialogueoptiontext');
		...
		if (current.question !== undefined && typeof (current.answers) == "object" && typeof (current.answers[0]) != "string") {
			...
			var actions = {
			    ...
			    fishbrap: function () {
			    	...
			    	fish.src = 'https://static.wikia.nocookie.net/image here';
			    	var brap = document.createElement('audio');
			    	brap.style.display = 'none';
			    	brap.src = 'https://static.wikia.nocookie.net/sound';
...
function create_dialogue(tokentravdialogue, dialoguename) {
	dialogues[dialoguename] = tokentravdialogue;
	dialogueNPCName[dialoguename] = tokentravdialogue.querySelector('.tokentravdialoguenpcname');
	dialogueText[dialoguename] = tokentravdialogue.querySelector('.tokentravdialoguetext');
	optionsList[dialoguename] = tokentravdialogue.querySelector('.tokentravdialogueoptionscontainer');
	restartMessage[dialoguename] = tokentravdialogue.querySelector('.tokentravdialoguerestart');
	
	var textSound = document.createElement('audio');
	textSound.src = 'https://static.wikia.nocookie.net/put audio thing here';
	...
	endSound.src = 'https://static.wikia.nocookie.net/put audio thing here';
...
if (tokentravdialogues.length) {
	for (var i = 0; i < tokentravdialogues.length; i++) {
		var tokentravdialogue = tokentravdialogues.item(i);
		try {
			create_dialogue(tokentravdialogue, 'dialogue' + i)
		} catch (error) {
			tokentravdialogue.querySelector('.tokentravdialoguetext').innerHTML = error.stack;
		}
	}
}