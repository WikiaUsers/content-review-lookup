...
var messageWallUserTags = {
    ...
    'DH2000': 'Tester',
    'FairySina': 'Administratorin',
    // erst Username  :  Text hinter Username
};
 
(function($) {
    for (var name in messageWallUserTags) {
        $('a.subtle[href$="Nachrichtenseite:' + name + '"]').after('<span style="color:white;background: -moz-linear-gradient(top, #0a0 25%, #070 75%); background: -webkit-linear-gradient(top, #3c3 25%, #3a3 75%);border-radius:10px;padding:2px 5px;font-size:9pt;font-weight:bold;vertical-align:top;">' + messageWallUserTags[name] + '</span>');
    }
}(jQuery));
