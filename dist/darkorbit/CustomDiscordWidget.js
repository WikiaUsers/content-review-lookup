/* retrieve json file */
$.getJSON("https://discordapp.com/api/guilds/564508076456148992/widget.json", function(json) {
            ...
            $(widgThemeDark).addClass('widget widget-theme-dark')
                ...
                $(widgFooter).addClass('widget-footer')
                    ...
                    $(invitelink).addClass('widget-btn-connect')
                                 .attr("href", "https://discord.gg/XDk6QGK")
                                 .attr("target", "_blank")
                                 .text("Connect")
                                 .appendTo(widgFooter);
    ...
    */
    ...
    if (json.members[j].bot)
        continue;
    var isVoice = false;
    
    if (json.members[j].hasOwnProperty('channel_id')) {
        if(json.members[j].channel_id == "572897344803307540") //check if channel_id of voice channell exist and increments voice users counter
        howmanyvoice++;
        isVoice = true;
        } else {
            isVoice = false;
