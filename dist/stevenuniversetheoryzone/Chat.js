...
setInterval(function () {
    ...
    $('.username').each(function () {
        if (this.innerHTML.match(/GeneralObsanite/)) {
            $(this).parent().addClass('founder').removeClass('chat-mod');
        }
        if (this.innerHTML.match(/Perlen297|Fudgecreep/)) {
            $(this).parent().addClass('bureaucrat').removeClass('chat-mod');
        }
        if (this.innerHTML.match(/JinxBinx|AndyToons/)) {
            $(this).parent().addClass('admin').removeClass('chat-mod');
        }
        if (this.innerHTML.match(/Sticks the Badger|Sharayna/)) {
            $(this).parent().addClass('supadmin').removeClass('chat-mod');
        }
        if (this.innerHTML.match(/Lan Mikoto/)) {
...
setInterval(function () {
    "use strict";
    $('.username').each(function () {
      if (this.innerHTML.match(/GoopHug/)) {
            $(this).parent().addClass('moderator');
        }
        if (this.innerHTML.match(/Lt Pudding/)) {
