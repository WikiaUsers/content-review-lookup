...
mw.hook('wikipage.content').add(function() {
    ...
    if (wikiaGallery) {
        // Use classList for better management than directly setting className
        // which would remove all other existing classes.
    ...
    // IMPORTANT: Update these arrays if you add or remove portal slides
    const portalSlideIds = [
        "hub",
        "Okami",
        "Okamiden"
        // Add any other actual slide IDs here
    ];

    // These are the specific IDs that map to a distinct background class on .portal.navigation
    // If a slide doesn't change the background, don't include its ID here.
    const backgroundMappingIds = [
        "hub",       // Assuming 'hub' has a default background or a specific one
        "Okami",
        "Okamiden"
    ...
    function updateSlide(newSlide) {
        ...
        if (portalNavigation) {
            ...
            // Add the new background class ONLY if it's in backgroundMappingIds
            if (backgroundMappingIds.includes(newId)) {
                portalNavigation.classList.add(newId);
            } else {
                // If the new slide ID doesn't have a specific background,
                // default to 'hub' or remove all specific backgrounds.
                portalNavigation.classList.add("hub"); // Default to hub background if not mapped
            }
        }
    ...
    // Initialize the first slide to 'hub' if no active slide is set by default
    // and if the portal navigation elements exist.
    const initialHubSlide = document.getElementById("hub");
    const portalNavigationContainer = document.querySelector(".portal.navigation");

    if (initialHubSlide && portalNavigationContainer && !document.querySelector(".portal__content__slide.active")) {
        initialHubSlide.classList.add("active");
        portalNavigationContainer.classList.add("hub"); // Ensure "hub" class is present initially
    }


    // --- Handler for clicking a game icon from the hub (prevents default link) ---
    // *FIXED*: Changed .portal__navigation__aux to .portal__contents__aux
    const hubGameIconLinks = document.querySelectorAll(".portal__contents__aux a");
    ...
    hubGameIconLinks.forEach(link => {
        link.addEventListener("click", function(e) {
            ...
            // *FIXED*: Changed .portal__navigation__aux to .portal__contents__aux
            const parentAuxDiv = this.closest(".portal__contents__aux");
            if (parentAuxDiv) {
    ...
    // *FIXED*: Changed .portal__navigation__navigator to .portal__content__navigator
    ...
    if (nextButton) {
        nextButton.addEventListener("click", function() {
            ...
            let newSlide;
            // Loop from the last slide back to the first (hub)
