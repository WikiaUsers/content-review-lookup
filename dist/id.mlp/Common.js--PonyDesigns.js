...
$(function() {
    ...
    function loadPage() {
        ...
        $.getJSON(
            '/id/api.php?action=query&prop=revisions&rvlimit=1&rvprop=content' +
    ...
    function parseText(text, which) {
        ...
        var types = ['Kristal', 'Bumi', 'Pegasus', 'Unikorn'];
        var freqs = ['always', 'usually', 'sometimes'];
        
        $.each(text.split('\n'), function(i, line) {
            if (!line || line.charAt(0) !== '*') {
                if (line.trim()) {
                    skipped[i.toString()] = [line, 'garis awal yang tidak valid'];
                }
                return true;
            }
            ...
            if (line.charAt(1) !== '*') {// One bullet = style
                ...
                while (designs[which][style] !== undefined) {
                    style = 'new-' + style;
                    skipped[i.toString()] = [
                        line, 'nama gaya duplikat (tidak dilewati)'
                    ];
                }
                designs[which][style] = {};
                ...
                if (page.indexOf('[') !== -1) {
                    ...
                    if (page) {
                        ...
                        skipped[i.toString()] = [line, 'tautan tidak valid'];
                        return true;
                    }
                ...
                if (species.indexOf(',') !== -1) {
                    ...
                    if (ucfirst(species[1].trim()) === 'Anak kuda') {
                        isFoal = true;
                    } else {
                        skipped[i.toString()] = [line, 'jenis poni tidak valid'];
                        return true;
                    }
                    species = species[0];
                }
                if (types.indexOf(species) === -1) {
                    skipped[i.toString()] = [line, 'spesies poni tidak valid'];
                    return true;
                }
                if (designs[which][style][page] !== undefined) {
                    skipped[i.toString()] = [line, 'duplikat poni dalam tipe'];
                    return true;
                }
                designs[which][style][page] = {
                    'link': pony,
                    'species': species,
                    'age': (isFoal ? 'Anak kuda' : 'Dewasa'),
                    'tags': {
                        'freq': 'always',
                        'sets': ''
                    }
                ...
                if (freqs.indexOf(tags[0]) === -1) {
                    skipped[i.toString()] = [line, 'tag frekuensi tidak valid'];
                } else {
                    designs[which][style][page].tags.freq = tags[0];
                }
                if (tags.length > 1) {
                    if (tags[1] !== 'set1' && tags[1] !== 'set2') {
                        skipped[i.toString()] = [line, 'tag set tidak valid'];
                    } else {
                        designs[which][style][page].tags.sets = tags[1];
                    }
                }
            } else {
                skipped[i.toString()] = [line, 'format garis tidak valid'];
                return true;
            }
        });
    ...
    function printSkipped(skips, type) {
        var text = '\n\n' + ucfirst(type === 'marks' ? 'Tanda Bakat' : type) + ':';
    ...
    function analyzeCharacter(target, pronoun) {
        ...
        $.each(matches, function(matchPony, matchList) {
            ...
            if (hierarchy.indexOf(matchRank) !== -1) {
                ...
                errors['match-sorting'] = [matchRank, 'jenis pencocokan tidak teridentifikasi'];
            }
        });
        ...
        if (output) {
            ...
            errors['pony-search'] = [target, 'poni tidak ditemukan'];
        }
        // Scale to content
    ...
    (function init() {
        ...
        $div.append($('<button />', {
            ...
            'html': 'Outputnya akan ditampilkan di sini.',
