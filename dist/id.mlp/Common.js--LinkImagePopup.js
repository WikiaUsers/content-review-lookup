...
mw.loader.using('mediawiki.util').then(function() {
    ...
    var names = {}, cacheEnabled = (window.linkImagePopupCachingDisabled ? false : true), pendingPages = {},
      categories = ['Poni alikorn', 'Antagonis', 'Karakter latar belakang', 'Poni Kristal',
      'Poni bumi', 'Karakter Equestria Girls', 'Karakter perempuan', 'Anak kuda', 'Karakter laki-laki',
      'Karakter non-poni', 'Makhluk', 'Naga', 'Griffon', 'Hewan peliharaan', 'Poni pegasus', 'Karakter pendukung',
      'Poni unikorn', 'Keluarga Apple',//character categories
      'Episode musim 1', 'Episode musim 2', 'Episode musim 3', 'Episode musim 4',
      'Episode musim 5', 'Episode musim 6', 'Episode musim 7', 'Episode musim 8',
      'Episode musim 9',//episode categories
      'Lokasi',//location categories
      ], exceptions = ['Alikorn', 'Poni Kristal', 'Poni bumi', 'Pegasus',  'Unikorn', 'Anak kuda',
      'Poni latar belakang', 'Hewan', 'Manusia', 'Benda mati', 'Makhluk', 'Naga', 'Griffon',
      ],//included pages with no representative image (also detected automatically)
      lists = ['poni komik', 'Karakter Equestria Girls', 'karakter non-poni',
      'poni/Poni alikorn', 'poni/Poni Kristal', 'poni/Poni bumi/Kuda betina', 'ponies/Poni bumi/Kuda jantan',
      'poni/Anak kuda', 'poni/Poni yang disebutkan', 'poni/Poni pegasus', 'poni/Poni unikorn',
      'poni komik/Poni bumi', 'poni komik/Poni pegasus', 'poni komik/Poni unikorn',
      'poni komik/Poni alikorn', 'poni komik/Poni Kristal',
    ...
    if (typeof window.sessionStorage !== 'undefined' && cacheEnabled &&
      typeof window.sessionStorage.linkPopupStorage !== 'undefined') {
        ...
        console.log('Data LinkPopupImage dimuat dari penyimpanan browser.');
    } else {
        ...
        $.each(categories, function(index, value) {
            pending += 1;
            $.getJSON('/id/api.php?action=query&list=categorymembers&cmtitle=Kategori:' + enc(value) +
              '&cmprop=title&cmtype=page&cmlimit=max&format=json', function(data) {
                data = data.query.categorymembers;
                $.each(data, function(idx, val) {
                    if (
                      ! names[val.title] &&
                      val.title.substring(0, 8) !== 'Daftar ' &&
                ...
                if (pending === 0) {
                    ...
                    $.each(lists, function(index2, value2) {
                        pending2 += 1;
                        $.getJSON('/id/api.php?action=parse&page=Daftar_' + enc(value2) +
                          '&prop=wikitext&format=json', function(data2) {
                            data2 = data2.parse.wikitext['*'];
                            var re = /\|\-\n\|.*\{\{[Cc]ellanchor\|([^\}]*)\}\}.*\[\[Berkas\:(.*(?:png|jpg)).*\n/g;
                            var match;
    ...
    $('#mw-content-text a').on('mouseover', function(e) {
        ...
        if (name.substr(0, 8) === 'Daftar ' && name.indexOf('#') !== -1) {
        ...
        if (! names[name]) {
            ...
            $.getJSON('/id/api.php?action=parse&page=' + enc(name) + '&section=0&prop=wikitext&format=json',
              function(data) {
                ...
                if (name.indexOf(' and ') !== -1) {
                    var iboxMatch = data.match(/\{\{[Ii]nfobox karakter/g);
                ...
                if (file2) { result += '|Berkas:' + file2; }
                $.getJSON('/id/api.php?action=query&prop=imageinfo&titles=Berkas:' + enc(result) +
            ...
            pendingPages[name] = true;
            $.getJSON('/id/api.php?action=query&prop=imageinfo&titles=Berkas:' + enc(names[name].file) +
