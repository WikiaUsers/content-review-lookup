importArticles({
    ...
    articles: [
        ...
        'MediaWiki:Wikia.js/test.js'
    ]
});

// This code is to revive [[Template:USERNAME]]
window.onload = function(){
    var username = document.getElementsByClassName('insertusername');
    for (var i = 0; i < username.length; i++){
       username[i].innerHTML= wgUserName;
    }
};
...
if (mw.config.get("wgPageName") == "KingdomHearts3DDDD_Wiki:Chat/Logs") {
...
$(document.defaultView || window).on('DOMContentLoaded', function(evt){
    var $grid_wrapper_data = $('div.grid-wrapper-d'),
        $grid_header_data = $grid_wrapper_data.children('h3.grid-header-d'),
        $grid_section_data = $grid_wrapper_data.children('div.grid-section-d'),
        $grid_items = $grid_section_data.children('span.grid_data');
    var $grid = $('<section />', { 'class': 'PortalGrid GridWrapper grid-wrapper', 'id': ($grid_wrapper_data.attr('data-id') || ''), 'data-title': ($grid_header_data.text() || '')} );
    if ($grid_header_data.length) $grid.append($('<header />', { 'class': 'PortalHeader GridHeader grid-header', html: $('<h2 />', { 'class': 'grid-heading', text: $grid_header_data.text() }) }) );
    $grid_items.each(function($item, index){
        var grid_image;
    });
});