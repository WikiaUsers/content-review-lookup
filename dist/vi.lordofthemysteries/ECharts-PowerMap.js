...
!(function(window, $) {
  ...
  // used different quote for in-function-variables, e.g. position: "function (point, params, dom, rect, size) {return [point[0], '10%'];}"
      if (typeof value === 'string' && (value.startsWith('function') || value.startsWith('(function'))) {
        return new Function('return (' + value + ')')();
      }
      if (typeof value === 'string' && value.startsWith('placeHolder_')) {
        const path = value.replace('placeHolder_', '');
        return path.split('.').reduce((acc, part) => acc?.[part], reviver.context);
      }
      return value;
    }
	  ...
	  // });
            ...
            `
            {
                title: {
                    ...
                    legend: [
                        {
                            // selectedMode: 'single',
                            data: "placeHolder_graphData.categoriesNames"
                        }
                    ],
                    ...
                    series: [
                        {
                            ...
                            layout: 'none',
                            data: "placeHolder_graphData.nodes",
                            links: "placeHolder_graphData.links",
                            categories: "placeHolder_graphData.categories",
                            roam: true,
                            label: {
                                position: 'right',
                                formatter: '{b}'
        ...
        try {
          reviver.context = {
                              ...
                              graphData: {
                                ...graphData,
                                categoriesNames: graphData.categories.map(a => a.name)
                              }
                            };
//           const reviverreviverSafe = reviver({ graphData });
          // Parse JSON with JSON5 { foo: 'bar' } -> { "foo": "bar" }
          //option = JSON5.parse(text)
          console.log('ECharts optionDummy:', optionDummy);
          option = JSON5.parse(optionDummy, reviver);
          console.log('Parsed ECharts option:', option);
        } catch (e) {
          $this.append(
            $('<pre>', {
              class: 'error',
