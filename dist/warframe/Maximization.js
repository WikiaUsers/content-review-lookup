...
//Last stable: https://jsfiddle.net/TunaInABottle/47enrfuc/1151/

$(function() {
  var inputTableClass = $(".maximization_warframe_input");
  var outputTableClass = $(".maximization_warframe_output");
  // Warframes
  var AshMap = {
    shuriken: [
  ...
  var EquinoxMap = {
    ...
    rest_and_rage: [
      ...
      ["equinox2_rng1", "Area range"],
      ["equinox2_rng2", "Cast range"],
      ["equinox2_eff", "Energy"]
    ],
    calm_and_frenzy: [
  ...
  var IvaraMap = {
    ...
    empowered_quiver: [
      ...
      ["ivara1_aug_str2", "Chance to resist status"]
    ],
    navigator: [
  ...
  function formula_loader(idCall, meta_array, option) {
      	//console.log("formula_loader: calling:" idCall);
        // TODO avoid formula_loader invokation party
    var i, j;
    if (option) { //required for warframes that have multi-state abiiities (e.g. Elemental Ward)
      value_assignment(idCall, meta_array, option);
    }
    ...
    function value_assignment(idCall, meta_array, option) {
      ...
      // add option to id if present (TODO remove when implementation will be indipendent from form)
      if (option) {
      	idName = idName + option + "_"
      }

    /*
    // REMOVE when testing is over
     $("#maximization_test").html(inputTableClass.attr("id").includes("chroma"));

      // If the id of the class has idCall in it
      // TODO idcall should be a string, not an array
      if ( inputTableClass.attr("id").includes(idCall[idCall.length - 1]) ) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      rows[2 * i] = $('<tr>', { //every ability/augment name goes in even rows
        append: [
          $('<td>', {
            append: [
            generateTooltip("ability", abilities[i])
            ]
          }),
          $('<td>', {
            append: [
            generateTooltip("mod", augments[i])
            ]
          })
        ]
      });
  ...
  function augmentGenerator(map, ability, augment, option) {
    ...
    return $('<table>', {
      append: [
        $('<tr>', {
          append: [
            generateTooltip("ability", ability),
            $('<td>', {
              text: augment
            })
          ]
  ...
  // makes the tooltip of the ability or mod and adds a link to the corrispective page.
  function generateTooltip(type, name) {
  	var link = name.replace(/ /g, "_");
  	var cssClass = "";
    if (type == "ability" || type == "mod" ) {
    	cssClass = type;
    } else {
  		throw new Error("generateTooltip: improper type given, expected: ability or mod, received: " + type);
    }
    return '<span class="' + cssClass + '-tooltip" data-param="' + name + '" style="white-space:pre"><a href="/wiki/' + link + '">' + name + '</a></span>';
  }
