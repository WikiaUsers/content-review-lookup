...
$( function() {
	$( '.mw-rollback-link > a' ).after( $( '<span>' ).addClass( 'edit-rollback' ).prop( 'title', 'Editar resumen de la reversión' ) );
		...
		if ( $( this ).parent().is( $rollback.parent() ) ) {
			...
			$rollback = $( '<div id="rollback-summary">' ).append(
				$( '<input type="text">' ).addClass( 'mw-ui-input rollback-text' ).prop( { maxlength: 250, spellcheck: true } ).val(
					'Reviertiendo ediciones de [[Especial:Contribuições/' + name + '|' + name + ']] ([[Usuário Discussão:' + name + '|disc]])'
				),
				$( '<input type="button">' ).addClass( 'mw-ui-button mw-ui-constructive rollback-submit-button' ).val( 'Reverter' )
			).insertAfter( this );
		}
