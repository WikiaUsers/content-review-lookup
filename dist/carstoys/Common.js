...
function ImageSwitch() {
	if(document.getElementById('imageswitch-button')) {
		document.getElementById('imageswitch-button').onclick = function() {
			if(document.getElementById('imageswitch-image1').style.display == 'none') {
				document.getElementById('imageswitch-image1').style.display = 'block';
				document.getElementById('imageswitch-image2').style.display = 'none';
			}
			else {
				document.getElementById('imageswitch-image1').style.display = 'none';
				document.getElementById('imageswitch-image2').style.display = 'block';
			}
		}
	}
}
...
/* Cool Section Gliding Stuff */


mw.loader.load( 'https://github.com/kswedberg/jquery-smooth-scroll/raw/master/src/jquery.smooth-scroll.js' );
$( '#WikiaPage a[href^=carstoys.wikia.com/wiki/Lightning_McQueen' ).smoothScroll();



( function ( mw ) {
if ( mw.config.get( 'wgPageName' ) === 'User:Iopl3887' ) {
var time = new Date().getTime(), text = document.createElement( 'p' );
text.id = 'time-text';
if ( time > 13 && time < 17 ) {
textContent = 'Hey';
...
textContent = 'We love nighttime too';
}
text.textContent = textContent + ', ' + mw.user.name();
document.getElementById( 'mw-content-text' ).appendChild( text );
}
}( mediaWiki ) );


var option_value = document.getElementById('mater').value; if (option_value == "Mater") { console.log("http://carstoys.wikia.com/wiki/Mater"); }