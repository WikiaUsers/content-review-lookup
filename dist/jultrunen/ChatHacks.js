...
if(wgPageName == 'Special:WikiActivity') {
			...
			for(var i = 0; i < pings.length; i++) {
				if(text.toLowerCase().indexOf(pings[i].toLowerCase()) != -1 || this != mainRoom.viewDiscussion) {
					...
					if(this == mainRoom.viewDiscussion) {
						...
						this.chatUL.children().last().children('.message').html(text.replace(phrase, '<span style="color:red;" title="' + phrase + '" ha sido nombrado.">' + phrase + '</span>'));
					}
					break;
				}
			}
	...
	NodeChatController.prototype.setAway = function (msg){
		...
		this.inlineAlert('Ahora tu estado es "Ausente"');
	...
	NodeChatController.prototype.setBack = function (){
		if( ! this.comingBackFromAway ){ // if we have sent this command (but just haven't finished coming back yet), don't keep spamming the server w/this command
			...
			this.inlineAlert('Has dejado de estar "Ausente"');
			this.socket.send(setStatusCommand.xport());
		}
	}
	...
	function Preparse(input) { //Parse slash commands.
		...
		switch(text[0]) {
			...
			case '/help': //Walrus
				...
				switch(text[1]) {
					case 'me':
						this.inlineAlert('Te nombras a ti mismo en tercera persona.');
						this.inlineAlert('Ejemplo: <span onclick="mainRoom.viewDiscussion.getTextInput().val(\'/me ama el chat\')">/me ama el chat</span>');
						break;
					case 'nc':
						this.inlineAlert('Link your message to <a href="http://inciclopedia.wikia.com/wiki/A_nadie_le_importa">Inciclopedia:A nadie le importa</a>.');
						this.inlineAlert('Ejemplo: <span onclick="mainRoom.viewDiscussion.getTextInput().val(\'/nc Me gusta\')">/nc Me gusta</span>');
						break;
					case 'away':
					case 'afk':
						this.inlineAlert('Te cambia el estado a "Ausente" o te lo quita.');
					...
					case 'private':
					case 'room':
						this.inlineAlert('Invoca un chat privado');
						this.inlineAlert('Los nombres deben ser separados por rayas (|).');
						this.inlineAlert('Ejemplo: <span onclick="mainRoom.viewDiscussion.getTextInput().val(\'/room Usuario\')">/room Usuario</span>');
						break;
					case 'mod':
						this.inlineAlert('Entrega el puesto de moderador del chat a determinado usuario.');
						this.inlineAlert('Ejemplo: <span onclick="mainRoom.viewDiscussion.getTextInput().val(\'/mod Usuario\')">/mod Usuario</span>');
						break;
					case 'block':
						this.inlineAlert('Bloquea al usuario para impedir enviarte mensajes privados.');
						this.inlineAlert('Ejemplo: <span onclick="mainRoom.viewDiscussion.getTextInput().val(\'/block Usuario\')">/block Usuario</span>');
						break;
					case 'unblock':
						this.inlineAlert('Desbloquea al usuario para poder enviarte mensajes privados.');
						this.inlineAlert('Ejemplo: <span onclick="mainRoom.viewDiscussion.getTextInput().val(\'/unblock Usuario\')">/unblock Usuario</span>');
						break;
					case 'kick':
					case 'kickban':
						this.inlineAlert('Echa y bloquea del chat a determinado usuario');
						this.inlineAlert('Ejemplo: <span onclick="mainRoom.viewDiscussion.getTextInput().val(\'/kickban Usuario\')">/kickban Usuario</span>');
						break;
					case 'enter':
					case 'join':
						this.inlineAlert('Entra a un chat determinado por su id.');
						this.inlineAlert('Ejemplo: <span onclick="mainRoom.viewDiscussion.getTextInput().val(\'/join 10\')">/join 10</span>');
						break;
					case 'leave':
					case 'part':
						this.inlineAlert('Quita el chat actual');
						this.inlineAlert('Un id puede ser especificado para cerrar determinado chat.');
						this.inlineAlert('Ejemplo: <span onclick="mainRoom.viewDiscussion.getTextInput().val(\'/part\')">/part</span>');
						break;
					case 'roomid':
					case 'id':
						this.inlineAlert('Regresa al chat de origen.');
	...
	$(function () {
		...
		$('#Write').append('<a class="button" href="javascript:toggleAway()" style="position:absolute; right:0; top:0;">Ausente</a><a class="button" href="javascript:active().clearWindow()" style="position:absolute; right:0; bottom:2px;">Limpiar</a>');
	...
	$(function () {
		if(document.body.className.indexOf('skin-oasis') == -1) {
			...
			for(var i in a) {
				if(a[i].href && a[i].href.indexOf('/wiki/Special:WikiActivity') != -1) {
	...
	function ChatCheck() {
		if($('.chat-join button').length != 0) {
			$('.chat-join button').replaceWith('<a class="button" onclick="OpenChatWindow()"><img src="https://images.wikia.nocookie.net/common/__cb36140/extensions/wikia/Chat/images/chat_icon.png" style="margin-right:3px;">Ingresar al chat</a>');
			clearInterval(window.chatcheck);
		}
	}
 
	function OpenChatWindow() {
		window.chatwindow = window.open('/index.php?title=Special:WikiActivity&useskin=wikia');
