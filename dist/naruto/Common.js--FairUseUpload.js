 ...
 * http://naruto.fandom.com/wiki/MediaWiki:Common.js/FairUseUpload.js
    ...
    mw.loader.using('mediawiki.util').then(function() {
        ...
        if (!mw.util.getParamValue('wpForReUpload')) {
            ...
            if (formstyle == "guided" || (formstyle == "" && window.location.search.indexOf('basic=true') == -1)) { // Add link to basic form
                $("#uploadtext").prepend('<div style="float: right;" id="uploadBasicLinkJS"><a href="https://naruto.fandom.com/index.php?title=Special:Upload&basic=true" onclick="javascript:setCookie(\'uploadform\', \'basic\', 30)">Switch to basic form</a></div>'); // Stretch table to full width            
                ...
                jQuery(function ($) {
                    ...
                    function verifySummary() {
                        ...
                        strBuilder += '|Characters in image = ' + $.trim($customRows.find('#characterBox').val()).replace(/\set\s/g, ', ') + '\n';
                        strBuilder += '|Jutsu in image      = ' + $.trim($customRows.find('#jutsuBox').val()).replace(/\set\s/g, ', ') + '\n';
