...
/* Source:     https://leagueoflegends.fandom.com/wiki/MediaWiki:Common.js/gridfiltering.js */
...
( function( $ ) {
	function gridFiltering() {
		...
		window.gridElements = []
		grid.find('.card-container').each(function() {
			var obj = {}
			var elem = $(this)
			obj['*'] = elem
	...
	function gridFilteringSwitches() {
		...
		for(x in gridFilters) {
			...
			if(gridFilters[x] == 'search') {
				var field = $('<input type="text" placeholder="ค้นหา..." />').appendTo(container).attr('id', container.attr('id')+'-field').data('type', 'search')
