$(function () {
    ...
    window.InappropriatedContent = (function (my, console, Math) {
 
        my = $.extend({
            question: 'This page contains inappropriate content? Are you sure you want to read it?',
            yes: 'Yes, please',
            no: 'No, not yet',
            isInappropriateContent: function () {
                return (/^Inappropriate\:/.test(document.title));
            },
            back: false
        ...
        function getBackgroundColor () {
            ...
            if ('transparent' !== color) return color;
            console.log('InappropriateContent: Cannot determine color');
            return color;
        }
 
        // Use LocalStorage, it doesn't get sent to the server every HTTP request
        var ids = $.storage.get('InappropriateContentJS');
        // Backwards compatibility. This block can be removed after a week or so
        if (!ids) {
            ids = $.cookies.get('inappropriate');
            if (ids) { // Old cookie found, convert to local storage
                ids = ids.split(',');
                $.cookies.del('inappropriate', {hoursToLive:0, path:'/', domain: location.host});
                $.storage.set('InappropriateContentJS', ids);
            } else {
                ids = [];
            }
        }
        if (my.isInappropriatedContent() && -1 === $.inArray(wgArticleId, ids)) {
            ...
            $('#yes').click(function () {
                ...
                $.storage.set('InappropriateContenttJS', ids);
            });
        }
 
        return my;
 
    }) (window.InappropriateContent, window.console || { log: $.noop }, Math);
 });
//