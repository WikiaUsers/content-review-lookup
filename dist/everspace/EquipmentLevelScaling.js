/*
 * Everspace 2 – Equipment Scaling Script v3
 *
 * Derived from:
 *   EquipmentLevelScaling.js [[MediaWiki:EquipmentLevelScaling.js?oldid=22618]]
 *   EnemyInfoboxSlider.js [[w:c:warframe:MediaWiki:EnemyInfoboxSlider.js]]
 *
...
;(function(window, $, mw) {
    ...
    mw.hook('wikipage.content').add(function ($content) {
		...
		const infoboxes = $content.find('.portable-infobox.type-equipment');
		...
		$(infoboxes).each(function(i) {
			...
			if (infobox.data('processed'))
                return true;

            infobox.data('processed', true );
			...
			scaler.updateFromButton = function() {
				...
				const settings = {
					...
					rank: wikiUserScalingConfig.rank
				};
				if( scaler.isLegendary ) {
				...
				const notifOptions = {
					title: 'Equipment Infobox',
					//autoHide: false,
					tag: 'infobox-config',
					classes: 'postedit'
				};
				const notifContent = $('<span>Configuration saved</span><svg class="wds-icon wds-icon-small" aria-hidden="true" focusable="false"><use xlink:href="#wds-icons-gear-tiny"></use></svg>');
