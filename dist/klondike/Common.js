...
/** Collapsible tables *********************************************************
 ...
 function GEcreateCollapseButtons()
 {
     ...
     for ( var i = 0; i < Tables.length; i++ ) {
         if ( hasClass( Tables[i], "GEcollapsible" ) ) {
     ...
     for ( var i = 0;  i < tableIndex; i++ ) {
         if ( hasClass( NavigationBoxes[i], "GEcollapsed" ) || ( tableIndex >= autoCollapse && hasClass( NavigationBoxes[i], "GEautocollapse" ) ) ) {
             GEcollapseTable( i );
         }
     }
 }
 
 addOnloadHook( GEcreateCollapseButtons );
 
function createGETooltips () {
     jQuery("div.GETooltip").tooltip();	
 }
 
addOnloadHook( createGETooltips );
...
/** Extra buttons *********************************************************
  *
  *  Description: extra edit buttons
  *  Maintainers: 
  */
var addExtraButtons = function(){
    ...
    mw.toolbar.addButton( {
        ...
        speedTip: 'To link to an NPC',
        tagOpen: '{{NPCLink|',
        tagClose: '}}',
        sampleText: 'Insert NPC here',
        imageId: 'button-comment'
    } );
 
    mw.toolbar.addButton( {
        imageFile: '//upload.wikimedia.org/wikipedia/commons/b/b8/Button_Globe.png',
        speedTip: 'To link to a Location',
        tagOpen: '{{GEOLink|',
        tagClose: '}}',
        sampleText: 'Insert Location here',
        imageId: 'button-comment'
    } );
 
    mw.toolbar.addButton( {
        imageFile: '//upload.wikimedia.org/wikipedia/commons/0/01/Button_no_source.png',
        speedTip: 'To link to a Mob',
        tagOpen: '{{MOBLink|',
        tagClose: '}}',
        sampleText: 'Insert Mob here',
        imageId: 'button-comment'
    } );
