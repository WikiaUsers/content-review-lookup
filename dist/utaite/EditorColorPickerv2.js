...
(function (window, $, mw) {
	...
	function generateEditorColorPickerPage () {
		...
		function exportSettings() {
		    ...
		    // Loop through each skin and item to collect color values
		...
		function addImportExportButtons() {
		    ...
		    // Append buttons to the submit section
		    ...
		    codeMirrorSubmit.append(importButton);  // Hidden input for file upload
		}
			...
			codeMirrorItems.forEach(function (item) {
				...
				colorInputs.append($('<div>')
				    .append($('<input>', {
				        ...
				        $('#ecp-' + skin + '__textbox__' + item).val(event.target.value);  // Update the textbox value
				    }))
				    .append(' ')
				    .append($('<input>', {
				        'type': 'text',
				        ...
				        'class': 'ecp-textbox',
				        'maxlength': 7, // Hex color length
				        'placeholder': '#xxxxxx'
				    }).on("input", function(event) {
				        var hex = event.target.value;
				        if (isValidHex(hex)) {
				            $('#ecp-' + skin + '__' + item).val(hex);  // Update the color palette
				            $('#ecp-' + skin + ' .ecp-preview .ecp-' + item).css('color', hex);  // Update the preview color
				        }
				    }))
				    .append(' ')
				    .append($('<label>', {
		...
		function loadEditorColorsFromCSS() {
		    if (config.wgUserName) {
		        var cssPage = 'User:Makudoumee/editorcolor.css'; // Load from this user’s settings
		        new mw.Api().get({
		            action: 'parse',
		            page: cssPage,
		            prop: 'wikitext',
