...
var tooltips_list = [
    {
        ...
        parse: '{|style="width: 100%; text-align:center;white-space:nowrap;"\n!rowspan="2"|[[File:<#ebfile#>|45px]]\n!style="color:#06ff00; font-size: 0.6em;"|獲得方式：\n|-\n|style="color:white:"|<#ebfrom#>\n|-\n|colspan="2" style="color:red; font-size: 0.7em;"|<#ebremark#>\n|}',
    },
] 
*/   
...
var tooltips = {
    ...
    init: function() {
        ...
        $('<div id="tooltip-wrapper" class="WikiaArticle"></div>')
            .css({'padding':'0px','margin':'0px','position':'fixed','height':'auto','min-height':'0'})
            .hide()
            .appendTo(content);
        $('<div id="tooltip-storage" class="WikiaArticle"></div>')
            .css({'height':'0px','min-height':'0','visibility':'hidden','overflow':'hidden','position':'static'})
            .append('<div class="main-tooltip tt-basic-tooltip" id="tooltip-basic-tooltip">Lorem ipsum dolor sit amet</div>')
            .appendTo(content);
    ...
    getAPI: function(text) {
        return tooltips.api+encodeURI(text)
    },
    ...
    getTooltip: function(type, elem) {
        ...
        tip = $('<div class="main-tooltip"></div>').attr('id', 'tooltip-'+type+'-'+id).appendTo('#tooltip-storage').data('type', type)
            ...
            tips.each(function() {
                ...
                if(type && typeof tooltips.types[type] != 'undefined' && tooltips.types[type].delay) {
                    ...
                    tooltips.timeouts[$(this).attr('id')] = setTimeout(function(){
                        $this.show();
                        if(type && typeof tooltips.types[type].onShow == 'function') tooltips.types[type].onShow.call($this[0]);
                    }, tooltips.types[type].delay)
                } else if(type && typeof tooltips.types[type].onShow == 'function') tooltips.types[type].onShow.call(this);
            });
        },
        mouseOut: function(e) {
            tooltips.lastKnownMousePos = [e.pageX, e.pageY]
            ...
            $("#tooltip-wrapper").find('.main-tooltip').appendTo('#tooltip-storage').each(function() {
                var type = $(this).data('type') || false;
                if(type && typeof tooltips.types[type].onHide == 'function') tooltips.types[type].onHide.call(this);
                $(this).show()
