						...
						new Modal.Modal({
							...
							events: {
								PatrolSubmit: function() {
											...
											if (revs.length>0) {
												betterDiff.patrolRevisions(
													...
													function (rev, left, total) {
														...
														if (left == 0) { details.prepend('<div class="success">Finished patrolling!</div>'); }
													}
												);
											} else {
									...
									} else { details.prepend('<div class="error">No filter specified.</div>'); }
								}
							}
						});
					settings.create();
		...
		quickDiff: function() {
			...
			var generateModal = function(event) {
				...
				var generateHeader = function(data, url) {
 					href = getURL(data.totitle, {diff: data.torevid});
					var header = '';
					...
					if (data.fromtimestamp) {
						href = getURL(data.totitle, {diff: data.torevid, oldid: data.fromrevid}); // Complete diff link
						...
						'<td class="diff-otitle diff-side-deleted" colspan="2">'+
							'<div id="mw-diff-otitle1">'+
								'<strong>'+
									'<a href="'+getURL(data.fromtitle, {oldid: data.fromrevid})+'" title="'+data.fromtitle.replace(/"/g, '&quot;')+'">'+
										'Revision as of '+
					...
					'<td class="diff-ntitle" colspan="'+(data.fromtimestamp ? '2' : '4')+'">'+
						'<div id="mw-diff-ntitle1">'+
							'<strong>'+
								'<a href="'+getURL(data.totitle, {oldid: data.torevid})+'" title="'+data.totitle.replace(/"/g, '&quot;')+'">'+
									'Revision as of '+
