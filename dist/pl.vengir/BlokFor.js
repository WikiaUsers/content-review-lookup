var strona = wgPageName + "?uselang=en&action=render";
$.get(strona, function(data){
  $('ul.comments li.SpeechBubble .permalink').last().attr("kiedys", ($(data).find('ul.comments li.SpeechBubble .permalink').last().text().replace(",","")));
});
...
(function (LockForums) {
    ...
    jQuery(function ($) {
        ...
        var expiryMilliseconds = LockForums.expiryDays * 24 * 60 * 60 * 1000,
            then = $('ul.comments li.SpeechBubble .permalink').last().attr("kiedys").replace(",",""),
            oldDate,
