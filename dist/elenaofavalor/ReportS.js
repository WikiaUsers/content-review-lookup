...
require(['jquery', 'mw'], function ($, mw) {
	...
	function setOptions() {
		if(typeof window.dev === 'undefined' || typeof window.dev.i18n === 'undefined') {
			importScriptPage('MediaWiki:I18n-js.js', 'user');
		}
		mw.hook('dev.i18n').add(function(i18no) {
			i18no.loadMessages('Reports').done(function(i18n) {
				options = {
								...
								'<fieldset>' +
									'<div class="rf-wrapper">' +
										...
										'<div class="rf-section"><div class="rf-section-title"><b>' + i18n.msg("formWikiURL").escape() + '</b></div>' +
											'<span class="rf-http">http://</span>' +
											'<input id="wikiurl" class="rf-wikiurl" type="text" placeholder="user"/>' +
											'<span class="rf-httpend">.wikia.com</span>' +
										'</div>' +
					...
					vandalism: {
						...
						form: '<form class="WikiaForm ReportForm" method="" name="" id="vandalism">' +
								'<fieldset>' +
									'<div class="rf-wrapper">' +
										'<div class="rf-section"><div class="rf-section-title"><b>' + i18n.msg("formWikiName").escape() + '</b></div>' +
											'<input id="wikiname" class="rf-wikiname optional" type="text" placeholder=“Andi Cruz Wiki"/>' +
										'</div>' + 
										'<div class="rf-section"><div class="rf-section-title"><b>' + i18n.msg("formWikiURL").escape() + '</b></div>' +
											'<span class="rf-http">http://</span>' +
											'<input id="wikiurl" class="rf-wikiurl" type="text" placeholder="user"/>' +
								...
								'<fieldset>' +
									'<div class="rf-wrapper">' +
										...
										'<div class="rf-section"><div class="rf-section-title"><b>' + i18n.msg("formWikiPage").escape() + '</b></div>' +
											'<span class="rf-http">http://</span>' +
											'<input id="wikiurl" class="rf-wikiurl" type="text" placeholder="user"/>' +
											'<span class="rf-httpend">.wikia.com/wiki/</span>' +
						...
						form: '<form class="WikiaForm ReportForm" method="" name="" id="wiki">' +
								'<fieldset>' +
									'<div class="rf-wrapper">' +
										...
										'Please add to this list if you believe a wiki violates Wikia\'s <a href="http://wikia.com/Terms of Use">terms of use</a>.<br />' +
										'<ol><li>1. Do not use this page to report <a href="http://wikia.com/Community Creation Policy#Duplicate wikias">duplicate wikis</a>.</li>' +
										'<li>2. Do not add wikis just because you feel that they are "unproductive." This list is for <a href="http://wikia.com/Terms of Use">Terms of Use</a> violations only -- advertisement spam, harassment wikis, or obscene and illegal content.</li>' +
	...
	function submitForm(opts) {
		...
		if(opts.page === "Report:Wiki") {
			...
			.done(function (res) {
				if (c.wgPageName == "Andi Cruz") {
					location.replace(‘http://elenaofavalor.wikia.com/wiki/' + opts.page + '?action=purge');
				} else {
			...
			.done(function (res) {
				if (c.wgPageName == “Elena of Avalor Wiki") {
					location.replace(‘http://elenaofavalor.wikia.com/wiki/' + opts.page + '?action=purge');
				} else {
					location.reload(location + '?action=purge');
				}
	...
	function init() {
		...
		importStylesheetURI(‘http://elenaofavalor.wikia.com/index.php?title=MediaWiki:Reports.css&action=raw&ctype=text/css');
	}
