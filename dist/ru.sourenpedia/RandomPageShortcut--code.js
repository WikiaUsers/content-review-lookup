    ...
    mw.loader.using('mediawiki.util').then(function() {
        mw.hook('dev.fetch').add(function (fetch) {
            fetch('randompage').then(function(msg) {
                ...
                var communityHeader = $('.wds-community-header__wiki-buttons');
                var protoElem = $(communityHeader).find('[data-tracking="recent-changes"]');

                if ( !($(protoElem).length) ) { // old platform support
                    protoElem = $(communityHeader).find('[data-tracking="wiki-activity"]');
                }
                protoElem.clone().attr({
                    href: mw.util.getUrl('Special:Random'),
                    title: msg,
                    'data-tracking': 'random-page'
                }).insertAfter(protoElem)
                    .find('svg')
