 ...
 * ScriptPage: https://naruto.fandom.com/fr/wiki/MediaWiki:Common.js/FairUseUpload.js
	...
	mw.loader.using('mediawiki.util').then(function() {
	    ...
	    if (!mw.util.getParamValue('wpForReUpload')) {
	        ...
	        if (formstyle == "guided" || (formstyle == "" && window.location.search.indexOf('basic=true') == -1)) { // Add link to basic form
	        	$("#uploadtext").prepend('<div style="float: right;" id="uploadBasicLinkJS"><a href="https://naruto.fandom.com/fr/index.php?title=Spécial:Téléverser&basic=true" onclick="javascript:setCookie(\'uploadform\', \'basic\', 30)">Basculer vers le formulaire basique</a></div>'); // Stretch table to full width            
	            ...
	            jQuery(function ($) {
	                ...
	                customRows += fromTemplate('Stands dans l’image', 'standBox', false, '[S’IL Y A] Liste des Stands sous forme de liens internes séparés par des virgules.');
	                ...
	                function verifySummary() {
	                    ...
	                    strBuilder += '|Stands dans image = ' + $.trim($customRows.find('#standBox').val()).replace(/\]\set\s/g, '], ') + '\n';
	                ...
	                $.getScript(mw.util.wikiScript('load') + 
	                    ...
	                    dev.minicomplete.load(
	                        $customRows.find('#descriptionBox'), $customRows.find('#sourceBox'),$customRows.find('#characterBox'), $customRows.find('#standBox')
	                    );
	                });
	            });
