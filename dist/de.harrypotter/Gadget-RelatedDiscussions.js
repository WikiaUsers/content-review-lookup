		...
		getThread(id,function (post) {
			...
			thread = $('<li />').append(
				$('<h3 />').append(
					...
					$('<a />',{ href: wgScriptPath + '/d/p/' + post.id }).text(post.title),
					' von ',
					$('<a />',{
			...
			for(i = 0; i < Math.min(post._embedded.contributors[0].count, 5); i++) {
                user = post._embedded.contributors[0].userInfo[i];
			...
			if(post.postCount) {
    			...
    			for(i = 0; i < Math.min(post.postCount, 2); i++) {
...
function getThread(threadId,callback) {
    $.getJSON('https://services.wikia.com/discussion/' + wgCityId + '/threads/' + threadId + '?responseGroup=full',function(data) {
       callback(data); 
    });
}