    	...
    	reprimands.forEach(function(reprimand, idx) {
            ...
            if(count >= 1) {
                tr.find('> :nth-child(3)').append(
					$('<time />',{class: 'discussions-reprimand-date', text: reprimand.reprimands[0].date, datetime: reprimand.reprimands[0].date})
				);
...
function actionDropdown(tr,id,username) {
	if(wgUserGroups.includes('sysop') || wgUserGroups.includes('threadmoderator')) {
        ...
        function getModalContent() {
            var modalContent = $('<div />').append(
				$('<form />',{class: 'discussions-reprimand-form'}).append(
                    ...
                    $('<div />',{class: 'form-field'}).append(
                        $('<input />',{type: 'hidden', id: 'reprimand-by', value: wgUserName})
                    ),
                    $('<div />',{class: 'form-field'}).append(
