...
// 		return window.UserAndIPTools.warn('Script double loaded, exiting...');
	...
	window.UserAndIPTools = $.extend({
		...
		import: function() {
			...
			this.imports.otherOnloadPromises.concat([
				this.api.get({
					action: 'query',
					list: 'users',
		...
		init: function(i18n) {
			...
			this.i18n = i18n;
			this.wg.wgArticlePath = this.wg.wgArticlePath.replace('UserAndIPTools', ''),
			...
			Object.keys(window).forEach(function(key) {
				...
				if (key.startsWith(prefix) && key !== prefix) {
					...
					console.log(oldPrefix, window[oldPrefix]);
		...
		appendTools: function() {
			...
			var links = {
				anon: {
					...
					'ip-tool-proxy-check': "//www.xmyip.com/proxy-check/${user}",
	...
	if (!UserAndIPTools.canRun()) return this.log('User does not have necessary rights or page is not supported, exiting...');
