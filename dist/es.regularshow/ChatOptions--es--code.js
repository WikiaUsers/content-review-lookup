...
function openOptions() {
	...
	var $optionsWindowHTML = $.showCustomModal( "Opciones", '<form method="" name="" class="WikiaForm "><fieldset><p style="font-size:120%; font-weight:bold; font-style:italic;">Cambios de color</p><p style="font-size:80%;">Introduce un <a href="http://www.w3schools.com/html/html_colornames.asp" target="_blank">nombre de un color</a> o <a href="http://html-color-codes.info/" target="_blank">color hex</a><p>Fondo del chat&nbsp;<input type="text" name="backgroundColourinput" id="backgroundColourinput" value="' + chatOptions.look.backgroundColor + '"/></p><br/><p>Fondo de tus propios mensajes&nbsp;<input type="text" name="selfPostColourinput" id="selfPostColourinput" value="' + chatOptions.look.selfPostColor + '"/></p><br/><p>Fondo de atrás&nbsp;<input type="text" name="surroundColourinput" id="surroundColourinput" value="' + chatOptions.look.surroundColor + '"/></p><br/><p>Color de las letras&nbsp;<input type="text" name="fontColourinput" id="fontColourinput" value="' + chatOptions.look.fontColor + '"/></p><br/><p style="font-size:120%; font-weight:bold; font-style:italic;">Fuente</p><p>Fuentes <select id="fontList"><option value="arial" style="font-family:arial;">Arial</option><option value="courier new" style="font-family:courier new;">Courier new</option><option value="georgia" style="font-family:georgia;">Georgia</option><option value="palatino linotype" style="font-family:palatino linotype;">Palatino linotype</option><option value="Comic Sans MS" style="font-family:Comic Sans MS;">Comic sans</option><option value="tahoma" style="font-family:tahoma;">Tahoma</option><option value="Trebuchet MS" style="font-family:Trebuchet MS;">Trebuchet MS</option><option value="Verdana" style="font-family:Verdana;">Verdana</option><option value="Lucida Console" style="font-family:Lucida Console;">Lucida Console</option></select></p><br/><p style="font-size:120%; font-weight:bold; font-style:italic;">Added functionality</p><input type="checkbox" name="chatHacks" value="chatHacks" id="chatHacks"/> Habilitar <a href="http://dev.wikia.com/wiki/MediaWiki:ChatHacks.js" target="_blank">chathacks</a>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="multiPM" value="multiPM" id="multiPM"/> Habilitar <a href="http://cod.wikia.com/wiki/MediaWiki:Chat.js/multipms.js" target="_blank">Privado grupal</a><br/><input type="checkbox" name="tabComplete" value="tabComplete" id="tabComplete"/>Habilitar <a href="http://dev.wikia.com/wiki/MediaWiki:Tabinsert.js" target="_blank">tab completar</a>&nbsp;&nbsp;&nbsp;<input type="checkbox" name="searchBar" value="searchBar" id="searchBar"/>Habilitar <a href="http://callofduty.wikia.com/wiki/MediaWiki:Chat.js/searchbar.js" target="_blank">barra de búsqueda</a><br/><input type="checkbox" name="multiKick" value="multiKick" id="multiKick" />Habilitar <a href="http://dev.wikia.com/wiki/MediaWiki:Multikick.js" target="_blank">multi expulsión</a>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="ignoreURL" value="ignoreURL" id="ignoreURL"/>Ignorar URLs en el chat principal<br /><input type="checkbox" name="stopSideScroll" value="stopSideScroll" id="stopSideScroll"/>Detener el scrollbar cuando alguien hace spam</fieldset></form>', {
		id: "optionsWindow",
        width: 600,
        buttons: [
		{
	...
	if (typeof window.customFonts !== "indefinido" && window.customFonts.length) {
...
function updateCookie() {
	...
	for (var m in chatOptions.modules) {
		if ( chatOptions.modules.hasOwnProperty( m ) ) {
			var module = chatOptions.modules[m];
			if (typeof module.element != 'indefinido' && $(module.element).attr("checked")) {
				module.enabled = true;
			} else {
...
if (!$("#chatOptionsButton").length) {
	$('.Rail').prepend('<div id="chatOptionsButton" onclick="openOptions();" style="margin:auto; cursor: pointer; font-size:150%; bottom:4px; padding-bottom:2px; border-bottom: 1px solid #CCCCCC; width:90%;" align="center"><img src="' + chatOptions.look.modalIcon + '" width="18px"/>&nbsp;Opciones</div>'); // Prevent multiple buttons from being appended
}
