...
function monsterCalc(){
    ...
    $.get('http://zh.btoky.wikia.com/api.php?format=json&action=expandtemplates&text={{Template:'+id+'|calc}}',function(d){
        ...
        $.get('http://zh.btoky.wikia.com/api.php?format=json&action=parse&text={{MonsterIcon|'+id+'|40}} <span style="font-size:1.7em; font-weight: bold">[['+u[0]+']]</span>',function(h){$('#monsterCalc').before(h.parse.text['*']);})
        for(i=1; i<=t[1]; i++){
...
var tooltips_list = [
    {
        ...
        onShow: function(handle) {$(handle).effect('opacity', {duration: 500, wait: false}).start(0,1)},
    },{
        classname: 'imgsrc-tooltip',
        parse: '['+'[File:<#imgsrc#>i.png|link=<#link#>]]',
        onShow: function() {$(this).effect('opacity', {duration: 500, wait: false}).start(0,1)},
    }
]
...
( function ( $, mw ) {
    ...
    function reportPage () {
            window.open().location.href = "http://zh.btoky.wikia.com/wiki/Project:回報?page=" + wgPageName.replace(/%/g, '%25');
    }
...
window.UserTagsJS = {
	...
	categ: { u:'Categorizer', },
	bureaucrat: { order: 1, link:'測試網站_维基:Administrators', },
	sysop: { order: 1, link:'測試網站_维基:Administrators',},
	rollback: { u: 'Special Officer (Rollback)', order: 1, link:'測試網站_维基:Administrators',},
	chatmoderator: { order: 2, link:'測試網站_维基:Administrators',},
// Custom Tags
	crypt: { u: 'Harbinger of Pestilence' },
	free: { u: 'Free Loader' },
	goobypls: { u: 'Gooby pls.' },
...
UserTagsJS.modules.custom = {
	'Btoky': ['csser', 'templater', 'spriter', 'categ',],
...
wgPageName === 'Test' && importScript('MediaWiki:Testjson.js');
wgPageName === '體力計算' && importScript('MediaWiki:Stamina.js');
wgPageName === '召喚獸搜尋器' && importScript('MediaWiki:PetSearch.js');
wgPageName === '填寫通關攻略' && importScript('MediaWiki:NewTeam.js');
wgPageName === '跑馬燈' && importScript('MediaWiki:跑馬燈.js');
wgPageName === '好友搜尋' && importScript('MediaWiki:LeaderFinder.js');
wgPageName === '好友搜尋' && importScript('MediaWiki:LeaderRegister.js');
//wgPageName === '公會搜尋器' && importScript('MediaWiki:GuildSearch.js');
wgPageName === '公會搜尋器' && importScript('MediaWiki:GuildRegister.js');
wgPageName === '公會搜尋器' && importScript('MediaWiki:Tablesorter.js');
wgPageName === '召喚獸編輯' && importScript('MediaWiki:Cardedit.js');
wgPageName === 'Template:Test2' && importScript('MediaWiki:Test.js');