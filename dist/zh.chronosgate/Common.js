...
$(function() {
    ...
    var a = $('<input type="button" id="ReportButton" style="vertical-align:top;margin-top:3px" value="\u56de\u5831\u9801\u9762\u554f\u984c" /><input type="button" id="PurgeButton" style="vertical-align:top;margin-top:3px;background-color:blue" value="\u986f\u793a\u6700\u65b0\u9801\u9762" />');
    $("header#WikiaPageHeader").children("a.comments").after(a);
    $("#ReportButton").click(function() {
            window.open().location.href = "http://zh.chronosgate.wikia.com/wiki/Project:回報?page=" + wgPageName.replace(/%/g, '%25');
        });
...
function dataSlider(){
    ...
    $('#dataSliderToggle').on('change',function(e,s){
        console.log(s);
        if(s){
            $('#monster-data>tbody>tr>*').slice(25,46).delay(500).fadeIn(500);
            $('#monster-data>tbody>tr>*').eq(24).fadeOut(500).delay(500);
        }else{
            $('#monster-data>tbody>tr>*').slice(25,46).fadeOut(500).delay(500);
...
function monsterCalc(){
    ...
    $.get('http://zh.chronosgate.wikia.com/api.php?format=json&action=expandtemplates&text={{Template:'+id+'|calc}}',function(d){
        ...
        $.get('http://zh.chronosgate.wikia.com/api.php?format=json&action=parse&text={{MonsterIcon|'+id+'|40}} <span style="font-size:1.7em; font-weight: bold">[['+u[0]+']]</span>',function(h){$('#monsterCalc').before(h.parse.text['*']);})
        for(i=1; i<=t[1]; i++){
...
wgPageName === '時空之門繁中維基:回報' && importScript('MediaWiki:Report.js');