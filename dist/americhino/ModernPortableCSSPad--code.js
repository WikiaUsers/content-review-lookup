...
(function ($, mw, window, console) {
	...
	function init(opt) {		
		...
		options = $.extend({
			...
			left: 0,
			top: 0,
			stylesheetURL: '/load.php?mode=articles&articles=u:americhino:MediaWiki:ModernPortableCSSPad.css&only=styles',
	...
	if (/\.css$/.test(mw.config.get('wgPageName')) && mw.config.get('wgNamespaceNumber') % 2 === 0)  {
		$(function ($) {
			...
			var buttonStyle = 'cursor: pointer; display: inline-block; border: 1px solid black; width: 20px; height: 20px; background-image: url(https://vignette.wikia.nocookie.net/botkylorens-test/images/0/0a/Modern-portable-css-pad-icons-small.png); background-position: 0 -20px; background-color: #fff;';
