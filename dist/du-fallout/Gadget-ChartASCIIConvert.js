// <nowiki>
/* Script for converting ASCII art to {{Chart}}
 * Script by Greg Ubben (User:GregU, http://en.wikipedia.org)
 * Oasis support by User:Porter21 (http://fallout.wikia.com)
 * Original script at http://en.wikipedia.org/wiki/User:GregU/familytree.js
 */
...
function update_menu (item)
{
   ...
   if (item == "wiki2art" || item == "art2wiki") {
      if (item == 'wiki2art') {
         buttonText = '{{chart}} to ASCII';
         buttonTooltip = 'Convert {{chart}} to ASCII art';
      } else {
         buttonText = 'ASCII to {{chart}}';
         buttonTooltip = 'Convert ASCII art to {{chart}}';
      }

      if (skin == 'monobook') {
         node = document.getElementById("t-whatlinkshere");
         addPortletLink ("p-tb", "javascript:" + item + "()", buttonText, "t-diagram", buttonTooltip, "", node);
      } else if (skin == 'oasis' || skin == 'wikia') {
         $('<a href="javascript:' + item + '()" id="t-diagram" title="' + buttonTooltip + '" class="wikia-button secondary" style="float: right; margin-right: 1%;">' + buttonText + '</a>').prependTo('#toolbar');
      }
   }
}
...
jQuery(function($) {
   if ((skin == 'monobook' || skin == 'oasis' || skin == 'wikia') && (wgAction == "edit" || wgAction == "submit")) {
     addChartConvert();
   }
});
// </nowiki>