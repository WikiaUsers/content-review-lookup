...
$( function() {
	$( '.mw-rollback-link > a' ).after( $( '<span>' ).addClass( 'edit-rollback' ).prop( 'title', 'Modifica sommario rollback' ) );
		...
		if ( $( this ).parent().is( $rollback.parent() ) ) {
			...
			$rollback = $( '<div id="rollback-summary">' ).append(
				$( '<input type="text">' ).addClass( 'mw-ui-input rollback-text' ).prop( { maxlength: 250, spellcheck: true } ).val(
					'Annulla modifiche consecutive di [[Speciale:Contributi/' + name + '|' + name + ']] ([[User talk:' + name + '|talk]])'
				),
