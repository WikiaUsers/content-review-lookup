...
function showTip(e) {
    ...
    if ($p.hasClass("selflink") == false) {
        $t.removeAttr("title");
        $p.removeAttr("title");
        var url = "/index.php?title=" + $t.data("tt").replace(/ /g, "_").replace(/\?/g, "%3F").replace(/\&/g, "%26").replace(/\+/g, "%2B") + "&action=render div.tooltip-content";
        ...
        $tfb.load(url, function() {
            ...
            if ($tfb.html() == "") $tfb.html('<div class="tooltip-content module" style="background:#000000 !important; color:#ffffff"><b>Lỗi</b><br />Phần này không có<br>hoặc chưa có bảng chú giải.</div>');
...
function bindTT() {
    ...
    $p = $t.parent();
    if ($p.hasClass("selflink") == false) {
        $t.data("tt", $p.attr("title").replace(" (trang không tồn tại)", "").replace("?", "%3F")).hover(showTip, hideTip).mousemove(moveTip);
    }
}
...
importArticles({
    type: 'script',
    articles: [
        "w:c:vi.blade-and-soul:MediaWiki:Common.js/Tooltips.js",
    ]
});