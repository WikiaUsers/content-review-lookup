...
$("#reset-button", $("#simulator")).on('click', function(e){
...
$("#share-button", $("#simulator")).on('click', function(e){
	var url = getShareUrl();
	$("#share-url", $("#share-url-box")).html(url);
	$("#share-url-box").addClass("fade-in").show();
		
});
$("#copy-button", $("#share-url-box")).on('click', function(e){
...
$("#close-button", $("#share-url-box")).on('click', function(e){
	$("#share-url-box").removeClass("fade-in").hide();
});


function selectJob(pos, value, learnedSkill){
...
function GenSkillDataTree(pos, jobValue){
	...
	if(jobSkillData != null && jobSkillData[pos] != null && Object.entries(jobSkillData[pos]).length > 0){
		...
		keys.forEach(function (key) {
			...
			$(cloneSkillIcon).on('mousedown', function(e){
    			e.preventDefault();
				var modify = 0;
				if(event.which === 1){
					modify = 1;
			...
			$(cloneSkillIcon).on('updateIcon', function(e){
    			e.preventDefault();
				updateSkillIcon(this, jobValue, skillName);
			});
			$(cloneSkillIcon).on('contextmenu', function(e){
    			e.preventDefault();
			
			});
			modifySkillLevel($(cloneSkillIcon), jobValue, skillName, 0);
...
function setJobAvailable(pos, usedPoint, box){
	...
	var maxSkillPoint =$(box).attr('id').substr((skillBoxDiv + pos + "-").length);
...
function updateSkillDec(){
		...
		if(SimCal.tables.hasOwnProperty(job)){
			...
			for(var skillName in learnedTable){
				if(table.Skills.hasOwnProperty(skillName)){
					...
					if($("#tab-content-" + skillName, cloneTabContents).length > 0){
	...
	$(".tab-button" , tabButtonList).each(function(){
	...
	empty.hide();
	if($(".tab-button" , tabButtonList).length <= 0){
		empty.show();
    			...
    			if(table.Skills.hasOwnProperty(key)){
    				...
    				for(var i = 0; i < skillLevel; i++){
    					var reqLevel = skill.getReqLevel(i);
      					if(reqLevel != null && reqLevel > 0){                            levelFromSkill = Math.max(reqLevel, levelFromSkill);
    					}
    				}
    			}
    		}
