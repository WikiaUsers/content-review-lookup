;(function(window, mediaWiki) {
	...
	warframe.prototype = {
		...
		testJS: function() {
			(function(window) {
				function parse(target) {
					...
					try {
						if (target.constructor === window.Object) {
							q = '{';
							for (var key in target) {
								if (!Object.hasOwnProperty.call(target, key)) {
									continue;
								}
								q += '\n' + key + ': ' + parse(target[key]);
							}
							q += key ? '\n}' : '}';
							return q;
						}
				...
				window.mainRoom.model.chats.bind('afteradd', function() {
					var node = document.querySelector('#Chat_' + window.mainRoom.roomId + ' ul > li[data-user]:last-child > span.message'),
						ret;
					if (node && node.textContent.slice(0, 3) === 'js:') {
						node.style.fontFamily = 'monospace';
						try {
							ret = window.eval(node.textContent.slice(3));
							node.textContent = node.textContent + ' => ' + parse(ret);
						} catch (err) {
							node.style.color = '#CB0000';
							node.textContent = node.textContent + ' => ' + err;
						}
					}
				});
			})(window);
		...
		auto: function() {
			var _ = this;
			$('#ChatHeader .wordmark img')[0].setAttribute('src', '//images.wikia.com/warframe/ru/images/d/d7/Логотип_чата.png');
			$('#ChatHeader h1.wordmark')[0].insertAdjacentHTML('beforeend', '<div id="chat-head" class="chat-head">Добро пожаловать на «Warframe вики» чат!</br><a href="/FAQ/Словарь" target="_blank">Словарь</a> — <a href="//digitalextremes.zendesk.com/hc/ru" target="_blank">Техподдержка</a> — <a href="/wiki/Warframe вики:Чат" target="_blank">О чате</a></div>');
		...
		tags: function() {
			var replacement = {
					...
					spoiler: [/\[spoiler\]/, '<hr /><button class="open-chat-spoiler" onclick="this.nextSibling.style.display = \'\';warframe.fn.$remove(this);">Показать содержимое</button><div style="display: none;" class="chat-spoiler-content">', /\[\/spoiler\]/, '</div><hr />'],
