...
// for comparison, our wiki limits template expansions to 2097152 characters per page (see http://runescape.wiki/api.php?action=parse)
...
function wiki(code,callback){
//  console.log('jc asking wiki to parse '+code);
...
var jc_dict={
    ...
    get:function(t,s){
        ...
        switch(t){
            ...
            case 'imgs':
                wiki(s.match(/^File:/)?('['+'['+s+']]'):('['+'[File:'+s+']]'),function(response){
...
Parse.prototype.run=function(){
    var statement=this;
    wiki(this.e.eval(),function(response){
...
Hiscores.prototype.skillnames=[
    ...
    'Smithing','Mining','Herblore','Agility','Thieving','Slayer','Farming','Runecrafting','Hunter','Construction','Summoning','Dungeoneering','Divination','Invention'
];
...
Hiscores.prototype.run=function(){
    ...
    $.ajax({
        dataType: 'text',
        url: '/cors/m=hiscore/index_lite.ws?player=' + charname,
        context: this,
        ...
        error: function(){
            jc_warn('The player "' + charname + '" does not exist, is banned or unranked, or we couldn\'t fetch your hiscores. Please enter the data manually.');
        }
    });
}
    ...
    $('.jcInput').each(function() {
        ...
        switch(v.type){
            ...
            case 'radio':
                $(this).empty();
                for(var i=0;i<v.range.length;++i){
                    // The '_blank' target is for previewing in the oasis skin. Wiki kills click events whose target isn't '_blank', to prevent links from opening in the preview (see WE.plugins.pagecontrols.renderDialog). This stops checkboxes and radio buttons from working
        ...
        if(v.type=='string'&&(v.range||v.namespaces)) {
            mw.loader.using( 'jquery.ui.autocomplete', function () {
                if ( v.range ) {
                    ...
                    $( v.$ctl ).autocomplete({
                        minLength: 3, // Matching Wiki's search minLength.
