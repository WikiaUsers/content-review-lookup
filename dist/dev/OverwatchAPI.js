...
$(function(){
    var $owapi = $("div#owapi-main");
    if ($owapi.length) {
        console.log("OW-Api: Initializing...");
		var decMark = defaultValue($owapi.data("decmark"), '.');
		var decSpaces = defaultValue($owapi.data("decspaces"), 2);
		var errorMsg = defaultValue($owapi.data("error"), "<span style=\"color:red; font-weight: bold\">Error: Unknown error occurred!</span>");
		var targetTemplate = $owapi.data("template");
		
		var battletag = /(.*?)#(\d*)/g.exec($owapi.data("battletag"));
		if(battletag.length !== 3) {
			console.log("OW-Api: Wrong nickname");
            $owapi.html(errorMsg);
            return;
		}
        var nick = battletag[1];
		var tag = battletag[2];
        var server = $owapi.data("server");
        var platform = $owapi.data("platform");
		...
		$.getJSON("https://owapi.net/api/v3/u/" + nick + '-' + tag + "/blob?platform=" + platform, function(data) {
			...
			$.each(serverdata.heroes.playtime.competitive, function (key, value) {
				...
				if(hero_stats_cp !== undefined) {
					...
					damage_cp            = defaultValue(hero_stats_cp.general_stats.all_damage_done, 0);
				...
				if(hero_stats_qp !== undefined) {
					...
					damage_qp            = defaultValue(hero_stats_qp.general_stats.all_damage_done , 0);
			...
			var games_played_cp = 0, games_won_cp = 0, games_tied_cp = 0, games_lost_cp = 0,
				...
				dmg_avg_cp      = 0, dmg_avg_qp   = 0, dmg_avg       = 0;
				...
				dmg_avg_cp = defaultValue((serverdata.stats.competitive.game_stats.all_damage_done / games_played_cp), 0);
                dmg_avg_qp = defaultValue((serverdata.stats.quickplay.game_stats.all_damage_done   / games_played_qp), 0);
                console.log(dmg_avg_cp);
                console.log(dmg_avg_qp);
               
			var query = "/api.php?action=parse&text={{" + targetTemplate +
				...
				"|dmg_cp_avg =" + roundFloat(dmg_avg_cp, decSpaces).toString().replace('.', decMark) +
				...
				"|dmg_qp_avg=" + roundFloat(dmg_avg_qp, decSpaces).toString().replace('.', decMark) +
				...
				"|dmg_cp_most =" + serverdata.stats.competitive.game_stats.all_damage_done_most_in_game +
				...
				"|dmg_qp_most=" + serverdata.stats.quickplay.game_stats.all_damage_done_most_in_game +
				...
				"|dmg_cp =" + serverdata.stats.competitive.game_stats.all_damage_done +
				...
				"|dmg_qp=" + serverdata.stats.quickplay.game_stats.all_damage_done +
				...
				"|dmg_avg=" + roundFloat((dmg_avg_cp + dmg_avg_qp) / 2, decSpaces).toString().replace('.', decMark) +
