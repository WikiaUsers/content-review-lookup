...
(function ($, mw) {
	...
	if (window.DupImageListLoaded) {
        return;
	}
	window.DupImageListLoaded = true;
	...
	function init (i18n) {
		...
		if ($('#mw-dupimages').length) {
			$('#mw-dupimages').prepend('<div id="dupImagesProgress" style="height: 0; text-align: center;"><img src="' + indicator + '" style="border: 0 none;" alt="' + i18n.msg('inProgress').escape() +'" /></div>');
			findDupImages();
		}
	}
    mw.hook('dev.i18n').add(function(i18no) {
        i18no.loadMessages('DupImageList').then(init);
    });
    importArticle({
        type: 'script',
