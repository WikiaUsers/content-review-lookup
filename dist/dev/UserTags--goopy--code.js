...
var importList = (function(config, alwaysOnly) {
	...
	var moduleMap = {
		explode: { s: 'u:dev:MediaWiki:UserTags/goopy/module.implode.js' }
	};
	...
	for (module in config.modules) {
		...
		if (moduleMap.hasOwnProperty(module)) {
			...
			script = 'u:dev:MediaWiki:UserTags/goopy/module.' + module + '.js';
		} else {
			continue;
		}
		if (hash[script] !== 1) {
	...
	if (!alwaysOnly) {
		list[list.length] = 'u:dev:MediaWiki:UserTags/goopy/core.js';
	}
	return list;
})(config, !loaderData);
