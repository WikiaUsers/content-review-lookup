...
(function ($, mw) {
  ...
  for (var ns in mwConfig.wgNamespaceIds) {
    if (
      mwConfig.wgNamespaceIds[ns] === 2 ||
      mwConfig.wgNamespaceIds[ns] === 3 ||
      mwConfig.wgNamespaceIds[ns] === 1200
    ) {
  ...
  var scriptRX = new RegExp(
    '^(?:' + mwConfig.wgServer + ')?' + mwConfig.wgScript + '\\?title=([^#&]+)',
  );
  ...
  function init(specialContribs) {
    ...
    mw.util.addCSS(
      '.mediawiki .user-blocked-partial {' +
        ...
        '}' +
        '.mediawiki .user-blocked-temp {' +
        (window.mbTempStyle || 'opacity: 0.7; text-decoration: line-through') +
        '}' +
        '.mediawiki .user-blocked-indef {' +
        (window.mbIndefStyle ||
          'opacity: 0.4; font-style: italic; text-decoration: line-through') +
        '}' +
        '.mediawiki .user-blocked-tipbox {' +
        (window.mbTipBoxStyle ||
          'font-size:85%; background:#FFFFF0; border:1px solid #FEA; padding:0 0.3em; color:#AAA') +
        '}',
    );

    waitingCSS = mw.util.addCSS(
      'a.userlink {opacity:' + (window.mbLoadingOpacity || 0.85) + '}',
    );
  ...
  function markBlocked(container) {
    var contentLinks = (container || mw.util.$content || $('#mw-content-text'))
      .find('a')
  ...
  function inHours(ms) {
    //milliseconds -> "2:30" or 5,06d or 21d
