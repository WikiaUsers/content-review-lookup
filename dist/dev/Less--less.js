...
function doXHR(url, type, callback, errback) {
    ...
    // @author Cqm <https://community.fandom.com/wiki/User:Cqm>
    if ( url.indexOf( '/load.php' ) === 0 ) {} else {
        url += '?action=raw&maxage=0&smaxage=0';
    }
    mw.log( url );
    // end dev wiki change
    ...
    function handleResponse(xhr, callback, errback) {
        ...
        // @author Cqm <https://community.fandom.com/wiki/User:Cqm>
        if ( xhr.status === 404 ) {
            mw.hook('less.404').fire(url);
        } else if ( xhr.status === 200 ) {
            mw.hook('less.200').fire(url);
        }
        // end dev wiki change
...
if (typeof define === "function" && define.amd) {
    define(
        // begin wikia change (modil support)
        define.mock ? 'less' : ['less'],
        // end wikia change
        function () {
