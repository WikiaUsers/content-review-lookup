    	...
    	if (cats.length > 0) {
			...
			if (cats.length < 4) {
				for (i = 1; i < cats.length; i++) {
					$('#top-hidden-categories').append(
						(', '), $('<a>', { 'class': 'mw-redirect', href: '/wiki/'+cats[i], title: cats[i], text: cats[i].replace('Category:','') } )
					);	
				}	
			} else {
				$('#top-hidden-categories').append(
						(', '), $('<a>', { 'class': 'mw-redirect', href: '/wiki/'+cats[1], title: cats[1], text: cats[1].replace('Category:','') } )
				);
				$('#top-hidden-categories').append(
