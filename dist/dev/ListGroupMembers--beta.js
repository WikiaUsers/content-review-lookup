require([
    ...
    _.init = function($content){
        _.$content = $content;
        _.options = _.getDefaults(_.config);
        _.debug = Boolean(_.options.debug) || mw.config.get("wgUserName") === "Ultimate Dark Carnage";
        if (_.debug) console.log(_.options);
    ...
    _.generateGroups = function(){
        if (_.debug) console.log(_.options.visibleGroups);
    ...
    _.loadInfo = function(group, users){
        _.users[group] = users.map(function(user){
            ...
            _u.promisify(user.deferred).then(function(){
                user.loaded = true;
                if (_.debug) console.log(user);
            });
            return user;
        });
        if (_.debug) console.log(_.users[group]);
    ...
    _.getDefaults = function(config){
        var options = Object.assign({}, _.defaults, _.options, config),
            defaultGroups = options.rights;
        if (_.debug) console.log(options, defaultGroups);
        options.visibleGroups = defaultGroups.filter(function(group){
            if (typeof options.exclude === "function"){
                return options.exclude.call(_, group);
            } else if (Array.isArray(options.exclude)){
                return options.exclude.indexOf(group) === -1;
            } else return true;
        });
        return options;
    };
    // Creating the UserData constructor
    function UserData(user, config){
    ...
    UserData.prototype.__defaults = Object.freeze({
        registration: null,
        primaryGroup: "",
        edits: 0,
        discordHandle: "",
    ...
    UserData.prototype.__getDefaults = function(config){
        return Object.assign({}, this.__defaults, config);
    };
    
    UserData.prototype.__getAjax = function(){
        return $.get(mw.util.wikiScript("wikia"), {
            controller: "UserProfilePage",
            method: "renderUserIdentityBox",
            title: "UserData:" + this.name,
            format: "json"
        });
    };
    
    UserData.prototype.__loadInfo = function(){
        return _u.promisify(this.__getAjax()).then(function(data){
            ...
            if (_.debug) console.log(obj);
            return obj;
        });
    };
    
    UserData.prototype.__generateURLs = function(){
    ...
    UserData.prototype.__getBadge = function(){
        var hasBadge = _.badges.hasOwnProperty(this.primaryGroup);
        return hasBadge ? _.wds.badge(_.badges[this.primaryGroup]).outerHTML 
            : null;
    };
    
    UserData.prototype.__getRegistration = function(time){
        var d = (_u.isset(time) || time !== null || time !== 0) ?
            time : "October 18, 2004";
        return new Date(time);
    };
    
    UserData.prototype.__render = function(){
    ...
    UserData.prototype.fetchInfo = function(){
    ...
    _.__render = function(renderAll){
        ...
        if (_.debug) console.log(renderConfig);
