...
(function(window, $, mw, options) {
	...
	// If the script is disabled, do not continue;
	...
	const isRan = "WikiActivity" in window.UCX ||
		("wikiActivityLoaded" in window && window.wikiActivityLoaded);
	
	// If the script has been ran, exit the script
	...
	// Initializes the WikiActivity script
	$.extend(wa, {
		...
		_prepare: function() {
			...
			return loader.then(this._init.bind(this));
		},
		// Gets subpage patterns
		_getSubpagePattern: function(type) {
			if (!("i18n" in window.dev)) return null;
			const subpageName = window.dev.i18n.msg("page-" + type + "-subpage").plain();
