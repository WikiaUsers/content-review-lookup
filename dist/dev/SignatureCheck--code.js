...
(function(window, $, mw, config) {
	...
	if (({edit:1, submit:1})[mw.config.get('wgAction')] !== 1) {
		return;
	}

        // FEATURE: added by [[User:Colouratura]]
        //
        // The ability to specify pages and namespaces to work on
        // This feature was added to support the use of pseudo-talk pages
        //
        // example:
        //     match: User:Colouratura/Talk
        //     config = {
        //         extraNamespaces: [
        //             {
        //                 namespace: 2,
        ...
        if ((config.hasOwnProperty('extraNamespaces') !== false) && (config.extraNamespaces.length > 0)) {
            var i;
            for (i = 0; i < config.extraNamespaces.length; i++) {
                if (havePTP) break;

                if ((config.extraNamespaces[i].namespace) === namespace) {
                    var pageTitle = window.location.toString().split('/wiki/')[1];

                    var j;
                    for (j = 0; j < config.extraNamespaces[i].patterns.length; j++) {
                        if (pageTitle.indexOf(config.extraNamespaces[i].patterns[j]) > -1) {
                            havePTP = true;
                            break;
                        }
                    }
	...
	if ((namespace & 1) !== 1) {
		if (namespace !== mw.config.get('wgNamespaceIds').forum && !havePTP) {
			return;
		}
		haveForum = true;
	}
	...
	// Blogs are edited using Special:CreateBlogPost which is a special page.
	if (namespace >= 200 || namespace < 0) {
		return;
	}

	// Don't do anything for undo actions
	if ((/(?:^\?|&)undo=/).test(window.location.search)) {
		return;
	}
