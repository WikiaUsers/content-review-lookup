...
(function (mw, $) {
	...
	function createForm(curPageName, options) {
		...
		var output = "";
		var formOutput = "";
		var dropList = createNamespaceCmbList([i18n("item_dropdown_allnamespaces").plain(), "main"]);
		var namespaceFieldset = new Html.Raw(Html.element("label", {
			"for": "namespaceDropdown"
		}, i18n("label_namespace").plain()) + Html.element("select", {
		...
		var formContent = [{
			...
			content: i18n("label_pagelinks").plain()
		}, {
			tag: "textarea",
			attribs: {
				"id": "pageNameId",
			...
			content: i18n("label_reason").plain(),
			"newline": true
		}, {
			tag: "input",
			attribs: {
			...
			content: i18n("label_fetch_from_category").plain()
		}, {
			tag: "select",
			attribs: {
				"id": "cmbCategories",
			...
			content: i18n("label_delete_links").plain(),
			newline: true
		}, {
			tag: "input",
			attribs: {
			...
			content: i18n("label_report").plain() + (options.reportStatus || ""),
			"newline": true
		}, {
			tag: "textarea",
			attribs: {
	...
	function showBackLinks() {
		...
		if (linkList.length > 0) {
			var counter = 0;
			reportStatus(i18n("label_report_search_backlinks").plain());
			sendMsg("", true);
			setProgress("wait");
			...
			var checkLinks = function (data) {
				if (data && data.query && data.query.backlinks) {
					...
					if (counter === linkList.length) {
						...
						reportStatus(i18n("label_fetching_backlinks").plain());
					}
				} else {
					reportStatus(i18n("label_error_fetch_links").plain());
				}
			};
			for (i = 0; i < linkList.length; i += 1) {
				processBackLinks(linkList[i], checkLinks, "", i18n("label_error_fetch_links_server").plain());
			}
		} else {
			reportStatus(i18n("label_report_nobacklinks").plain());
		}
	}
			...
			if (count > 0) {
				...
				var saveNewContent = function (data) {
					if (data && data.edit && data.edit.result === "Success" && !data.edit.nochange) {
						sendMsg(page + i18n("label_success_linksremoved").plain());
						sweepedLinks += 1;
						setProgress("add");
						reportStatus(sweepedPages + " " + i18n("label_success").plain());
					} else {
						if (data.error){
				...
				sendMsg(i18n("report_link_not_found").plain() + " - " + page);
	...
	function removeBackLinks() {
		...
		if (linkList.length) {
			...
			if (linkSweepConfirmation) {
				confirmAction = window.confirm(i18n("label_submit_warning").plain());
				if (!confirmAction) {
					return;
				}
			}
			startCleaning = function (tmpResultList) {
				if (tmpResultList.length) {
					...
					sendMsg(i18n("report_found_backlinks").plain());
								...
								if (data.query && data.query.pages) {
									...
									if (listLength > 0) {
										if (confirmAction) {
											...
											reportStatus(i18n("report_action_cancel").plain());
										}
									}
								} else {
									reportStatus(i18n("report_no_links_found").plain());
									setProgress("reducemax");
	...
	var showLinkSweeperModal = function (event) {
		...
		window.dev.showCustomModal("Link Sweeper", sweeperForm, {
			...
			width: 325,
			buttons: [{
				message: i18n("btn_show_back_links").plain(),
				id: "showBacklinks",
				handler: function () {
					showBackLinks(currPages);
				}
			}, {
				message: i18n("btn_add_category_content").plain(),
				id: "addCatMembersButton",
				handler: function () {
					var category = $("#cmbCategories").val();
					if (category === "Custom") {
						category = prompt(i18n("label_category_fetch", category).plain());
					}
				...
				message: i18n("btn_fetch_redlinks").plain(),
				...
				message: i18n("btn_cancel").plain(),
				id: "cancelButton",
				handler: function () {
					bContinueProcess = false;
					window.dev.showCustomModal.closeModal($("#backLinkForm"));
				}
			}, {
				message: i18n("btn_remove_backlinks").plain(),
				id: "removeButton",
				defaultButton: true,
				handler: function () {
					removeBackLinks();
	...
	function main(i18no) {
		...
		defaultRemovalMsg = i18n("default_removalsummary").plain();
		setNamespaces();
	...
	function preload() {
		...
		window.dev.i18n.loadMessages('Linksweeper').done(main);
	}
