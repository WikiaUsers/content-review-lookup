...
mw.loader.using('mediawiki.api', function() {
	...
	var i18n; // holds the loaded I18n-js instance once messages are ready
	...
	// Uses .plain() (no wikitext/HTML parsing) since the HTML is built manually below.
	function msg(key) {
	...
	function fetchBlockData() {
		api.get({
			...
			block_type = data[0].partial !== undefined ? msg("block-type-partial") : msg("block-type-sitewide"); // Determine if block is sitewide or partial.
			...
			function main(user, blockreason, blockID, blockperformer, blockperformer_url, blocktime, blockexpire, block_type, partial_block_pages, partial_block_features, partial_block_namespaces_titles) {			
				...
				api.parse(blockreason).done(function(parsedBlockReason) {
					...
					var headerLine = i18n.msg("user-blocked", user, wiki_name, "<span style=\"font-size: 8pt;\">" + lang_for_wiki_name + "</span>").parse();
					
					// HTML design
				textParagraph.innerHTML = "<center><div style=\"font-size: 15pt; line-height: 1em\">" + headerLine + "</div><a href=\"" + wiki_url + "/wiki/User:" + user + "\">" + wiki_url + "/wiki/User:" + user + "</a></center><hr style=\"border: 1px solid rgb(var(--theme-alert-color--rgb)); background-color: rgb(var(--theme-alert-color--rgb));\"><span style=\"position: relative; float: right; border: 1.5px dotted; padding: 0 0.25em 0 0.25em\"><a href=\"" + wiki_url + "/wiki/Special:Log?type=block&page=User:" + url_user + "\">" + msg("block-log") + "</a></span><div style =\"font-size: 15pt; text-decoration: underline;\">" + msg("block-information") + "</div><b><i>" + msg("username") + " </i></b>" + user + "<br/><b><i>" + msg("block-id") + " </b></i>" + blockID + "<br/><b><i>" + msg("block-performer") + " </b></i><a href=\"" + wiki_url + "/wiki/User:" + blockperformer_url + "\">" + blockperformer + "</a><br/><b><i>" + msg("block-type") + " </b></i>" + block_type + partial_block_details + "<br/><b><i>" + msg("block-start") + " </b></i>" + blockStartDateTime + "<br/><b><i>" + msg("block-expiry") + " </b></i>" + blockEndDateTime + "<br/><b><i>" + msg("block-reason") + " </b></i><blockquote style=\"border-left: 5px solid rgba(var(--theme-alert-color--rgb), 0.5); padding-left: 0.5em;\">" + parsedBlockReason + "</blockquote>";
