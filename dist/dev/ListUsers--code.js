...
$(function() {
	...
	new mw.Api().loadMessagesIfMissing(['talkpagelinktext', 'contribslinks', 'nchanges']).then(function() {
		var msgTalk = mw.message('talkpagelinktext').text();
		var msgContribs = mw.message('contribslinks').text();
		var msgEdits = mw.message('nchanges', '$1').text();
		function createList (group, au) {
			...
			for (var i in au) {
				...
				if (listUsers.talk) {
					html += ' <a href="' + articlePath.replace('$1', 'User_talk:' + enc) + '">(' + msgTalk + ')</a>';
				}
				if (listUsers.contribs) {
					html += ' <a href="' + articlePath.replace('$1', 'Special:Contributions/' + enc) + '">(' + msgContribs + ')</a>';
				}
				if (listUsers.editcount) {
					html += ' ' + msgEdits.replace('$1', au[i].editcount);
				}
				html += '</li>';
