...
.then(function() {
	...
	if (
		...
		!/content-moderator|sysop|vanguard|content-volunteer|soap|helper|wiki-specialist|wiki-representative|staff/.test(config.wgUserGroups.join('\n'))
	) {
		return;
	}
	...
	$('.page-header__contribution-buttons .wds-list, .page-header__actions .wds-list').first().append(
		$('<li>').append(
			$('<a>', {
				...
				href: new mw.Title('ChangeContentModel', -1).getUrl({ pagetitle: config.wgPageName.replaceAll('_',' ') }),
			})
		)
	);
});