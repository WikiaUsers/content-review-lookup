...
$(function () {
  ...
  function fetchLanguage() {
    return new Promise(function (resolve, reject) {
      ...
      fetch('https://ark-data.seen-von-ragan.de/data/loc/ark_data_' + lang + '.json')
  ...
  function fetchData(type, mod) {
    return new Promise(function (resolve, reject) {
      if (cache[type] && cache[type][mod]) {
        resolve(cache[type][mod]);
        return;
      }
  
      fetch('https://ark-data.seen-von-ragan.de/data/mods/' + mod + '/' + type + '-wiki.json')
        .then(function (response) { return response.json(); }) // parse JSON
        .then(function (data) {
          if (!cache[type]) {
            cache[type] = {};
  ...
  function addData(type, mod, item) {
    Promise.all([fetchLanguage(), fetchData(type, mod)])
      .then(function (values) {
        processResult(values[1], item, type, values[0]);
      });
  }

  function processResult(data, item, type, langData) {
    var items = findItems(data, item, type, langData);
    if (lang) {
      items = items.map(function (item) {
        var langObj = langData[type].find(function (obj) { return obj.blueprint == item.blueprint;} );
        return langObj ? Object.assign({ localizedName: langObj.name }, item) : item;
      });
    }
  ...
  function findItems(data, itemName, type, langData) {
    var re = new RegExp(itemName, 'i');
    var localizedItems = (langData[type] || [])
      .filter(function (langObj) { return re.test(langObj.name); })
      .map(function (langObj) { return langObj.blueprint; });
    return data.filter(function (obj) {
      return re.test(obj['dossierName']) || 
        re.test(obj['name']) ||
        localizedItems.indexOf(obj['blueprint']) > -1;
    });
  }
