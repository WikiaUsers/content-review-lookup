//Comments using the asterisk/slash format (/**/) are section headers (for multiple different scripts with similar functions). The slash (//) comments are for //stand-alone scripts.
...
/*add pages to be purged every 24 hours directly below*/
	...
	mw.loader.using('mediawiki.api').then(function() {
		try {
			...
			if (Date.now() - lastPurgeTime.valueOf() <= 1 * 60 * 60 * 1000)
				return;
		} catch(e) {
			return;
		}
...
window.UserTagsJS = {
	modules: {},
	tags: {
		// groupname: { u:'Text when users sex is not listed', m:'When user is male', f:'When user is femal', link:'Link the tag should lead to', title:'Text to display on hover', order:-1/0 }
