...
//   2. Making sure there is a heading identified by the "fpheader" class.
...
var fpmobilecollapse = fpmobilecollapse || {
    initialize : function() {
        ...
        $( '.fpbox.mobilecollapsible' ).each( function() {
            var fpheader = $( this ).children( '.fpheader' ).not( '.nomobile, .notoggle' );
            if ( fpheader.length > 0 && $( this ).children( '.fpbody' ).length > 0 ) {
                $( this ).addClass( 'mobilecollapsible' + index );
                fpheader.first().html( $( '<a class="togglecollapse" href="javascript:fpmobilecollapse.toggle( ' + index + ' )"></a>' ).html( fpheader.html() ) );
            }
            ++index;
        } );
    },
