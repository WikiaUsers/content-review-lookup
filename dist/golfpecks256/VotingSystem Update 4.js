...
if ( document.getElementById("ScoreVoteCountContainerMulti") !== null ) {
	...
	for (var i = 0; i < SupportVoteScores.length; i++) {
		...
		if (key === "" && ExceptComb[1]) {
			var tmpContainer = SupportVoteScores[i].parentElement.parentElement.parentElement;
			key = document.getElementById(tmpContainer.innerHTML.includes("SupportBlue") ? "489bce" : "5fa41e").innerHTML;
		}
	...
	for (var key in dictVotesCount) {
		if (dictVotesCount.hasOwnProperty(key)) {
			...
			currentVoteStatus += '"' + key + '" has score "' + (!ExceptComb[1] ? tmpCurrScore.toFixed(2) : '{{{CurrScore' + key + '}}}') + '"';
			currProds.push(key);
	...
	var bestProd = countMax === 1 ? currProds[maxIX] : "";

	if (counter > 0) {
		if (ExceptComb[0]) currentVoteStatus += ', ' + outputWord + ' is "' + (bestProd !== "" ? bestProd + ' ' + ResultTrue : ResultNull) + '"';
		currentVoteStatus += '.';
	} else {
	...
	if (ExceptComb[1]) {
		var blueKey = document.getElementById("489bce").innerHTML;
		var greenKey = document.getElementById("5fa41e").innerHTML;

		var sumB = dictVotesCount[blueKey] / (dictVotesCount[blueKey] + dictVotesCount[greenKey]);
		var sumG = dictVotesCount[greenKey] / (dictVotesCount[blueKey] + dictVotesCount[greenKey])

		var scoreB = dictVotesNumSum[blueKey] * sumB;
		var scoreG = dictVotesNumSum[greenKey] * sumG;

		var holderB = '{{{CurrScore' + blueKey + '}}}';
		var holderG = '{{{CurrScore' + greenKey + '}}}';

		currentVoteStatus = currentVoteStatus.replace(holderB, scoreB.toFixed(2));
		currentVoteStatus = currentVoteStatus.replace(holderG, scoreG.toFixed(2));
	}
