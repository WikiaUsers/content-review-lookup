	...
	for (var key in dictVotesCount) {
		if (dictVotesCount.hasOwnProperty(key)) {
			...
			if (!ExceptComb[1]) {
				...
				tmpCurrScore = dictVotesNumSum[keyB] * (dictVotesCount[keyB] / (dictVotesCount[keyB] + dictVotesCount[keyG]));
			} else if (key === keyG) {
				tmpCurrScore = dictVotesNumSum[keyG] * (dictVotesCount[keyG] / (dictVotesCount[keyB] + dictVotesCount[keyG]));
			}
