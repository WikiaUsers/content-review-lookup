...
mwCustomEditButtons['reftools'] = [refbuttons, 'commons/e/ea/Button_easy_cite.png', 'Добавить шаблон цитирования']



function refbuttons() {
 if ( ! $('#citeselect').length )
  $('#editform').prepend(
    '<div id="citeselect">'+
    '<input type=button value="Web" onclick="citeWeb()" />'+
    '<input type=button value="News" onclick="citeNews()" />'+
    '<input type=button value="Книга" onclick="citeBook()" />'+
    '<input type=button value="Статья" onclick="citeJournal()" />'+
...
function citeJournal() {
  ...
  form = '<div id="citediv'+numforms+'">'+
    ...
    '<td width="120"><label for="pmid">&nbsp;PMID: </label></td>'+
      '<td width="400"><input type="text" tabindex=1 style="width:100%" id="pmid"></td></tr>'+
    '<tr><td width="120"><label for="refname">&nbsp;Имя сноски: </label></td>'+
      '<td width="400"><input type="text" tabindex=1 style="width:100%" id="refname"></td></tr>'+
    '</table>'+
    '<input type="button" value="Добавить" onClick="addcites()">'+
 '</fieldset></div>';
...
function errorCheck() {
  ...
  templates = false //templates = document.getElementById('templates').checked;
    ...
    if (samecontent) {
      ...
      while (p<allrefs.length && !skipcheck) {
        if (allrefscontent[i] == allrefscontent[p] && i != p) {
          ...
          errorlist[q] += '<td width="25%">Несколько сносок имеют одинаковое содержание, вместо этого надо использовать <a href="//ru.wikipedia.org/wiki/Википедия:Сноски#Повторное_использование_одной_и_той_же_сноски">именованные сноски</a></td></tr>';
          q++;
    ...
    for (var k=0; k<namedrefs.length; k++) {
      ...
      while (z<namedrefs.length && !skipcheck) {
        if (namedrefs[k] == namedrefs[z] && k != z) {
          ...
          errorlist[q] += '<td width="25%">Повторяющиеся сноски с одинаковым <a href="//ru.wikipedia.org/wiki/Википедия:Сноски#Повторное_использование_одной_и_той_же_сноски">именем</a></td></tr>';
          q++;
    ...
    for (var p=0; p<namedrefcalls.length; p++) {
      ...
      if (!skipcheck) {
        if (!NRcallError(namedrefs, namedrefcalls[p])) {
          ...
          errorlist[q] += '<td width="25%"><a href="//ru.wikipedia.org/wiki/Википедия:Сноски#Повторное_использование_одной_и_той_же_сноски">Именованная сноска</a> используется, но её текст не задан</td></tr>';
          q++;
