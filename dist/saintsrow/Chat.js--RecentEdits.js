/* recent edits */
  ...
  +'&rcprop=user|title|ids|timestamp|loginfo&rcexcludeuser='+wgUserName, 
  function(result) { 
      ...
      if ($('#RecentEdits').size()) {
        ...
        $(recentchanges).each(function( index, value ) {
          if (!$('#RecentEdits #rcid'+value.rcid).size()) {                  //if entry does not exist
            ...
            commentType[wgNamespaceIds['top_10_list_talk']]  = {1:"Comment",    2:"Comment reply"};
            commentType[0]  = {1:"Other",    2:"Other reply"};

            if (value.title.split("/@comment").length > 1) {
              if (typeof commentType[value.ns] == "undefined") {
