...
$(function() {
    ...
    $.each(catList, function(index, value) {
        function getList(cmcontinue) {
            $.getJSON('/api.php?action=query&prop=imageinfo&iiprop=metadata&generator=categorymembers&gcmtitle=Category:' +
              ...
              '&gcmprop=title&gcmtype=file&gcmlimit=max&format=json&rawcontinue=&gcmcontinue=' + cmcontinue, function(data) {
                if (data.query) {
    ...
    function stepTwo() {
        ...
        $.each(IDlists, function(index, value) {
            $.getJSON('https://www.googleapis.com/youtube/v3/videos?part=id&id=' + 
    ...
    function results() {
        ...
        if (! $.isEmptyObject(videoList)) {
            ...
            $.each(videoList, function(key, value) {
                $('#dv-results-1').append('<li><a href="/' + encodeURIComponent(value.replace(/ /g, '_')) +
        ...
        if (noId.length) {
            ...
            $.each(noId, function(index, value) {
                $('#dv-results-2').append('<li><a href="/' + encodeURIComponent(value.replace(/ /g, '_')) +
        ...
        if (nonYT.length) {
            ...
            $.each(nonYT, function(index, value) {
                $('#dv-results-3').append('<li><a href="/' + encodeURIComponent(value.title.replace(/ /g, '_')) +
