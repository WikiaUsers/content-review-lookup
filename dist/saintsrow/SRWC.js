...
function emoticonReload(omitMessage) {
	$.ajax({
		...
		'success': function(data) {
			mw.config.set({"wgChatEmoticons":data});
			if (!omitMessage) {
...
function randomEmoticon() {
  var emot = mw.config.get("wgChatEmoticons").split("<!-- Begin random list -->")[1].split("http");
  emot.shift();
...
function getOgg(oggFile) {
  ...
  if (!$("script[src*='OggPlayer.js']").size()) $("head").append('<script src="https://saintsrow.wikia.com/extensions/OggHandler/OggPlayer.js?12"></script>');

  $.getJSON("/api.php?action=parse&format=json&disablepp=1&prop=text&text={{audio|"+oggFile.substring(5)+"}}", function(data) { 
	var theOgg = $("#ogg_player_1", data.parse.text["*"]);
	if (!theOgg[0]) mainRoom.inlineAlert("File not found.");
	else {
	  $("#ogg_player_1").remove();
	  $(".Chat").after(theOgg);
	  $("#ogg_player_1 button").click();
	  $('#ogg_player_1 audio').prop('volume', chatOptions.options.chatVolume/100);
	  $("#ogg_player_1").append( $('<img />').attr('src', "https://images.wikia.nocookie.net/saintsrow/images/thumb/2/2f/UI_N-A.png/20px-UI_N-A.png").css({"vertical-align":"bottom", "cursor":"pointer"}).bind("click", function() { $("#ogg_player_1").remove(); }) );
	}
  });
}
...
function customStatusChange(status) {
	...
	if (status.attributes.isModerator) $("li.User[data-user='"+status.attributes.name+"']").addClass('chat-mod');

	doCustomStatusChange(status.attributes.name, status.attributes.statusState);
	var stat = status.attributes.statusMessage;
	if (stat == "logo") {
		$(".ChatWindow .Rail").css("background-image", 'url("https://goo.gl/hhFYaf?'+Date.now()+'")');
	} else if(stat.length) {
	  if (stat.substring(0,stat.indexOf(":")) == "emoticon") {
		image = stat.substring(stat.indexOf(":", (stat.indexOf(":")+1) )+1);
		trigger = stat.substring(stat.indexOf(":")+1,stat.indexOf(":",stat.indexOf(":")+1)).trim();
		mw.config.values["wgChatEmoticons"] = "\n*"+image+"\n**"+trigger+"\n"+mw.config.values["wgChatEmoticons"]+"\n*"+image+"\n**"+trigger+"\n";
...
function updateChatSkin(save) {
	...
	if (chatOptions.options.hideBackground) $('body')[0].style.setProperty('background-image', 'none', 'important' );
	else $("body").css('background-image','');
	$('body')[0].style.setProperty('background-color', chatOptions.options.pageColor, 'important' );
	...
	if (chatOptions.options.emoticonDisable) mw.config.set({"wgChatEmoticons":""}); 
	else if(mw.config.get("wgChatEmoticons") == "") emoticonReload();
...
function showUserOptions() {
	...
	$("#ModalTable h1").html('Go to <a href="https://saintsrow.wikia.com/wiki/User:'+wgUserName+'/chat.js?action=edit" target="_blank">User:'+wgUserName+'/chat.js</a> and add: ');
...
function showOptionsPanel() {
	...
	+'<div title="Enable sound notifications for new messages">'
		...
		+'<div>'
			+'<input type="checkbox" name="soundNameOnly" value="soundNameOnly" id="soundNameOnly"> <label for="sounds">Name Only</label>'
	...
	+'<div style="margin-left: 1em;">'
		+'<div title="Time to wait before testing connection"><input type="number" type="text" name="timeoutTime" id="timeoutTime" default="60" style="width: 3em;"> <label for="timeoutTime">Timeout (s)</label></div>'
	+'</div></div>'
...
function onInit() {
	$("#chatwelcome").remove();
	mainRoom.inlineAlert('<div id="chatwelcome">Chat initialised, Playa.<br>Confused?  See <a href="https://saintsrow.wikia.com/wiki/Help:Chat">Help:Chat</a> and <a href="https://saintsrow.wikia.com/wiki/Help:Chat_Options">Help:Chat Options</a><br>Need Help?  Use our <a href="https://saintsrow.wikia.com/wiki/Saints_Row_Wiki:Forums">Forums</a>.<br>Need ideas? Check the <a href="https://saintsrow.wikia.com/wiki/Saints_Row_Wiki:To-do_list">To-do list</a>! or <a href="https://saintsrow.wikia.com/wiki/Special:Random?action=edit">Edit a random article</a>!</div>', "alertInit");
	...
	if (typeof FlashTitle == "function") {
		window.alert("Incompatible outdated script detected, refreshing chat window.");
