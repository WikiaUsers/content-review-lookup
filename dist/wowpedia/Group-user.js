...
$(function() {
  $("div.amdiv-text").each(function() {
    ...
    adt = adt.replace(/\[\[([^\]]*?)\]\]/g,function(match,p1,offset,string) {
      if (match.slice(0,7) === "[[File:") {
        var split = match.replace(/[\[\]]/g,"").split("|");
        var file = split.shift().slice(5);
        var md5 = hex_md5(file);
        var path = mw.config.get("wgServer")+"/media/"+md5.slice(0,1)+"/"+md5.slice(0,2)+'/'+file;
        var imglink = "";
        var title = "";
        for (s in split) {
          if (split[s].slice(0,5) === "link=") imglink = split[s].slice(5);
          else if (!split[s].match(/\d+\w+/)) title = split[s];
        }
        var output = "";
        if (imglink !== "") output = '<a href="'+mw.config.get("wgServer")+'/'+encodeURIComponent(imglink.replace(/ /g,"_"))+'">';
        output += '<img src="'+path+'" alt="'+title+'">';
        if (imglink !== "") output += "</a>";
        return output;
      }
      var split = match.replace(/[\[\]]/g,"").split("|");
      var output = '<a href="'+mw.config.get("wgServer")+'/'+encodeURIComponent(split[0].replace(/ /g,"_"))+'">';
