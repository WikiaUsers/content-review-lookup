(function($) {
    function observe(elem, selector, callback) {
        if(MutationObserver) {
            ...
            $(elem).each(function() {
                observer.observe(this, { childList: true, characterData: true, attributes: true })
            });
        } else {
    ...
    window.PFD = {
        ...
        updateForm: function(textarea) {
            ...
            if(skin == 'monobook') mw.util.addCSS('.sprite.question { background-position:-1144px -20px; height:12px; width:12px; background-color:transparent; background-image:url(https://images.wikia.nocookie.net/__cb1475889905/common/skins/shared/images/sprite.png); background-repeat: no-repeat }');

            PFD.textarea = $(textarea).addClass('pfd-initialized');
            PFD.textarea.height('auto').attr('rows', 9);
            
            if(PFD.templates.length > 1) {
                PFD.textarea.before('<div id="fileDescTemplatesSwitch" style="display:none"><select id="fileDescriptionTemplate"></select>&nbsp;<span style="display:inline-block;vertical-align:middle;cursor:help;" class="sprite question" title="'+PFD.msg('template-change-notice')+'"></span></div>');
        ...
        changeTemplate: function(id) {
            ...
            if(PFD.templates[id].altdesc) {
                ...
                $('#fileDescTemplatesMoreInfo').append(' <span style="display:inline-block;vertical-align:middle;cursor:help;" class="sprite question" title="'+PFD.msg('template-change-notice')+'"></span>');
