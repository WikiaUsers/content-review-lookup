    ...
    window.Skins = {
        ...
        resizeSlideshow: function(width) {
            ...
            ss.find('.skin-image img').each(function() {
                var rx = /\/([0-9]+)px-/
                var oldW = rx.exec(this.src)[1]
                if(oldW < W) this.src = this.src.replace(rx, '/' + slideW + '-')
            })
        },
