/**
 * http://dev.wikia.com/wiki/MediaWiki:Tooltips.js
 */
...
var tooltips = tooltips || {
  ...
  addType: function(tt) {
    if (typeof tooltips.types[tt.classname] == 'undefined') {
      ...
      if (typeof tt.parse == 'string' || typeof tt.parse == 'function') var parse = tt.parse;
      else var parse = false;
      if (typeof tt.text == 'string' || typeof tt.text == 'function') var text = tt.text;
      else var text = false;
      ...
      if (typeof obj.text == 'string') obj.parameters = tooltips.getParameters(obj.text);
      else obj.parameters = [];

      if (typeof tt.delay == 'string' || typeof tt.delay == 'number') obj.delay = parseInt(tt.delay);
      else obj.delay = false;
  ...
  sameWidth: function() {
    if ($("#tooltip-wrapper").find('.main-tooltip').length == 1) {
      ...
      var width = 0;
      $("#tooltip-wrapper").find('.main-tooltip').each(function() {
  ...
  wrapperPosition: function(mouseX, mouseY) {
    ...
    $("#tooltip-wrapper").find('.main-tooltip').each(function() {
      if (typeof $(this).data('outerheight') != 'undefined') tipH += $(this).data('outerheight');
    });
    $("#tooltip-wrapper").find('.main-tooltip').each(function() {
      if (typeof $(this).data('outerwidth') != 'undefined') tipW = Math.max(tipW, $(this).data('outerwidth') + parseInt($("#tooltip-wrapper").css('padding-left')) + parseInt($("#tooltip-wrapper").css('padding-right')));
    });
    ...
    if (toRight >= tipW) $("#tooltip-wrapper").css({
      left: coordX + 'px',
      top: coordY + 'px'
    });
    else $("#tooltip-wrapper").css({
      left: coordX - tipW - tooltips.offsetX * 2 + 'px',
      top: coordY + 'px'
    });
  },
  handlers: {
