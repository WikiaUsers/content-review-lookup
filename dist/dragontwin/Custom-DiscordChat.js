...
(function (window, $, mw) {
    ...
    window.DiscordChat = {
        ...
        rail: function () {
            ...
            var isDefault = (displayMode === 'default' || displayMode === 'compact'); 
            var isExpand = !isDefault;
            
            var ofOnline = window.DiscordChat.formatCount(totalOnline) + (isDefault ? '' : ' ' + onlineLabel);
            var ofMembers = window.DiscordChat.formatCount(totalMembers) + (isDefault ? '' : ' ' + membersLabel);
            
            var statusClasses = 'stats-column' + (isDefault ? ' is-default' : ' is-expanded');
                               ...
                               '<div class="chat-container-capsule">' +
                                 '<div class="stats-wrapper">' +
                                   ...
                                   '<div class="' + statusClasses + '">' +
                                     '<span class="stat-members" title="' + ofMembers + '">' + ofMembers + '</span>' +
                                     '<span class="stat-online" title="' + ofOnline + '">' + ofOnline + '</span>' +
                                   '</div>' +
                                 '</div>' +
            ...
            if ($target.length) { $target.after(element); } 
            else { $('#WikiaRail').prepend(element); }
        ...
        thriftData: function() {
            ...
            var displayMode = (window.DiscordChat.messages['Custom-DiscordChat-display'] || 'default').toLowerCase();
            var isDefault = (displayMode === 'invert' || displayMode === 'compact');
            var isExpand = !isDefault;
    ...
    // Initialize using Fandom's loader
    mw.loader.using(['mediawiki.api', 'mediawiki.util'], function () {
        importArticles({
            ...
            type: 'style',
            articles: [
                'MediaWiki:Custom-DiscordChat.css'
            ]
        });
