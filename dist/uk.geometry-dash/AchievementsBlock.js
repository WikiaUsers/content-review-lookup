...
$(document).ready(function addCustomAchievementsModule() {
    ...
    if ( $("#WikiaRail.loaded").length === 0 ) {
        ...
        var header = (data.wgUserName == data.wgTitle)?'Ваші досягнення:':('Досягнення користувача ' + data.wgTitle + ':');
 
        $.get(
            '/uk/index.php?action=render&title=MediaWiki:Custom-UserAchievement-' + data.wgTitle, 
            function(c) {
                if (!c) return false;
                $("#WikiaRail").prepend(
