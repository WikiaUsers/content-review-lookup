(function() {
	...
	mw.loader.using( 'mw.Api' ).then(function() {
		var api = new mw.Api();
		var actualPageLinks = "Ссылки не найдены";
		var linkBase = location.protocol + '//' + location.host + '/ru/wiki/';
		var whatReallyLinksHerePage = "Служебная:Реальные_ссылки_сюда";
		...
		function generateSpecialPage() {
            ...
            filterForm.append($('<br />')).append($('<div style="width: 50%;height: 20px;margin-top: 5px;float: left;">').append($('<div style="width:20px;height:20px;background-color: #FFC107;display: inline-block;">')).append($('<span style="font-size:11px;vertical-align: top;line-height: 20px;display: inline-block;margin-left: 5px;">').text('Нет текста для отображения')));
            filterForm.append($('<div style="width: 50%;height: 20px;margin-top: 5px;float: right;">').append($('<div style="width:20px;height:20px;background-color: #5ff765;display: inline-block;">')).append($('<span style="font-size:11px;vertical-align: top;line-height: 20px;display: inline-block;margin-left: 5px;">').text('Частичное совпадение')));
            ...
            searchBoxForm.submit(function(e) {
                ...
                if (searchText !== '') {
                	updateTitleHeading('Реальные ссылки сюда');
                    setTimeout(function() {
			...
			var firstHeadingText = 'Реальные ссылки сюда';
			...
			if (mw.config.get("wgPageName").indexOf('/') != -1) {
                ...
                if (subSearchPage !== '') {
                    ...
                    firstHeadingText = 'Страницы, реально ссылающиеся на «' + subSearchPage + '»';
                }
            }
        ...
        function highlightFilters(filterText) {
            ...
            $('#FilterCount').text(filterCount+' совпадений с фильтрами.');
        }
        
        function updateTitleHeading(newText) {
        	$('#firstHeading').text(newText);
        	document.title = newText + ' — Убежище';
        }
		        	...
		        	if (linkInfos[titleLink].linkshere !== undefined) {
			            ...
			            for (var linkInf in linkInfos[titleLink].linkshere) {
			            	if (linkInfos[titleLink].linkshere[linkInf].redirect !== undefined) {
                                numLinks -= 1;
                                if (!hasRedirects) {
                                    $(RedirectArea).append('<div><h2>Перенаправления:</h2></div>');
                                    hasRedirects = true;
                                }
                        ...
                        if (!isRedirectPage) {
                        	$('#LinkCount').html('Найдено 0 ссылок со страниц');
                            $(mainLinkArea).append(actualPageLinks);
                            $('#LoadingInfo').hide();
                            $('#LoadingInfo').find('#RecordInfo').text('Обработано 0/0 записей.');
		...
		function checkIfLinksReal(pageLinks, basePageName, IsRedirectPage) {
			...
			api.get({
				...
				if (data.error === undefined) {
		            ...
		            numLinks -= 1;
		            currNumProcessing -= 1;
		            $('#LoadingInfo').find('#RecordInfo').text('Обработано '+(totalLinks-numLinks).toString()+'/'+totalLinks.toString()+ ' записей.');
		            if (numLinks <= 0) {
		                ...
		                $('#LoadingInfo').hide();
		                $('#LoadingInfo').find('#RecordInfo').text('Обработано 0/0 записей.');
		                $('#LinkCount').text(linkData.links.length+' ссылок со статей найдено.');
		                $('#RedirectCount').text(linkData.Redirects.length+' с перенаправлений.');
		                totalLinks = 0;
		                processingLinks = false;
		            }
		        } else {
		...
		$(document).ready(function() {
            ...
            var whatReallyLinksHereElem = $('<li class="overflow">').append($('<a href="' + concatPageLink() + '" data-name="whatreallylinkshere">').text('Реальные ссылки сюда'));
