...
function addFastDeleteButtons() {
	...
	switch( skin ) {
		...
		case 'wikia':
			...
			for( var i = 0; i < fdButtons.length; i++ ) {
				afd = '<a class="wikia-button" href="/index.php?title=' + encodeURIComponent( wgPageName ) + '&redirect=no&action=delete&submitdelete=true&autocloseit=true&wpReason=' + encodeURIComponent( fdButtons[i].summary ) + '" title="one-click delete: ' + fdButtons[i].summary+'" data-id="delete">'+ fdButtons[i].label + '</a>';
				if( location.href.match( /[&?]diff=/ ) && !$( '#UserProfileMasthead' ).length ) {
					$( 'div.WikiaPageHeader > p' ).before( afd );
				} else {
