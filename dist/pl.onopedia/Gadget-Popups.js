//<nowiki>
//+[pl-fix] pg.re.contribs,pg.re.email,pg.re.backlinks
//+[fix] mouseOverWikiLink2 with popupOnlyArticleLinks=false and Vector
//+[fix] WikEd previews
//+[fix] Opera menu
...
//<NOLITE>
		...
		return f("<a class='image' ? href=\"?\">?</a>", (caption!='')? "title=\"" + escapeQuotesHTML(caption) + "\"" : '', escapeQuotesHTML(Insta.conf.paths.articles + Insta.conf.locale.image + ':' + filename), img);
//</NOLITE>
	}
...
function setRegexps() {
	...
	// [pl-fix] pg.re.contribs,pg.re.email,pg.re.backlinks
	pg.re.contribs  =RegExp('(title=|/)'  + sp + '(?:%3A|:)Wk%C5%82ad' + '(&target=|/|/' + mw.config.get('wgFormattedNamespaces')[pg.nsUserId]+':)(.*)') ;
	pg.re.email     =RegExp('(title=|/)'  + sp + '(?:%3A|:)E-Mail'     + '(&target=|/|/(?:' + mw.config.get('wgFormattedNamespaces')[pg.nsUserId]+':)?)(.*)') ;
...
function setBrowserHacks() {
	...
	if (typeof window.opera != 'undefined') {
		if (leadingInteger(opera.version()) < 10) { useOriginal=true; }	// new CSS should probably work, but not tested
...
function generalLink(l) {
	...
	var ret='<a href="' + url + '"';
	if (typeof l.title!='undefined' && l.title) { ret += ' title="' + escapeQuotesHTML(l.title) + '"'; }
	if (typeof l.onclick!='undefined' && l.onclick) { ret += ' onclick="' + escapeQuotesHTML(l.onclick) + '"'; }
...
function editCounterLink(l) {
	...
	var url;
	var soxredToolUrl='//toolserver.org/~tparis/pcount/index.php?name=$1&lang=$2&wiki=$3';
...
//</NOLITE>
	...
	// regexps
	newOption('popupDabRegexp', '{{([dD]isambig|[uU]jednoznacznienie|[sS]u)}}');
...
addOnloadHook( function() {
   	if(typeof doLivePreview != "function" || typeof $j != "function") return;
	$j("#wpPreview").click(function(){
...
wikEdSetupHook.push(function() {
	$j("#wikEdLocalPreview").click(function(){
		var i = setInterval(function(){
			var p = document.getElementById("wikEdPreviewBox");
			p.ranSetupTooltipsAlready = false;
			setupTooltips(p);
			clearInterval(i);
		}, 500);
