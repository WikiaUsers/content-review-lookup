...
/*
  ...
  Authors:
    ...
    V2.3: September 2010: [[User:Infovarius]] (correct categories in templates)
...
var HotCat = {
 ...
 ,category_canonical : 'Категория'
   ...
   // MediaWiki 1.16 or later; otherwise, set it to the preferred category name (e.g., "Категория").
 ,category_summary : 'Category'
   // Overall default category name (english). Used for summary description as short alias.
  ...
  function change_category (wikitext, toRemove, toAdd, key) {
    ...
    if (toAdd && toAdd.length > 0) {
      ...
      if (matches && matches.length > 0) {
        ...
        if (wgNamespaceNumber == 10 && !toRemove) newcatstring = '<noinclude>' + newcatstring + '</noinclude>';
        if (cat_point >= 0) {
    ...
    if (wgArticleId > 0) {
      if (changes == 1) {
        ...
        for (var i=0; i < deleted.length; i++) {
          summary.push ('−[[' + HotCat.category_summary + ':' + deleted[i] + ']]');
        }
        if (deleted.length == 1)
          shortSummary.push ('−[[' + HotCat.category_summary + ':' + deleted[0] + ']]');
        else if (deleted.length > 1)
          shortSummary.push ('− ' + HotCat.messages.multi_change.replace ('$1', "" + deleted.length));
        // Added
        for (var i=0; i < added.length; i++) {
          summary.push ('+[[' + HotCat.category_summary + ':' + added[i] + ']]');
        }
        if (added.length == 1)
          shortSummary.push ('+[[' + HotCat.category_summary + ':' + added[0] + ']]');
        else if (added.length > 1)
          shortSummary.push ('+ ' + HotCat.messages.multi_change.replace ('$1', "" + added.length));
        // Changed
        for (var i=0; i < changed.length; i++) {
          if (changed[i].from != changed[i].to) {
            summary.push ('±[[' + HotCat.category_summary + ':' + changed[i].from + ']]→[['
                         + HotCat.category_summary + ':' + changed[i].to + ']]');
          } else {
            summary.push ('±[[' + HotCat.category_summary + ':' + changed[i].from + ']]');
          }
        }
        if (changed.length == 1) {
          if (changed[0].from != changed[0].to) {
            shortSummary.push ('±[[' + HotCat.category_summary + ':' + changed[0].from + ']]→[['
                         + HotCat.category_summary + ':' + changed[0].to + ']]');
          } else {
            shortSummary.push ('±[[' + HotCat.category_summary + ':' + changed[0].from + ']]');
          }
