...
window.allWikis = [];
window.rlWikis = {};
window.langs = ["BG","BS","CS","DE","EL","EN","ES","ET","FI","FR","HU","ID","IT","JA","KO","NL","PL","PT","PT-BR","RU","SK","UK","TH","TR","VI","ZH","ZH-CN","ZH-HANS"];
		...
		for (c in data.query.recentchanges) {
			...
			if (change.type == "edit" || change.type == "new") {
				...
				if (change.logaction=="delete") {
					...
					h+='<li>(<a href="'+d+'/Special:Log/curseprofile" target="_blank">Profile log</a>)&nbsp;'+grcDTF.format(da)+' &nbsp; <a href="'+d+'/User:'+u+'" target="_blank">'+change.user+'</a> (<a href="'+d+'/User_talk:'+u+'" target="_blank">talk</a> | <a href="'+d+'/Special:Contributions/'+u+'" target="_blank">contribs</a>) edited the '+change.logparams[0].replace("profile-","")+' on '+change.title.replace("UserProfile:","")+'\'s profile. &nbsp;'+(comment==""?'':'('+comment+')')+'</li>';
				}
				else ns++;
...
window.loadWikiList = function() {
	...
	api.get( {
		'action': 'allsites',
		'do': 'getSiteStats',
		'filter': 'wikis|md5_key,ss_total_edits,wiki_domain,wiki_display_name,wiki_managers'
	}).done(function(data) {
		wikis = data.data.wikis;
		for (w in wikis) {
			var wiki = wikis[w];
			allWikis.push({"key":wiki.md5_key,"edits":(wiki.ss_total_edits == undefined)?null:wiki.ss_total_edits,"domain":wiki.wiki_domain,"displayname":wiki.wiki_display_name,"wiki_manager":wiki.wiki_managers[0]});
	    }
