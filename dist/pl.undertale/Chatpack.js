...
if(wgCanonicalSpecialPageName == 'Chat') {
	...
	$(function () {
		...
		importScriptURI('http://c.wikia.com/wiki/User:Monchoman45/ChatHacks.js/i18n.js?action=raw&ctype=text/javascript');
	...
	function OpenChatWindow() {
		window.chatwindow = window.open('/wiki/Special:Chat?useskin=oasis', 'chat');
...
// nav linkfix
$('#WikiHeader > nav > ul > li.nav-item.marked > ul > li:nth-child(5) a[data-canonical="chat"]').replaceWith('<a class="subnav-2a" style="cursor: pointer;" onclick="OpenChatWindow()">Czat</a>');
 
// MULTIKICK
// by Madnessfan34537 and Callofduty4
function showPopup() {
        $('body').append('<section style="left: 50%; top: 50px; width: 434px; z-index: 2000000002; margin-left: -222px;" class="modalWrapper" id="masskicker"><button class="close wikia-chiclet-button" onclick="cancelKick()"><img src="https://images.wikia.nocookie.net/__cb57523/common/skins/oasis/images/icon_close.png"></button><h1>Multikick</h1><section class="modalContent"><div><form method="" name="" class="WikiaForm "><fieldset><p>Wprowadź nicki użytkowników do wyrzucenia, rozdzielając je przecinkiem i spacją.</p><input type="text" name="multikickinput" id="multikickinput" /></fieldset></form><div style="float:right;"><a onclick="kickUsers()" class="wikia-button">Gotowe</a>&nbsp;<a onclick="cancelKick()" id="cancel" class="wikia-button secondary">Anuluj</a></div></section></section>');
...
$('<a id= "multiKickerButton" class="wikia-button" href="javascript:showPopup()" style="position:absolute; right:75px; top:22px;">Multikick</a>').appendTo('.Write');
// MULTIPRIV
// by Joeytje50
if(wgCanonicalSpecialPageName == 'Chat') {
function createGroupPM() {
var users = prompt('Podaj uczestników multipriva – rozdziel nicki przecinkiem','');
