...
/*
  Purpose: For ImageAnnotator gadget ("add note") on File: pages.
  Source:  Commons: http://commons.wikimedia.org/w/index.php?title=MediaWiki:LAPI.js
  Documentation: http://commons.wikimedia.org/wiki/Help:Gadget-ImageAnnotator
    installed by phlox 2009-09-05
*/
...
if (!Array.lastIndexOf) {
  ...
  {
    if (target === null) return -1;
    if (typeof (target.indexOf) == 'function') return target.lastIndexOf (elem, from);
    if (typeof (target.length) == 'undefined') return -1;
    var l = target.length;
...
LAPI.DOM =
{
  ...
  currentStyle : function (element, property)
  {
    function normalize (prop) {
      // Don't use a regexp with a lambda function (available only in JS 1.3)...
    ...
    if (window.getComputedStyle) // Gecko etc.
      return window.getComputedStyle (element, null).getPropertyValue (property);
    else if (element.currentStyle) // IE, has subtle differences to getComputedStyle
      return element.currentStyle[normalize (property)];
    else // Not exactly right, but best effort
      return element.style[normalize (property)];      
  }
...
LAPI.DOM.DocumentFacade.prototype =
{
  ...
  getElementsByTagName : function (tag)
  {
    ...
    function traverse (node, tag)
    {
      ...
      while (curr) { traverse (curr); curr = curr.nextSibling; }
    }
...
LAPI.WP = {
  ...
  getPreviewImage : function (title, doc)
  {
    ...
    for (var i = 0; i < imgs.length; i++) {
      ...
      //console.info('phlox- SRC=',src,' TITLE=',title);
      //var re = new RegExp ('^http://images\\d.wikia.nocookie.net.*' + (title.replace (/ /g, '_').replace (/(\.svg)$/i, '$1.png').escapeRE ()) + '$')
      //if (src.search (re) == 0)  // Apparently, the imgs array has both the svg as well as the png version.  This rejects the svg and waits for the png.
      if (src.search (new RegExp ('^http://images\\d.wikia.nocookie.net.*' + title.replace (/ /g, '_').replace (/(\.svg)$/i, '$1.png').escapeRE () + '$')) == 0)
        return imgs[i];
    }
    //console.debug('phlox-LAPI getpreviewImage regex failed to find img ;-).');
    //console.debug('phlox-title=',title);
    return null;
  },
  ...
  pageFromLink : function (lk)
  {
    ...
    if (!href) return;
    if (LAPI.DOM.hasClass (lk, 'new')) { // href="/index.php?title=...&action=edit"
  ...
  LAPI.Ajax.getPage (
    ...
    , function (request, failureFunc) {
        ...
        try {
          ...
          revision_id = request.responseText.match (/var wgCurRevisionId = (\d+);/);
...
LAPI.Ajax.apiGet = function (action, params, success, failure)
{
  ...
  if (params != null) {
    if (typeof (params) == 'string') {
      if (!params.test (/format=[^&]+/)) params = params + '&format=json';
      is_json = params.test (/format=json(&|$)/); // Exclude jsonfm, which actually serves XHTML
    } else {
  ...
  LAPI.Ajax.get (
      uri, params
    , function (request, failureFunc) {
        if (is_json && request.responseText.charAt (0) != '{') {
          failureFunc (request);
        } else {
          success (
              request
            , (is_json ? eval ('(' + request.responseText + ')') : null)
            , original_failure
          );
        }
      }
