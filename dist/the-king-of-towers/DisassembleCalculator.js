(function($){
	$(function(){
		var target=$('#DisassembleCalculator');
		...
		var types = [{returnRate: 0.6, name: '60%'}, {returnRate: 0.8, name: '80%'},{returnRate: 1, name: '100%/Solvent'}];
		var level=1, type=0, labs='', labels='', returnType=0;
		data.forEach(function(obj,id){labs += '<label><input type="radio" name="type" value="'+id+'"'+(id===0?' checked':'')+'> '+obj.name+'</label>';});
		types.forEach(function(obj,id){labels += '<label><input type="radio" name="disType" value="'+id+'"'+(id===0?' checked':'')+'> '+obj.name+'</label>';});
		$('#DisassembleCalculator_labels').html(labs).find('input[type=radio]').on('change', function(){type = $(this).val()*1;update();});
		$('#DisassembleCalculator_types').html(labels).find('input[type=radio]').on('change', function() {returnType = $(this).val()*1;update();});
		$('<input type="number" name="level" value="1" min="1" max="110" step="1">').appendTo($('#DisassembleCalculator_level').empty()).on('input', function(){level = $(this).val()|0;update();});
		...
		function update(){
			...
			totalG = Math.ceil(totalG * types[returnType].returnRate);
			totalGS = Math.ceil(totalGS * types[returnType].returnRate) - 1;
			if (totalGS < 0) totalGS = 0;
