...
$('.jcConfig').each(function() {
	...
	$form = $('<form />').attr({'action': '#', 'id': 'jcForm' + formId}).submit(function() {
		function dispResult(response) {
			...
			$('#bodyContent #' + resultId).empty().removeClass('jcError').html(response.parse.text['*']);
		}
		...
		if (formError) {
			showError('Yhdessä tai useammassa kohdassa on virhe.');
		} else {
	...
	$form.append($table);
	$('#bodyContent #' + formId).empty().append($form);
