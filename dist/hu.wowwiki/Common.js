...
function showTip(e) {
	...
	if ($p.hasClass("selflink") == false) {
		...
		var url = "/wiki/" + encodeURIComponent(decodeURIComponent($t.data("tt"))) + "?action=render " + 'div[class*="tooltip-content"]';
...
function bindTT() {
	var $t=$(this);
    var $p=$t.parent();
	if ($p.hasClass("selflink") == false && $p.attr('title') != undefined) {
...
function versionsInit() {
	...
	var baseEditLink = $("#content .versions-compatible").first().parentsUntil("#content").addBack().prev("h2, h3").first().find(".editsection a").attr("href");
	...
	var ttstore = {'#': $("#content .versions-compatible").first()}, editlinks = {}, conditionals = {'#': 'default'};
	var tips = $("#item-versions .versions-compatible").not(".versions-compatible .versions-compatible"), headers = tips.prev("h3").find(".mw-headline");
	...
	if ((mw.config.get('wgAction') != "edit" && mw.config.get('wgAction') != "submit")) {
		...
		tabs.appendTo('#firstHeadingTitle');
	}
	...
	function versionsShow(key) {
		...
		if ($("#content .versions-compatible").first()[0] != ttstore[key][0])
			$("#content .versions-compatible").first().replaceWith(ttstore[key]);
