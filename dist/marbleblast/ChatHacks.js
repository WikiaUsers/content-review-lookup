		...
		for(var i in this.model.users.models) {
			if(this.model.users.models[i].attributes.name == chat.attributes.name) {
				if(this.model.users.models[i].attributes.isStaff) {
					icon = ' <img class="stafficon" src="https://images.wikia.nocookie.net/marbleblast/images/7/7e/Marble3.jpeg">';
				}
				else if(this.model.users.models[i].attributes.isModerator) {
					icon = ' <img class="modicon" src="https://images.wikia.nocookie.net/lcf119testinggrounds/images/d/d1/MBUMarbleDefault-Chat.png">';
				}
				break;
			}
		}
			...
			for(var i = 0; i < pings.length; i++) {
				if(text.toLowerCase().indexOf(pings[i].toLowerCase()) != -1 || this != mainRoom.viewDiscussion) {
					if(!window.hasFocus) { //Only annoy people if the window isn't focused
						document.getElementById('sound').innerHTML = '<audio src="https://images.wikia.nocookie.net/lcf119testinggrounds/images/d/da/Bumper.ogg" autoplay=""></audio>';
...
importScriptPage('MediaWiki:Chat_Hacks_Commands.js','nigmatechlabs');