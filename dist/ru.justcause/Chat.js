...
;(function(window, mediaWiki) {
	...
	justcause.prototype = {
		...
		auto: function() {
			var _ = this;
			$('#ChatHeader .wordmark img')[0].setAttribute('src', 'https://vignette.wikia.nocookie.net/justcause/images/8/89/Wiki-wordmark.png/revision/latest/scale-to-width-down/212?cb=20190214200449&path-prefix=ru');
			$('#ChatHeader h1.wordmark')[0].insertAdjacentHTML('beforeend', '<div id="chat-head" class="chat-head">Добро пожаловать на «Just Cause вики» чат!</br><a href="/FAQ/Словарь" target="_blank">Словарь</a> — <a href="/wiki/Just Cause вики:Чат" target="_blank">О чате</a></div>');
			...
			var ignored = JSON.parse(window.localStorage.getItem('just-cause-wiki-chat')) || [],
		...
		tags_i: function() {
			...
			this.modalWindow({
				id: 'ajaxTagsModal',
				title: 'Теги',
				section: '<div id="ajax-onload" style="position: static;"><div><img src="https://vignette.wikia.nocookie.net/justcause/images/6/62/%D0%98%D0%B4%D1%91%D1%82_%D0%B7%D0%B0%D0%B3%D1%80%D1%83%D0%B7%D0%BA%D0%B0.gif/revision/latest?cb=20190223144524&path-prefix=ru"><br><span>Идёт загрузка...</span></div></div>'
			}, true, function() {
				$ajax({
					url: '/index.php',
					params: {
						title: 'Just_Cause_вики:Чат/Теги',
						action: 'render'
					},
					selector: '#ajax-load',
		...
		tags: function() {
			var replacement = {
					...
					spoiler: [/\[spoiler\]/, '<hr /><button class="open-chat-spoiler" onclick="this.nextSibling.style.display = \'\';just-cause.fn.$remove(this);">Показать содержимое</button><div style="display: none;" class="chat-spoiler-content">', /\[\/spoiler\]/, '</div><hr />'],
					...
					soundcloud: [/\[(soundcloud|sc)\]\s*?(&lt;|<).+"<a\s*?href="/gi, '<iframe class="chat-soundCloud" scrolling="no" frameborder="no" src="', /("\s*?(&gt;&lt;="").+\[\/(sc|soundcloud)\]<\/a>|"\s*&gt;&lt;.+<\/a>\s*?\[\/(sc|soundcloud)\])/gi, '">Отсутствует поддержка видео</iframe>'],
