(function () {
    // запуск только в чате + двойная защита (из NewMessageCount)
    if (mw.config.get('wgCanonicalSpecialPageName') !== 'Chat' || window.loadedTabinsert) {
        return;
    }
    window.loadedTabinsert = true;

    // https://archive.is/6cy2m
    function getCaretPos(ctrl) {
        var CaretPos = 0; // IE Поддержка
    ...
    //Вставка вкладок для имён пользователей
    ...
    $('[name="message"]').keydown(function(e) {
        if (e.keyCode == 9) {
            ...
            if ((!curval.charAt(carpos) || curval.charAt(carpos).match(/\s/)) && !curval.charAt(carpos - 1).match(/\s/) && this.value.length) {
                ...
                $('.User').each(function() {
                    if ($(this).attr('data-user') !== undefined) userList.push($(this).attr('data-user'));
                });
                var matchedNames = [];
