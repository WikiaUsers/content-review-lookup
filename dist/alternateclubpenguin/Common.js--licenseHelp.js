			...
			contents: [
				{
					...
					contents: "This short FAQ will answer some of the common questions that you may have about how to license files, whether you've got to license a file in the past or not. Select the next item from the navigation bar above and lets get started!",
					isStatic: true
				}
			]
		},
		...
		},
		{
			...
			contents: [
				...
				},
				{
					...
					contents: "Although this scenario is unlikely, you may once a while need to upload a file that needs to be given a licensing that does not appear in the dropdown- which only covers the more common licenses. You can go to the <a href=\"/wiki/Category:Copyright_tags\">copyright tags category</a> and see if you can find what you need. If so, upload your file without a licensing, and once uploaded, edit the file page and replace the placeholder template in the 'Licensing' section with the template that you need. If you still cannot find it, you you can contact an admin about this, ask in chat for help, or even create a new licensing template that matches the copyright status of your file, which will also be usable for future files."
				},
				{
					label: "But wait, there is no dropdown!",
					contents: "If you upload a file via <a href=\"/wiki/Special:Upload\">Special:Upload</a> or <a href=\"/wiki/Special:MultipleUpload\">Special:MultipleUpload</a>, the licensing dropdown is found on the bottom of the page. If, however, you upload your file via the 'Add a Photo' popup interface, accessible via the right rail on articles in the wiki, it will not be visible by default. For that, you can press the 'More Options' in the popup section, which will also show further options, such as changing the target file name or adding an upload summary.<br />If, however, you're uploading a new version of an existing file, you won't be able to modify the file's licensing. You will only be able to do that after the upload."
				}
			]
		}
	];
