(function($){
	...
	function main() {
		var $dropdown = $("<div>").addClass("wds-button wds-is-secondary wds-dropdown");
		$("<a>").addClass("wds-dropdown__toggle").attr("title", "Resource Packs").html("🎨").css({ fontSize:"13px" }).appendTo($dropdown);
		_content = $("<div>").addClass("wds-dropdown__content wds-is-right-aligned").appendTo($dropdown);
	...
	function renderPacksList() {
		...
		_content.html("");
		var $list = $("<ul>").addClass("hsw-pack-list").appendTo(_content);
		...
		selectedIds.reverse().forEach(function(selectedId, i){
			...
			var $li = $("<li>").html(renderPackBox(pack, true)).addClass('selected').appendTo($list).on('click', function(){
		...
		$("<li class='default'>"+renderPackBox({ name:"Default", author:"Mojang" }, 'default')+"</li>").appendTo($list);
		...
		unselectedPacks.forEach(function(pack){
			$("<li>").html(renderPackBox(pack, false)).addClass('unselected').appendTo($list).on('click', function(){
	...
	function renderPackBox(pack, selected) {
		return [
			"<div class='hsw-pack-box'>",
				"<div class='check'>"+(selected === true ? "<img src='https://static.wikia.nocookie.net/hypixel-skyblock/images/e/eb/Yes.svg' />" : (selected === false ? "➕" : ''))+"</div>",
				"<div class='pack-title'>"+pack.name+"</div>",
				(selected != 'default' ? "<a class='pack-site' href='"+pack.packsUrl+"' target='_blank' >"+EXTERNAL_LINK_ICON+"</a>" : ""),
