...
mw.loader.using(['mediawiki.api', 'mediawiki.util', 'mediawiki.Uri', 'ext.fandom.ContentReview.legacyLoaders.js']).then(function () {
    ...
    function userBelongsToGroup() {
        ...
        return mw.config.get("wgUserGroups").some(function (g) {
            return groups.includes(g);
        });
    }

    // Autoconfirmed or above
    if (userBelongsToGroup('autoconfirmed', 'rollback', 'threadmoderator', 'content-moderator', 'codeeditor', 'sysop', 'bureaucrat')) {
        importArticles({
            type: 'script',
            articles: [
    ...
    // Rollback or above
    if (userBelongsToGroup('rollback', 'threadmoderator', 'content-moderator', 'codeeditor', 'sysop', 'bureaucrat')) {
        importArticles({
            type: 'script',
            articles: [
    ...
    if (userBelongsToGroup('content-moderator', 'codeeditor', 'sysop', 'bureaucrat')) {
        importArticles({
            type: 'script',
            articles: [
