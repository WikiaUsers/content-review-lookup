/**Provenientes de Steven Universe Wiki, créditos a ellos**/
	...
	'soundcloud': function (s, t) {
		...
		if (t.charAt(0) !== '/') {
			try {
				t = t.split('="');
				t[1] = t[1].replace('"', '');
				s = s.replace('[soundcloud="' + t[1] + '"]', '<iframe class="chat-soundcloud" src="//w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/' + mw.html.escape(t[1]) + '&show_artwork=false" frameborder="0"></iframe>');
			} catch (e) {
				console.log(e);
			}
		}
	...
	'youtube-audio': function (s, t) {
		...
		if (t.charAt(0) !== '/') {
			try {
				t = t.split('="');
				t[1] = t[1].replace('"', '');
				s = s.replace('[youtube-audio="' + t[1] + '"]', '<audio class="chat-audio" controls preload="auto"><source src="//www.youtubeinmp3.com/fetch/?video=https://www.youtube.com/watch?v=' + mw.html.escape(t[1]) + '" type="audio/mp3"></audio>');
			} catch (e) {
				console.log(e);
			}
		}
	...
	'youtube': function (s, t) {
		...
		if (t.charAt(0) !== '/') {
			try {
				t = t.split('="');
				t[1] = t[1].replace('"', '');
				s = s.replace('[youtube="' + t[1] + '"]', '<iframe class="chat-ytvideo" src="https://www.youtube.com/embed/' + mw.html.escape(t[1]) + '" frameborder="0" allowfullscreen></iframe>');
			} catch (e) {
				console.log(e);
			}
		}
