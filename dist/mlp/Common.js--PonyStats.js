...
$(function() {
    var $content = $('#mw-content-text > p'),
      ...
      ponylists = ['List of ponies/Earth ponies', 'List of ponies/Pegasus ponies',
          'List of ponies/Unicorn ponies', 'List of Crystal Ponies',
          'List of ponies/Foals', 'List of ponies/Mentioned ponies'],
      otherlists = ['List of Wonderbolts', 'List of comic ponies',
            ...
            $('.listofponies tr:not(:first)', $html).each(function() {
                if (
                  ...
                  (which === 'other' && index === 2 && $(this).find('td:first').find('a[href^="/wiki/"]').length > 0)
                    ...
                    hasarticle[{'pony':{0:'Earth',1:'Pegasus',2:'Unicorn',3:'Crystal',4:'Foal',5:'Mentioned'},
                      'other':{1:'Comic',2:'EG'}}[which][index]] += 1;
                    hasarticle.total += 1;
                    thiscount += 1;
                }
            });
    ...
    $content.html(
      '<p><span style="font-size:125%;">Progress: <span>0</span>%.</span><br />' +
        'Please make suggestions and bug reports <a href="/wiki/User_talk:Bobogoobo" ' +
        'title="User talk:Bobogoobo">here</a>.<br /><b>This page is currently unmaintained. ' +
        'Please make suggestions if you would like to see the project revived.</b></p>' +
      '<p>Note that this page does not include ponies who appear only in merchandise or in ' +
        '<a href="/wiki/Project:Workshop/List_of_prose_ponies">literature</a> (yet).</p>' +
      '<h2>Main lists</h2><ul id="ps-ponies"></ul>' +
      '<h2>Other lists</h2><ul id="ps-other"></ul>' +
      '<h2>Totals</h2><p>These include main lists, except Crystal, plus comic ponies. ' +
        'Total characters includes all but Crystal and Wonderbolts.</p><ul id="ps-total"></ul>' +
      '<h2>Statistics</h2><ul id="ps-stats"></ul>'
    );
        ...
        $.getJSON('/api.php?action=parse&page=' + page + '&prop=text&format=json', function(data) {
            ...
            $('#ps-ponies').append($('<li />', {
                ...
                'html': '<a href="/wiki/' + page + '" title="' + page.replace(/_/g, ' ') + '">' +
                  ($('#title-meta', $html).text() || page.replace(/_/g, ' ')).replace('List of ', '') + '</a>: '
            }));
            $li = $('#ps-' + page.replace('/', '-'));
        ...
        $.getJSON('/api.php?action=parse&page=' + page + '&prop=text&format=json', function(data) {
            ...
            switch (index) {
                ...
                case 2:
                    totals.chars += chars;
                    $li.append(
                        doStuff($list, 3, '', 'F', ' females, '),
                        doStuff($list, 3, '', 'M', ' males, '),
                        doStuff($list, 3, '', ' ', ' n/a | '),
                        doStuff($list, 2, '', 'Athlete', ' athletes, '),
                        doStuff($list, 2, '', 'Fashionista', ' fashionistas, '),
                        doStuff($list, 2, '', 'Drama', ' dramas, '),
                        doStuff($list, 2, '', 'Eco-kid', ' eco-kids, '),
                        doStuff($list, 2, '', 'Techie', ' techies, '),
                        doStuff($list, 2, '', 'Rocker', ' rockers, '),
                        doStuff($list, 2, '', 'Staff', ' staff, '),
                        doStuff($list, 2, '', 'Animal', ' animals, '),
                        doStuff($list, 2, '', 'Comedian', ' comedians, '),
                        doStuff($list, 2, '', 'Undefined', ' undefined')
                    );
                    break;
            }
    ...
    function finish() {
        ...
        $('#ps-stats').append(
            ...
            '<h3>Names</h3><div id="ps-names"><p>Excludes Crystal and Wonderbolts.</p></div>',
            '<h3>Colors</h3><p>Counts main lists except Crystal, plus EG (skin/hair/eyes in that case).' +
              ' May be slightly inaccurate in Chrome.</p>',
            '<h4>Coat</h4><div id="ps-colors-coat"></div>',
            '<h4>Mane</h4><div id="ps-colors-mane"></div>',
            '<h4>Eyes</h4><div id="ps-colors-eyes"></div>'
        );
        ...
        (function nameStats() {
            ...
            $('#ps-names').append('<p>Out of ' + names.total + ' characters\' names, ' +
              names.status.official + ' (' + (names.status.official / names.total * 100).toFixed(2) + '%) are official ' +
              'and ' + names.status.poll + ' (' + (names.status.poll / names.total * 100).toFixed(2) + '%) are open for ' +
              '<a href="/wiki/FW:PN" title="FW:PN">voting</a>.</p>' +
              '<p>Further stats in this section only include official names.</p>'
            );
            ...
            $('#ps-names').append($('<ul></ul>').append(
                '<li>Average name length: ' + (totalLength / names.status.official).toFixed(2) + '</li>',
                '<li id="ps-names-words">Most common words: </li>',
                '<li>This section currently under construction. c:</li>'
            ));
            ...
            $('#ps-names').append('<table id="ps-names-letters" style="' +
              ...
              '<caption><b>Letter Frequency</b></caption><tr></tr></table>');
            ...
            for (var type in {'coat':'','mane':'','eyes':''}) {
                ...
                for (var color3 in {'r':'','g':'','b':''}) {
                    ...
                    $div.find('#ps-colors-' + type + '-' + color3).parent().next().children().append(
                        '<li>Max frequency: ' + max[type][color3] + '</li>',
                        '<li>Mean value: ' + getStat.mean(colors[type][color3].count) + '</li>',
                        '<li>Median value: ' + getStat.median(colors[type][color3].count) + '</li>'
                    );
                }
            }
        }());
