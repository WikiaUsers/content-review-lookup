...
* General (information is not level-dependent) - all buildings
 * 'width'            (the width of the building)
 * 'length'           (the length of the building)
 * 'townhall level'   (level of Town Hall that unlocks the building)
 * 'number available' (number of buildings available at specified town hall level)
 * 'levels'           (returns number of buiding levels defined)
 * 'description'      (in-game building description)
 *
* General (information is not level-dependent) - defense buildings
 * 'preferred target' (None, PEKKA, etc.)
 * 'multiplier'       (Damage multiplier vs. preferred target)
 * 'ground attack'    (whether a building can attack ground targets (true/false))
 * 'air attack'       (whether a building can attack air targets (true/false))
 ...
 *
* General (information is not level-dependent) - resource buildings
 *'resource'          (what does the building store/produce)
 *
* Level-specific information - all buildings
 *
 * 'hitpoints'        (building hit points)
 * 'build cost'       (build cost (Normal=elixir, Dark=DE, Gold))
 * 'build time'       (build time in hours)
 * 'townhall level'   (town hall required to upgrade building to that level)
 *
* Level-specific information - defense buildings
 * 'dps'              (damage per second)
 * 'death dama)ge'     (damage caused upon death (undefined if none))
 * 
* Level-specific information - resource buildings
 *'capacity'          (amount of resource it can)
 *'gem boost'         (amount of gems needed to boost)
 *'production'        (amount of resource it produces per hour)
 *
 ...
 * (Note that filters are not implemented yet.)
