...
//experimental JS, for the template HomePageSpoilerWarning
// Check if user has confirmed before
if (!localStorage.getItem('spoilerWarning')) {
    // Load the spoiler warning template onto the home page
    mw.loader.using('mediawiki.api', function() {
        new mw.Api().get({
            action: 'parse',
            text: '{{HomePageSpoilerWarning}}',
            format: 'json'
        }).done(function(data) {
...
$(document).on('click', '#spoiler-confirm', function() {
    $('#spoiler-warning').fadeOut();
    localStorage.setItem('spoilerWarning', 'true');
});