// <nowiki>
// Imported from https://en.wikipedia.org/w/index.php?title=MediaWiki:Gadget-popups.js&oldid=1222213787
//
...
$(function () {
	...
	function setUserInfo() {
		...
		// [PATCHED] Add patrol option
		pg.user.canReview = false;
		pg.user.canPatrol = false;
		if (getValueOf('popupReview') || getValueOf('popupPatrol')) {
			getMwApi()
				.get(params)
				.done(function (data) {
	...
	function setMainRegex() {
		var reStart = '[^:]*://';
		var preTitles = // [PATCHED] Make index.php optional to work with our short urls
	...
	function loadDiff(article, oldid, diff, navpop) {
		...
		mw.loader.using('mediawiki.api').then(function () {
			...
			api.get(params).then(function (data) {
				...
				addPatrolLink(navpop, 'popupMiscTools'); // [PATCHED]
	...
	// [PATCHED] Add option to patrol
	// Put a "mark patrolled" link to an element target
	function addPatrolLink(navpop, target) {
		if (!pg.user.canPatrol) {
			return;
		}
		// If 'newRev' is older than 'oldRev' than it could be confusing, so we do not show the patrol link.
		if (navpop.diffData.newRev.revid <= navpop.diffData.oldRev.revid) {
			return;
		}
		// Only allow patrolling prev/diff links
		if ( [ popupString('last'),  popupString('diff') ].indexOf( navpop.parentAnchor.textContent ) === -1 ) {
			return;
		}
		// Only add for unpatrolled edits
		if ( !navpop.parentAnchor.closest( '.revisionpatrol-unpatrolled, .mw-changeslist-reviewstatus-unpatrolled' ) ) {
			return;
		}
		var a = document.createElement('a');
		a.innerHTML = popupString('mark patrolled');
	...
	function globalSearchLink(l) {
		...
		// [PATCHED] Changed the base URL below. The original was specific to Wikipedias
		var base = 'https://infinity-nikki.fandom.com/zh/wiki/Special:Search?fulltext=1&search=';
	...
	function setOptions() {
		...
		newOption('popupPatrol', true); // [PATCHED] Add patrol option
