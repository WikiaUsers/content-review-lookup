...
function createDrTerrorWidget() {
   ...
   function updateTerrorWidget(tZoneOffset, cTerrorZone, w) {
      ...
      var startTerror = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + ((now.getUTCDay() > 3 ? 10 : 3) - now.getUTCDay()), tZoneOffset, 0, 0, 0));
      ...
      var startVolcano = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + ((now.getUTCDay() > 6 ? 10 : 6) - now.getUTCDay()), tZoneOffset, 0, 0, 0));
...
';
   ...
   // Delete this
   window.importArticle({ type: 'style', article: 'w:c:boombeach:MediaWiki:Wikia.css' });

   // Import required CSS
   window.importArticle({
     type:    'style',
     article: 'MediaWiki:Common.css/DrTerror.css'
   });
