...
function hasClassPrefix(element, classNamePrefix) {
   var cName = element.className;

   if (cName.substring(0, cName.length - 1) == classNamePrefix)
      return cName.substring(classNamePrefix.length);
...
function doQuizAnswer(Answer, correctAnswer) {
...
function createQuizAnswer() {
   ...
   for (var index = 0; index < paras.length; index ++) {
      if (hasClassTest(paras[index], "check-quiz-answer")) {
         var correctAnswer  = paras[index].id;
         var form           = document.createElement("form");
         var input1         = document.createElement("input");
         var input2         = document.createElement("input");
         var span           = document.createElement("span");

         form.id            = "quiz_form_" + offset;
         form.correctAnswer = correctAnswer;

         input1.id          = "quiz_input_" + offset;
         input1.size        = 10;
 
         input2.id          = "quiz_submit_" + offset;
         input2.offset      = offset;
         input2.type        = "button";
         input2.value       = "Submit";
         input2.onclick     = function() {
            var tempInput = document.getElementById("quiz_input_"  + this.offset);
            var tempForm  = document.getElementById("quiz_form_"   + this.offset);
            var tempSpan  = document.getElementById("quiz_result_" + this.offset);

            if (tempInput.value == tempForm.correctAnswer)
               tempSpan.innerHTML = " = Correct Answer!";
            else
               tempSpan.innerHTML = " = Wrong Answer!";

            tempSpan.innerHTML += " (Answer == '" + tempInput.value + "', len:" +
               tempInput.value.length + ", Correct Answer == " +
               tempForm.correctAnswer + ", len:" + tempForm.correctAnswer.length + ")";
         };

         span.id = "quiz_result_" + offset;
         span.innerHTML = " =  ???";

         form.appendChild(document.createTextNode("Answer: (" + correctAnswer +
...
addOnloadHook(createQuizAnswer);

/* GorillaMan's quiz code */
/* Check for Answer = A */
