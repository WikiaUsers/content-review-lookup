 ...
 */
 ...
 /*
  * ChatHack_Redux.js, by Captain Barracuda
  * version 1.0
  * Features:
  * 	Supports regex for pings. 
  */
 
if(wgCanonicalSpecialPageName == 'Chat') { // Checks that the current page is the chat page
	...
	console.log("ChatHack_Redux has been loaded");
	...
	NodeChatDiscussion.prototype.iconPing = function (chat) {
		...
		if(mainRoom.isInitialized && chat.attributes.name != wgUserName && !chat.attributes.isInlineAlert) {
			...
			for(var i = 0; i < pings.length; i++) {
				// PING
				//if(text(pings[i].toLowerCase().indexOf.toLowerCase()) != -1) {
				if (mustPing(text, pings[i])) {
	...
	// CONTENT ADDED BY THE REDUX VERSION
	const REGEX_CHECKER = new RegExp('^/.+/$');
	
	function mustPing(text, ping) {
		/*
		 * Two forms of ping text:
		 * 	- Regex (starts and end with /)
		 * 	- Non-Regex
		 */
		if (REGEX_CHECKER.test(ping)) {
			return ping.test(text);
		} else if (text.toLowerCase() === ping.toLowerCase()) {
			return true;
		} else {
			return false;
		}
