...
$('body').append('\n<div id="GEchartpopup" style="display:none;"><div id="closepopup"><a href="javascript:popupChart(false)">Close popup [X]</a></div><div id="itemstats"></div>'+
'<form action="javascript:addItem(\'popup\')" id="chartPropertiespopup"><label>Average: <input type="number" min="1" id="averagepopup"> days</label><div class="moreitems">Add more item(s): <input type="text" id="extraItempopup">&nbsp;<b>Note:</b> If you add more items, the averages will not be shown.</div><button>Submit</button><button type="reset">Reset<span class="resetallfields"> all fields</span></button></form><a class="GEPermLink" id="GEPermLinkpopup" title="Permanent link to the current chart settings and items. Right click to copy the url." href="javascript:void(0)">Permanent link</a><div id="addedItemspopup"></div>'+
...
function chartPermLinkUrl() {
	...
	urlHash += '#i='+items;
	var url = 'http://rs.wikia.com/wiki/GEMW/C'+urlHash;
	return url;
}
...
function addItem(i,it) {
	...
	if (item&&item.length) {
		if ($('#addedItems'+i+' [data-item], #GEdataprices'+i+'[data-item]').map(function() { return $(this).attr('data-item').toLowerCase(); }).get().indexOf(item.toLowerCase())!=-1 || _GEC.AIQueue.indexOf(item.toLowerCase())!=-1) {
			if (!it) alert(item+' is already in the graph.')
		...
		$.get('/api.php?action=parse&format=json&page=Exchange:'+encodeURIComponent(item)+'/Data', function(response) {
			if (response.error) {
				...
				if (_GEC.AIData.length && _GEC.AIQueue.length == _GEC.AILoaded.length) loadChartsQueueComplete(_GEC.chartid);
					...
					if (!_GEC.AIData.length) setChartRange(_GEC.chartid);
				}
				$('#extraItem'+_GEC.chartid).prop('disabled',false).val('');
				return false;
			}
...
function loadChartsQueueComplete(c) {
	if (getSeriesIndex(_GEC.chartid,'7-day volume')!=-1) {
	...
	for (var i=0;i<data.length;i++) {
		...
		if (!($('#addedItems'+c).html()||'').replace(/\s+/,'').length) {
			$('#addedItems'+c).html('Remove items from graph: <a href="javascript:removeGraphItem(\''+iname+'\',\''+c+'\')" data-item="'+iname+'">'+iname+'</a>');
			itemhash = '#i='+iname;
		} else {
...
function popupChart(i) {
	...
	$('#overlay').click(function() {popupChart(false)});
	if (typeof(i)=='number') {
		$(document).keydown(function(e) { if (e.which == 27) popupChart(false); })
	} else {
...
function addPopupPriceInfo(c) {
	...
	itemStats += $('#GEdataprices'+c).attr('data-value').length?'<tr><th><a href="/wiki/Value" target="_blank" title="Value">Value</a></th><td>'+toKMB($('#GEdataprices'+c).attr('data-value'))+'</td></tr>':'';
	itemStats += $('#GEdataprices'+c).attr('data-limit').length?'<tr><th><a href="/wiki/Grand_Exchange#Trade_Restrictions" target="_blank" title="Grand Exchange buying limit">Buy limit</a></th><td>'+toKMB($('#GEdataprices'+c).attr('data-limit'))+'</td></tr>':'';
...
function getData(c,isSmall,avginput) {
	...
	if (volumes.length >= 10&&!isSmall) {
		yAxis = [{
			title: {
				text: isIndexChart?'Index history':'Price history',
				offset: 60,
				rotation: 270,
				style: {
					color: 'black',
...
Highcharts.setOptions({
	lang: {
		resetZoom: '<span style="font-size:11px;">reset zoom</span>',
