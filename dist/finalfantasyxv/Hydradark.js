...
var fp = fp || {
  equalizeColumns : function() {
    ...
    if ( $( '.fp2columnonly' ).css( 'display' ) == 'block' ) {
    ...
    } else if ( $( '.fp3columnonly' ).css( 'display' ) == 'block' ) {
      $( '.fpmain' ).each( function (index) {
        var leftHeight = $( this ).find( '.fpsection1' ).height() + $( this ).find( '.fpsection4' ).height();
  ...
  resetSectionBoxHeights : function ( sections ) {
    sections.each( function () {
      $( this ).find( '.fpbox' ).not( '.fpnoresize' ).each( function () {
        $( this ).height( 'auto' );
      } );
    } );
  },
  ...
  adjustSectionBoxHeights : function ( heightToAdd, sections ) {
    ...
    sections.each( function() {
      boxes = $( this ).find( '.fpbox' ).not( '.fpnoresize' );
      lastBox = boxes.last();
