/** <pre>
 * Taken from: https://runescape.fandom.com/wiki/MediaWiki:Common.js/highlightTable.js
 * 
 * highlightTable.js
 *
 * Description:
...
;(function ($, mw) {
    ...
    function highlightTable() {
		...
		mw.hook('wikipage.content').add(function(pSection){
            ...
            var resetAllButtonExists = $("."+tableClass+'-reset-all').length > 0;
            if(resetAllButtonExists) {
            	$("."+tableClass+'-reset-all').show().append(
                    $('<input>')
                        .attr({
                            'type': 'button',
                            'value': 'Reset'
