...
// @namespace   https://en.wikipediawikipedia.org/wiki/User:Cacycle/
...
if (wikEd.config.homeBaseUrl === undefined) { wikEd.config.homeBaseUrl = '//health-and-medicine.wikia.com/wiki/'; }
...
wikEd.InitTranslations = function () {
	wikEd.InitObject(wikEd.config.translations, {
		...
		'ar':  wikEd.config.homeBaseUrl + 'w/index.php?action=raw&ctype=text/javascript&title=User:??????05/wikEd_international_ar.js',
		...
		'he':  wikEd.config.homeBaseUrl + 'w/index.php?action=raw&ctype=text/javascript&title=User:??????/wikEd_international_he.js',
			...
			linkTitle = linkTitle.replace(/([cghjsu])(x+)/gi,
				function(p, p1, p2) {
					...
					if ( (xLength / 2 - xCount) > 0) {
						...
						accentChar = 'CGHJSUcghjsu'.substr(pos, 1);
...
function CustomizeModificationsOfSidebar() {
	...
	ModifySidebar( 'add', 'toolbox', 'CategoryTree', 'http://health-and-medicine.wikia.com/wiki/Special:CategoryTree' );
	// removes [[Special:Upload]] from toolbox
	ModifySidebar( 'remove', 'toolbox', 'Upload file', 'http://health-and-medicine.wikia.com/wiki/Special:Upload' );
}
