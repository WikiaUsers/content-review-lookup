...
/* required for DataTables (which can use Responsive for a similar effect) */
...
/* dev:DataTables.js */
// ImportJS does not load quickly at this time, creating a race condition
importArticles({ type: 'script', articles: [ 'u:dev:MediaWiki:DataTables.js' ]});
...
(function($, mw) {
  ...
  function initializeDT () {
    $.getJSON(
      ...
      function(dtConfig) {
        function createButtonAction(text, column) {
			return function(e, dt, node, config) {
			  e.preventDefault();
			  dt.rowGroup().dataSrc(column);
			  if (column == 0) {
				dt.column(0).visible(false);
			  } else if (column == 2) {
				dt.column(0).visible(true);
			  }
			  dt.order.fixed( {pre: [[ column, 'asc' ]]} ).draw();
			};
		  }
		$.each(dtConfig.setManifest.buttons, function(i, v) {
		  if (v.text == 'by Region') {
			v.column = 0;
			var action = createButtonAction(v.text, v.column);
			v.action = action;
		...
		$('table.data.setManifest').not('.dataTable').DataTable(dtConfig.setManifest);
		...
		$('table.data.setsContainingCard').not('.dataTable').DataTable(dtConfig.setsContainingCard);
      });
  };
