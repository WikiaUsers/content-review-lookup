  ...
  $("#SRWmap").prepend(
    ...
    .append(
      ...
      $("<button>", {
        ...
        title:"New Pin",
        class:"MapControls",
        html:$("<img>", { src:"https://vignette.wikia.nocookie.net/"+wgDBname+"/images/5/5e/Ui_map_marker.png" })
      }).hover( function() {
...
function dirty(what, which, where) {
  ...
  SRW.Map.dirty[what][which][where] = (SRW[what][which][where]+"").split("/"+wgDBname+"/images/").pop();
}
...
//old kludge fix stopped working at some point, thanks wikia. MediaWiki:Common.js/SRWMaps.js?diff=next&oldid=342873
  this.filePrefix = "https://vignette.wikia.nocookie.net/"+wgDBname+"/images/";
  this.padding=500;
...
function Cat(params) {
  ...
  this.setParams = function(params) {
    ...
    if (this.icon) this.icon = SRW.Map.filePrefix+this.icon.split("/"+wgDBname+"/images/").pop();
    if (this.image) this.image = SRW.Map.filePrefix+this.image.split("/"+wgDBname+"/images/").pop();
              ...
              if ($(target).val() !== decodeURIComponent(cat[field].split("/").pop())) {
                ...
                .always(function (data) {
                  ...
                  if (data.statusText == "error" || typeof data.image == "undefined" || typeof data.image.error != "undefined") {
                    ...
                    if (!$("#CatEditBox input.pending").length) $("#CatEditDone").removeClass("pending").addClass("ok");
                    newImage = SRW.Map.filePrefix+data.image.imageserving.split("/revision/latest").shift().split("/"+wgDBname+"/images/").pop();
                  ...
                  .always(function (data) {
                    ...
                    if (data.statusText == "error" || typeof data.image == "undefined") {
                      ...
                      if (typeof data.image.imageserving == "string" && $("#edit"+CatID+" #image"+CatID).val() == "") {
                        autoImage = SRW.Map.filePrefix+data.image.imageserving.split("/revision/latest").shift().split("/"+wgDBname+"/images/").pop();
