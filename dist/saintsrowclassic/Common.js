...
$(function() { /* Run when page has loaded */
  ...
  $('.page-content a[href*="//'+mw.config.get("wgServerName")+'"][target="_blank"]').attr("target","");
  ...
  */
  ...
  if (mw.config.get("wgCanonicalNamespace") == "Forum" && typeof mw.util.getParamValue('action') != "string" && mw.config.get("wgArticleId")) {
...
window.initScripts = function() { 
  ...
  if ($("#SRWmap").length) loadScripts.push('u::MediaWiki:Common.js/SRWMaps.js');
  if ($(".ns-2").length || $(".ns-3").length || $(".mw-special-UserProfileActivity").length || $(".mw-special-Contributions").length)	
    loadScripts.push('MediaWiki:Common.js/usericons.js');	// Import custom usericons -custom.
  ...
  if (mw.util.getParamValue('action') == "edit" || mw.util.getParamValue('action') == "submit") {
    ...
    loadScripts.push('MediaWiki:Common.js/stdSummaries.js'); // edit page summaries - rewritten
    loadScripts.push('MediaWiki:Common.js/stdTemplates.js'); // edit page template - rewritten
    loadScripts.push('MediaWiki:Common.js/htmlentities.js'); // button to encode htmlentities  -custom
    loadScripts.push('MediaWiki:Common.js/tabber.js');

  } else if (mw.config.get("wgPageName") == "Special:RecentChanges") {

    loadScripts.push('MediaWiki:Common.js/RecentChangesUpdate.js'); // Autoload new Recentchanges -rewritten

  } else {

    if (mw.config.get("wgPageName").indexOf("Saints_Row_Wiki:Polls") != -1 || $(".SRWpoll").length)
	  loadScripts.push('MediaWiki:Common.js/polls.js');        // -custom

    loadScripts.push('MediaWiki:Common.js/tabber.js');		// copied external script locally to ensure loading
    loadScripts.push('MediaWiki:Common.js/ResizeSlideshow.js');	// Resize Slideshows to fit page width -custom

    loadScripts.push('MediaWiki:Common.js/countdown.js');		/* Import javascript countdown.   */
    loadScripts.push('MediaWiki:Common.js/tablePopout.js');	// Enables popout tables -rewritten
    loadScripts.push('MediaWiki:Common.js/ReferencePopups.js');	// reference popups - needs rewrite

    loadScripts.push('MediaWiki:Common.js/View_Source.js');	/* "view source" link */
    loadScripts.push('MediaWiki:Common.js/PageRefresh.js');	// "purge" and "null edit" links -rewritten
  }
  if (1)
    ...
    loadScripts.forEach(function(v){
      ...
      if (vW == "sr") vW = mw.config.get("wgServerName").split(".")[0];
