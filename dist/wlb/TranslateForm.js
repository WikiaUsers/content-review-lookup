...
var $ = this.jQuery,
    ...
    get: function(name) {
        var m = '(' + name + ')';
        if (messages[_tr.language.toUpperCase()][name]) m = messages[_tr.language.toUpperCase()][name];
...
if(_tr.pagename === 'Translate:Requests') {
    var buttonappend = '<a class="wikia-button" id="wlb-submit" onclick="openFormTranslate()">' + msg.get('button') + '</a>';
...
// template TBD, then uncomment
if (lang2 === "XM" ) {
/*page = '{{Translate header}}\n{{Translation|' + lang + '|' + lang2 + '}}\n\n{{Multidone}}<!-- add e.g. |de = done when you added your translation-->\n\n\'\'\'{{int:i18n-form-wiki}}\'\'\': [[w:c:' + wikiurl + ']]\n\n\'\'\'{{int:i18n-form-items}}\'\'\': ' + items + '\n\n\'\'\'{{int:i18n-form-extrainfo}}\'\'\': ' + extrainfo + '\n\n\'\'\'{{int:i18n-form-signature}}\'\'\': ' + signatureplace + '\n[[Category:New translations]]';*/
}
...
console.log('Performed checks...');
    ...
    // Ajax URL
    var url = _tr.server + '/api.php?action=edit&title=Translate:' + encodeURIComponent(header) + ' (' + encodeURIComponent(lang) + '+%E2%86%92+' + encodeURIComponent(lang2) + ')&text=' + encodeURIComponent(page) + '&summary=New+translate+request+(' + encodeURIComponent(lang) + '+%E2%86%92+' + encodeURIComponent(lang2) + ')&token=' + encodeURIComponent(_tr.edittoken);
