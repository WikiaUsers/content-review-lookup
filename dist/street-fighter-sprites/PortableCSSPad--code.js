//THIS CODE FOR THE - PortableCSSPad - SCRIPT WILL BE STORED ON THIS PAGE SHOULD ANYTHING HAPPEN TO THE ORIGINAL PAGE ON dev.wikia/com/PortableCSSPad - ALL RIGHTS BELONG TO THE ORIGINAL AUTHOR "Mathmagician"
...
(function ($, mw, window, console) {
	...
	if (/\.css$/.test(mw.config.get('wgPageName')) && mw.config.get('wgNamespaceNumber') % 2 === 0)  {
		$(function ($) {
			...
			var validatorURL = 'https://jigsaw.w3.org/css-validator/validator?uri=' + window.encodeURIComponent(stylesheetURL) + '&profile=css3&usermedium=all&warning=1&vextwarning=&lang=' + mw.config.get('wgUserLanguage');
			var buttonStyle = 'cursor: pointer; display: inline-block; border: 1px solid black; width: 20px; height: 20px; background-image: url(https://images.wikia.nocookie.net/dev/images/1/19/PortableCSSPad-icons.png); background-position: 0 -20px; background-color: #fff;';
