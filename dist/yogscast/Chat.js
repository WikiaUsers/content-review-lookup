...
/*
* Wikia to Slack Connection
* !version 1.0.0
* !author Shufflertoxin
*/

var YogscastDiscord = {
  "async": true,
  "crossDomain": true,
  "url": "http://yogscastwiki.x10host.com/YOGSCAST/discord/",
  "method": "POST",
  "processData": false
}
function DiscordToken(){
    $.ajax(YogscastDiscord).done(function (response) {
      $.discord = response['token']; 
      console.log($.discord);
      //Discord
    $.ajax({
		url: "https://discordapp.com/api/channels/136213532801892352/messages",
		type: "POST",
        headers: {
		...
		data: JSON.stringify({
			content: window.wgUserName + ": Joined the Chat!",
			channel_id: "136213532801892352",
		})
	});
    });
    
}
window.onload = DiscordToken();


$('textarea[name=message]').on("keydown",function search(e) {
    if(e.keyCode == 13) {
        if ( !window.mainRoom || !window.mainRoom.active ) {
            return; // prevent activation on PMs
        }
	var formData = '*Name1:* ';
	var firstName = "Test"; // For Success/Failure Message
	// Check for white space in name for Success/Fail message
	if (firstName.indexOf(' ') >= 0) {
		firstName = name.split(' ').slice(0, -1).join(' ');
	}
	$.ajax({
		url: "https://hooks.slack.com/services/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",
		type: "POST",
		data: JSON.stringify({
			text: $(this).val(),
	...
	$.ajax({
		...
		data: JSON.stringify({
			content: window.wgUserName + ": " + $(this).val(),
			channel_id: "136213532801892352",
		})
	});
...
$.ajax({
		...
		data: JSON.stringify({
			content: window.wgUserName + ": Left the Chat!",
			channel_id: "136213532801892352",
		})
	});
$.ajax({
