...
function addFastDeleteButtons() {
	...
	switch( skin ) {
		...
		case 'wikia':
			...
			for( var i = 0; i < fdButtons.length; i++ ) {
				var afd = '<a class="wikia-button" href="/index.php?title=' + encodeURIComponent( wgPageName ) + '&action=delete&submitdelete=true&wpReason=' + encodeURIComponent( fdButtons[i].summary ) + '" title="one-click delete: ' + fdButtons[i].summary+'" data-id="delete">'+ fdButtons[i].label + '</a>&nbsp;';
