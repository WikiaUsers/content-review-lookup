...
var LD = {
	buildings:['Ακαδημία','Αλάνα','Αποθήκη','Αποστακτήριο','Αρχείο Ναυτικών Χαρτών','Αρχιτεκτονικό γραφείο','Δημαρχείο','Εμπορικός Λιμένας','Εργαστήριο','Θέση Εμπορικών Συναλλαγών','Η Κατοικία του Κυβερνήτη','Κρησφύγετο','Λατομείο','Μαύρη Αγορά','Μουσείο','Ναυπηγείο','Ναός','Ξυλοκόπος','Ξυλουργός','Οινοποιείο','Οπτικός','Παλάτι','Πειρατικό Φρούριο','Περιοχή Δοκιμών Πυροτεχνημάτων','Πρεσβεία','Πύργος Αλχημιστή','Στρατώνες','Ταβέρνα','Τείχος','Υαλουργείο'],
...
function hardcap(t,s)
{
	...
	switch(s)
	{
		case 1:
			...
			e += '<br>'+prntf(LD.hardcap.e1,[FrmtNumToStr((wc[84]*5+3*85*32000+2500)),icon(5,9,12),lnk(LD.steamlift,LD.steamlift),lnk(LD.hardcap.name+'#'+LD.hardcap.namewithoutA,LD.hardcap.namewithoutA)]);
			c = '-s';
			break;
		case 2:
			title += LD.hardcap.title2;
			e = '<br>'+prntf(LD.hardcap.e2,[FrmtNumToStr((wc[84]*5+3*85*32000+2500)*2),icon(5,9,12),lnk(LD.steamlift,LD.steamlift),lnk(LD.hardcap.name+'#'+LD.hardcap.namewithA,LD.hardcap.namewithA)]);
			c = '-sa';
			break;
		case 3:
...
function Tips(q,r,s,a)
{
	...
	switch(r)
	{
		...
		case 'Α': // Μείωση Αλφαδιού
			var h = ['<th colspan="'+(r==='Γ'?2:(r==='Α'?3:1))+'">'+(r!=='Τ'?'Έρευνες':'Έρευνα')+'</th>',r!=='Τ'?'<th rowspan=2>Σύνολο<br>έρευνών</th>':'','<th>Τροχαλία</th>'+(r==='Γ'||r==='Α'?'<th>Γεωμετρία</th>':'')+(r==='Α'?'<th>Αλφάδι</th>':''),'<th colspan=2>Μειωτικό κτήριο</th>','<th rowspan=2>Σύνολικό<br>ποσοστό</th>','<th colspan=3>Ποσότητα</th>','<th>Επίπεδο</th><th>Ποσοστό</th>','<th>μείωσης</th><th>τελική</th><th>αθροιστική</th>'];
			var t = '<table class="darktable zebra" style="display:inline-block;text-align:center;line-height:1.0;border:none !important;background:transparent;font-size:10px;padding:0px;border-collapse:collapse;margin-left:auto;margin-right:auto"><thead><tr>'+h[0]+h[1]+h[3]+h[4]+h[5]+'</tr><tr>'+h[2]+h[6]+h[7]+'</tr></thead><tbody>';
			for(var i=0;i<31;i++)
			{
				t += '<tr><td>2 %</td>'+(r==='Γ'||r==='Α'?'<td>4 %</td>':'')+(r==='Α'?'<td>8 %</td>':'')+(r==='Τ'?'':'<td>'+(r==='Γ'?6:(r==='Α'?14:0))+' %</td>')+'<td>'+i+'</td><td>'+i+' %</td><td>'+((r==='Τ'?2:(r==='Γ'?6:(r==='Α'?14:0)))+i)+' %</td><td>'+FrmtNumToStr(Math.ceil(((r==='Τ'?2:(r==='Γ'?6:(r==='Α'?14:0)))+i)*q/100))+icon(s,12,10)+'</td><td>'+FrmtNumToStr(Math.floor((100-((r==='Τ'?2:(r==='Γ'?6:(r==='Α'?14:0)))+i))*q/100))+icon(s,12,10)+'</td><td>'+FrmtNumToStr(Math.floor((100-((r==='Τ'?2:(r==='Γ'?6:(r==='Α'?14:0)))+i))*a/100))+icon(s,12,10)+'</td></tr>';
			}
			t += '</tbody><tfoot><tr>'+h[2]+h[1]+h[6]+h[4]+h[7]+'</tr><tr>'+h[0]+h[3]+h[5]+'</tr></tfoot></table>';
			t += '<table class="darktable zebra" style="display:inline-block;text-align:center;line-height:1.0;border:none !important;background:transparent;font-size:10px;padding:0px;border-collapse:collapse;margin-left:auto;margin-right:auto"><thead><tr>'+h[0]+h[1]+h[3]+h[4]+h[5]+'</tr><tr>'+h[2]+h[6]+h[7]+'</tr></thead><tbody>';
			for(var i=31;i<62;i++)
			{
				t += '<tr><td>2 %</td>'+(r==='Γ'||r==='Α'?'<td>4 %</td>':'')+(r==='Α'?'<td>8 %</td>':'')+(r==='Τ'?'':'<td>'+(r==='Γ'?6:(r==='Α'?14:0))+' %</td>')+'<td>'+i+'</td><td>'+i+' %</td><td>'+((r==='Τ'?2:(r==='Γ'?6:(r==='Α'?14:0)))+i)+' %</td><td>'+FrmtNumToStr(Math.ceil(((r==='Τ'?2:(r==='Γ'?6:(r==='Α'?14:0)))+i)*q/100))+icon(s,12,10)+'</td><td>'+FrmtNumToStr(Math.floor((100-((r==='Τ'?2:(r==='Γ'?6:(r==='Α'?14:0)))+i))*q/100))+icon(s,12,10)+'</td><td>'+FrmtNumToStr(Math.floor((100-((r==='Τ'?2:(r==='Γ'?6:(r==='Α'?14:0)))+i))*a/100))+icon(s,12,10)+'</td></tr>';
			}
...
function createTable(jsn)
{
	...
	switch(jsn.name)
	{
		...
		case 'Υαλουργείο':
			wps = ['<th>Αύξηση παραγωγής</th>','<th class="title6">'+icon(jsn.name=='Λατομείο'?2:jsn.name=='Οινοποιείο'?1:jsn.name=='Πύργος Αλχημιστή'?4:jsn.name=='Υαλουργείο'?3:0)+'</th>'];
			break;
		case 'Αποστακτήριο':
	...
	for(var y=0;y<(ps[1]-ps[0]+1);y++)
	{
		...
		var w4 = max*0.25-2500<0?0:Math.ceil((max*0.25-2500)/wc[84]); // Επίπεδα για μέγιστη μείωση χωρίς αμβροσία
		...
		var w9 = max*0.125-2500<0?0:Math.ceil((max*0.125-2500)/wc[84]); // Επίπεδα για μέγιστη μείωση με αμβροσία
		...
		switch(jsn.name)
		{
			...
			case 'Θέση Εμπορικών Συναλλαγών':
				...
				break;
			case 'Κρησφύγετο':
				var txt = '<table class="darktable zebra" style="text-align:center;line-height:1.0;border:none !important;background:transparent;font-size:10px;padding:0px;border-collapse:collapse;margin-left:auto;margin-right:auto"><thead><tr><th>Αριθμός<br>κατασκόπων</th><th>Συνολική διάρκεια<br>εκπαίδευσης</th></tr></thead><tbody>';
				for(var z=0;z<y+parseInt(ps[0]);z++)
				{
					txt += '<tr><td>'+(z+1)+'</td><td>'+BT((z+1)*Math.round(900.12*Math.exp(-0.0513*(y+parseInt(ps[0])))),1)+'</td></tr>';
				}
				txt += '</tbody></table>';
			...
			case 'Εμπορικός Λιμένας':
				...
				for(var z=0;z<19;z++)
				{
					...
					port2 += z<12?'<tr><td>'+(z+1)+'</td><td>'+FrmtNumToStr(resources.loadspeed[y+parseInt(ps[0])-1]+resources.loadspeed[z])+'</td><td>'+(z+13)+'</td><td>'+FrmtNumToStr(resources.loadspeed[y+parseInt(ps[0])-1]+resources.loadspeed[z+12])+'</td><td>'+(z+25)+'</td><td>'+FrmtNumToStr(resources.loadspeed[y+parseInt(ps[0])-1]+resources.loadspeed[z+24])+'</td><td>'+(z+37)+'</td><td>'+FrmtNumToStr(resources.loadspeed[y+parseInt(ps[0])-1]+resources.loadspeed[z+36])+'</td></tr>':'';
				}
...
var createForestMineTable = function(jsn)
{
	...
	var plus = {
		...
		'Αμπελώνες':['Το Ατμοκίνητο Πιεστήριο κρασιού δίνει<br /><b>20 %</b> περισσότερη παραγωγή.','Ατμοκίνητο Πιεστήριο κρασιού','ΑτμοκίνητοΠιεστήριοΚρασιού','Οινοποιείο','Οινοποιείο.png'],
