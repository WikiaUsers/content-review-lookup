...
function hardcap(t,s)
{
	...
	switch(s)
	{
		case 1:
			...
			e += '<br>'+prntf(LD.hardcap.e1,[FrmtNumToStr((40*(8000*4+32000)+2500)),icon(5,9,12),lnk(LD.steamlift,LD.steamlift),lnk(LD.hardcap.name+'#'+LD.hardcap.namewithoutA,LD.hardcap.namewithoutA)]);
			c = '-s';
			break;
		case 2:
			title += LD.hardcap.title2;
			e = '<br>'+prntf(LD.hardcap.e2,[FrmtNumToStr((40*(8000*4+32000)+2500)*2),icon(5,9,12),lnk(LD.steamlift,LD.steamlift),lnk(LD.hardcap.name+'#'+LD.hardcap.namewithA,LD.hardcap.namewithA)]);
			c = '-sa';
			break;
		case 3:
...
function Tips(q,r,s,a)
{
	...
	switch(r)
	{
		...
		case 'Α': // Μείωση Αλφαδιού
			...
			var t = '<table class="darktable zebra" style="text-align:center;line-height:1.0;border:none !important;background:transparent;font-size:10px;padding:0px;border-collapse:collapse;margin-left:auto;margin-right:auto"><thead><tr>'+h[0]+h[1]+h[3]+h[4]+h[5]+'</tr><tr>'+h[2]+h[6]+h[7]+'</tr></thead><tbody>';
			for(var i=0;i<33;i++)
			{
...
function createTable(jsn)
{
	...
	for(var y=0;y<(ps[1]-ps[0]+1);y++)
	{
		...
		var w4 = max*0.54-2500<0?0:Math.ceil((max*0.54-2500)/8000); // Επίπεδα για μέγιστη μείωση χωρίς αμβροσία
		...
		var w9 = max*0.27-2500<0?0:Math.ceil((max*0.27-2500)/8000); // Επίπεδα για μέγιστη μείωση με αμβροσία
		...
		if(w4>320) { cnt++; hc = (cnt==1?'<tr><td colspan="'+(c-(jsn.name=='Δημαρχείο'?1:0))+'">'+hardcap(y+parseInt(ps[0])-1,1)+'</td></tr>':''); }
		if(w9>320) { cnt2++; hc = (cnt2==1?'<tr><td colspan="'+(c-(jsn.name=='Δημαρχείο'?1:0))+'">'+hardcap(y+parseInt(ps[0])-1,2)+'</td></tr>'+(jsn.name=='Δημαρχείο'?'<tr style="background:lightgreen"><td colspan="'+(c-(jsn.name=='Δημαρχείο'?1:0))+'">'+hardcap(y+parseInt(ps[0])-1,3)+'</td></tr>':''):''); }
		body += hc+tr;
	}
