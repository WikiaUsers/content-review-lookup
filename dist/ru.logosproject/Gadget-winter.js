if (!(new Date().getMonth() > 2 && new Date().getMonth() < 11)) {
...
!r&&!a.flakeBottom;if(g.getComputedStyle)try{w="relative"===g.getComputedStyle(a.targetElement,null).getPropertyValue("position")}catch(d){w=!1}t=a.usePositionFixed;h&&(l&&!a.disabled)&&(a.init(),a.active=!0)};a.autoStart&&a.events.add(g,"load",y,!1);return this}.apply({},[window,document]);

(function() {
var unstuck = false;
if(!window.localStorage||window.localStorage.snow!="true"){snowStorm.stop();snowStorm.freeze();unstuck=true}else{snowStorm.start();snowStorm.show();snowStorm.resume();}
if(window.localStorage){
  var buttonContain = document.createElement("div");
  buttonContain.style.position = "fixed";
  buttonContain.style.background = "#266494";
  buttonContain.style.borderRadius = "100%";
  buttonContain.style.cursor = "pointer";
  buttonContain.style.padding = "5px";
  buttonContain.style.top = "3rem";
  buttonContain.style.right = "1rem";
  var flake = document.createElement("i");
  flake.classList = "fa fa-snowflake-o";
  flake.style.verticalAlign = "middle";
  flake.style.color = "#D3DEE9";
  flake.style.fontSize = "1.5em";
  buttonContain.appendChild(flake);
  buttonContain.addEventListener("click", function() {if(unstuck){snowStorm.disabled = 0;unstuck=false} snowStorm.toggleSnow(); localStorage.snow = snowStorm.active.toString();});
