$(document).ready(function (){
	...
	$('#editform').before('<div id="wiki-editor-tabs"><div class="wikitext">'+($text_wikitext[mw.config.get( 'wgUserLanguage' )]||'Викитекст')+'</div><div class="preview">'+($text_preview[mw.config.get( 'wgUserLanguage' )]||'Предпросмотр')+'</div><div class="changes">'+($text_changes[mw.config.get( 'wgUserLanguage' )]||'Изменения')+'</div></div>');
		...
		$changes_api.post(postdata).done( function ( postResult ) {
			...
			try {
				...
				if (diff){
					...
					$changes_content.html( '<p style="color:#9DA7B1;text-align:center;font-style:italic" class="note-text">('+($text_nochanges[mw.config.get( 'wgUserLanguage' )]||'Без изменений')+')</p>' )
					.append( '<div class="visualClear"></div>' );
				}
