...
- DarkGames26: Nose pos solo hice copy paste y ya, supongo que eso cuenta (cabe recalcar que tengo permiso para tomar el CSS) (?)
*/
...
function customizeUserList() {
	var interval = setInterval(function () {
		$('.listusers-result-table-rows td:first-child a').each(function(){
			if ($(this).text() === 'Wall') {
		    	this.href = this.href.replace("User_talk:", 'Muro:');
			}this.href = this.href.replace(/%20/g, "_").replace('User:', 'Usuario:');
		});
	}, 100 );
}
function customizeContribsTools() {
		$('.mw-contributions-user-tools a[href*="Usuario%3A"], .UserProfileActivityModeration__links a[href*="Usuario%3A"]').each(function(){
	    	 this.href = this.href.replace(/\+/g, '_').replace("Usuario%3A", 'Usuario:');
		 });
}
...
$('td.user > a').filter(function() {
		       ...
		       $(this).siblings('.wds-avatar').find('.wds-avatar__image').wrap("<a href='/es/wiki/Usuario:"+$(this).text().replace(/ /g, '_')+"'></a>");
			});
}
...
$(function() {
        ...
        if (config.wgPageName == "Especial:ListaUsuarios") {
        	customizeUserList();
        	}
        if (config.wgPageName == "Especial:Leaderboard") {
        	customizeLeaderboardAvatars();
        	}
        if (config.wgPageName == "Especial:Comunidad") {
