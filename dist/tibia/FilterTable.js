	...
	$("table.filterable").each(function(){
		...
		$(this).find("tr:first-child th, tr:first-child td").each(function(){
			if (!$(this).hasClass("unfilterable")){
				...
				$(this).append($('<div class="filterMenu" style="top:'+$(this).height()+'px;display:none;"></div>'));
				for (j=0; j<cols.length; j++){
					/* order:x; for number options will use first 3 int to sort, if string will make an invalid css [ie order:abc;] but do nothing*/
					$(this).find(".filterMenu").append('<div style="order:'+ String(cols[j]).substr(0, 3)+';"><input id="opt'+j+(1+i)+'" type="checkbox" value="'+cols[j]+'" col="'+(i+1)+'" class="filterOption" checked><label for="opt'+j+(1+i)+'">&nbsp;'+cols[j]+'</label></div>');
				}
			}
			i++;
		});
