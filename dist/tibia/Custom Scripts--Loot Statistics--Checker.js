...
'Yielothax': '9.1'
    ...
    lootparser_creature_special_names = {
      ...
      creature_items_name_change = {
        'Acolyte of the Cult': {'Book': 'Book (Orange)'},
        'Adept of the Cult': {'Book': 'Book (Orange)'},
        'Barbarian Brutetamer': {'Book': 'Book (Grey Small)'},
        'Blue Djinn': {'Book': 'Book (Blue)'},
        'Demodras': {'Book': 'Book (Gemmed)'},
        'Dragon Lord': {'Book': 'Book (Gemmed)'},
        'Enlightened of the Cult': {'Book': 'Book (Orange)'},
        'Frost Dragon': {'Book': 'Book (Gemmed)'},
        'Ghost': {'Book': 'Book (Orange)'},
        'Green Djinn': {'Book': 'Book (Green)'},
        'Novice of the Cult': {'Book': 'Book (Orange)'},
        'Orc Shaman': {'Book': 'Book (Grey Small)'},
    ...
    check_all = function (filter_function) {
      var dook = function (c, r) {
        var i;
        if (lootparser_creature_special_names.hasOwnProperty(c)) { c = lootparser_creature_special_names[c]; }
        i = $('#stats_result').find('a[title="Loot Statistics:' + c + '"]').parent();
        if (!i.size()) { alert('couldn\'t find for remove\n' + '[title="Loot Statistics:' + c + '"]\n\n\n' + r); }
        else { i.remove(); }
      },
      doajax = function (id) {
        var tthis = $(this), t = $.trim($(this).text().replace('Loot Statistics:', '').replace(' ', '_'));
        $.ajax({cache: false, async: true, url: '/index.php?title=Loot_Statistics:' + t + '&action=raw', success: function (r) {
          if (r === '') { window.setTimeout(function () { doajax(id); }, 3000); }
          else {
            var iserror, creature = r.match(/name\s*=\s*([\.\w \(\)]*)/);
            creature = (creature === null ? false : $.trim(creature[1]).replace(/_/g, ' '));
            if (creature === false) { alert('problem with\n' + r); }
            else {
              iserror = filter_function(r, creature);
              if (iserror === false) {
                dook(creature, r);
              }
              else if (typeof iserror === 'object') {
                tthis.parent().append('<br />');
/*jslint unparam: true*/
                $.each(iserror, function (i, v) { tthis.parent().append(v + '<br />'); });
/*jslint unparam: false*/
          ...
          if (a.status !== 404) { alert(a.status + b + c); window.setTimeout(function () { doajax(id); }, 3000); }
      ...
      total_requests = requests_to_do = $('#stats_result').find('a').each(doajax).size();
    };
...
/*jslint regexp: true*/
              var tt, item, times, amount, total, x, y, ret = [], tmpt, tmps,
              tmp = r.match(/\{\{\s*Loot2[\s\|]*version\s*=\s*(?:\d+\.\d+)[\s\|]*[^}]*/g), tmpa = [],
              versions = r.match(/\{\{\s*Loot2[\s\|]*version\s*=\s*\d+\.\d+/g), versions_n = [], page_name, tmpv;
/*jslint regexp: false*/
              if (versions !== null) {
                for (x = 0; x < versions.length; x++) {
                  y = 0;
                  while (versions[x].substr(y - 1, 1).match(/\d|\./)) { y--; }
                  versions_n[x] = parseFloat(versions[x].substr(y), 10);
                }
/* possible incorrect version */
                page_name = lootparser_creature_special_names.hasOwnProperty(creature) ? lootparser_creature_special_names[creature] : creature;
                for (x = 0; x < versions_n.length; x++) { for (y = 0; y < versions_n.length; y++) {
                  if (x !== y && versions_n[x] === versions_n[y]) {
                    ret.push('Repeated template version:' + versions_n[x]);
                  }
                } }
                if (lootparser_versions_ex[current_tibia_version].changed.hasOwnProperty(page_name)) {
                  tmpv = parseFloat(lootparser_versions_ex[current_tibia_version].changed[page_name], 10);
                  for (x = 0; x < versions_n.length; x++) {
                    if (versions_n[x] > tmpv) { ret.push('Possible incorrect version:' + versions_n[x]); }
                  }
                }
                if (lootparser_versions_ex[current_tibia_version]['new'].hasOwnProperty(page_name)) {
                  tmpv = parseFloat(lootparser_versions_ex[current_tibia_version]['new'][page_name], 10);
                  for (x = 0; x < versions_n.length; x++) {
                    if (versions_n[x] !== tmpv) { ret.push('Possible incorrect version:' + versions_n[x]); }
                  }
                }
/* possible non existing version */
                for (x = 0; x < versions_n.length; x++) {
                  if (
                    versions_n[x] > current_tibia_version &&
                    !lootparser_versions_ex[current_tibia_version]['new'].hasOwnProperty(page_name) &&
                    !lootparser_versions_ex[current_tibia_version].changed.hasOwnProperty(page_name)
                  ) { ret.push('Possible non existing version:' + versions_n[x]); }
                }
...
/* wrong parameters */
              if (tmp !== null) { for (x = 0; x < tmp.length; x++) {
                tmpa[x] = tmp[x].split(/[\r\n|]+/);
                for (y = 0; y < tmpa[x].length; y++) {
                  tmpt = $.trim(tmpa[x][y]);
                  if (tmpt.indexOf(',') !== -1) {
/*jslint regexp: true*/
                    tt = tmpt.match(/(.*?\(.*?)\s*,/);
...
/*jslint regexp: false*/
                      if (tt !== null) {
                        ...
                        else if ((tmps = item.match(/\s+(?:boots|legs)$/i)) !== null) {
  ...
  $.ajax({
    ...
    success: function (obj) {
      var x;
      if (typeof obj.query !== 'undefined' && typeof obj.query.userinfo !== 'undefined'
      && obj.query.userinfo.groups !== 'undefined') {
