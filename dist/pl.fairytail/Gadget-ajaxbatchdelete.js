...
$( function () {
	if( skin === 'monobook' ) {
		addPortletLink( 'p-tb', wgServer + wgScriptPath + '/index.php?title=Special:BlankPage&blankspecial=ajaxjqbd', 'Masowe usuwanie');
	} else {
		$( '#my-tools-menu' ).prepend( '<li class="custom"><a href="' + wgServer + wgScriptPath + '/index.php?title=Special:BlankPage&blankspecial=ajaxjqbd" rel="nofollow">Masowe usuwanie</a></li>' );
	}
} );
...
function createAjaxDeleteForm() {
	var	pageHeading = ( skin === 'oasis' ) ? ( $( '.AdminDashboardArticleHeader' ).length ? '.AdminDashboardArticleHeader > h1' : '.WikiaPageHeader > h1' ) : 'h1.firstHeading',
		...
		bdelFormHtml = '<form id="ajaxdeleteform" action="javascript:void(0);"><textarea style="height: 20em; width: 50%;" id="abd-textarea">'
			+ '</textarea><p><label for="abd-reason">Powód usunięcia: </label><input type="text" style="width: 20em;" id="abd-reason" />'
	...
	$bodyId.text( 'Lista stron do usunięcia:' );
...
function ajaxDeleteStart() {
	...
	var	txt = document.getElementById( 'abd-textarea' ),
		...
		badchars = /(\#|\<|\>|\[|\]|\{|\}|\|)/;
	if( page === '' ) {
		$( '#abd-output' ).append( '* Zrobione! Całość usunięta lub następna linia jest pusta.\n' );
		document.getElementById( 'abd-startbutton' ).removeAttribute( 'disabled');
	} else {
		if( badchars.test( page ) ) {
			$( '#abd-output' ).append( '! Wykryto niedozwolone znaki, pominięto:' + page + '\n' );
