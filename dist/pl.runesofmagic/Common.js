...
$j('.changecolor').html('<a class="nocolors">Przywróć domyślne kolory</a><a class="addcolors">Pokaż kolory według rzadkości</a>');
...
$j('.nocolors').click(function(){
  $j(this).closest('table').addClass("deletecolors");
  $j('.addcolors').show();
  $j('.nocolors').hide();
});

$j('.addcolors').click(function(){
  $j(this).closest('table').removeClass("deletecolors");
  $j('.nocolors').show();
  $j('.addcolors').hide();
});