// Dieses Skript wurde von Finner (englisches WarframeWiki) erstellt.
     ...
     for (var k = 0; k < count.length; k++) {
         ...
         if ((i === (loopLimit1 + 1)) && (nextDate.getTime() <= now.getTime())) {
             ...
             if ($('.pcPlanet').length > 0) {
                 if (pc !== -1) document.getElementById('pcPlanet_' + (k - a)).innerHTML = PCPlanetTracker((i - 1) * loopTime[k].innerHTML);
                 else a += 1;
             }
             if ($('.ps4Planet').length > 0) {
                 if (ps4 !== -1) document.getElementById('ps4Planet_' + (k - b)).innerHTML = PS4PlanetTracker((i - 1) * loopTime[k].innerHTML);
                 else b += 1;
             }
             if ($('.xb1Planet').length > 0) {
                 if (xb1 !== -1) document.getElementById('xb1Planet_' + (k - c)).innerHTML = XB1PlanetTracker((i - 1) * loopTime[k].innerHTML);
                 else c += 1;
             }
             if ($('.nswPlanet').length > 0) {
                 if (nsw !== -1) document.getElementById('nswPlanet_' + (k - d)).innerHTML = NSWPlanetTracker((i - 1) * loopTime[k].innerHTML);
                 else d += 1;
             }
         } else {
...
function PS4PlanetTracker(countXinterval) {
    if ((countXinterval + 3) % 4 === 0) {
...
function XB1PlanetTracker(countXinterval) {
    if ((countXinterval + 3) % 4 === 0) {
...
function NSWPlanetTracker(countXinterval) {
    if ((countXinterval + 3) % 4 === 0) {
