...
/*
  ...
  Authors:
    ...
    V2.6: Feb 2011: [[User:Ciencia Al Poder]] & [[User:Lupo]]: get rid of old URI-param based edit
          interface, preventing unwanted single-category changes following malicious links
...
if (typeof (HotCat) == 'undefined' && wgAction != 'edit') { // Guard against double inclusions, and inactivate on edit pages
...
if (typeof (window.HotCat) != 'undefined') {
	HotCat.messages.cat_removed = 'eliminada la [[Categoría:$1]]';
	HotCat.messages.template_removed = 'eliminada la {{[[Categoría:$1]]}}';
	HotCat.messages.cat_added = 'añadida la [[Categoría:$1]]';
	HotCat.messages.cat_keychange = 'Cambiada la clave de ordenación de [[Category:$1]]: ';
	...
	HotCat.messages.cat_resolved = ' (redirección a [[Categoría:$1]] reparada)';
	HotCat.messages.uncat_removed = 'eliminada {{uncategorized}}';
	HotCat.messages.prefix = '';
	HotCat.messages.using = ' usando [[wikipedia:Wikipedia:HotCat|HotCat]]';
	...
	if (window.wgUserLanguage == 'es') {
		...
		HotCat.messages.multi_error = 'No se pudo obtener el texto de la página del servidor. Por lo tanto, tus cambios de categoría no pueden ser guardados. Disculpa los inconvenientes.';
	}

	HotCat.category_canonical = 'Categoría'
	HotCat.categories = 'Categorías';
	HotCat.disambig_category = null;
	HotCat.redir_category = null;

	HotCat.tooltips.change = 'Modificar';
	HotCat.tooltips.remove = 'Quitar';
	...
	HotCat.tooltips.down = 'Sustituir esta categoría por una subcategoría';
	HotCat.tooltips.up = 'Sustituir esta categoría por una de las categorías superiores';

	HotCat.addmulti = '<span>+<sup>+</sup></span>';
	HotCat.multi_tooltip = 'Modificar varias categorías';

	//HotCat.disable = function() { return (wgNamespaceNumber != 0 && wgNamespaceNumber != 6 && wgNamespaceNumber != 14 && wgNamespaceNumber != -1); };

	HotCat.uncat_regexp = null;

	HotCat.template_categories = {
		'Artículos para expandir sobre Pokémon': '[Ee]xpandir\\|[Pp]okémon',
		'Artículos para expandir sobre Pokémon Mundo Misterioso': '([Aa]rtículo MM Incompleto|[Ee]xpandir\\|[Ppokémon Mundo Misterioso)',
		'Artículos para expandir sobre Pokémon de los personajes': '[Ee]xpandir\\|[Pp]okémon de los personajes',
		'Artículos para expandir sobre TCG': '[Ee]xpandir\\|TCG',
		'Artículos para expandir sobre anime': '[Ee]xpandir\\|[Aa]nime',
		'Artículos para expandir sobre ciudades y pueblos': '[Cc]iudad o Pueblo Incompleto',
		'Artículos para expandir sobre edificios': '[Ee]xpandir\\|[Ee]dificios',
		'Artículos para expandir sobre episodios del anime': '[Ee]P Incompleto',
		'Artículos para expandir sobre lugares': '[Ee]xpandir\\|[Ll]ugares',
		'Artículos para expandir sobre manga': '[Ee]xpandir\\|[Mm]anga',
		'Artículos para expandir sobre objetos': '[Ee]xpandir\\|[Oo]bjetos',
		'Artículos para expandir sobre personajes': '[Ee]xpandir\\|[Pp]ersonajes',
		'Artículos para expandir sobre rutas': '[Ee]xpandir\\|[Rr]utas',
		'Artículos para expandir sobre videojuegos': '[Ee]xpandir\\|[Vv]ideojuegos'
	};
	...
	HotCat.use_up_down = false; // Sin flechas arriba/abajo: cambiar por subcategoría o categoría superior
	HotCat.single_minor = true; // solo 1 categoría: marcar como menor

	HotCat.blacklist = /([Aa]rtículos para expandir sobre|[Ee]xpandir|[Ii]ncompleto)/;
...
(function () {
  ...
  function change_category (wikitext, toRemove, toAdd, key) {
    ...
    function find_insertionpoint (wikitext) {        
      ...
      if (index < 0) {
        // Find the index of the last interwiki link... (approximation without API)
        var match = /((^|\n\r?)(\[\[\s*\w+\s*:[^\]]+\]\]\s*))+$/.exec (copiedtext);
        if (match)
          index = match.index;
      }
      return index;
  ...
  function initiateEdit (doEdit, failure) {
    ...
    var uri     = wgServer + wgScriptPath
                ...
                + '&prop=info%7Crevisions&inprop=watched&intoken=edit&rvprop=content%7Ctimestamp'
                + '&meta=siteinfo|userinfo&uiprop=options';
  ...
  function performChanges (failure, singleEditor) {
    ...
    if (!commitForm) {
      ...
      if (singleEditor && !singleEditor.noCommit && !HotCat.no_autocommit && editToken) {
        formText += '<input type="hidden" name="wpEditToken" value="' + editToken + '" />';
        formText += '<input type="hidden" name="wpSave" value="wpSave" />';
      } else {
        formText += '<input type="hidden" name="wpDiff" value="wpDiff" />';
      }
      formText += '</form>';
    ...
    for (var i=0; i < toEdit.length; i++) {
      ...
      {
        result = change_category (result.text, toEdit[i].originalCategory, null, null);
        if (!result.error) {
          changes++;
    ...
    if (error !== null) { // Do not commit if there were errors
      if (commitForm.wpSave) {
        var action = commitForm.wpSave;
        action.name = 'wpDiff';
        action.value = 'wpDiff';
      }
    }
  ...
  var suggestionConfigs = {
    searchindex : {name: HotCat.engine_names.searchindex, engines: ['opensearch'], cache: {}, show: true, temp: false, noCompletion : false}
   ,pagelist    : {name: HotCat.engine_names.pagelist, engines: ['internalsearch'], cache: {}, show: true, temp: false, noCompletion : false}
   ,combined    : {name: HotCat.engine_names.combined, engines: ['opensearch', 'internalsearch'], cache: {}, show: true, temp: false, noCompletion : false}
   ,subcat      : {name: HotCat.engine_names.subcat, engines: ['subcategories'], cache: {}, show: true, temp: true, noCompletion : true}
   ,parentcat   : {name: HotCat.engine_names.parentcat, engines: ['parentcategories'], cache: {}, show: true, temp: true, noCompletion : true}
  };
