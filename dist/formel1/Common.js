...
function createToggle()
{
	...
	for(var i=0; i < document.getElementsByTagName("span").length; i++)
	{
		...
		if(elemClasses[0] == 'Toggler' || elemClasses[0] == 'Jutsu' || elemClasses[0] == 'SB')
		{
			var SpoilerLink= document.createElement("a");
			var attrVar;
			SpoilerLink.setAttribute('id', obj.className);
			SpoilerLink.setAttribute('href', "javascript:clickToggler('" + obj.className + "');");
...
function clickToggler(hideID)
{
	...
	var clickElement = hideID.split(' ');
	ToggleClasses('span', clickElement[0], clickElement[1]);
	ToggleClasses('div', clickElement[0], clickElement[1]);
}
function ToggleClasses(tag, ToggleName, ToggleId)
{
	var obj;
	var myFirst;
	myFirst = '1';
	...
	for(var i=0; i < document.getElementsByTagName(tag).length; i++)
	{
		obj= document.getElementsByTagName(tag)[i];
		var elemClasses = obj.className.split(' ');
		//Für alle ein-ausblenden
		if(ToggleId == 'Master' && elemClasses[1] == ToggleName)
		{
			if (myFirst == '1')
			{
			...
			obj.className = myFirst + elemClasses[1] + ' ' + elemClasses[2];
		}
		else
		{
			if(elemClasses[1] == ToggleName && elemClasses[2] == ToggleId)
			{
				if(elemClasses[0] == 'toToggle')
					obj.className = 'unToggle ' + elemClasses[1] + ' ' + elemClasses[2];
				else if(elemClasses[0] == 'unToggle')
					obj.className = 'toToggle ' + elemClasses[1] + ' ' + elemClasses[2];
			}
			else if(elemClasses[1] == ToggleName && elemClasses[2] != ToggleId)
			{
				obj.className = 'toToggle ' + elemClasses[1] + ' ' + elemClasses[2];
			}
		}
