	    ...
	    if (!mw.util.getParamValue('wpForReUpload')) {
	        ...
	        if (formstyle == "guided" || (formstyle == "" && window.location.search.indexOf('basic=true') == -1)) { // Add link to basic form
	            ...
	            jQuery(function ($) {
	                ...
	                customRows += fromTemplate('Nom Fichier Original', 'originalfileBox', false, '[OPTIONNEL] Nom de l’image sur How I Met Your Mother Wiki (Wiki anglais). (Ne pas saisir «&nbsp;File:&nbsp;»)');
	                ...
	                function verifySummary() {
	                    ...
	                    var enfilename = "",
	                    originalfile = document.getElementById('originalfileBox').value.trim();
	    
	                    if (originalfile !== "") {
	                        enfilename = '\n[[en:File:' + originalfile + ']]';
	                    }
	                ...
	                $.getScript(mw.util.wikiScript('load') + 
	                    ...
	                    dev.minicomplete.load(
	                        $customRows.find('#descriptionBox'), $customRows.find('#sourceBox'), $customRows.find('#characterBox')
	                    );
	                });
	            });
