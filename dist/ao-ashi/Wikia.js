...
/* Configuration for [[w:c:dev:MassCategorization]] */
...
(function () {
    ...
    function fixCrotchShotsInPopularPages() {
        document.querySelectorAll('#recirculation-rail .thumbnails a:not(.rail-sponsored-content) > img[src*="/smart/"]').forEach(function (img) {
            img.src = img.src.replace('/smart/', '/top-crop/');
        });
    }
    ...
    if ($rail.filter('.is-ready').length) {
...
/*This page is adapted from My Hero Academia Wiki.*/