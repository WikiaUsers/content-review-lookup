...
if(wgCanonicalSpecialPageName == 'Chat') {
	...
	NodeChatController.prototype.openPrivateRoom = function(users) {
		...
		$.ajax({
			...
			success: $.proxy(function(data) {
				$().log("Versuche einen Privatchat mit diesem Benutzer herzustellen:  " + users.join(','));	
	...
	NodeRoomController.prototype.onJoin = function(message) {
		...
		if(typeof connectedUser == "undefined"){
			...
			if( !this.isPrivate ) {
				...
				var newChatEntry = new models.InlineAlert({text: $.msg('Benutzer dazugekommen: ', [joinedUser.get('name')] ) });
	...
	NodeRoomController.prototype.onPart = function(message) {
		...
		if(typeof connectedUser != "undefined"){
			...
			//TODO: move it to other class
			if( !this.isPrivate ) {
				var newChatEntry = new models.InlineAlert({text: $.msg('Benutzer gegangen: ', [connectedUser.get('name')] ) });
				this.model.chats.add(newChatEntry);
			}
