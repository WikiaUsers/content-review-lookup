...
// Written by Joeytje50
// Original can be found at http://runescape.wikia.com/wiki/User:Joeytje50/monstercalc.js
	...
	if ($('span#GEPrice').length != 0) {
			...
			$('span.GEItem').each(function(){//for each of the prices, run this script
				...
				$('input#GEPriceCalc' + i).bind('keyup click', function(){//when text is entered...
					...
					if ($(this).val().replace(/[^0-9]/g, '').length == 0) {
						...
						$('#GEPCalcResult' + $(this).attr('class')).html(warn + rs.addCommas($(this).val().replace(/[^0-9]/g, '') * $('span.GEItem span.' + $(this).attr('class')).html().replace(/[^0-9]/g, '')) + warn2)
					}
				})
				i++
			})
		...
		$('input#GEPriceCalc').bind('keyup click', function(){
			...
			if ($('input#GEPriceCalc').val().replace(/[^0-9]/g, '').length == 0) {
				...
				$('#GEPCalcResult').html(warn + rs.addCommas($('input#GEPriceCalc').val().replace(/[^0-9]/g, '') * GEItem.replace(/[^0-9]/g, '')) + warn2)//add commas back to the result too after calculating the price
			}
		})
