/*
 * Duplicate of https://github.com/Wikia/app/blob/dev/resources/jquery/jquery.tablesorter.js with additional functionality
 * allows table cache to be regenerated dynamically, e.g. if table contents is changed via JS
 * use $(table).trigger('updateSortableCache'); to do so
 * to use this, use the 'sortable2' class instead of 'sortable'
 * 
 * Other functionality is the same, see http://community.wikia.com/wiki/Help:Sorting for help
 * 
 * Other changes: all uses of 'tablesorter' replaced with 'tablesorter2'
 * Adds both jquery-tablesorter2 and jquery-tablesorter classes to prevent 
 *     duplicate sorting and make css still work
 * Added hook to load the sorter (this is in another script in the repo)
 */
...
( function( $, mw ) {
    ...
    $.tablesorter2 = {
        ...
        construct: function( $tables, settings ) {
            return $tables.each( function( i, table ) {
                ...
                if ( $table.hasClass('jquery-tablesorter') || $table.hasClass('jquery-tablesorter2') ) {
                    return;
                }
                // Quit if no tbody
                if ( !table.tBodies ) {
                ...
                $table.addClass( "jquery-tablesorter jquery-tablesorter2" );

                // New config object.
                table.config = {};

                // Merge and extend.
                config = $.extend( table.config, $.tablesorter2.defaultOptions, settings );

                // Save the settings where they read
                $.data( table, 'tablesorter2', config );
                ...
                $table.on('updateSortableCache', function () {
                    if (!firstTime) {
                        cache = buildCache(table);
                    }
                });
    ...
    $.fn.tablesorter2 = function( settings ) {
    ...
    ts.addParser( {
        ...
        format: function( s ) {
            return $.tablesorter2.formatDigit( s.replace( ts.rgx.currency[1], '' ) );
        },
        type: 'numeric'
    } );
    ...
    ts.addParser( {
        id: 'number',
        is: function( s, table ) {
            return $.tablesorter2.numberRegex.test( $.trim( s ));
        },
        format: function( s ) {
            return $.tablesorter2.formatDigit(s);
        },
        type: 'numeric'
    } );
