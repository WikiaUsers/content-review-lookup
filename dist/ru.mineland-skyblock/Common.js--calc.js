    ...
    var cache = {},
        ...
        helper = {
            ...
            submitForm: function () {
                ...
                if (formError) {
                    helper.showError.call(self, 'Одно или несколько полей содержат недопустимое значение.');
                    return;
                }
    ...
    Calc.prototype.setupCalc = function () {
		...
		$table.append($('<tr>').addClass('jcTable-toprow').append(
			...
			.append(
				...
				$('<a>').text('просмотр шаблона').addClass('noselect')
					.attr('href', self.template && '/wiki/'+self.template || '#')
					.attr('target', '_blank'),
				$('<a>').text('просмотр калькулятора').addClass('noselect')
