...
function cardList_buildForm(){
	$("#cardList-select").append($('<select id="cardList-select-t"> <optgroup label="陣營"> <option value="t-a">劍術之城</option> <option value="t-t">技巧之場</option> <option value="t-m">魔法之派</option> <option value="t-f">妖精</option>  </optgroup> <optgroup label="稀有度"> <option value="r-1">★</option> <option value="r-2">★★</option> <option value="r-3">★★★</option> <option value="r-4">★★★★</option> <option value="r-5">★★★★★</option> <option value="r-6">★★★★★★</option> <option value="r-7">★★★★★★★</option> </optgroup> <optgroup label="COST"> <option value="c-1">1-5 BC</option> <option value="c-2">6-10 BC</option> <option value="c-3">11-15 BC</option> <option value="c-4">16-20 BC</option> <option value="c-5">21 BC或以上</option> </optgroup> <optgroup label="力量屬性"> <option value="f-a">劍術</option> <option value="f-t">技巧</option> <option value="f-m">魔法</option> </optgroup> <optgroup label="型系" id="types"> </optgroup> </select>'));
}

function quickList_buildForm(){
	$("#c-kw").append($('<input type="text" id="kw" size="30">'));
	$("#c-type").append($('<select id="type"> <option value="">全部</option> <option value="劍術之城">劍術之城</option> <option value="技巧之場">技巧之場</option> <option value="魔法之派">魔法之派</option> <option value="妖精">妖精</option> </select>'));
