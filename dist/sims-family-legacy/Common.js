...
;Dependencies
This template needs the following Java Script, if not already present,:{{Hide|1=
<pre>
/************* General functions *************/
...
Both of them to be placed in [[MediaWiki:Common.js]].

;Syntax
Type <code><nowiki>{{T|Hide|text}}</nowiki></code> where you want to show/hide the text.
...
;Samples
<code><nowiki>{{Hide|Text to be hidden}}</nowiki></code> results in:<br/>
{{Hide|Text to be hidden}}

<code><nowiki>{{Hide|1=Text with the equal sign "="}}</nowiki></code> results in:<br/>
...
results in {{hide|mode=start}}
{| class=wikitable
| tata || titi
|}
{{hide|mode=end}}

<code><nowiki>{{Hide|showbutton=+|hidebutton=-|Text to be hidden}}</nowiki></code> results in:<br/>
{{Hide|showbutton=[+]|hidebutton=[-]|Text to be hidden}}

;Models
- http://openwetware.org/wiki/Template:Hide openwetware.org

<includeonly>[[Category:General Templates|{{PAGENAME}}]] </includeonly><noinclude>[[Category:Template Documentation|{{PAGENAME}}]]</noinclude>