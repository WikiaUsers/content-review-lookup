...
// lock blog comments for blogs that haven't been commented on for more than 180 days. By: [[User:Joeyaa|Joey Ahmadi]]
...
$(function() {
    function disableCommenting() {
        if (wgNamespaceNumber == 500 && $('#article-comments-ul li').size() > 1) {
            ...
            if (old > 180) {
                $('#article-comm').attr('disabled','disabled').text('This blog post hasn\'t been commented on for over 180 days. There is no need to comment.');
