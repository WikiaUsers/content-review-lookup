...
window.snowStorm = {
	...
	flakesMax: 90,			// Limit total amount of snow made (falling + sticking)
	...
	snowColor: 'Black',		// Don't eat (or use?) yellow snow.
...
snowStorm.timers = [];
snowStorm.flakes = [10000000];
snowStorm.disabled = false;
snowStorm.active =  true;
snowStorm.meltFrameCount = 30;
...
function doStart() {
	if (!snowStorm.excludeMobile || !snowStorm.internal.isMobile) {
		if (snowStorm.freezeOnBlur) {
			snowStorm.events.add(snowStorm.internal.isIE ? document : window, 'reload', doDelayedStart);
		} else {
			doDelayedStart();
		}
	}
