...
function stageTableRelated(){
    $('.stageTable td.stageTD').each(function(){
        var t = $(this).parent().nextUntil('tr:not(:has(td.needRowSpan))');
        $(this).attr('rowSpan',t.length+1);
        t.children('td:first-child').remove()
...
$(function(){
    ...
    if(x.style.pointerEvents === 'auto') importStylesheet('User:夜暮/frames1pe.css');
    else importStylesheet('User:夜暮/frames1.css');
...
function staminaSlider(){
    ...
    ).css({background: 'black'}).hide().on('mousedown',function(e){e.stopPropagation();});
    r();
