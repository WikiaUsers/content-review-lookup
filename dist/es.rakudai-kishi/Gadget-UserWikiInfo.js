// <pre>
/*
 * UserWikiInfo v4.4: Una colección de enlaces útiles relacionados con el usuario que aparece en contribuciones, página de usuario y discusión, con recuento de ediciones y avatar, para Monobook
 *
...
(function($, mw) {
	...
	_dataRecv = function(data) {
		...
		if (!_isIP) {
			$uwi.find('.userlink > a').eq(0).attr('href', mw.util.getUrl(mw.config.get('wgFormattedNamespaces')['2'] + ':' + u));
		}
		$uwi.find('.talklink > a').eq(0).attr('href', mw.util.getUrl(mw.config.get('wgFormattedNamespaces')['3'] + ':' + u));
	...
	_contribsData = function(data) {
		...
		text = text.substring(3, text.indexOf('</p>')).replace(new RegExp('[^0-9\|]', 'g'), '');
