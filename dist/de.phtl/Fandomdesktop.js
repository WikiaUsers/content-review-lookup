...
$(function () {
	...
	if (mw.config.get('skin') === 'oasis') {
		...
		glnbutt.templates = {
			...
			normal:
				'<a {url} class="global-navigation__link">' +
					'<div class="global-navigation__icon">{icon}</div>' +
					'<div class="global-navigation__label">{text}</div>' +
				'</a>',
			parent:
		...
		glnbutt.createTemplate = function (template, args) {
			...
			return glnbutt.templates[template].replace('{text}', args.text).replace('{url}', args.url ? 'href="' + args.url + '"' : '').replace('{icon}', args.icon).replace('{shortName}', args.shortName).replace('{has-background}', args.hasBackground);
		};
