...
CSC148, Winter 2019
Bogdan Simion, Sadia Sharmin, and Michael Liut
...
class Election:
    ...
    def read_results(self, instream: IO[str]) -> None:
        ...
        for line in instream:
            data = line.split(",")
            riding_name = data[1].strip().strip('"')
            party_name = data[13].strip().strip('"')
            number_of_votes = int(data[17])

            self.update_results(riding_name, party_name, number_of_votes)
    ...
    def riding_winners(self, riding: str) -> List[str]:
        ...
        for party, votes in self._results[riding].items():
            if votes > most_votes:
                riding_winners = [party]
                most_votes = votes
            elif votes == most_votes:
                riding_winners.append(party)
    ...
    def popular_vote(self) -> Dict[str, int]:
        ...
        for riding_result in self._results.values():
            for party, votes in riding_result.items():
                if party not in total_votes_per_party:
                    total_votes_per_party[party] = votes
                else:
                    total_votes_per_party[party] += votes

        return total_votes_per_party
    ...
    def party_seats(self) -> Dict[str, int]:
        ...
        for riding in self._ridings:
            riding_winner = self.riding_winners(riding)

            for party in self._results[riding]:
                if party not in party_seats:
                    party_seats[party] = 0

            if len(riding_winner) == 1:
                party_seats[riding_winner[0]] += 1
    ...
    def election_winners(self) -> List[str]:
        ...
        ['pc']
        """
        party_seat = self.party_seats()
        election_winners = []
        most_seats = 0

        for party, seats in party_seat.items():
            if seats > most_seats:
                election_winners = [party]
                most_seats = seats
...
class Jurisdiction:
    ...
    def read_results(self, year: int, month: int, day: int, instream: IO[str]) \
        ...
        add to them.
        """
        elec_date = date(year, month, day)

        if elec_date in self._history:
            self._history[elec_date].read_results(instream)
        else:
            new_elec = Election(elec_date)
            new_elec.read_results(instream)
            self._history[elec_date] = new_elec
    ...
    def party_wins(self, party: str) -> List[date]:
        ...
        for election_date, election in self._history.items():
            if party in election.election_winners():
                party_wins.append(election_date)
    ...
    def party_history(self, party: str) -> Dict[date, float]:
        ...
        for election_date, election in self._history.items():
            popular_vote = election.popular_vote()
            popular_vote_total = 0

            for votes in popular_vote.values():
                popular_vote_total += votes

            if popular_vote_total == 0 or party not in popular_vote:
                party_history[election_date] = 0.0

                return party_history

            percentage = popular_vote[party] / popular_vote_total
            party_history[election_date] = float(percentage)
    ...
    def riding_changes(self) -> List[Tuple[Set[str], Set[str]]]:
        ...
        for i in range(len(dates) - 1):
            prev_elec_ridings = self._history[dates[i]].ridings_of()
            next_elec_ridings = self._history[dates[i + 1]].ridings_of()

            removed_ridings = set(prev_elec_ridings) - set(next_elec_ridings)
            added_ridings = set(next_elec_ridings) - set(prev_elec_ridings)

            riding_changes.append((removed_ridings, added_ridings))
