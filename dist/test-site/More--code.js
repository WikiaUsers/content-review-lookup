//<source lang="javascript">
    //Inspired from http://dev.wikia.com/wiki/AutoEditPages and http://dev.wikia.com/wiki/ListAdmins
$(function ($) {
    "use strict";
    if (!$('#More').length) return;

    if (mw.config.get('skin') === 'monobook') {
          mw.util.addPortletLink('creates', '#', 'More', 'creates');
       } else {
             if ($('.creates').length === 0) {
             $('#AccountNavigation > li > .subnav > li:last-child').after(
               $('<li/>').append('<a style="cursor:pointer" class="creates">More</a>')
             );
             $('#More').prepend('<li class="custom"><a style="cursor:pointer" class="creates">More</a></li>');
             }
       }
