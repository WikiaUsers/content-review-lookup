var BGLinks = (function() {
  ...
  var searchNode = function(node, inserted_nodes) {
    ...
    var book_chap = "((?:("+book_string+apoc_string+")(?:\\.)?"+unicode_space+"*?)?(?:(\\d*):)?(\\d+(?:(?:ff|f|\\w)|(?:\\s?(?:-|â€“|â€”)\\s?\\d+)?)))([^a-z0-9]*)";
    var regex_string = "(?:"+book_string+apoc_string+")(?:\\.)?"+unicode_space+"*?\\d+:\\d+(?:ff|f|\\w)?(?:\\s?(?:(?:(?:-|â€“|â€”)\\s?(?:(?:"+book_string+apoc_string+")(?:\\.)?\\s)?)|(?:(?:,|;|&amp;|&|and|cf\\.|cf)))\\s?(?:(?:(?:vv.|vs.|vss.|v.) ?)?\\d+\\w?)(?::\\d+\\w?)?)*";
    ...
    if (verse_match === null) {
      ...
      var newLinkNodeClicked = function() {return false;};
      while (matched === book_chap_regex.exec(verse_match[0])) {
        ...
        if (that.clickTooltip === true) {
          newLinkNode.onclick=newLinkNodeClicked;
        }
