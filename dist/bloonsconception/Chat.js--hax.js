//THIS CODE HAS BEEN TAKEN FROM Plants vs. Zombies Character Creator Wiki, if you have any questions, we can answer you
/* 
		...
		for(var i in this.model.users.models) {
			if(this.model.users.models[i].attributes.name == chat.attributes.name) {
				if(this.model.users.models[i].attributes.isStaff) {
					icon = ' <img class="stafficon" src="https://images.wikia.nocookie.net/__cb20140904103123/bloonsconception/images/7/78/STARBURST.svg">';
				}
				break;
			}
		}
			...
			for(var i = 0; i < pings.length; i++) {
				if(text.toLowerCase().indexOf(pings[i].toLowerCase()) != -1 || this != mainRoom.viewDiscussion) {
					if(!window.hasFocus) { //Only annoy people if the window isn't focused
						document.getElementById('sound').innerHTML = '<audio src="https://images.wikia.nocookie.net/bloonsconception/images/e/e4/Towerlevelup.ogg" autoplay=""></audio>';
