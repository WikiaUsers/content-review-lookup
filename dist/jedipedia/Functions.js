...
function storePageName()
{
   ...
   var edit = $("#WikiaPageHeader > h1, #firstHeading").text();
   edit = edit.replace(/Bearbeite: /,'');
   edit = edit.replace(/Bearbeiten von/,'');
...
function initCountdowns (klasse)
{
  i = 0;
  $("." + klasse).each(function(){
    value = parseInt($(this).text());
    $(this).attr("class",klasse + "-" + i);
    Countdowns(klasse + "-" + (i++), value); // format: <tag class="countdown">Time in seconds</tag>
  });
}

function Countdowns (klasse,value)
{
  if (value == 0)
  {
    $("." + klasse).text($("." + klasse).attr("countdown-ablaufnachricht"));
    return;
  }
  ...
  s = value%60;
  $("." + klasse).text((d!=0?d+' '+(d==1?'Tag':'Tage')+' ':'')+(h<10?"0"+h:h)+":"+(m<10?"0"+m:m)+":"+(s<10?"0"+s:s));
  setTimeout("Countdowns('"+klasse+"',"+(--value)+")",1000);
}
...
function rewriteTitle() {
    if(typeof(window.SKIP_TITLE_REWRITE) != 'undefined' && window.SKIP_TITLE_REWRITE)
        return;
    var text = document.getElementById("title-meta");
    if (text)
      document.getElementById("WikiaPageHeader").getElementsByTagName('h1')[0].innerHTML = text.innerHTML;
    return true;
}
...
function showEras(className){
    if(typeof(SKIP_ERAS) != 'undefined' && SKIP_ERAS)
        return;
    $($("#"+className)).insertBefore("#WikiaPageHeader > h1");
    $($("#"+className)).insertBefore("#firstHeading"); /* Monobook */
    $("#WikiaPageHeader > #"+className+", #content > #"+className).css("display","block");
}