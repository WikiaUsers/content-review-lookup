...
/* This function is used only for doradexEntry template. It colorizes the borders. */
var doradex_entry = ".doradex-entry";

var doradexdex_entry_function = function() {
	if(check_element_exists(doradex_entry) !== false){
	    // Get class for color
	    var doradex = document.querySelectorAll(doradex_entry);
	    var doradex_list = document.querySelector(doradex_entry + " ul");
	    var doradex_list_element = document.querySelectorAll(doradex_entry + " ul li");
	    var doradex_colors;

	    for(var i = 0; i < doradex.length; i++){
	    	doradex_colors = type_compare(doradex[i].classList);
	    	doradex_list.style.borderColor = doradex_colors[0];

	    	for(var j = 0; j < doradex_list_element.length; j++)
	    		doradex_list_element[j].style.borderBottomColor = doradex_colors[1];
		}
	}
};
page_load(doradex_entry_function);
