    ...
    TooltipsEditor = that = window.TooltipsEditor = {
        ...
        refreshLuaCache: function () {
            ...
            api.get({
                ...
                console.log('Кэш обновлен!', data);
            })
            // Fandom doesn't like catch as a method name
            ["catch"](function (err) {
                mw.notify("Ошибка при обновлении кэша lua", {
                    title: "Неперехваченная ошибка",
                    type: "error"
                });
                console.error(err);
            });
        ...
        updateActions: function (keys) {
            ...
            $log.empty().append($("<li>", {
                text: len ? "Несохраненные изменения: " : "Никаких изменений внесено не было",
                "class": "actions-none"
            }));
            ...
            $log.append($("<li>", {
                text: "Сохранение изменений также приведет к сортировке всех всплывающих подсказок в алфавитном порядке.",
                "class": "actions-none"
            }));
        ...
        generateSearch: function () {
            ...
            if (val.trim() === "") return $results.html("<p>Введите поисковый запрос, чтобы начать поиск.</p>");
        ...
        openEditor: function (values) {
            ...
            var $button2 = $("<span>", {
                ...
                html: $("<button>", {
                    ...
                    "class": "oo-ui-buttonElement-button",
                    click: function () {
                        that.reset(confirm("Вернуться на главную страницу без сохранения?"));
                    },
                }),
            });
            var $button3 = $("<span>", {
        ...
        onSave: function (oldKey) {
            ...
            if (!values.key) return alert("Вам нужно ввести ключ всплывающей подсказки!");
            ...
            that.forEachInputBox(function (inter, i, otherinputboxes) {
                ...
                if (!val.optional) {
                    if (!values[inter]) {
                        pass = false;
                        return alert("Вам нужно ввести всплывающую подсказку " + val.display + "!");
                    }
                }
            });
            if (!pass) return;
                ...
                $("<div>", {
                    ...
                    html: [
                        ...
                        $("<div>", {
                            ...
                            html: [
                                $("<div>", {
                                    text: "Поиск по существующей всплывающей подсказке: ",
                                        ...
                                        $("<span>", {
                                            ...
                                            html: [
                                                $("<button>", {
                                                    ...
                                                    "class": "oo-ui-buttonElement-button",
                                                    text: "Добавить новую всплывающую подсказку",
                                                    click: function () {
                                                        that.openEditor({});
                                                    },
                                                }),
                ...
                $("<div>", {
                    ...
                    html: Array.prototype.concat(
                        [
                            ...
                            "для дополнительного форматирования. Перейдите по ссылкам для получения дополнительной информации.",
                            "<br>",
                            $("<fieldset>", {
                                id: "TooltipsEditor-insertionTool",
                                html: [
                                    $("<legend>", {
                                        text: "Набор инструментов (Нажмите, чтобы вставить)"
                                    }),
                                    $("<div>", {
                                        html: [
                                            $("<span>", {
                        ...
                        [
                            $("<span>", {
                                text: "Заголовок всплывающей подсказки: ",
        ...
        editorCloseHandler: function () {
            ...
            if (confirm("Вы уверены, что хотите сохранить и закрыть редактор?")) {
                that.modal.hide();
                mw.notify("Другое всплывающее окно должно указывать на успешное редактирование.", {
                    title: "Обработка Вашей правки",
                    type: "info",
                });
            } else return;
            ...
            api.postWithEditToken({
                ...
                if (saved) {
                    mw.notify("Просмотрите свои изменения прямо сейчас!", {
                    ...
                    mw.notify("Никаких изменений внесено не было.", {
                        title: "Tooltips Saved!",
                        type: "info"
                    });
                }
        ...
        onClick: function (tooltips) {
            ...
            $(window).on("beforeunload", function () {
                if (!that.closing)
                    return "Вы уверены, что хотите выйти со страницы и отменить внесенные изменения?";
            });

            that.modal.show({
                title: "Tooltips Editor",
                onHide: function () {
                    if (!that.closing && confirm("Вы уверены, что хотите выйти из редактора и отменить внесенные изменения?")) {
                        that.closing = true;
                        return true;
            ...
            $.when.apply($, promises).then(that.mainProcess, function (code, e) {
                return alert("Не удалось проанализировать всплывающие подсказки: ", e), console.warn("Не удалось проанализировать всплывающие подсказки: ", e);
            }).catch(console.warn);
        },
        ...
        secretDebugTool: function () {
            var it = prompt("Пожалуйста, введите элемент, чтобы увидеть его JSON");
            if (it !== null && it !== "") {
                if (it in that.json) {
                    mw.notify("Смотрите результаты в консоли.", {
                        title: "Отладка: Найдено",
                        type: "warn"
                    });
                    console.log(that.json[it]);
                } else mw.notify("Элемент не найден.", {
                    title: "Отладка: Сбой",
                    type: "warn"
                });
            }
