...
mw.loader.using(["mediawiki.api"]).then(function () {
    ...
    function copyToClipboard(text, notice) {
        ...
        if (confirm(notice + "Следующее будет скопировано в ваш буфер обмена:\n\n" + text + "\n\nНажмите кнопку ОК для подтверждения")) {
            var $temp = $("<textarea>");
            var brRegex = /<br\s*[\/]?>/gi;
            $("body").append($temp);
    ...
    (function () {
        ...
        function errorHandler(err) {
            mw.notify("Смотрите веб-консоль для получения подробной информации", {
        ...
        if ($(ELECTED_BUTTON).length > 0)
            $(ELECTED_BUTTON).show().click(function () {
                Promise.all([getElectionData(), getLocalData(), getLocalElections()]).then(function (data) {
                    ...
                    if (!pastElectionData) {
                        alert("На данный момент API не предоставляет данные о прошлых выборах.");
                        return;
                    }
            ...
            $(CURRENT_BUTTON).show().click(function () {
                Promise.all([getElectionData(), getLocalData()]).then(function (data) {
                    ...
                    if (!currentElectionData) {
                        alert("На данный момент API не предоставляет текущие данные о выборах.");
                        return;
                    }
                    var localData = data[1].mayors,
    ...
    (function () {
        ...
        $(BUTTON).show().click(function () {
            getBingoData().then(function (data) {
                if (!data.goals)
                    mw.notify("На данный момент API не предоставляет целей для бинго", { title: "Данных пока нет", type: "error" });
                else {
