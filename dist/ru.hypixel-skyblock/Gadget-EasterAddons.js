...
mw.loader.using(["mediawiki.util", "mediawiki.Uri"], function () {
    ...
    window.EasterAddons = EasterAddons = that = {
        ...
        init: function () {
            console.log("[Easter Addons] Инициализация..");
            $("<link>", {
                rel: "stylesheet",
        ...
        calculate: function (index) {
            ...
            if (++this.debugCounter[0] >= 10) {
                if (this.DEBUG)
                    console.log("[EasterAddons Debug Mode] В среднем за последние 10 попыток: " + this.debugCounter[1] / 10 + " Мин: " + this.debugCounter[2] + " Max: " + this.debugCounter[3]);
                // reset counters
        ...
        startAll: function () {
            this.pause = false;
            console.log("[Easter Addons] Запуск анимаций");
        },
        stopAll: function () {
            this.pause = true;
            console.log("[Easter Addons] Приостановка анимации");
        },
        startDebug: function () {
            this.DEBUG = true;
            this.canvas.addClass("debug-mode");
            console.log("[Easter Addons] Вход в режим отладки");
        },
        stopDebug: function () {
            this.DEBUG = false;
            this.canvas.removeClass("debug-mode");
            console.log("[Easter Addons] Выход из режима отладки");
        },
    };
