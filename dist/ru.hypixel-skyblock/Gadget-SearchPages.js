/* jshint
	...
	browser: true, jquery: true,Page content to match
...
mw.loader.using(['mediawiki.util', 'mediawiki.api', 'ext.codeEditor.ace']).then(function() {	
	...
	function filterMatch(data, titleSearch, contentSearch, caseInsensitive, regex, regexFlags) {
		if (regexFlags.match(/[^gmisuy]/)) return error("Недопустимые флаги регулярных выражений \"" + regexFlags + "\"");
		if (regex) {
			try {
	...
	function error(msg) {
		console.warn("Ошибка при получении данных: " + msg.replace(/(\/.+?\/[a-z]*)/));
		
		$('#SP-note').html($("<b>", { 
			html: "Ошибка при получении данных: " + msg.replace(/(\/.+?\/[a-z]*)/, $('<span>', {
	...
	function onDelete(pages) {
		...
		$.when.apply($, pages.map(function(page) {
			...
			mw.notify('Удаление выбранных страниц завершено.');
			$(inputSelector).prop('disabled', false);
		});
	}
	...
	function onSubmit() {
		...
		getData().then(function(d) {
			...
			if (!d.length) {
				$('#SP-note').text('Ни одна страница не соответствовала вашему запросу.');
				$('#SP-resultsToolsWrapper, .SP-buttonGroup, #SP-resultsAmount').addClass('hidden');
				return;
			}
			$('#SP-PageList').html(d);
	...
	function initInterface() {
		(document.getElementById('firstHeading') || document.getElementsByClassName('page-header__title')[0]).innerHTML = 'Поиск по страницам';
		document.title = 'Поиск по страницам | ' + mw.config.get('wgSiteName') + ' | Fandom';
		...
		$('#mw-content-text').html($('<div>', {
			html: [
				'<b>Пространство имен для поиска: </b>',
				generateNamespaces(),
				createCheckbox("Показывать перенаправления", "Показывать перенаправления в результатах", "showredir"),
				createCheckbox("Без учета регистра", "Сделайте сопоставители нечувствительными к регистру", "case-insensitive"),
				createCheckbox("Регулярное выражение", "Заставьте сопоставители использовать регулярные выражения", "regex", function() {
					toggleEditors();
				...
				$('<div>', {
					...
					html: [
						$('<b>', {
							title: "Флаги регулярных выражений для использования",
				...
				'<b>Заголовок страницы должен соответствовать: </b>',
				...
				'<br>',
				'<b>Содержимое страницы должно соответствовать: </b>',
				'<br>',
				...
				'<br>',
				$('<h2>', {
					html: ["Результаты поиска", "&nbsp;".repeat(2), $('<button>', { 
						id: "SP-submit", 
						text: "Submit",
						click: onSubmit,
					})],
				...
				$('<big>', {
					id: "SP-note",
					text: "Отправляйте входные данные для просмотра страниц.",
				}),
				$('<div>', { 
					id: "SP-results", 
					html: [
						$('<div>', {
							...
							class: "hidden",
							html: [
								canDelete ? createCheckbox('Удалить найденное', 'Удалить все найденные страницы', 'del-found', function() {
								...
								canDelete ? $('<div>', {
									...
									html: [
										$("<b>", {
											text: "Создать уровень защиты: ",
											id: "SP-protect-level-sign",
										}),
									].concat([
										createInput('Только администраторы', 'Редактировать удаленную страницу могут только администраторы.', 'pl-admin-only', {
											input: {
												type: "radio",
												checked: true,
												value: "sysop",
										...
										createInput('Только администраторы', 'Редактировать удаленную страницу могут только автоматически подтвержденные пользователи.', 'pl-auto-only', {
											input: {
												type: "radio",
												value: "autoconfirmed",
												name: "pl-protect-options",
										...
										createInput('Причина защиты', 'Причина защиты', 'protect-reason'),
									]),
								}) : "",
								'<br>',
								$('<b>', {
			...
			html: [
				...
				$('<button>', {
					...
					click: function() {
						...
						$el.attr('checked', !checked);
						$el.text(!checked ? "Инвертировать выделение" : "Необращенный выбор");
