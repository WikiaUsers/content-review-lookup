/** HalloweenEffects
 * Скрипт для добавления эффектов для празднования Хэллоуина.
 */
...
window.HalloweenEffects = HalloweenEffects = that = Object.assign(this, {
	N: 5, // количество спрайтов
	H: 80, // ширина и высота спрайта в пикселях
	T: 5000, // время анимации в мс
	MINWAIT: 3000, // минимальное время задержки в мс
	MAXWAIT: 7000, // максимальное время задержки в мс
	MORESEP: 0, // требуется большее разделение между изображениями ('1' означает, что размер изображения в 1 раз больше)
	MAXTRIES: 200, // максимум попыток программы выделить позицию для спрайта, пока вместо этого он снова не отобразится в исходном положении; гарантия производительности
	X_BOUNDARIES: false, // ограничено ли отображение спрайтов очень близко к верхним и нижним краям (рекомендуется false)
	Y_BOUNDARIES: true, // ограничены ли спрайты отображением очень близко к левым и правым краям
	avoidElements: [ // селекторы для элементов, чтобы избежать установки спрайтов на
	...
	calculate: function (index) {
		...
		if (++this.debugCounter[0] >= 10) {
			if (this.DEBUG)
				console.log("[HalloweenEffects Debug Mode] В среднем за последние 10 попыток: " + this.debugCounter[1] / 10 + " Мин: " + this.debugCounter[2] + " Max: " + this.debugCounter[3]);
			// reset counters
		...
		if (tries < this.MAXTRIES) {
			...
			console.log("[HalloweenEffects] Элемент настройки " + index + " на предыдущую позицию.\nПопробуйте изменить размер вашего браузера и/или использовать режим 'свернуть', чтобы повысить производительность.");
	...
	startAll: function () {
		this.pause = false;
		console.log("[Halloween Effects] Запуск анимаций");
	},
	stopAll: function () {
		this.pause = true;
		console.log("[Halloween Effects] Приостановка анимации");
	},
	startDebug: function () {
		this.DEBUG = true;
		this.canvas.addClass("debug-mode");
		console.log("[Halloween Effects] Вход в режим отладки");
	},
	stopDebug: function () {
		this.DEBUG = false;
		this.canvas.removeClass("debug-mode");
		console.log("[Halloween Effects] Выход из режима отладки");
	},
});
