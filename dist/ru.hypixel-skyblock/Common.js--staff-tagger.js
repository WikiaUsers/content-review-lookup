...
/** Метчик персонала
 * 
 * Код для автоматизации некоторой части действий администратора по продвижению/понижению в должности персонала
 * 
 * Опция "Пометка персонала" появляется в шаблоне {{User rights request}} - Этот шаблон обычно используется на [[HSW:STFREQS]]
 * Также может быть запущен нажатием кнопки "staff tagger" в выпадающем меню на любой странице пользователя
 */
        ...
        $.Deferred(function (def) {
            if (mw.util.StaffUtil) {
                ...
                $.ajax({
                    cache: true,
                    dataType: "script",
                    url: "https://hypixel-skyblock.fandom.com/ru/wiki/MediaWiki:Gadget-StaffColorsUpdater.js"
                }).done(function () {
                    def.resolve(mw.util.StaffUtil);
                });
            }
