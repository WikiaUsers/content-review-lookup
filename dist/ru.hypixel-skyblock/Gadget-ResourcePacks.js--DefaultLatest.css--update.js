...
/*/*(() => {
	const files = {
		'Коллекция - Фермерство (агрономия)': [
			{ name: 'Кактус' }, //
			{ name: 'Зелёный краситель', mcName: 'Зелёный краситель' },
			{ name: 'Марковь' },
			{ name: 'Cocoa Beans' },
		...
		VanillaMobs: [
			...
			{ name: 'Enderman' },
			{ name: 'Ghast' },
			{ name: 'Sea Guardian', mcName:'Guardian' },
			{ name: 'Iron Golem' },
			{ name: 'Kada Knight', mcName:'Chicken Zombified Piglin Jockey' },
			{ name: 'Magma Cube' },
			{ name: 'Mooshroom' },
			{ name: 'Ocelot' },
			{ name: 'Pig' },
			{ name: 'Rabbit', mcName:'Brown Rabbit' },
			{ name: 'Sheep' },
			{ name: 'Skeleton Horse' },
			...
			{ name: 'Wither (Invulnerable)', mcName: 'Blue Wither' },
			{ name: 'Wither' },
			{ name: 'Wolf' },
			{ name: 'Zombie Horse' },
			{ name: 'Zombie Pigman' },
	...
	const mappedFiles = Object.keys(files)
		.map((key) =>
			files[key].map((file) => {
				...
				file.enchName ??= `Зачарование ${file.name}`;
	...
	function fetchUrls(files, wiki) {
		const url = `https://${wiki}.fandom.com/ru/api.php?${new URLSearchParams({
			action: 'query',
			format: 'json',
			prop: 'imageinfo',
			iiprop: 'url',
...
*		const cssEntries = ['/* Скрипт для обновления этой страницы включен DefaultLatest.css/update.js /'],
			missing = [],
			sbMissing = [],
			previewUrls = [],
			enchantedNames = [];
		...
		mappedFiles.forEach((file) => {
			...
			if (mcUrl) {
				...
				if (file.ench) enchantedNames.push(`${file.enchName.replace(/^Зачарование /, '')}.${file.sbEnchExt ?? file.sbExt ?? 'png'}`);
		...
		previewUrls.push({ label: 'Цветная броня СкайБлока ', urlPairs: [] });
		...
		console.log('Завершена генерация CSS! Просмотрите результаты в режиме на экране!');

		openModal(
			[
				"<div style='color:white; background:darkgrey; padding:8px 8px; margin:0 8px;'>",
				missing.length > 0 ? 'Ссылки на следующие элементы, не найденные в MC wiki:' : '',
				missing.length > 0 ? `<pre style="background:darkred; margin-bottom:10px;">${missing.join('\n')}</pre>` : '',
				sbMissing.length > 0 ? "Следуя правилам css, в SB Вики нет соответствующего файла по используемому имени:" : '',
