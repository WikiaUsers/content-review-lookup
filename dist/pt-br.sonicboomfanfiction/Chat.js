...
var chatOptions = {
	look: {
		...
		backgroundColor: getCookie("customisation", 0),
                modalIcon: "http://i.imgur.com/nsTQqya.png"
	},
	modules: {
		chatHacks: {
			element: "#chatHacks",
		...
		ignoreURL: {
			...
			loaded: false,
			load: function () {
				$('head').append('<style type="text/css">li[data-user="The_Black_Soldier"] {display:none;}</style>');
				this.loaded = true;
			}
		},
		stopSideScroll: {
...
function openOptions() {
	...
	var $optionsWindowHTML = $.showCustomModal( "Opções", '<form method="" name="" class="WikiaForm "><fieldset><p style="font-size:120%; font-weight:bold; font-style:italic;">Mudanças de cores</p><p style="font-size:80%;">Coloque o <a href="http://www.w3schools.com/html/html_colornames.asp" target="_blank">nome da cor</a> ou <a href="http://html-color-codes.info/" target="_blank">código hex</a><p>Cor do fundo do chat&nbsp;<input type="text" name="backgroundColourinput" id="backgroundColourinput" value="' + chatOptions.look.backgroundColor + '"/></p><br/><p>Cor do fundo da sua fala&nbsp;<input type="text" name="selfPostColourinput" id="selfPostColourinput" value="' + chatOptions.look.selfPostColor + '"/></p><br/><p>Cor da parte externa do chat&nbsp;<input type="text" name="surroundColourinput" id="surroundColourinput" value="' + chatOptions.look.surroundColor + '"/></p><br/><p>Cor da fonte do chat&nbsp;<input type="text" name="fontColourinput" id="fontColourinput" value="' + chatOptions.look.fontColor + '"/></p><br/><p style="font-size:120%; font-weight:bold; font-style:italic;">Fonte</p><p>Font family <select id="fontList"><option value="arial" style="font-family:arial;">Arial</option><option value="courier new" style="font-family:courier new;">Courier new</option><option value="georgia" style="font-family:georgia;">Georgia</option><option value="palatino linotype" style="font-family:palatino linotype;">Palatino linotype</option><option value="Comic Sans MS" style="font-family:Comic Sans MS;">Comic sans</option><option value="tahoma" style="font-family:tahoma;">Tahoma</option><option value="Trebuchet MS" style="font-family:Trebuchet MS;">Trebuchet MS</option><option value="Verdana" style="font-family:Verdana;">Verdana</option><option value="Lucida Console" style="font-family:Lucida Console;">Lucida Console</option></select></p><br/><p style="font-size:120%; font-weight:bold; font-style:italic;">Mais Funcionalidades</p><input type="checkbox" name="chatHacks" value="chatHacks" id="chatHacks"/> Ativar <a href="http://c.wikia.com/wiki/User:Monchoman45/ChatHacks.js" target="_blank">chathacks</a>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="multiPM" value="multiPM" id="multiPM"/> Ativar <a href="http://callofduty.wikia.com/wiki/User:Madnessfan34537/multipm.js" target="_blank">multi PM</a><br/><input type="checkbox" name="tabComplete" value="tabComplete" id="tabComplete"/>Ativar <a href="http://runescape.wikia.com/wiki/User:Joeytje50/tabinsert.js" target="_blank">tab complete</a>&nbsp;&nbsp;&nbsp;<input type="checkbox" name="searchBar" value="searchBar" id="searchBar"/>Ativar <a href="http://callofduty.wikia.com/wiki/MediaWiki:Chat.js/searchbar.js" target="_blank">barra de pesquisa</a><br/><input type="checkbox" name="multiKick" value="multiKick" id="multiKick" />Ativar <a href="http://callofduty.wikia.com/wiki/User:Madnessfan34537/multikick.js" target="_blank">multi kick</a>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="ignoreURL" value="ignoreURL" id="ignoreURL"/>Ignorar GL (The Black Soldier) no chat principal<br /><input type="checkbox" name="stopSideScroll" value="stopSideScroll" id="stopSideScroll"/>Parar a barra de rolagem lateral quando alguém fizer spam</fieldset></form>', {
		id: "optionsWindow",
	    width: 600,
	    buttons: [
		{
...
$('.Rail').prepend('<div id="chatOptionsButton" onclick="openOptions();" style="margin: auto; cursor: pointer; color: #6ACB00; font-size: 147%; font-family: Oswald; text-transform: uppercase; margin-top: 6px; bottom: 4px; padding-bottom: 2px; border-bottom: solid 4px #038307; width: 92%; border: 5px solid rgb(106, 203, 0); border-left: 5px solid rgb(56, 105, 2); border-right: 5px solid rgb(56, 105, 2); background: transparent; border-radius: 23px;" align="center"><img src="' + chatOptions.look.modalIcon + '" width="25px"/>Opções</div>'); // Prevent multiple buttons from being appended
}
...
var Ichidoise = {
    ...
    doceIchi : "Amor Doce",
    ichoDoce: function() { 
                return this.ichiDoce + " Joga " + this.doceIchi + " pq sim.";
                }
};
...
function Horas() {
    ...
    var oi; 
    if (hour < 12) {
       oi = "Bom Dia" + " " + wgUserName + "!";
    } else {
       oi = "Boa Noite" + " " + wgUserName + "!";
    }
    document.getElementById('ChatHeader').innerHTML = oi;
}