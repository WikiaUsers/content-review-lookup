...
$( '.mw-rollback-link > a' ).after( $( '<span>' ).addClass( 'edit-rollback' ).prop( 'title', 'Bewerk terugdraaisamenvatting' ) );
	...
	if ( $( this ).parent().is( $rollback.parent() ) ) {
		...
		$rollback = $( '<div id="rollback-summary">' ).append(
			$( '<input type="text">' ).addClass( 'mw-ui-input rollback-text' ).prop( { maxlength: 250, spellcheck: true } ).val(
				'Bijdrage van [[Speciaal:Bijdragen/' + name + '|' + name + ']] ([[Overleg gebruiker:' + name + '|overleg]]) teruggedraaid'
			),
			$( '<input type="button">' ).addClass( 'mw-ui-button mw-ui-constructive rollback-submit-button' ).val( 'Terugdraaien' )
		).insertAfter( this );
	}
