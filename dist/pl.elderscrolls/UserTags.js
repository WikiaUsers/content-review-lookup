...
if (wgNamespaceNumber === 2) {
	...
	(function (window, $, mw, config) {
		...
		var loaderData = (function ($, mw) {
			...
			if (siteSkin === 'oasis') {
				...
				if (({
					...
					2: 1,
					3: 1
				})[namespace] === 1 && username.indexOf('/') !== -1) {
					return;
				}
		...
		if (loaderData) {
			...
			config.modules.stopblocked = config.modules.stopblocked || config.modules.stopblocked === void 0;
			// Only Monobook needs this, OasisTagsModule can scrape the blocked state from
			// the DOM instead.
			config.modules.isblocked = config.modules.isblocked || (config.modules.isblocked === void 0 && loaderData.skin === 'monobook');
		...
		var importList = (function (config, alwaysOnly) {
			...
			var moduleMap = {
				explode: {
					s: 'w:dev:UserTags/module.implode.js'
				},
				prefLanguages: {
					s: 'w:dev:UserTags/module.prefLanguages.js',
					always: true
				}
			};
		...
		for (var i = 0, len = data.length ; i < len ; ++i) {
			...
			/*jshint forin:true */
			var list = [],
				hash = {}, exts = config.extensions || {}, script, data, module;
			for (module in config.modules) {
				// Skip already loaded
				if (exts[module] !== void 0) {
					continue;
				}
		...
		if (!importList.length) {
			...
			try {
				delete window.UserTagsJS;
			} catch (e) { /* IE8 throws just to increase it's crapness */ }
			return;
		}
