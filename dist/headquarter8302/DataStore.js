...
(function (window) {
  ...
  window.dev.dataStore = window.dev.dataStore || {
    ...
    util: {
      ...
      msgFmt: function msgFmt() {
        var _DataStore;
        return (_DataStore = "[DataStore] ").concat.apply(_DataStore, arguments);
      }
    }
  };
  ...
  try {
    ...
    throw new Error(window.dev.dataStore.util.msgFmt("Either LocalStorage or SessionStorage is unavailable and can't be accessed. To preserve data integrity, DataStore will intentionally throw. No data has been read or written.".concat(" Cause: ").concat(error.message)));
  }
    ...
    remove: function remove(key) {
      switch (this.storeType) {
        ...
        case window.dev.dataStore.storeTypes.SESSION:
          ...
          break;
        default:
          console.warn(window.dev.dataStore.util.msgFmt("Unable to remove key: ".concat(key).concat(" because it can't be found")));
          break;
      }
    },
    /**
