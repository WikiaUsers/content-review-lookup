...
function timeStamp_CumulativeCosts_js() {
   return "2015.08.28 15:24 (UTC-7)";
}

function populateCumulativeCosts() {
   function reconcile(goldarray, elixirarray) {
      ...
      for (var i = 1; i <= thLevels; i ++) {
         if (goldarray[0][i] > 0 && elixirarray[0][i] > 0) {
            var min0 = goldarray[0][i];
            var min1 = goldarray[1][i];

            if (elixirarray[0][i] < min0) {
               min0 = elixirarray[0][i];
               min1 = elixirarray[1][i];
            }

            eitherarray[0][i]  = min0;
            eitherarray[1][i]  = min1;
            goldarray[0][i]   -= min0;
            goldarray[1][i]   -= min1;
            elixirarray[0][i] -= min0;
            elixirarray[1][i] -= min1;
         }
         else {
            eitherarray[0][i] = 0;
            eitherarray[1][i] = 0;
      ...
      return [eitherarray, goldarray, elixirarray];
   }

   if (wgPageName !== 'Cumulative_Costs')
      return;

   var gold1      = createCumulativeCostsTable('buildings',  'gold');
   var gold2      = createCumulativeCostsTable('walls',      'gold');
   var elixir1    = createCumulativeCostsTable('all',        'elixir');
   var elixir2    = createCumulativeCostsTable('walls',      'elixir', true);
   ...
   // Reconcile gold-only, elixir-only and either wall levels
   var ret    = reconcile(gold2, elixir2);
   var either = ret[0];
   gold2      = ret[1];
   elixir2    = ret[2];
   ...
   // Header row
   var headers  = ['Gold', 'Elixir', 'Either', 'Dark Elixir', 'Time', 'Gems', 'Cost'];
   var thLevels = buildingInfo('town hall', 'levels');
      ...
      for (var col = 0; col < headers.length; col ++) {
         ...
         switch (headers[col]) {
            case 'Gold':
               ...
               break;
            case 'Elixir':
               value = elixir1[0][lvl] + elixir2[0][lvl];
               td.innerHTML = (value ? value.format('#,##0') : '-');
               break;
            case 'Either':
               value = either[0][lvl];
            ...
            case 'Gems':
               ...
               tmp1 = doCalcResourceToGems(gold1[0][lvl] + gold2[0][lvl] + either[0][lvl]) +
                  doCalcResourceToGems(elixir1[0][lvl] + elixir2[0][lvl]) + 
                  darkelixir[1][lvl] + research[1][lvl] +
                  time1[1][lvl] + time2[1][lvl];
               tmp2 = doCalcResourceToGems(gold1[0][lvl] + gold2[0][lvl]) +
                  doCalcResourceToGems(elixir1[0][lvl] + elixir2[0][lvl] + either[0][lvl]) + 
                  darkelixir[1][lvl] + research[1][lvl] +
                  time1[1][lvl] + time2[1][lvl];
               value = (tmp2 < tmp1 ? tmp2 : tmp1);
               td.innerHTML = (value ? value.format('#,##0') : '-');
               break;
            case 'Cost':
               var tmp1, tmp2;
   ...
   for (var col = 0; col < headers.length; col ++) {
      ...
      switch (headers[col]) {
         case 'Gold':
            ...
            break;
         case 'Elixir':
            value = elixir1[0].reduce(function(a, b) { return a + b; }) +
                    elixir2[0].reduce(function(a, b) { return a + b; });
            td.innerHTML = (value ? value.format('#,##0') : '-');
            break;
         case 'Either':
            value = either[0].reduce(function(a, b) { return a + b; });
            td.innerHTML = (value ? value.format('#,##0') : '-');
            break;
         case 'Dark Elixir':
         ...
         case 'Gems':
            ...
            tmp1 = doCalcResourceToGems(gold1[0].reduce(function(a, b) { return a + b; }) +
               gold2[0].reduce(function(a, b) { return a + b; }) +
               either[0].reduce(function(a, b) { return a + b; })) +
               doCalcResourceToGems(elixir1[0].reduce(function(a, b) { return a + b; }) +
               elixir2[0].reduce(function(a, b) { return a + b; })) +
               ...
               time2[1].reduce(function(a, b) { return a + b; }) + gems;
            tmp2 = doCalcResourceToGems(gold1[0].reduce(function(a, b) { return a + b; }) +
               gold2[0].reduce(function(a, b) { return a + b; })) +
               doCalcResourceToGems(elixir1[0].reduce(function(a, b) { return a + b; }) +
               elixir2[0].reduce(function(a, b) { return a + b; }) +
               either[0].reduce(function(a, b) { return a + b; })) +
   ...
   for (var lvl = 1; lvl <= thLevels; lvl ++) {
      ...
      for (var i = maxlab; i >= 1; i --) {
         if (buildingInfo('laboratory', 'required town hall', i) <= lvl) {
            labtime = buildingInfo('laboratory', 'upgrade time', i);
            break;
         }
      }
   ...
   for (var col = 2; col <= builders + 1; col ++) {
      ...
      var rsrch      = research[0].reduce(function(a, b) { return a + b; }) +
         buildingInfo('laboratory', 'upgrade time').reduce(function(a, b) { return a + b; });
...
function createCumulativeCostsTable(tableType, resourceType, noTable) {
   if (!noTable) {
   ...
   if (tableType === 'buildings') {
     ...
     if (wIdx >= 0)
      ...
      if (resourceType === 'elixir') {
         ...
         sList = spellInfo('list', 'normal available');
      }
      else {
         tList = troopInfo('list', 'dark trainable');
         sList = spellInfo('list', 'dark available');
         hList = heroInfo('list');
      }
   }
   else if (tableType === 'laboratory') {
      tList = troopInfo('list', 'trainable');
      sList = spellInfo('list', 'available');
   }
   ...
   for (var i = 0; i < bList.length; i ++) {
      ...
      for (var lvl = 1; lvl <= thLevels; lvl ++) {
         ...
         if (resource === 'time') {
            ...
            td.innerHTML = formatDurationInSeconds((curr - cumulative));
         }
         else
      ...
      if (cumulative) {
         ...
         if (resource === 'time')
            td.innerHTML = formatDurationInSeconds(indiv);
         else
         ...
         if (resource === 'time')
            td.innerHTML = formatDurationInSeconds(cumulative);
         else
...
function formatDurationInSeconds(duration, longFormat) {
   var longRet     = [];
   var retVal      = '';
   var units       = [24 * 60 * 60, 60 * 60,      60,         1];
   var abbrevs     = [         'd',     'h',      'm',      's'];
   var longabbrevs = [       'day',  'hour', 'minute', 'second'];
   ...
   for (var i = 0; i < units.length; i ++) {
      if (duration >= units[i]) {
         ...
         if (i === units.length - 1)
            qty = Math.round(duration / units[i]);
         else
            qty = Math.floor(duration / units[i]);

         if (longFormat)
            longRet.push(qty.format('#,##0') + ' ' + longabbrevs[i] + (qty === 1 ? '' : 's'));
         else
   ...
   if (longFormat)
      return longRet.slice(0, -1).join(', ') + (longRet.length > 1 ? ' and ' : '') + longRet.slice(-1);
   else
