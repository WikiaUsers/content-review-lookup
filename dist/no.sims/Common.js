...
/* Add report script */
importScriptPage('User:Lost Labyrinth/QReportAdder.js', 'sims');

/* Remove report script */
importScriptPage('User:Lost Labyrinth/QReportRemoval.js', 'sims');
...
if (mwCustomEditButtons) {
    mwCustomEditButtons[mwCustomEditButtons.length] = {
        ...
        "tagOpen": "\n<gallery>\nImage:",
        "tagClose": "|[[Simmenes Wiki]] Logo\nImage:Wiki.png|[[Simmenes Wiki]] Logo\nImage:Wiki.png|Eine [[Simmenes Wiki]] Logo\n<\/gallery>",
        "sampleText": "Wiki.png"
    };
}
if (mwCustomEditButtons) {
...
function bindTT() {
    $t = $(this);
    $p = $t.parent();
    if ($p.hasClass("selflink") == false) $t.data("tt", $p.attr("title").replace(" (siden eksisterer ikke)", "").replace("?", "%3F")).mouseover(showTip).mouseout(hideTip).mousemove(moveTip);
}
...
function findDupImages(gf) {
    ...
    $.getJSON(url, function(data) {
        if (data.query) {
            ...
            for (pageID in pages) {
                dils = "," + dil.join();
                if (dils.indexOf("," + pages[pageID].title) == -1 && pages[pageID].title.indexOf("Fil::") == -1 && pages[pageID].duplicatefiles) {
                    ...
                    for (x = 0; x < pages[pageID].duplicatefiles.length; x++) {
                        ...
                        dil.push("Fil:" + pages[pageID].duplicatefiles[x].name.replace(/_/g, " "));
                    }
                    output += "</ul>\n\n"
                }
            }
...
AjaxRCRefreshText = 'Oppdater siden automatisk';
AjaxRCRefreshHoverText = 'Oppdater siden automatisk';
ajaxPages = ["Spesial:Siste endringer", "Spesial:WikiActivity"];
