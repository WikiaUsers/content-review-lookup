...
( function ( $, mw ) {
	...
	var HC = window.HotCat = {
		...
		messages: {
			...
			template_removed: 'Удаление {{[[Category:$1]]}}',
			cat_added: 'Добавлена[[Category:$1]]',
			cat_keychange: 'новый ключ сортировки для [[Category:$1]]: "$2"', // $2 is the new key
			cat_notFound: 'Категория "$1" не найдена',
			cat_exists: 'Категория "$1" уже существует; не добавлена.',
			cat_resolved: ' (перенаправление [[Category:$1]] открыто)',
			uncat_removed: 'удален шаблон {{uncategorized}}',
			separator: '; ',
			...
			// see localization hook below.
			multi_error: 'Не удалось получить текст страницы с сервера. Поэтому ваша категория меняется ' +
			'невозможно сохранить. Приносим свои извинения за доставленные неудобства.',
		...
		disambig_category: 'Неоднозначности',
		// Any category in this category is deemed a (soft) redirect to some other category defined by a link
		// to another non-blacklisted category. If your wiki doesn't have soft category redirects, set this to null.
		// If a soft-redirected category contains more than one link to another non-blacklisted category, it's considered
		// a disambiguation category instead.
		redir_category: 'Перенаправления категории',
		...
		tooltips: {
			...
			undo: 'Отменить',
			down: 'Открыт для изменения и отображения подкатегорий',
			up: 'Открыт для изменения и отображения надкатегорий'
		},
