if(document.cookie.indexOf('grinch') == -1) {
/** @license
...
$('.toolbar .tools').prepend('<li><a href="#" title="Bye-bye snow">I\'m a grinch.</a></li>').click(function() {snowStorm.stop(); hater();}); 

function hater() {
document.cookie = "grinch=true;"
                + "expires=Mon, 24 December 2012 18:40:22 GMT";
}
};
...
$('.stagger').each(function() {
  ...
  this.style.webkitAnimationDelay = -(j*delay) + 'ms';

  this.innerHTML = '';
  var _this = this;

  oldHTML.replace(/(.)/g, function(c) {
    var span = document.createElement('span');
    if (c == ' ') c = '&nbsp;'
    // span.style.webkitAnimation = 'bounce 0.8s ' + -delay * i + 'ms alternate infinite';
    span.style.webkitAnimationDelay = -(delay * i) + 'ms';
    span.style.mozAnimationDelay = -(delay * i) + 'ms';
    span.style.msAnimationDelay = -(delay * i) + 'ms';
    span.style.oAnimationDelay = -(delay * i) + 'ms';
...
$('#cart-link').find('a').addClass('stagger')
...
var bgSwitch = setInterval(switchBgColors,2000);
function switchBgColors() {
  var bgClasses = ['.bgA','.bgB','.bgC','.bgD','.bgE'];
  bgClasses.forEach(function(bgClassName) {
    $(bgClassName).css('background',bgStyles[Math.floor(Math.random() * bgStyles.length)]);
  });
};</script>