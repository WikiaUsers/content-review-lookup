...
* This is the code to accompany [[LogoLine:File Upload Wizard]].
* ===============================================================
*/
...
function fuwUpdateOptions() {
   ...
   fuwGet('fuwSubmitText').innerHTML = opts.OptionFree ? 
         ("<b>No</b>, I want to upload this file here on this wiki only.<br/>" + 
          "<small>This way it can be used only on LogoLine. However, somebody " +
          "else might still decide to copy it to Wikia or use it elsewhere later. If you " +
...
fuwGlobal.prototype.getLicense = function() {
   ...
   if (! license.special) {
      ...
      var simpleLicense = (opts.OptionOwnWork ? opts.OwnWorkLicense : null) ||
       ...
       // "PD-author" needs parameter, at least on Wikia
...
fuwGlobal.prototype.getUserStatus = function() {
   ...
   // 'notAutoconfirmed': can't use local upload, but may use script to prepare upload for Wikia
...
function fuwSetEnabled(tg, condition) {
   ...
   } catch (e) { alert("Element not found: " + ($.isDomElement(tg) ? tg.id : tg)); }
}
   ...
   $.ajax({
      ...
      // callback function, called when API query has succeeded:
         ...
         if (exists) {
            ...
            // However, we want a direct link to the Community Central.
            if (isCommons) {
               descURL = descURL.replace(/logohistory\.wikia\.com/, "community.wikia.com");
...
fuwValidateNFArticle = function() {
   ...
   name = name.replace(/^https?:\/\/logohistory\.wikia\.com\/wiki\//, "");
   name = name.replace(/^https?:\/\/logohistory\.wikia\.com\/w\/index\.php\?title=/, "");
   name = name.replace(/_/g, " ");
...
function fuwSubmitCommons() {
   ...
   alert("You will now be redirected to the Community Central. \nPlease use the Community Central upload form to add categories to your file description, and then complete the upload.");
   window.location = url;
}
