 ...
 * TODO: Improve user interface and user friendly "Options" explanation 
 *
 * @version 1.3.1 (Bejeweled wiki)
 * @author Callofduty4
...
var chatOptions = {
	look: {
		...
		backgroundColor: getCookie("customisation", 0),
                modalIcon: "https://openclipart.org/image/18px/svg_to_png/202775/gear-3d.png"
	},
	modules: {
		chatHacks: {
			element: "#chatHacks",
			...
			load: function () {
				if ($("#pingspan").length > 0 || this.loaded)
					return;
				importScript("MediaWiki:Chat.js/ChatHacks.js");
				this.loaded = true;
			}
		},
		EmoticonsWindow: {
			element: "#EmoticonsWindow",
			enabled: isEnabled("EmoticonsWindow"),
			loaded: false,
			load: function () {
...
function openOptions() {
	...
	var $optionsWindowHTML = $.showCustomModal( "Options", '<form method="" name="" class="WikiaForm "><fieldset><p style="font-size:120%; font-weight:bold; font-style:italic;">Color options</p><p style="font-size:80%;">Enter a <a href="http://www.w3schools.com/html/html_colornames.asp" target="_blank">color name</a> or <a href="http://html-color-codes.info/" target="_blank">color hex</a><p>Chat background <input type="text" name="backgroundColourinput" id="backgroundColourinput" value="' + chatOptions.look.backgroundColor + '"/></p><br/><p>Self-post background <input type="text" name="selfPostColourinput" id="selfPostColourinput" value="' + chatOptions.look.selfPostColor + '"/></p><br/><p>Surround <input type="text" name="surroundColourinput" id="surroundColourinput" value="' + chatOptions.look.surroundColor + '"/></p><br/><p>Font color <input type="text" name="fontColourinput" id="fontColourinput" value="' + chatOptions.look.fontColor + '"/></p><br/><p style="font-size:120%; font-weight:bold; font-style:italic;">Font</p><p>Font family <select id="fontList"><option value="roboto" style="font-family:roboto;">Roboto</option><option value="arial" style="font-family:arial;">Arial</option><option value="courier new" style="font-family:courier new;">Courier new</option><option value="georgia" style="font-family:georgia;">Georgia</option><option value="palatino linotype" style="font-family:palatino linotype;">Palatino linotype</option><option value="Comic Sans MS" style="font-family:Comic Sans MS;">Comic sans</option><option value="tahoma" style="font-family:tahoma;">Tahoma</option><option value="Trebuchet MS" style="font-family:Trebuchet MS;">Trebuchet MS</option><option value="Verdana" style="font-family:Verdana;">Verdana</option><option value="Lucida Console" style="font-family:Lucida Console;">Lucida Console</option></select></p><br/><p style="font-size:120%; font-weight:bold; font-style:italic;">Additional Options</p><input type="checkbox" name="chatHacks" value="chatHacks" id="chatHacks"/> Enable <a href="http://bejeweled.wikia.com/wiki/MediaWiki:Chat.js/ChatHacks.js" target="_blank">ChatHacks</a>    <input type="checkbox" name="multiPM" value="multiPM" id="multiPM"/> Enable <a href="http://callofduty.wikia.com/wiki/User:Madnessfan34537/multipm.js" target="_blank">multi-PM</a><br/><input type="checkbox" name="EmoticonsWindow" value="EmoticonsWindow" id="EmoticonsWindow"/>Enable <a href="http://runescape.wikia.com/wiki/User:Joeytje50/tabinsert.js" target="_blank">EmoticonsWindow</a>   <input type="checkbox" name="pmAlert" value="pmAlert" id="pmAlert"/>Enable <a href="http://callofduty.wikia.com/wiki/MediaWiki:Chat.js/searchbar.js" target="_blank">PM Alert</a><br/><input type="checkbox" name="multiKick" value="multiKick" id="multiKick" />Enable <a href="http://callofduty.wikia.com/wiki/User:Madnessfan34537/multikick.js" target="_blank">multi-kick</a>    <input type="checkbox" name="ignoreURL" value="ignoreURL" id="ignoreURL"/>Ignore URL in main chat<br /><input type="checkbox" name="stopSideScroll" value="stopSideScroll" id="stopSideScroll"/>Stop the sidescroll bar to appear after someone spams</fieldset></form>', {
		id: "optionsWindow",
	    width: 600,
	    buttons: [
		{
	...
	if (chatOptions.modules.EmoticonsWindow.enabled)
		$("#EmoticonsWindow").attr("checked",true);
	if (chatOptions.modules.pmAlert.enabled)
		$("#pmAlert").attr("checked",true);
...
function updateCookie() {
	...
	// Set the cookies
	setCookie("customisation", chatOptions.look.backgroundColor + ", " + chatOptions.look.fontColor + ", " + chatOptions.modules.chatHacks.enabled + ", " + chatOptions.look.fontFamily + ", " + chatOptions.modules.EmoticonsWindow.enabled + ", " +  chatOptions.modules.multiKick.enabled + ", " + chatOptions.modules.multiPM.enabled + ", " + chatOptions.modules.pmAlert.enabled + ", " + chatOptions.modules.ignoreURL.enabled + ", " + chatOptions.modules.stopSideScroll.enabled + ", " + chatOptions.look.surroundColor + ", " + chatOptions.look.selfPostColor);
	updateChatSkin();
	cancelChanges();
}
...
if (!$("#chatOptionsButton").length) {
	$('.Rail').prepend('<div id="chatOptionsButton" onclick="openOptions();" style="margin:auto; cursor: pointer; font-size:150%; bottom:4px; padding-bottom:2px; border-bottom: 1px solid #CCCCCC; width:90%; font-family: Roboto" align="center"><img src="' + chatOptions.look.modalIcon + '" width="18px"/> Options</div>'); // Prevent multiple buttons from being appended
}
 
window.onload = updateChatSkin();

// ChatAnnouncements; for Chat moderators and above 
importArticles({
    type: 'script',
    articles: [
