...
(function (mw, $, window, document) {
    ...
    var MAP_ICONS = '//images3.wikia.nocookie.net/wowwiki/images/f/f3/Map-sprites.png',
        LEGEND_BG = '//images1.wikia.nocookie.net/__cb20130626204249/wowwiki/images/5/50/Map-legend-bg.jpg',
    ...
    function initMaps ($maps) {
        ...
        mw.util.addCSS('\
            ...
            .map.map-embed.map-preload {\
                ...
                background-image: url("http://wowwiki.com/skins/common/images/ajax.gif");\
        ...
        var tooltips = ({
            ...
            autoClose: function () {
                if (tooltips.$autoShow) {
                    tooltips.$autoShow.tipsy('hide');
                    tooltips.$autoShow = false;
                }
            },
            
            init: function () {
                callbacks.close.add(this.autoClose);
                return this;
            }
        ...
        $.extend(Trigger.prototype, {
            ...
            createMap: function () {
                log('Trigger.createMap');
                var w = this.$map.data('size') || MAP_PREVIEW_SIZE,
                    h = Math.round(w * 3 / 4);
                    content;
                    
                /*
                return '<div class="map map-preload map-temp" style="width: ' +
                    w + 'px; height: ' + h + 'px;"></div>';
                */
                content  = '<div style="position: relative;">';
                content += '<img src="' + this.src + '" width="' +
                    w + 'px" height="' + h + 'px">';
                if (this.x || this.y) {
                    content += '<div class="map-Icon map-Blip" style="left: ' +
                        this.x + '%; top: ' + this.y + '%;"></div>';
                }
                content += '</div>';
                return content;
            },
            ...
            gravity: function () {
                var $this = $(this),
                    top = $this.offset().top,
                    h = Math.round(($this.data('size') || MAP_PREVIEW_SIZE) * 3 / 4);
                return top - h - 20 < $(window).scrollTop() ? 'nw' : 'sw';
            },
            
            addTipsy: function () {
                log('Trigger.addTipsy');
                this.$map
                .tipsy({
                    delayIn: 50,
                    opacity: '1.0',
                    ...
                    title: $.proxy(this.createMap, this)
                })
        ...
        function Coords ($map) {
            ...
            //<div data-legend="Point of Interest::Graveyard" data-y="60.5" data-x="39.4" data-icon="Star" data-title="Graveyard" style="left: 39.4%; top: 60.5%;" class="map-Icon map-Star" data-note="mn1" original-title="Graveyard @&nbsp;39.4,60.5" id="mn1"></div>
        ...
        $.extend(Legend.prototype, {
            ...
            walkGroups: function (nodes, key, list) {
                ...
                walker = new SortedObject(list.indexed);
                walker.each(this.walkIndexed, sub);
                walker = new SortedObject(list.named);
            ...
            render: function () {
                var walker = new SortedObject(this.parsed),
                    nodes = [];
        ...
        $.extend(MapLightbox.prototype, {
            ...
            initLargeMap: function () {
                ...
                tooltips.add(this.$icons, 'manual', this.autoShow);
