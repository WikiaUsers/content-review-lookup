...
ChatGame = {
    displayDisclaimer: function() {
        if (!JLAPI.cookie.load("ChatGame.accepted")) {
            ...
            if (a === true) {
                ...
                JLAPI.InlineAlert("ChatGame privacy policy: <a href='http://d97.wikia.com/wiki/MediaWiki:Cg.js/doc'>http://d97.wikia.com/wiki/MediaWiki:Cg.js/doc</a>");
                return false;
            }
        } else {
            return true;
    ...
    updateinfo: function() {
        ...
        infotext = ChatGame.gameList[a].info;
        $("#gameInfo").html(infotext);
    },

    communicate: function(command) {
        JLAPI.message.send("!ChatGame " + ChatGame.otherPlayerName + " " + encrypt("CG" + ChatGame.currentGameType + ChatGame.currentGameRoom + command, ChatGame.currentGamePass));
    },
    ...
    parse: function(source, encryptedMessage) {
        decryptedMessage = decrypt(encryptedMessage, ChatGame.currentGamePass);
            ...
            if (e == "!ChatGame") { // check it's a message for us
                ...
                if (c.slice(10, 16) == "invite" && !ChatGame.gameInProgress) {
                    if (c.slice(c.indexOf("name:") + 5).trim() == wgUserName) {
                        ...
                        JLAPI.InlineAlert(inviteName + " wants to play " + inviteGame + " in room " + inviteRoom + "!<span class='invitationlinks' id='invite-" + ChatGame.inviteIdentifier + "'><br />" + inviteReplyLinks + "</span>");
                    }
    ...
    openInviteWindow: function() {
        if (!ChatGame.gameInProgress) {
            ...
            var generatedPassword = passGenerate(16);
            for (i = 0; i < ChatGame.gameList.length; i++) {
            ...
            JLAPI.modal.form("Start a new game", "",
                ...
                "<span id='invitePlayerDropdown'></span>" + "<br />" +
                "By clicking 'OK' you understand that the Chat Game script will automatically send messages in chat under your name. (<a href='http://d97.wikia.com/wiki/MediaWiki:Cg.js/doc' target='_blank'>more info</a>)"
