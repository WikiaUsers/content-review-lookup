...
timer.setAttribute("style", "font-size:120%; text-align:center; border:2px solid grey; background-color: #bad1f5; color: black; border-radius: 12px; margin: 3px 35px 8px 35px;");
...
var tcrRefresh = setInterval(function()
			...
			if (released) {
			    trueColorsDiv.innerHTML = "True Colors";
                        trueColorsDiv.setAttribute("style", "font-size:120%; text-align:center; border:2px solid grey; background-color: #bad1f5; color: black; border-radius: 12px; margin: 3px 35px 8px 35px;");
				$("#tcr_img").attr("src", "https://vignette.wikia.nocookie.net/kccreations/images/3/32/True_Colors.jpg");
				tcr_img.setAttribute("height", 150);
				remainingReleaseTimeInMs = Date.UTC(2015, 4, 18, 0, 0, 0);
    			$("#tcr_timer").html("True Colors has finally been released in the U.S!");
				...
				tcr_img.setAttribute("height", 150);
				remainingReleaseTimeInMs = Date.UTC(2015, 4, 18, 0, 0, 0);
    		    $("#tcr_timer").html("Time left until U.S release: " + differenceInTime(currentTimeInMs, remainingReleaseTimeInMs) );
                    $("#tcr_imgLink").attr("href", "http://zedd.wikia.com/wiki/True_Colors_(album)");
                }
