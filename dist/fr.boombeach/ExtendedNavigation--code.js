...
function timeStamp_ExtendedNavigation_js() {
  return "2014.04.08 16:49 (UTC-7)";
}
 
// Importing required CSS
window.importArticle({
  type:    'style',
  article: 'MediaWiki:ExtendedNavigation/code.css'
});
...
function subNavLevel(strNav, strReturnLink) {
  ...
  if (strNav.substr(0, 8) === '<nowiki>' &&
    ...
    // Explicitly changing ampersands as well rather than relying on encodeURIComponent,
    // as that seems to change too many characters
    strNav[0] = '/wiki/' +
      encodeURI(strNav[0].replace(new RegExp(' ', 'g'), '_')).replace(new RegExp('&', 'g'), '%26');
  }
  ...
  switch (strReturnLink) {
    ...
    case 'findlink': {
      // For findlinks, we need to replace some special characters
      // that encodeURI/encodeURIComponent doesn't
      strNav[0] = strNav[0].replace(new RegExp('\'', 'g'), '%27');
