/* Any JavaScript here will be loaded for all users on every page load. */

  // ==== Imports.js customization ====
window.mbIndefStyle = 'color: #ff0000; opacity: 0.7; font-style: italic; text-decoration: line-through; text-decoration-thickness: 0.1px; font-weight: 300; letter-spacing: 0.8px';

window.mbTooltip = 'blocked by $2 for $1 with the reason: \"$3\" ($4 ago)';

window.AutoCreateUserPagesConfig = {
    content: {
        2: '{{NewUser}}',
    },
    summary: 'Automatically creating user page via JS',
    notify: true
};

  // ============================================ Search Bar
mw.hook('wikipage.content').add(function ($content) {
    var $placeholder = $('#search-placeholder');
    if (!$placeholder.length || $('#inpage-search').length || window.location.href.includes('action=')) return;

    var debounceTimer;

    // SEARCH BAR UI
    var $container = $('<div id="inpage-search" style="margin: 15px 0; padding: 15px; background: var(--theme-page-background-color--secondary); border: 1px solid var(--theme-border-color); border-radius: 8px;"></div>');
    var $input = $('<input type="search" id="pageSearchInput" placeholder="Filter Change Log (e.g., Winter, V74, Fixed)..." style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; background: var(--theme-page-background-color); color: var(--theme-page-text-color); box-sizing: border-box;">');
    var $status = $('<div id="searchStatus" style="font-size: 12px; margin-top: 8px; color: var(--theme-page-text-color); opacity: 0.8; font-weight: bold;"></div>');
    
    $container.append($input, $status);
    $placeholder.replaceWith($container);

    function getUpdateGroups() {
        var groups = [];
        // Map only versions (H3)
        $('.mw-parser-output > h3').each(function() {
            var $header = $(this);
            var $contentBetween = $header.nextUntil('h3, h2'); // For the next H3 or next YEAR (H2)
            groups.push({
                header: $header,
                content: $contentBetween,
                fullText: ($header.text() + ' ' + $contentBetween.text()).toLowerCase()
            });
        });
        return groups;
    }

    function clearHighlights() {
        $('.page-search-highlight').each(function() {
            var parent = this.parentNode;
            $(this).replaceWith(this.textContent);
            if (parent) parent.normalize();
        });
    }

    function applyHighlight($elements, term) {
        var regex = new RegExp('(' + term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + ')', 'ig');
        
        $elements.find('span, li, b, i, code, h3').addBack('h3').not('h2').contents().filter(function() {
            return this.nodeType === 3 && this.textContent.match(regex);
        }).each(function() {
            var highlightedText = this.textContent.replace(regex, '<span class="page-search-highlight">$1</span>');
            $(this).replaceWith(highlightedText);
        });
    }

    function executeSearch() {
        var query = $input.val().trim().toLowerCase();
        var groups = getUpdateGroups();
        var matchCount = 0;

        clearHighlights();

        if (!query) {
            $('.mw-parser-output > *').show();
            $status.text('');
            return;
        }

        $('.mw-parser-output > *').not('#inpage-search, .log-header, .noprint').hide();
        // Keeps the years (H2) visible only as divisors (without counting as a result)
        $('.mw-parser-output > h2').show(); 

        groups.forEach(function(group) {
            // Check if the term is in the version title (H3) or in the items below it
            if (group.fullText.indexOf(query) !== -1) {
                group.header.show();
                group.content.show();
                applyHighlight(group.header.add(group.content), query);
                matchCount++; // It only counts if the term is in H3 or in the content of the version
            }
        });

        $status.text(matchCount > 0 ? 'Found ' + matchCount + ' corresponding versions.' : 'No results found.');
    }

    $input.on('input', function() {
        clearTimeout(debounceTimer);
        debounceTimer = setTimeout(executeSearch, 250);
    });

    if (!$('#page-search-highlight-style').length) {
        $('<style id="page-search-highlight-style">')
            .text('.page-search-highlight { background: #fff176 !important; color: #000 !important; border-radius: 2px; padding: 0 1px; }')
            .appendTo('head');
    }
});

 // ============================== Calculator ==================================================
mw.hook('wikipage.content').add(function ($content) {
    if ($('#hospital-calc-loaded').length) return;
    var $placeholder = $('#hospital-calc-placeholder');
    var $dataJson = $('#calc-data-json');
    if (!$placeholder.length || !$dataJson.length) return;

    var data = JSON.parse($dataJson.text());

    // --- UI Construction ---
    var $container = $('<div id="hospital-calc-container" style="padding: 20px; border: 2px solid var(--theme-border-color); border-radius: 10px; background: var(--theme-page-background-color--secondary);"></div>');
    $container.append('<h2 style="margin-top:0;">Hospital Resource Calculator</h2>');

    var $inputGrid = $('<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 15px;"></div>');

    // Generate Inputs for Main and Bonus items
    [data.main_items, data.bonus_items].forEach(function(group) {
        group.forEach(function(item) {
            var maxAttr = item.max ? ' max="' + item.max + '"' : '';
            var $field = $('<div style="margin-bottom:10px;"></div>')
                .append('<label style="display:block; font-size:12px; font-weight:bold;">' + item.label + '</label>')
                .append('<input type="number" id="inp-' + item.id + '" class="calc-trigger" value="0" min="0"' + maxAttr + ' style="width:100%; padding:5px; box-sizing:border-box;">');
            $inputGrid.append($field);
        });
    });

    $container.append($inputGrid);

    // --- Results Dashboard ---
    var $dashboard = $('<div style="margin-top: 20px; padding: 15px; background: rgba(0,0,0,0.1); border-radius: 5px;">' +
        '<div style="font-size: 1.2em; margin-bottom: 10px;"><strong>Potential Delivery: </strong><span id="res-potential">0</span> patients</div>' +
        '<div style="font-size: 1.2em; margin-bottom: 10px;"><strong>Actual Delivery: </strong><span id="res-actual" style="color:#44ff44;">0</span> patients <small>(Limited by Beds)</small></div>' +
        '<hr style="opacity:0.2;">' +
        '<div><strong>Emergency Support: </strong><span id="res-emergency">None</span></div>' +
    '</div>');

    $container.append($dashboard);
    $placeholder.replaceWith($container);
    $container.append('<div id="hospital-calc-loaded" style="display:none;"></div>');

    // --- Calculation Logic ---
    function updateCalculations() {
        var vals = {};
        // Get all values
        data.main_items.concat(data.bonus_items).forEach(function(item) {
            vals[item.id] = parseFloat($('#inp-' + item.id).val()) || 0;
        });

        // Formula
        var mainSum = (vals.bed * 2) + (vals.desk * 0.95) + (vals.heal * 3.5);
        var bonusSum = (vals.soda * 0.5) + (vals.bench * 0.75) + (vals.trash * 0.5);
        
        var potential = Math.round((mainSum + bonusSum) / data.config.divider);
        var actual = Math.min(potential, vals.bed);

        // Update UI
        $('#res-potential').text(potential);
        $('#res-actual').text(actual);

        // Emergency Logic
        var $emergency = $('#res-emergency');
        if (actual >= 40) {
            $emergency.text('Big (100% chance)').css('color', '#ff4444');
        } else if (actual >= 10) {
            $emergency.text('Medium (75% chance)').css('color', '#ffaa00');
        } else if (actual >= 1) {
            $emergency.text('Small (50% chance)').css('color', '#44aaff');
        } else {
            $emergency.text('None').css('color', 'inherit');
        }
    }

    $('.calc-trigger').on('input change', updateCalculations);
});