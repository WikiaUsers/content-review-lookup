...
function spellcheck()
{
  ...
  scJS.src = '//toolserver.org/~apper/sc/es/checkArticle.php?pageName=' + escape(mw.config.get( 'wgPageName' ));
  document.getElementsByTagName('head')[0].appendChild(scJS);
}
 
function markWordStart(text, hint)
{
  if (typeof bodyContent == 'undefined' && !(bodyContent = mw.util.$content[0]))
...
function RP_load()
{
  ...
  if ( typeof DontAutorunRP == 'undefined' || DontAutorunRP == false )
  {
    ...
    if (mw.config.get( 'wgAction' ) == 'view' && (($.inArray( mw.config.get( 'wgNamespaceNumber' ), [0, 4, 8, 10, 12, 14, 100, 102, 104]) > -1 && !mw.config.get( 'wgPageName' ).match( "^(Anexo:Falsos_amigos|Wikipedia:(Café|Consultas|Corrector_ortográfico/Listado|Informes_de_error|Tablón_de_anuncios_de_los_bibliotecarios|Taller_idiomático|Vandalismo_en_curso))" ) ) || (typeof RPonAllPages != 'undefined' && RPonAllPages == true)))
    {
      spellcheck();
    }
  }
