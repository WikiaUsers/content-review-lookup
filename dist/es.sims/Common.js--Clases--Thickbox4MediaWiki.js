/* <pre>
 * Thickbox4MediaWiki. Based on Thickbox 3.1 By Cody Lindley (http://www.codylindley.com)
...
var Thickbox = {
	...
	init: function() {
		...
		$('#bodyContent').bind('click.thickbox', Thickbox.triggerEvent);
	},
	triggerEvent: function(e) {
		...
		if (Thickbox.isTag(target,'img')) { // Gallery o thumb
			...
			if (!Thickbox.isTag(a,'a') || !Thickbox.isClass(a,'image')) return true;
			...
			// Wikia gallery
			if (Thickbox.isTag(gb,'span') && Thickbox.isClass(gb,'wikia-gallery-item')) {
				var t = gb.parentNode;
				if (Thickbox.isTag(t,'div')) {
					var root = $(t);
					if (Thickbox.isClass(t,'wikia-gallery-row')) {
						t = t.parentNode;
						root = $(t).children('div.wikia-gallery-row');
					}
					if (Thickbox.isClass(t,'wikia-gallery')) {
						a.blur();
						Thickbox.getCaption = Thickbox.getCaptionWikia;
						Thickbox.galleryData = root.children('span.wikia-gallery-item').children('div.thumb').find('a.image');
	...
	getCaptionEmpty: function(elem) {
		return $('<div></div>').text(elem.attr('title')).html().replace(/"/g,"&quot;");
	},
	...
	navigate: function() {
		...
		var url = Thickbox.getUrlFromThumb(gitem.get(0).firstChild.src);
		Thickbox.updateNavigation();
