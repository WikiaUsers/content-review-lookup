$('td.nojavascript').attr('class', '') //Suppression de la class nojavascript
$('#acc-tableau-1').hover(function() {changeText('romanfics')}, function() {changeText()});
$('#acc-tableau-3').hover(function() {changeText('mangafics')}, function() {changeText()});
$('#acc-tableau-2').hover(function() {changeText('fictions')}, function() {changeText()});
function changeText(wiki) {
    switch(wiki){
        case 'romanfics':
            $('#acc-tableau-notice').html('<div class="javscript-notice-acc-tableau">ROMANFICS est le wiki consacré aux fanfictions sur le récit. Découvrez de nombreux écrits sur vos séries littéraires préférées : Harry Potter, Hunger Games, Twilight, et bien d\'autres !</div>');
            break;
        case 'fictions':
            $('#acc-tableau-notice').html('<div class="javscript-notice-acc-tableau">(Fan)Fictions est le premier wiki à vous proposer de lire des aventures originales de héros tout droits sortis de l\'imagination de nos auteurs !</div>');
            break;
        case 'mangafics':
            $('#acc-tableau-notice').html('<div class="javscript-notice-acc-tableau">MANGAFICS est le wiki spécialisé dans les fanfictions de mangas. Retrouvez vos héros favoris et découvrez de nouvelles inédites, inventées par des fans, pour des fans !</div>');
            break;
        default:
            $('#acc-tableau-notice').html('');
    }
}

/*#################################
  #### Création des fanfiction ####
  #################################*/
/*Editor WYSIAWYG*/
    ...
    if(targeT == 10){
        ...
        <form name="newchapterform" class="form" id="formwikifanfic">\
	        <div style="height:300px;overflow:hidden">\
    		    <div id="divtodisplaynone" style="height:300px">\
			        <fieldset>\
		    		    <legend><big>Informations au sujet de la fanfiction</big></legend>\
	    			    <table>\
    					    <tr>\
					    	    <td><label for="titre">Titre : </label></td>\
