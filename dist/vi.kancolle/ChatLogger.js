...
//This script is a modified version of http://runescape.wikia.com/wiki/User:Joeytje50/ChatLogger.js
...
function getLog(exists,content) {
	...
	var msgtime = allmsgs[0].parentNode.getElementsByClassName('time')[0].innerHTML;
	for (i=1;allmsgs[i];i++) {
		message += '<|>'+allmsgs[i].innerHTML;
		msguser += '<|>'+allmsgs[i].parentNode.getAttribute('data-user');
		msgtime += '<|>'+allmsgs[i].parentNode.getElementsByClassName('time')[0].innerHTML;
	}
