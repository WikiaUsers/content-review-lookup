/*An edited version of EmoticonsWindow by KockaAdmiralac */
...
$(function() {
    ...
    var EmoticonsWindow = {
        ...
        config: $.extend({
            ...
            chatOptionsIntegration: (mw.config.get('wgCityId') === '885297')
        ...
        preload: function() {
            $.get(mw.util.wikiScript('load'), {
                mode: 'articles',
                articles: 'MediaWiki:Custom-plugin-i18n/EmoticonsWindow.json',
                only: 'styles'
            }, $.proxy(function(d) {
         ...
         */
        insertUI: function() {
            mw.util.addCSS('.chat-button{margin: 10px auto;text-align:center;}.EmoticonsWindowIcon{max-width:60px;max-height:60px;cursor:pointer;padding:10px;}#EmoticonsWindowList{height:400px;overflow-y:auto;}.EmoticonsWindowButton{width:90px;text-align:center;}' + (this.config.chatOptionsIntegration ? '' : '.EmoticonsWindowButton{position:absolute;top:10px}'));
            this.insertElements();
            this.insertButton();
        },
        /**
