...
/*****
  ...
  # Original Authors: Soap Shadow and Speedit. Changes made by Wither
  # Based on a script from the My Little Pony Wiki
  —————————————————————————————————————————————————————————————————————————
**/

importStylesheet("MediaWiki:Custom Discord Widget.js/Style.css");
$.getJSON("https://discordapp.com/api/guilds/403110929031757836/widget.json", function(json) {
    //Get IDs of the Staff and Bots and mark them as such in the widget
    /*we'll mark Awesomeclash as Moderator rather than Head of Defence and 
    Ancient One as Admin rather than his custom The Ancient One role. Same goes 
    for other users with custom roles. */
    function getOwner() {return ["403106266458619905"];}
    function getCo_Owners() {return ["405632100994121728", "390183834685276162"];}
    function getAdmins() {return ["404209913481396225", "490444452502962177",
    "419809623516643329"];}
    function getMods() {return ["407946926412005379", "405712757867020308",
    "405342325846441984"];}
    function getBots() {return [//We have a lot of bots, so this'll be a long one
        "356065937318871041", //Aki
        "118578193278959616", //Anth
        "177735619668213760", //ArrayBot
        "185476724627210241", //Ayana
        "413728456942288896", //BoxBot
        "247852652019318795", //Dad Bot
        "270904126974590976", //Dank Memer
        "170915625722576896", //DiscordRPG
        "155149108183695360", //Dyno
        "184405311681986560", //FredBoat♪♪
        "424606447867789312", //IdleRPG
        "195244363339530240", //KawaiiBot
        "159985870458322944", //MEE6
        "160105994217586689", //Miki
        "116275390695079945", //Nadeko
        "439205512425504771", //NotSoBot
        "305177429612298242", //RPGBot
    ...
    $($widgetHeaderMembersOnline)
        ...
        "</strong> Writers and Readers Online")
        .appendTo($widgetHeader);
    ...
    $($adminsOnlineContainer).addClass("widget-role-container")
    .appendTo($widgetBodyUsers);
 
    // Create container for the server owners
    
    var $OwnerContainer = document.createElement("div");
    $($OwnerContainer).addClass("widget-role-container").appendTo
    ($widgetBodyUsers);
 
    var $OwnerTitle = document.createElement("div");
    $($OwnerTitle)
        .addClass("widget-role-name-owner")
        .text("Owner")
        .appendTo($adminsOnlineContainer);
    
    var $CoOwnersOnlineContainer = document.createElement("div");
    $($CoOwnersOnlineContainer).addClass("widget-role-container").
    appendTo($widgetBodyUsers);
 
    var $CoOwnersOnlineTitle = document.createElement("div");
    $($CoOwnersOnlineTitle)
        .addClass("widget-role-name-co-owners")
        .text("Co-Owners")
        .appendTo($adminsOnlineContainer);
    ...
    $($moderatorsOnlineContainer).addClass("widget-role-container")
    .appendTo($widgetBodyUsers);
    ...
    var $botsContainer = document.createElement("div");
    $($botsContainer)
        .addClass("widget-role-container")
        .appendTo($widgetBodyUsers);
    var $botsTitle = document.createElement("div");
    $($botsTitle)
        .addClass("widget-role-name-bots")
        .text("Bots")
        .appendTo($botsContainer);
    ...
    for (var i = 0; i < membersOnline; i++) {
        ...
        if (owner.includes(member.id)) {$(createWidgetMember(member))
        .appendTo($OwnerContainer);} 
        else if (co_owners.includes(member.id)) {$(createWidgetMember(member))
        .appendTo($CoOwnersOnlineContainer);} 
        else if (mods.includes(member.id)) {$(createWidgetMember(member))
        .appendTo($moderatorsOnlineContainer);} 
    ...
    $($widgetFooterText)
        ...
        .append("You are expected and required to follow the " + 
        "<a href=\"https://fanficages.wikia.com/wiki/Rules_and_Guidelines\">" +
        " wiki policies</a>.")
    ...
    function createWidgetMember(member) {
        ...
        railModuleTitleIconPath.style.fill = "crimson";
        ...
        $($railModuleTitle).append("Fanficages Wiki Discord Server");
    ...
    $('.CustomDiscordIntegrator').each(function () {
        ...
        $($containerTitle).append("Fanficages Wiki Discord Server");
