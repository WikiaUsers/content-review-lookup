...
CreaEnlacesDex = function(){
	...
	this.generaciones = ['Primera', 'Segunda', 'Tercera', 'Cuarta'];
...
CreaEnlacesDex.prototype = {
	...
	genPoke: function(){
		...
		this.link(T_UPS+'dp/'+zPadLeft(sn,3)+'.php',T_PS+' DP',T_PS+': Diamante y Perla');
		//UPC
		n && this.link('http://www.upokecenter.com/dex/?number='+sn,'Ultimate PokéCenter','The Ultimate Pokémon Center');
		// Pokexperto
		n && this.link('http://www.pokexperto.net/index2.php?seccion=nds/nationaldex/pkmn&pk='+sn,'Pokexperto 3-4Gen','Pokexperto: 3ª y4ª'+T_G);
		// Otras/otros idiomas
		...
		this.link(T_ULP+'dp/pokedex/'+m,T_L+' 4Gen [en]',T_LP+'4ª'+T_G+T_EN);
	...
	genMov: function(){
		...
		i && this.link(T_UBP+i.replace(/\b(\w)/g, function(s,p){return p.toUpperCase();})+'_(move)','Bulbapedia [en]','Bulbapedia'+T_EN);
		i && g < 4 && this.link(T_ULP+'rs/attacks/'+i,T_L+' 3Gen [en]',T_LP+'3ª'+T_G+T_EN);
		i && this.link(T_ULP+'dp/attacks/'+i,T_L+' 4Gen [en]',T_LP+'4ª'+T_G+T_EN);
		i && this.link('http://veekun.com/dex/moves/'+i.toLowerCase(),'Veekun [en]','Veekun'+T_EN);
		i && g < 4 && this.link(T_USA+'/'+i.toLowerCase().replace(/\s/g,'')+'.shtml',T_S+' 3Gen [en]',T_S+': 3ª'+T_G+T_EN);
		i && this.link(T_USA+'-dp/'+i.toLowerCase().replace(/\s/g,'')+'.shtml',T_S+' 4Gen [en]',T_S+': 4ª'+T_G+T_EN);
	},
