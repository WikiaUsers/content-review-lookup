...
var i18n = {
	blockedNotice: wgULS('您不能编辑此精灵图，因为您已被封禁。', '您不能編輯此 Sprite，因為您已被封鎖。'),
	blockedReason: '原因：$1',
	browserActionNotSupported: wgULS('您的浏览器不支持。', '您的瀏覽器不受支援。'),
	changesSavedNotice: wgULS('已保存您的更改。', '已儲存您的變更。'),
	controlNewName: wgULS('新建名字', '新建名稱'),
	ctxDeleteImage: wgULS('删除', '刪除'),
	ctxDownloadImage: wgULS('下载', '下載'),
	ctxReplaceImage: wgULS('替换', '取代'),
	diffError: wgULS('无法获取差异', '無法取得差異'),
	diffErrorMissingPage: wgULS('无法获取页面', '無法取得頁面'),
	dupeName: wgULS('此名字已存在。', '此名稱已存在。'),
	dupeNamesNotice: wgULS('存在重复的名字，在保存之前请解决此问题。', '存在重複的名稱，請在儲存之前解決此問題。'),
	errorApi: wgULS('API错误', 'API 錯誤'),
	errorConnection: wgULS('连接错误', '連線錯誤'),
	errorConnectionText: wgULS('请检查您的网络连接', '請檢查您的網路連線'),
	errorGeneric: wgULS('错误', '錯誤'),
	errorHttp: wgULS('HTTP错误', 'HTTP 錯誤'),
	genericError: wgULS('出现了一些错误', '出現了一些錯誤'),
	...
	namePlaceholder: wgULS('输入一个名字', '輸入一個名稱'),
	noPermissionNotice: wgULS('您没有权限编辑此精灵图。', '您沒有權限編輯此 Sprite。'),
	panelChangesIdTitle: wgULS('数据更改', '資料變更'),
	panelChangesNoDiffFromCur: wgULS('与当前版本无差异。', '與目前的版本無差異。'),
	panelChangesSheetTitle: wgULS('精灵图表更改', 'Sprite 表變更'),
	panelChangesTitle: wgULS('检查您的更改', '檢查您的變更'),
	panelCloseTip: wgULS('关闭', '關閉'),
	panelConflictCurText: wgULS('当前文本', '目前的文字'),
	panelConflictReview: wgULS('检查更改', '檢查變更'),
	panelConflictSave: wgULS('保存', '儲存'),
	panelConflictText: wgULS('发生了编辑冲突，并且无法自动解决。', '發生編輯衝突，且無法自動解決。'),
	panelConflictTitle: wgULS('编辑冲突', '編輯衝突'),
	panelConflictYourText: wgULS('您的文本', '您的文字'),
	panelDiscardContinue: wgULS('继续编辑', '繼續編輯'),
	panelDiscardDiscard: wgULS('放弃更改', '取消變更'),
	panelDiscardText: wgULS('您确定要放弃您的更改吗？', '您確定要取消您的變更嗎？'),
	panelDiscardTitle: wgULS('未保存的更改', '未儲存的變更'),
	panelEcchangesReturn: wgULS('返回编辑冲突表单', '返回編輯衝突表單'),
	panelEcchangesTitle: wgULS('检查您的手动更改', '檢查您的手動變更'),
	sectionPlaceholder: wgULS('输入一个分类名', '輸入一個分類名'),
	sectionUncategorized: wgULS('未分类', '未分類'),
	titleEditing: wgULS('正在编辑$1', '正在編輯 $1'),
	toolbarHelp: wgULS('帮助', '說明'),
	toolbarHelpPage: 'en:Help:Sprite editor',
	toolbarNewImage: wgULS('新建图像', '新增圖片'),
	toolbarNewSection: wgULS('新建分类','新增分類'),
	toolbarRedo: '重做',
	toolbarReviewChanges: wgULS('检查更改', '檢查變更'),
	toolbarSave: wgULS('保存', '儲存'),
	toolbarSummaryLabelTip: wgULS('剩余字节数', '剩餘位元組數'),
	toolbarSummaryPlaceholder: wgULS('您所作更改的摘要', '您所做的變更的摘要'),
	toolbarToolDeprecate: wgULS('弃用', '棄用'),
	toolbarToolDeprecateTip: wgULS('将名字切换为已弃用', '將名稱切換為已棄用'),
	toolbarTools: '工具',
	toolbarUndo: wgULS('撤销', '還原')
};
	...
	if ( imageEditingSupported ) {
		sheetRequest = contentRequest.then( function() {
			...
			if ( !settings.sheet ) {
				...
				settings.sheet = urlParts[2] + '?format=original';
