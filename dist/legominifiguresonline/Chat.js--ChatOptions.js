 ...
 */
function getCookieValueAtPosition( cookie_name, pos ) {
...
function isEnabled(module) {
	...
	switch (module) {
		case "chatHacks":
			c = getCookieValueAtPosition("customisation", 2);
			break;
		case "tabComplete":
			c = getCookieValueAtPosition("customisation", 4);
			break;
		case "multiKick":
			c = getCookieValueAtPosition("customisation", 5);
			break;
		case "multiPM":
			c = getCookieValueAtPosition("customisation", 6);
			break;
		case "searchBar":
			c = getCookieValueAtPosition("customisation", 7);
			break;
		case "stopSideScroll":
			c = getCookieValueAtPosition("customisation", 9);
			break;
		case "ignoreURL":
			c = getCookieValueAtPosition("customisation", 8);
			break;
    	case "notifications":
			c = getCookieValueAtPosition("customisation", 12);
			break;
		case "ping":
			c = getCookieValueAtPosition("customisation", 13);
			break;
		case "chatlogger":
			c = getCookieValueAtPosition("customisation", 14);
			break;
		default:
			return false;
	}
...
var chatOptions = {
	look: {
		fontColor: getCookieValueAtPosition("customisation", 1),
		fontFamily: getCookieValueAtPosition("customisation", 3),
		surroundColor: getCookieValueAtPosition("customisation", 10),
		selfPostColor: getCookieValueAtPosition("customisation", 11),
		backgroundColor: getCookieValueAtPosition("customisation", 0),
	...
	modules: {
		...
		stopSideScroll: {
			...
			loaded: false,
			load: function () {
			importScriptPage("MediaWiki:Chat.js/notifications.js","legominifiguresonline");
					this.loaded = true;
			}
		},
		ping: {
			...
			loaded: false,
			load: function () {
			importScriptPage("MediaWiki:Chat.js/ping.js","legominifiguresonline");
					this.loaded = true;
			}
		},
		chatlogger: {
			element: "#chatlogger",
			enabled: isEnabled("chatlogger"),
			loaded: false,
			load: function () {
			importScriptPage("MediaWiki:Chat.js/chatlogger.js","legominifiguresonline");
					this.loaded = true;
			}
		}
	}
...
function openOptions() {
	...
	var $optionsWindowHTML = $.showCustomModal( "Options", '<form method="" name="" class="WikiaForm "> <fieldset> <h2>Colour changes</h2> <p style="font-size:80%;">Enter a <a href="http://www.w3schools.com/html/html_colornames.asp" target="_blank">colour name</a> or <a href="http://html-color-codes.info/" target="_blank">colour hex</a> </p> <table> <tr> <td> Chat background: </td> <td> <input type="text" name="backgroundColourinput" id="backgroundColourinput" value="' + chatOptions.look.backgroundColor + '" /> </td> </tr><tr> <td> Self-post background: </td> <td> <input type="text" name="selfPostColourinput" id="selfPostColourinput" value="' + chatOptions.look.selfPostColor + '" /> </td> </tr> <tr> <td> Surround: </td> <td> <input type="text" name="surroundColourinput" id="surroundColourinput" value="' + chatOptions.look.surroundColor + '" /> </td> </tr> <tr> <td> Font colour: </td> <td> <input type="text" name="fontColourinput" id="fontColourinput" value="' + chatOptions.look.fontColor + '" /> </td> </tr> </table> <h2>Font</h2> <p>Font family <select id="fontList"> <option value="arial" style="font-family:arial;">Arial</option> <option value="courier new" style="font-family:courier new;">Courier new</option> <option value="georgia" style="font-family:georgia;">Georgia</option> <option value="palatino linotype" style="font-family:palatino linotype;">Palatino linotype</option> <option value="Comic Sans MS" style="font-family:Comic Sans MS;">Comic sans</option> <option value="tahoma" style="font-family:tahoma;">Tahoma</option> <option value="Trebuchet MS" style="font-family:Trebuchet MS;">Trebuchet MS</option> <option value="Verdana" style="font-family:Verdana;">Verdana</option> <option value="Lucida Console" style="font-family:Lucida Console;">Lucida Console</option> </select> </p> <h2>Added functionality</h2> <input type="checkbox" name="chatHacks" value="chatHacks" id="chatHacks" />Enable <a href="http://c.wikia.com/wiki/User:Monchoman45/ChatHacks.js" target="_blank">chathacks</a>&nbsp;&nbsp;&nbsp;&nbsp; <input type="checkbox" name="multiPM" value="multiPM" id="multiPM" />Enable <a href="http://callofduty.wikia.com/wiki/User:Madnessfan34537/multipm.js" target="_blank">multi PM</a><br/> <input type="checkbox" name="tabComplete" value="tabComplete" id="tabComplete" />Enable <a href="http://runescape.wikia.com/wiki/User:Joeytje50/tabinsert.js" target="_blank">tab complete</a>&nbsp;&nbsp;&nbsp; <input type="checkbox" name="searchBar" value="searchBar" id="searchBar" />Enable <a href="http://callofduty.wikia.com/wiki/MediaWiki:Chat.js/searchbar.js" target="_blank">search bar</a><br/> <input type="checkbox" name="multiKick" value="multiKick" id="multiKick" />Enable <a href="http://callofduty.wikia.com/wiki/User:Madnessfan34537/multikick.js" target="_blank">multi kick</a>&nbsp;&nbsp;&nbsp;&nbsp; <input type="checkbox" name="ignoreURL" value="ignoreURL" id="ignoreURL" />Ignore URL in main chat <br /> <input type="checkbox" name="stopSideScroll" value="stopSideScroll" id="stopSideScroll" />Stop the sidescroll bar to appear after someone spams<br/> <input type="checkbox" name="notifications" value="notifications" id="notifications" />Enable notifications<br/> <input type="checkbox" name="ping" value="ping" id="ping" />Enable ping on every message<br/> <input type="checkbox" name="chatlogger" value="chatlogger" id="chatlogger" />Chatlogger</fieldset> </form>', {
		id: "optionsWindow",
	    width: 600,
	    buttons: [
		{
	...
	if (chatOptions.modules.notifications.enabled)
		$("#notifications").attr("checked",true);
	if (chatOptions.modules.ping.enabled)
		$("#ping").attr("checked",true);
	if (chatOptions.modules.chatlogger.enabled)
		$("#chatlogger").attr("checked",true);
...
if (!$("#chatOptionsButton").length) {
	$('#ChatHeader h1').append('<span id="chatOptionsButton" onclick="openOptions();">Options</span>'); // Prevent multiple buttons from being appended
}
