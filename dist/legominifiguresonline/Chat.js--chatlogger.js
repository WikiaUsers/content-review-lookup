/* Modified Chat Logger Script
* Original by Joeytje50
* Modified by Spydar007
* Slight changes by Sloshedtrain */
...
$('form#Write').append('<a class="wikia-button" href="javascript:submitLog()" style="position:absolute; right:50px; top:0;" title="Update Chat Logs">Update</a>');
...
function dcRestart() {
	if ($('.inline-alert:contains("You have been disconnected")').length > 0) {
		submitLog('restart')
	}
	callAPI({
		'action': 'query',
...
var logInterval = logInterval?logInterval:900000;
...
function getLog(exists,content) {
	...
	var ChatLog='';
	for (i=0;message[i];i++) {
	    console.log(content.indexOf('['+msgtime[i]+'] <'+msguser[i]+'> '+message[i]) == -1);
	    if (content.indexOf('['+msgtime[i]+'] <'+msguser[i]+'> '+message[i]) == -1)
...
function submitLog(restart) {
	...
	callAPI({
		...
		'prop': 'info|revisions',
		'intoken': 'edit',
		'titles': 'LEGO_Minifigures_Online_Wiki:Chat/Logs/'+date,
		'rvprop': 'content',
		'rvlimit': '1',
		'indexpageids': 'true',
	}, 'GET', function(response) {
		...
		callAPI({
			...
			'action': 'edit',
			'title': 'LEGO_Minifigures_Online_Wiki:Chat/Logs/'+date,
