...
// Викификатор, ибо иначе не работает 
...
(function($, wikificator) {
    ...
    var p = {},
        ...
        lang = $.getUrlVar('uselang') || wgContentLanguage;// use rules by content;
    p.debug = $.getUrlVar('debug');
    // these ((())) are made for reasons
    if (((wikificator && wikificator.loaded === true) || (!wikificator.forced && $.inArray(mw.config.get( 'wgAction' ), ['edit', 'submit']) === -1))) return;
