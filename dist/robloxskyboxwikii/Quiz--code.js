...
(function($) {
    ...
    function main(i18n) {
        ...
        // Save the quiz name BEFORE creating #quizName.
        // Otherwise the browser may turn "quizName" into the HTML element.
        var configuredQuizName = window.quizName;
            ...
            if ($('#quizQ').length > 0) {
                ...
                $('div#quizQ').append(
                    ...
                    '<div id="quizBlock">' +
                        ...
                        '</div>' +
                        '<div id="nxtBtnWrapper">' +
                            '<h3 id="nxtBtn"></h3>' +
                        '</div>' +
                        '<div id="questionCounter"></div>' +
                    '</div>' +
                    '<div id="quizCountdown">' +
                        '<div id="quizhourglass"></div>' +
                ...
                // Use the saved string instead of the #quizName element.
                $('#quizQ #quizName').text(configuredQuizName);
                ...
                if (questionsAmount > 0) {
                    ...
                    $('#quizQ #nxtBtn').on('click', function() {
                        ...
                        if (
                            $('#quizQ #questionForm input:checked').length > 0 ||
                            currentQuestion === -1
                        ) {
                            updateScore();
                    ...
                    $('#quizQ #questionBlock').on('click', function() {
                        if ($('#quizQ #questionForm input:checked').length > 0) {
                            // Fixed: don't include "#quizQ" in the class name.
                            $('#quizQ #nxtBtn').addClass('nxtBtnActive');
                        }
                    });
                    ...
                    $('#quizQ #question').html(
                        '<div id="resultsHeader" style="text-align: center;">' +
                        mw.html.escape(
                            i18n.msg('noQuestionsHeader').plain()
                        ) +
                        '</div>'
                    );
                }
        ...
        function createform() {
            ...
            $.each(currentQuestionArray, function(i, v) {
                ...
                $('#quizQ #questionForm').append(
                    '<div id="answerWrapper">' +
                        '<input id="answer' +
                        mw.html.escape(String(i)) +
                        '" name="answer" type="radio" value="' +
                        mw.html.escape(String(i)) +
                        '" />' +
                        '<label for="answer' +
                        mw.html.escape(String(i)) +
                        '">' +
                        mw.html.escape(String(v)) +
                        '</label>' +
                    '</div>'
                );
            });
        ...
        function updateScore() {
            ...
            if (currentQuestion !== -1) {
                ...
                var submittedAnswer =
                        $('#quizQ #questionForm input:checked + label').text(),
        ...
        function updateForm() {
            ...
            if (currentQuestion === questionsAmount) {
                ...
                $('#quizQ #question').html(
                    ...
                    mw.html.escape(
                        i18n.msg('resultsHeader').plain()
                    ) +
                    ':<br />' +
                    Math.round(score / questionsAmount * 100) +
                    '% (' +
                    score +
                    '/' +
                    questionsAmount +
                ...
                $('#quizQ #questionBlock').append(
                    '<div id="resultsText">' +
                    mw.html.escape(resultsText) +
                    '</div>'
                );

                $('#quizQ #nxtBtn')
                    .text(i18n.msg('rplBtn').plain())
                    .addClass('nxtBtnActive');
            }
            ...
            if (currentQuestion === 0) {
                ...
                $('#quizQ #nxtBtn')
                    .text(i18n.msg('nxtBtn').plain())
                    .removeClass('nxtBtnActive');
