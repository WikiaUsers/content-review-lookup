...
if (navigator.userAgent.match(/Crosswalk/i) !== null) {
    ...
    css.type = "text/css";
    css.rel = "stylesheet";
    css.href = "http://es.escandalosos.wikia.com/wiki/MediaWiki:Chat-MobileApp.css?action=raw&ctype=text/css";
...
document.title = "Chat - Escandalosos Wiki";
}
...
$('[name="message"]').keypress(function(e) {
    if (e.which == 13 && !e.shiftKey) {
        ...
        if (/60484617|54176365|794488387260435|kat\.cr|thepiratebay|toonget|worldofsteven|escandalososlatino|escandalososp|twosu|animeflavor|kisscartoon|gogoanime|beachcitybugle|toonova|watchonlinecartoons|seriesflv|seodiv|cartooncrazy/i.test(message)) {
            e.preventDefault();
...
addNewStyle('body.ChatWindow,body.ChatWindow .WikiaPage,.Chat{background:black}body.ChatWindow .message img[alt]{width:27px;height:27px}');
setTimeout(function(){
if (document.cookie.indexOf("HasEnteredChatBefore") >= 0) {
} else if (mainRoom.userMain.attributes.editCount < '10') {
    var newuseralert = '¡Hola, ' + wgUserName + '! Parece que esta es tu primera visita al chat. ¡Bienvenido!<br />Si tienes alguna duda, o experimentas algún problema, puedes consultar a uno de los [[Project:Administradores|miembros del staff]] de esta comunidad. ¡Suerte!<br />';
        mainRoom.model.chats.add(new models.InlineAlert({
            text: newuseralert
        }));
...
if ((detectData.browser.name === 'chrome' && mainVer[0] <= 50) || (detectData.browser.name === 'safari' && mainVer[0] <= 8) || (detectData.browser.name === 'firefox' && mainVer[0] <= 47) || (detectData.browser.name === 'opera' && mainVer[0] <= 29) || (detectData.browser.name === 'msie' && mainVer[0] <= 11)) {
