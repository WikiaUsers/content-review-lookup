		...
		init: function() {
			...
			document.addEventListener('dblclick', function(event) {
				...
				if (event.target && (event.target.id == 'mapImage' || event.target.id == 'templateImage')) {
					...
					var img = new Image();
					img.onload = function () {
						img.setAttribute('width', (loadedImages['File:Map Marker 55.png'].size*localZoom)+'px');
						img.setAttribute('style', 
							'left:'+(event.layerX-(loadedImages['File:Map Marker 55.png'].size*localZoom/2))+'px; '+
							'top:'+(event.layerY-(loadedImages['File:Map Marker 55.png'].size*localZoom/2))+'px; '+
					...
					img.setAttribute('src', loadedImages['File:Map Marker 55.png'].src);
					var newMarker = {
						src: 'File:Map Marker 55.png',
				...
				if (event.target && /^marker\d+/.test(event.target.id)) {
					...
					var menu = $(
						'<div class="markerSettings" style="z-index:99999; position:absolute;">'+
							'<ul>'+
								'<li rel="'+event.target.id+'" class="markerSettings-35">35px</li>'+
								'<li rel="'+event.target.id+'" class="markerSettings-55">55px</li>'+
								'<li rel="'+event.target.id+'" class="markerSettings-75">75px</li>'+
								'<li rel="'+event.target.id+'" class="markerSettings-95">95px</li>'+
								'<li rel="'+event.target.id+'" class="markerSettings-130">130px</li>'+
			...
			document.addEventListener('click', function(event) {
				...
				} else if (event.target && event.target.closest('#regionSelect') && sett.r !== event.target.id) {
					closeMarkerSettings();
					sett = {r: event.target.id};
		...
		urlQuery: function(r, m) {
			...
			if (!r || !m) {
				(new URL(window.location.href)).searchParams.forEach(function(v, k){
					if (['r', 'm'].includes(k)) {query[k] = v;}
				});
			}
			if (query.m && !query.r) {
				var map = 'File:'+decodeURIComponent(query.m.replace(/_/g, ' '))+' Map Template.png';
				for (var reg in mapRefs) {
					if (reg !== 'QuickGen' && mapRefs[reg].indexOf(map) !== -1) {
						query.r = reg;
						break;
					}
				}
	...
	if (['Genshin_Impact_Wiki:Map_Generator', 'Special:Map'].includes(config.wgPageName) && config.wgAction == 'view') {
		...
		api.get({action: 'query', prop: 'revisions', titles: 'MediaWiki:Custom-MapGenerator.json', rvprop: 'content', rvslots: '*'}).then(function(data){
			...
			mapGenerator.loadImages([
				'File:Map Marker 35.png',
				'File:Map Marker 55.png',
				'File:Map Marker 75.png',
				'File:Map Marker 95.png',
				'File:Map Marker 130.png'
			]);
			mapGenerator.init();
		});
	}
