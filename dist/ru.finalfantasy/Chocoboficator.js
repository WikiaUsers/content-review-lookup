// ВНИМАНИЕ! Внося изменения в код, не забывайте обновлять справку на странице [[Чокобопедия:Чокобофикатор]] 

/* <pre> */ 

	var txt;
	
	var wmFullText = 'Chocoboficator will process ALL text on this page. Continue?';
	var wmCantWork = 'Chocoboficator cannot work in your browser';
	var wmWontWork = 'Chocoboficator will not work in Netscape 4.x and less';
...
{key = window.event.keyCode;if (key==10){Chocobofy();}}
//======================================
function Chocobofy()
{
...
if (wgNamespaceNumber % 2 || wgNamespaceNumber==4) {
 ...
 if (sigs && sigs.length > 1)
   if (!confirm('Пожалуйста, не обрабатывайте Чокобофикатором реплики других участников. Вы уверены, что хотите продолжить?')) 
     return;
}
//var noChocobo = ReplaceElements( '\<math\>(.|\r|\n)+?\<\/math\>, "\x03", "\x04" );
//That variant will make ReplaceTags() function unnecessary, but it's so ugly...

var noChocobo = ReplaceTags( 'nowiki', "\x03", "\x04" );
