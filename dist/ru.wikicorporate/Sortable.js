(() => {
    'use strict';
    if (window.sortableAdapterES6Loaded) return;
    window.sortableAdapterES6Loaded = true;

    const SCRIPT_PAGE = 'MediaWiki:Sortable/Main.js'; 
    const VERSION = '1.0'; 
    ...
    const checkDOM = () => {
        if (document.querySelector('.wiki-sortable')) {
            loadScript();
            observer.disconnect();
        }
    };
