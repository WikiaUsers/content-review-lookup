(() => {
    ...
    mw.loader
        .using(["mediawiki.api", "mediawiki.util", "mediawiki.Title"])
        .then(() => {
            ...
            const fetchAndParseModules = () => {
                return api
                    ...
                    .then((parseResponse) => {
                        return parseResponse && parseResponse.parse && parseResponse.parse.text
                            ? parseResponse.parse.text["*"]
                            : null;
                    })
                    .catch((e) => {
            ...
            const fetchPopularPages = () => {
                ...
                if (wikiId) {
                    promiseChain = promiseChain.then(() => {
                        return fetch(
                            `https://services.fandom.com/recommendations/recommendations?wikiId=${wikiId}&articleId=${articleId}`
                        )
                            .then((response) => (response.ok ? response.json() : Promise.reject()))
                            .then((data) => {
                                const localItems = (data.article_recommendation || []).filter(
                                    (item) =>
                                ...
                                shuffleArray(localItems).forEach((i) =>
                                    addArticle(i.article_title, i.url, i.thumbnail_url)
                                );
                            })
                            .catch(() => {});
                    });
                ...
                promiseChain = promiseChain.then(() => {
                    if (articles.length < 5) {
                        return $.getJSON(mw.util.wikiScript("wikia"), {
                        ...
                        return api
                            ...
                            .then((mwData) => {
                                if (mwData.query && mwData.query.pages) {
                                    mwData.query.pages.forEach((p) =>
                                        addArticle(p.title, p.fullurl, p.thumbnail ? p.thumbnail.source : undefined)
                                    );
                                }
                            })
                            .catch(() => {});
                ...
                return $.getJSON(apiUrl, {
                    ...
                    .then((postsData) => {
                        ...
                        const articleIds = uniquePosts
                            .filter(
                                (p) =>
                                    p._embedded &&
                                    p._embedded.thread &&
                                    p._embedded.thread[0] &&
                                    p._embedded.thread[0].containerType === "ARTICLE_COMMENT"
                            )
                            .map((p) => p.forumId)
                            .filter(Boolean);
                        ...
                        if (articleIds.length > 0) {
                            articlePromise = $.getJSON(apiUrl, {
                                controller: "FeedsAndPosts",
                                method: "getArticleNamesAndUsernames",
                                stablePageIds: articleIds.join(","),
                                format: "json",
                            })
                                .then((titleData) => {
                            ...
                            return uniquePosts.map((post) => {
                                ...
                                if (containerType === "FORUM") {
                                    threadUrl =
                                        ...
                                        "/f/p/" +
                                        post.threadId +
                                        (post.isReply ? "/r/" + post.id : "");
                                    if (!displayTitle) displayTitle = "Обсуждение на форуме";
                                } else if (containerType === "WALL" || containerType === "MESSAGE_WALL") {
                                    const cleanWallName = (post.forumName || "")
                                        .replace(/ Message Wall$/, "")
                                        .replace(/^Стена обсуждения:/, "");
                                    threadUrl =
                                        mw.util.getUrl("Message_Wall:" + cleanWallName) +
                                        "?threadId=" +
                                        post.threadId +
                                        (post.isReply ? "#" + post.id : "");
                                    if (!displayTitle || displayTitle.indexOf("@") === 0)
                                        displayTitle = `Сообщение на стене`;
                        ...
                        data() {
                            return {
                                ...
                                popularTitle: mw.message("rail-popular-pages-header").exists()
                                    ? mw.message("rail-popular-pages-header").text()
                                    : "Популярные страницы",
                                fallbackImage: (mw.config.get("wgArticlePath") || "/wiki/$1").replace(
                                    "$1",
                                    "Special:FilePath/Site-favicon.ico"
                                ),
                            };
                        },
                        computed: {
            ...
            const insertIntoDOM = () => {
                ...
                let rightRailWrapper =
                    wikiaRail.closest(".right-rail-wrapper") || wikiaRail.closest(".page__right-rail");

                // Подготавливаем фрагмент для вставки
                const fragment = document.createDocumentFragment();

                dataPromise
                    .then(([renderedHtml, popularData, socialData, filesData]) => {
