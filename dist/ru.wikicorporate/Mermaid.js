(() => {
    'use strict';
    if (window.mermaidAdapterES6Loaded) return;
    window.mermaidAdapterES6Loaded = true;

    const SCRIPT_PAGE = 'MediaWiki:Mermaid/Main.js'; 
    const VERSION = '1.0'; 
    ...
    const checkDOM = () => {
        if (document.querySelector('.wiki-mermaid')) {
            loadScript();
            observer.disconnect();
        }
    };
