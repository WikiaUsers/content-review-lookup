...
(() => {
    ...
    const hotkey = isMac ? 'Ctrl+Shift+W' : 'Ctrl+Alt+W';
    ...
    window.Wikify = (inputOrText) => {
        ...
        if (typeof inputOrText === 'string') {
            ...
            if (inputOrText instanceof HTMLElement) {
                inputElement = inputOrText;
            } else if (inputOrText && inputOrText.jquery && inputOrText.length) {
                inputElement = inputOrText[0];
            } else {
                inputElement = document.getElementById('wpTextbox1');
            }
        ...
        const hideTemplates = () => {
            ...
            while (true) {
                ...
                if (left !== -1 && (left < right || right === -1)) {
                    ...
                    right += 2;
                    tpl = txt.slice(left, right);
                    txt = `${txt.slice(0, left)}\x01${hidden.push(tpl)}\x02${txt.slice(right)}`;
        ...
        const processLink = (link, left, right) => {
            ...
            const inLink = right.slice(0, left.length);
            const afterLink = right.slice(left.length);
            const uniLeft = left.charAt(0).toUpperCase() + left.slice(1);
            const uniRight = (right.charAt(0).toUpperCase() + right.slice(1)).replace(/[_\u00A0]/g, ' ');
            
            if (uniRight.startsWith(uniLeft) && /^[a-zа-яё]*$/.test(afterLink)) {
        ...
        const processAllText = () => {
            ...
            const editForm = document.getElementById('editform') || document.forms.editform;
            if (window.auto_comment && window.insertSummary && editForm && !editForm.wpSection.value) {
        ...
        if ($input && inputElement) {
            ...
            if (caretPosition) {
                const codeMirrorScroll = document.querySelector('.CodeMirror-vscrollbar');
                const scrollElement = codeMirrorScroll || inputElement;
                const textScroll = scrollElement.scrollTop;
                ...
                scrollElement.scrollTop = textScroll;
            } else {
                processAllText();
            }
        } else {
    ...
    if (editForm) {
        editForm.addEventListener('keydown', (e) => {
            if (e.ctrlKey && !e.metaKey && e.code === 'KeyW') {
                const isValidModifier = isMac ? (e.shiftKey && !e.altKey) : (!e.shiftKey && e.altKey);
                if (isValidModifier) {
                    e.preventDefault(); 
                    window.Wikify();
                }
