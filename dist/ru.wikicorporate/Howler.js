(() => {
    'use strict';
    if (window.howlerAdapterES6Loaded) return;
    window.howlerAdapterES6Loaded = true;

    const SCRIPT_PAGE = 'MediaWiki:Howler/Main.js'; 
    const VERSION = '1.0'; 
    ...
    const checkDOM = () => {
        if (document.querySelector('.wiki-howler')) {
            loadScript();
            observer.disconnect();
        }
    };
