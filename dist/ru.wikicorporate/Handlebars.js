(() => {
    'use strict';
    if (window.handlebarsAdapterES6Loaded) return;
    window.handlebarsAdapterES6Loaded = true;

    const SCRIPT_PAGE = 'MediaWiki:Handlebars/Main.js'; 
    const VERSION = '1.0'; 
    ...
    const checkDOM = () => {
        if (document.querySelector('.wiki-handlebars')) {
            loadScript();
            observer.disconnect();
        }
    };
