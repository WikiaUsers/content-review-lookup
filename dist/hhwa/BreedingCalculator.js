...
function buildDataLists(){
    ...
    parent_type_pools[TYPE_GROUPS_DRAGON_ISLANDS_ID] = [];
    parent_type_pools[TYPE_GROUPS_ARCTIC_ISLES_ID] = [];
    
    for (var key in parent_type_groups){
        var group_is_arctic = false;
        var type_group = key.split(",");
        
        for (var index in arctic_types){
            if (listContainsElements(type_group, [arctic_types[index]])){
                parent_type_pools[TYPE_GROUPS_ARCTIC_ISLES_ID].push(type_group);
        ...
        if (!group_is_arctic){
            parent_type_pools[TYPE_GROUPS_DRAGON_ISLANDS_ID].push(type_group);
        }        
    ...
    for (var dragon_id in breeds){
        ...
        var dragon_types = breeds[dragon_id].types;
        var num_types = dragon_types.length;
        
        breeds[dragon_id].requiredtypes = filterTypes(dragon_types, "recessive");
        ...
        var environments = breeds[dragon_id].environments;
        ...
        for (var index in environments){
            ...
            environment_breed_ids[key].push(dragon_id);
        }
        ...
        if (required_level !== undefined){
            ...
            var sorted_types = mergeSort(dragon_types, "level_requirements");
...
function getPossibleParents(){            
    ...
    if (breedable){
        ...
        if (default_offspring_explanations[dragon_id]){
            default_offspring_message = "<br>" + default_offspring_explanations[dragon_id] + "<br>";
        }
...
function buildOffspringList(first_parent_id, second_parent_id){
    ...
    var first_parent_is_arctic = breedInhabitsEnvironment(first_parent_id, ENVIRONMENT_ARCTIC_ISLES_ID);
    var second_parent_is_arctic = breedInhabitsEnvironment(second_parent_id, ENVIRONMENT_ARCTIC_ISLES_ID);
    ...
    if (pair_is_arctic){
        ...
        for (var index in possible_offspring){
            ...
            if (breedInhabitsEnvironment(breed_id, ENVIRONMENT_ARCTIC_ISLES_ID)){
        ...
        if (possible_offspring.length === 0){
            ...
            if (breedTypePresent(id_list, "diamond")){            
                ...
                var arctic_breeds = environment_breed_ids[ENVIRONMENT_ARCTIC_ISLES_ID];
                for (var index in arctic_breeds){
                    if (listContainsElements(unfiltered_types, breeds[arctic_breeds[index]].types)){
...
function buildParentList(dragon_id){
    ...
    var breed_types = breeds[dragon_id].types;
    ...
    if (parentDataIsValid(parent_data)){   
        ...
        var breed_type_req = breeds[dragon_id].requiredtypes;
        ...
        if (breed_type_req.length === 1){        
            if (getTypeClassification(breed_type_req[0]) === "recessive"){
                ...
                for (var index in parent_type_pools[TYPE_GROUPS_DRAGON_ISLANDS_ID]){
                    var pool = parent_type_pools[TYPE_GROUPS_DRAGON_ISLANDS_ID][index];
...
function getEnvironmentKey(dragon_id){
    ...
    for (var index in arctic_types){           
        if (listContainsElements(breed_types, [arctic_types[index]])){
            // Arctic Isles dragons have access to all breeds
            return TYPE_GROUPS_ALL_ID;                    
        }
    }
    
    return TYPE_GROUPS_DRAGON_ISLANDS_ID;
}
...
function isSingleTypeOffspringException(breed_types, parent1_types, parent2_types){   
    ...
    if (breed_types.length === 1){
        ...
        if (type_pool.length === 1){
            ...
            if (single_type === "diamond" && listContainsElements(prismatic_types, [breed_types[0]])){
                if (!listContainsElements(arctic_types, breed_types)){              
...
function getSingleTypeExceptionParents(id){
    ...
    if (type){
        if (getTypeClassification(type) !== "recessive"){
            var offspring_is_arctic = listContainsElements(arctic_types, [type]);
            var pure_diamond_group = new DragonGroup(parent_type_groups["diamond"], ["diamond"]);
            ...
            if (offspring_is_arctic) {
                ...
                for (var type_index in all_types) {
                    var type_to_check = all_types[type_index];
...
function getCorrespondingArcticDiamondHybridGroup(type) {
    if (listContainsElements(arctic_types, [type])) {
        var pure_and_diamond_group = parent_type_groups[type + ",diamond"];
    ...
    var rows = handler.joinRows(
        some_type_groups.map(function (group_pair) {                        
            ...
            if (left_group && right_group){
                ...
                handler.wrapCell(handler.renderDragonTypes(left_types), "center"),
                handler.wrapCell(handler.renderDragonTypes(right_types), "center"),
