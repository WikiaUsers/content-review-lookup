/*************
Title		:   BlockUserNotification
Description	:   EXCLUSIVELY FOR 'adoptme' WIKI ONLY - Whenever a user gets blocked, users will have a one-time full-screen notification of their block status and information.
Author		:   Vastmine1029
Version		:	Alpha (No version will be specified until Alpha stage is complete.)
*************/
...
mw.loader.using('mediawiki.api', function() {
	...
	bkusers: user,
		...
		text_to_format = "<div class=\"default-background\" style=\"line-height: 1.5; border: 2px solid red; font-family: rubik, helvetica, arial, sans-serif;\"><center><span style=\"font-size: 16pt;\">'''You have been blocked from using [[w:c:adoptme|Adopt Me! Wiki]].'''</span></center><hr/>You will not be able to do the following, until your block ends:<li>Discussing or upvoting in [https://adoptme.fandom.com/f Discussions]; and</li><li>Editing any pages on [[w:c:adoptme|Adopt Me! Wiki]].</li><hr/><span style=\"font-size: 16pt; text-decoration: underline;\">Block Information</span><br/>Block Performer: [[User:" + blockperformer + "|" + blockperformer + "]]<br/>Block Start: " + blockStartDateTime + " <br/>Block Expiry: " + blockEndDateTime + "<br/>Block Reason:<blockquote style=\"margin-top: 0.5em;  margin-bottom: 0.5em;\"><i>" + blockr + "</i></blockquote><hr/>Please abide the [[Rules and Guidelines]] so that [[w:c:adoptme|Adopt Me! Wiki]] can be a safe and fun place for all Fandom community members.<br/><center><div class=\"wds-button block-notif-dismiss\" id=\"block_notif_dismiss\" style=\"cursor: pointer; background-color: salmon; border-color: salmon; margin: 1em 0 0.5em 0; color: #3A3A3A;\">Dismiss</div></center></div>";
		...
		try {
			...
			console.log("Cookie exists with value \"" + cookie_read_status + "\"!");
		}
		catch(err) {
			console.error(err);
		...
		if (cookie_read_status === "1") {
			console.error(user + " has already dismissed notification. i.e., Cookie value is \"" + cookie_read_status + "\".");
			return;
		}
...
/* Current Issue(s):
* When a moderator changes an existing block, the cookie does not update. A code needs to be written up, if possible, to do such update.
*/