 ...
 * Взято с английской Terraria Wiki, на пару дней =)
 * window.Snowstorm
 ...
 * A new layer is created every 600-1100 milliseconds with 1 snowflake per region
 * 
...
(function ($, window, document, Math) {
	...
	var _container,
		_layer,
		_active = false,
		_regions = 6,          // user customizable
		_timeoutSeconds = 300, // user customizable
		_windCounter = -1,
		_windDrift;
	...
	function snow() {
		var $window = $(window),
			width = $window.width(),
			height = $window.height() - 35 - 150,    // the -150 magic number is to match up w/ Terraria Wiki background
	...
	window.Snowstorm = {
		...
		start: function () {
			...
			if (!_active || !con) {
				// setup timeout
				if (typeof _timeoutSeconds === 'number' && _timeoutSeconds > 0) {
