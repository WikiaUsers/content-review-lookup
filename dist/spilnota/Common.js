importArticles({
    type: "script",
    articles: [ 
        'MediaWiki:CodeGenerator.js',       //CodeGenerator
        'MediaWiki:Requests.js'  //Request filling system
  ]
});
...
(function ($) {
    ...
    f.init = function ($content) {
        // fix lost mw-content-text
        if ($content && f.fixContentHook && $content.selector !== '#mw-content-text') {
            f.fixContentHook = false;
            f.init($('#mw-content-text'));
        }
    ...
    f.update = function (targets, d) {
        var wordmark, wordmark_body,
            ...
            data = ['activeusers', 'admins', 'articles', 'edits', 'hub', 'images', 'lang', 'pages', 'founder'],
...
window.LockOldBlogs = {
    expiryMessage: "Цей блоґ не коментували впродовж <expiryDays> днів. Немає сенсу коментувати його.",
};