...
$(function () {
        ...
        '<div id="GeneratorBody">' +
            '<fieldset>' +
                '<legend>' +
                    ...
                    '<div id="SwitchToJS" class="codegenerator-field-switch" style="margin-right:none;opacity:0.7">JS</div>' +
                '</legend>' +
                '<div id="CSSParams" />' +
                '<div id="JSParams" style="display:none;" />' +
    ...
    $('#JSParams').append(
        '<h2 style="margin-top:15px;">Оформление вики</h2>' +
        '<div id="FlagBody" class="codegenerator-group" style="opacity:1; text-align:left;">' +
            '<div class="codegenerator-flag-js"><input class="codegenerator-masthead-nick" style="width:19%" placeholder="Ник"/> : <input class="codegenerator-input codegenerator-masthead-flags" placeholder="Текст на табличках (можно несколько через запятую)" /></div>' +
            '<div class="codegenerator-flag-js"><input class="codegenerator-masthead-nick" style="width:19%" placeholder="Ник"/> : <input class="codegenerator-input codegenerator-masthead-flags" placeholder="Текст на табличках (можно несколько через запятую)" /></div>' +
            '<button id="codegenerator-flag-jsAdd" type="button" class="wikia-button">Добавить ещё!</button>' +
        '</div>' +
        '<h2 style="margin-top:15px">Результат</h2>' +
        '<button id="JSSubmit" type="button" class="wikia-button" style="display:block;">Показать результат</button>'
    );

    $('#SwitchToJS').click(function () {
        $('#CSSParams').hide();
        $('#JSParams').show();
        $('#SwitchToJS').fadeTo(500, 1);
    ...
    $('#CSSSubmit').click(function () {
        ...
        if (cursRes) {
            Result += '/* Курсор на вики */\n' +
                'body {\n' +
                '    cursor: url("' + $('#CursorNormal').val() + '");\n' +
                '}\n' +
                'a:link, a:visited,\n' +
                'a:hover {\n' +
                '    cursor: url("' + $('#CursorLink').val() + '");\n' +
                '}\n\n';
        }
        if (commRes) {
            var commBQ = 
    ...
    // JS
    $('#codegenerator-flag-jsAdd').click(function () {
        $('#codegenerator-flag-jsAdd').before('<div class="codegenerator-flag-js"><input class="codegenerator-masthead-nick" style="width:19%" placeholder="Ник"/> : <input class="codegenerator-input codegenerator-masthead-flags" placeholder="Текст на табличках (можно несколько через запятую)" /></div>');
    });

    $('#JSSubmit').click(function () {
        // Пока что только для флагов
        $('#CodeGenOutput').val('');
        var flagBody = 'MastRights = {};\n',
            flagImpt = '\nimportScriptPage("MediaWiki:Masthead.js", "ru.c");';
        $('.codegenerator-flag-js').each(function () {
            var user = $(this).find('.codegenerator-masthead-nick').val(),
                userflags = $(this).find('.codegenerator-masthead-flags').val().split(',').join('","');
            if (user === '' || userflags === '') {
                return;
            }
            flagBody += 'MastRights["' + user + '"] = ["' + userflags + '"];\n';
        });
        $('#CodeGenOutput').val(flagBody + flagImpt);
    });
});