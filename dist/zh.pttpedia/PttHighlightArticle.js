// this file has move to [[mediawiki:PttHighlight.js]]
...
var pttHighlightArticle = {
  ...
  /^(\s*作者 )(.*)( 看板 )(.*)$/, function command(match) {
    ...
    __proto__: Parser.prototype,
    regexp: /^(\s*\d+\s.|\s*★\s*.)( \d|\d\d|爆|X.|  )(.*$)$/,
    command: function command(match) {
      if (match[1].match('★')) match[1] = this.token(match[1], 'announcement')
      match[2] = this.markPushCount(match[2]);
  ...
  processTemplate: function processTemplate(root) {
    ...
    var rawList = root.querySelectorAll('.ptt-article.raw');
