...
/version 1.2.0
/Original version coded by Callofduty4 and Madnessfan34537, modified by Fang³ for use on /Merlin Wiki
*/
...
var surroundColour = getCookie("customisation", 10);
var selfPostColour = getCookie("customisation", 11);
...
function loadChatSkin() {
	...
	$('.ChatHeader').css({"background-color":backgroundColour, "font-family":fontChoice});
        ...
        selfPostElement.innerHTML = '.Chat .you{background:' + selfPostColour + ' !important;}';
        $('head').append(selfPostElement);
        $('.Write [name="message"]').css({"color":fontColour});
        $('.Write .message').css({"background-color":backgroundColour});
        $('.ChatHeader .User .username').css({"color":fontColour});
...
function updateCookie() {
	...
	surroundColour = $('#surroundColourinput').val();
        selfPostColour = $('#selfPostColourinput').val();
...
function setDaCookie() {
	setCookie("customisation", backgroundColour + ", " + fontColour + ", " + chatHacksEnabled + ", " + fontChoice + ", " + tabCompleteEnabled + ", " +  multiKickEnabled + ", " + multiPMEnabled + ", " + searchBarEnabled + ", " + ignoreURLEnabled + ", " + stopSideScrollEnabled + ", " + surroundColour + ", " + selfPostColour);
}
...
if ( $.inArray('chatmoderator', wgUserGroups) > -1 || $.inArray('sysop', wgUserGroups) > -1 || $.inArray('vstf', wgUserGroups) > -1 ) {
	multiKickDisplayer = '';
}
else {
...
// Add Options button
$('.Rail').prepend('<div onclick="openOptions()" style="margin:auto; cursor: pointer; font-size:150%; bottom:4px; padding-bottom:2px; border-bottom: 1px solid #CCCCCC; width:90%;" align="center"><img src="http://aux.iconpedia.net/uploads/16357735571081531132.png" width="18px"/>&nbsp;Options</div>');
