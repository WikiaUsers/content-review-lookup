...
function checkUpForm(){
 ...
 if (lcns.value == '' && /^[^\{\}]*(\{\{Изображение[^\{\}]+\}\}[^\{\}]*)*$/i.exec(desc.value))
  wrn.innerHTML += '<li>Не указана лицензия. Выберите лицензию в выпадающем списке выше.</li>'
 if (lcns.value != '' && lcns.value != 'subst:ET' && /\{\{Обоснование добросовестного использования/i.exec(desc.value))
 ...
 if (lcns.value == '' && /\{\{Обоснование добросовестного использования\s*((\|\s*)+([\wа-яё]+\s*)+=\s*)*(\|\s*)*\}\}/i.exec(desc.value))
