(function($, mw) {
    ...
    function addOasisPurgeButton() {
        $('.WikiaPageHeader > .wikia-menu-button').after('<a id="purge" '
            + 'class="wikia-button" href="javascript:void(0)" title="Demotivate the page...">Demotivate</a>');
    }
    function addPurgeButton() {
        switch (mw.config.get('skin')) {
            case 'monobook':
                $('#p-cactions > .pBody > ul').append('<li id="ca-purge">'
                    + '<a id="purge" href="javascript:void(0)" title="Demotivate the page...">Demotivate</a></li>');
                break;
            case 'oasis':
            case 'wikia':
                addOasisPurgeButton();
