...
(function() {
  if (
    !window.Promise ||
    mw.config.get('wgCanonicalSpecialPageName') != 'CreatePage' ||
    (window.ArticleDraft && window.ArticleDraft.init)
  ) return;
  ...
  window.BlogDraft = $.extend(Object.create(null), {
    ...
    loadDrafts: function() {
      return Promise.resolve(
        Object.keys(localStorage)
          .filter(function(key) {
            return key.indexOf('ArticleDraft-') == 0;
          })
          .map(function(key) {
            return key.slice(10);
          })
    ...
    load: function(title) {
      return Promise.resolve(localStorage.getItem('ArticleDraft-' + title));
    },
    // -> Promise<>
    save: function() {
      try {
        return Promise.resolve(localStorage.setItem('ArticleDraft-' + this.getTitle(), this.getContent()));
      } catch(e) {
    ...
    // title -> Promise<>
    del: function(title) {
      return Promise.resolve(localStorage.removeItem('ArticleDraft-' + title));
    },
    ...
    preload: function() {
      if (++this._preload == 2) {
        dev.i18n.loadMessages('ArticleDraft').then(this.init.bind(this));
      }
    },
    init: function(i18n) {
      this.i18n = i18n;
  ...
  importArticles(
    ...
    },
    {
        type: 'style',
        articles: [
            'u:cjrichards-and-applemasterexpert:MediaWiki:BlogDraft.css'
        ]
    }
  );
