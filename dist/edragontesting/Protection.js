...
-----------------------
 AUTHORS & MAINTAINERS
-----------------------
- FaceBound: version 1 with template
- E12Dragon: version 2
...
function addProtectionIcon(type, level) {
	...
	if (type == 'comment' && level == 'sysop') {
		...
		iconsmall = 'lock';
	}else if (config.wgCanonicalNamespace == 'User' || config.wgCanonicalNamespace == 'User_blog') {
		if (type == 'comment') {
			iconsmall = 'checkmark';
		}else {
			iconsmall = 'user';
	...
	if (config.wgCanonicalNamespace == 'User' || config.wgCanonicalNamespace == 'User_blog') {
