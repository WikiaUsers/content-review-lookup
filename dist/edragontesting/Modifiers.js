...
$(document).ready(function() {
	...
	$("span#targetHPHarness").html('<div id="targetHPInput">Target HP: <input type="text" value="0" id="targetHP" style="text-align: right; width: 60px; background-color:white;"></input></div>');
   ...
   $("#changeBonusButton").click(function() {
	  ...
	  var poisonASMultiplier = [0,35,40,45,50,55,60,65,68]
		...
		function calcDPH(initialDPH,isHero) {
		...
		$(".DPH").each(function() {
			...
			if ($(this).hasClass("Hero") === true) {
				calcNewDPH = calcDPH(initialDPH,true);
			} else {
				calcNewDPH = calcDPH(initialDPH,false);
			}
		...
		$(".HP").each(function() {
			...
			var roundedHP = Math.floor(calcNewHP * 100)/100; //Use floor function to round down to 2 d.p.
