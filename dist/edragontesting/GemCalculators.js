...
------------------------
- Spottra and Dahimi: original version
- Pkukiss: Adding builder base calculators
- MagmaHound: General maintenance and additional gem calculators
- E12Dragon: Adding time to XP calculator
*/
...
// Deprecated since Builder Base timers use the same formula as HV as of BB2.0
...
$(document).ready(function() {
    ...
    //Builder Base Resources to Gems
    ...
    //Gems to Builder Base Resources
    ...
    //Time to XP
    var attributes = 'type="text" value="0" style="text-align: right; width: 40px; background-color:white;"';
    $("span#xpDaysHarness").html('<input id="xpDays"' + attributes + '></input>');
    $("span#xpHoursHarness").html('<input id="xpHours"' + attributes + '></input>');
    $("span#xpMinutesHarness").html('<input id="xpMinutes"' + attributes + '></input>');
    $("span#xpSecondsHarness").html('<input id="xpSeconds"' + attributes + '></input>');

	//Clicking calculate button
    $("#xpCalculate").click(function() {
        var output = $('#xpOutput');
        var days = Math.max(0, parseInt(document.getElementById('xpDays').value) || 0);
		var hours = Math.max(0, parseInt(document.getElementById('xpHours').value) || 0);
		var minutes = Math.max(0, parseInt(document.getElementById('xpMinutes').value) || 0);
		var seconds = Math.max(0, parseInt(document.getElementById('xpSeconds').value) || 0);
        var secondsinput = 86400 * days + 3600 * hours + 60 * minutes + seconds;

		//Output the result
        var result = calculateXP(secondsinput);
        output.text(result);
    });
});