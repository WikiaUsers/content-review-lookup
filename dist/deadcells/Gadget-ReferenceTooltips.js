...
var addedHeightOffset = -28.8; // Add an additional tooltip height offset specific to Dead Cells Wiki
...
function rt( $content ) {
	...
	function Tooltip( te ) {
				...
				this.panelDisabled.$element.append(
					$( '<table>' )
						...
						.append(
							$( '<tr>' ).append(
								$( '<td>' ).append(
									$( '<img>' ).attr( 'src', 'https://deadcells.gamepedia.com/load.php?modules=ext.popups.images&image=footer&format=rasterized&lang=ru&skin=vector&version=0uotisb' )
								),
								$( '<td>' )
		...
		this.calculatePosition = function ( ePageX, ePageY ) {
			...
			teOffset = {
				top: $window.scrollTop() + teOffset.top + addedHeightOffset,
