...
var chatOptions = {
	...
	modules: {
		...
		Toolbox: {
			element: "#Toolbox",
			enabled: isEnabled(" Toolbox"),
			loaded: false,
			load: function () {
				if ($("#pingspan").length > 0 || this.loaded)
					return;
				importScript('http://garagekids.wikia.com/wiki/ChatToolbox/code.js');
				this.loaded = true;
			}
		},
		searchBar: {
...
function openOptions() {
	...
	if (chatOptions.modules.searchBar.enabled)
        ...
        if (chatOptions.modules.Toolbox.enabled)
		$("#Toolbox").attr("checked",true);
	...
	if (wgServer !== "http://garagekids.wikia.com")
...
function updateCookie() {
	...
	// Set the cookies
	setCookie("customisation", chatOptions.look.backgroundColor + ", " + chatOptions.look.fontColor + ", " + chatOptions.modules.chatHacks.enabled + ", " + chatOptions.look.fontFamily + ", " + chatOptions.modules.tabComplete.enabled + ", " +  chatOptions.modules.multiKick.enabled + ", " + chatOptions.modules.multiPM.enabled + ", " + chatOptions.modules.toolbox.enabled + ", " + chatOptions.modules.searchBar.enabled + ", " + chatOptions.modules.ignoreURL.enabled + ", " + chatOptions.modules.stopSideScroll.enabled + ", " + chatOptions.look.surroundColor + ", " + chatOptions.look.selfPostColor);
	updateChatSkin();
	cancelChanges();
}
...
if (!$("#chatOptionsButton").length) {
	$('.Rail').prepend('<div id="chatOptionsButton" onclick="openOptions();" style="margin:auto; cursor: pointer; font-size:135%; bottom:4px; padding-bottom:2px; border-bottom: 1px solid #4b6884; width:90%;" align="center"><img src="' + chatOptions.look.modalIcon + '" width="18px"/>&nbsp;Settings</div>'); // Prevent multiple buttons from being appended
}
