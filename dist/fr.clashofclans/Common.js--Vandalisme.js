...
// de fr.inazuma-eleven.wikia.com/wiki/MediaWiki:Wikia.js
$(function() {
	...
	function callAPI(data, method, callback) {
		...
		$.ajax({
			...
			error: function(xhr, error) { showError('AJAX error: ' + error.info); }
		});
	}
	...
	function cvuSubmit() {
		...
		callAPI({
			...
			var content = page.revisions[0]['*'];
                        var template = '{{Vandalisme|Exemple}}';
                        content = content.replace(template, '{' + '{Vandalisme|' + editor + '}' + '}');
	...
	if(wgPageName=='Wiki_Clash_of_Clans:Vandalisme') {
		var $div = $('#cvu_guide');
		var $form = $('<form />');
        ...
        function removeUser(user, state) {
                var template = '\{\{Vandalisme\|' + user.split("_").join(" ") + '(\|.*?)?\}\}';
		...
		callAPI({
			...
			if (content.match(template) == null) {
				showError('Erreur.');
                        ...
                        content = content.replace('{' + '{Vandalisme|' + user.split("_").join(" "), '{' + '{Vandalisme|Exemple}' + '}');
