...
( window.dev = window.dev || {} ).editHelp = {
	i18n: {
		en: {
			noTemplateSelected: '(Seleccione aquí o en el panel de edición)',
			noParamSelected: '(Seleccione aquí o en el panel de edición)',
			noDocumentation: 'No hay documentación disponible',
			unknownParam: 'No hay documentación disponible',
			docNotLoaded: 'No se pudo cargar la documentación'
		}
	}
};
...
( function ( $, mw, editHelp ) {
	...
	model = ( function () {
		...
		function normalize( name ) {
			...
			name = /^Plantilla:/i.test( name ) ? name.substr( 9 ) : name;
			return $.ucFirst( name );
		}
			...
			$.extend( CacheBuilder.prototype, {
				...
				request: function ( cmcontinue ) {
					var query = {
						...
						cmtitle: 'Categoría:Documentación de plantillas',
						cmprop: 'title',
						cmnamespace: 10,
						cmlimit: 500
					};
		...
		docCache = ( function () {
			...
			function Doc( name, data ) {
				var $doc = $( '<div>' + data + '</div>' )
					...
					$usage = $doc.find( '#Uso' ).parent().next(),
					$p = $doc.children( 'p ' ),
					doc = this;
				this.title =  name;
		...
		function createDocPane() {
			...
			.append( '<div id="doc" class="init">' +
				'<div class="doc-section doc-template doc-header">' +
					'<div class="doc-caption">Plantilla:</div>' +
					'<select>' +
				...
				'<div class="doc-section doc-param doc-header">' +
					'<div class="doc-caption">Parámetro:</div>' +
					'<select></select>' +
				'</div>' +
			...
			html = {
				...
				template: {
					...
					setTitle: function ( title ) {
						var	page = 'Plantilla:' + title,
							href = formatPage( page );
						this.$title.html(
							formatLink( page, href, page ) + ' (' +
							formatLink( 'Editar', href + '?action=edit', 'Editar plantilla' ) + ')' +
							formatLink( '?', formatPage( 'Ayuda:Plantillas' ), 'Más acerca de las plantillas', 'right' )
						);
					},
				...
				param: {
					...
					setDesc: function ( param ) {
						var desc = param.desc;
						if ( param.choice ) {
							desc += ' Valores posibles: <br><table>';
