function filterTable(){
	$("table.filterable").each(function(){
		var i=0;
		var cols;
		$(this).find("tr").eq(0).find("th, td").each(function(){
			if (!$(this).hasClass("unfilterable")){
				...
				$(this).find(".filterMenu").append('<div><input type="button" value="All Check" class="all_check"><input type="button" value="All Uncheck" class="all_uncheck"></div>');
				for (j=0; j<cols.length; j++){
	...
	$(".all_check").click(function(){
		...
		$(this).closest("table").find("tbody tr:nth-child(n+1)").attr("condition",1);
		$(this).closest("table").find("tbody tr:nth-child(n+1)").show();
	});
	
	$(".all_uncheck").click(function(){
		$(this).parent().parent().find('input[type="checkbox"]').prop('checked', false);
		$(this).closest("table").find("tbody tr:nth-child(n+1)").attr("condition",-1);
		$(this).closest("table").find("tbody tr:nth-child(n+1)").hide();
	});
}
