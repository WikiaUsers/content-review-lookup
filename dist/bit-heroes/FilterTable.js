// Original script from https://pad.fandom.com/wiki/MediaWiki:FilterTable.js 
  ...
  $(".filterOption").click(function(){
    ...
    $(this).closest("table").find("tr:nth-child(n+1)").each(function(){
      if ($(this).find("td:nth-child("+col+")").text()==val){
        ...
        if (cond==0) {
          $(this).show();
          $(this).nextAll("." + $(this).attr("class") + ".info").show();
        }
        else {
          $(this).hide();
          if (!$("." + $(this).attr("class") + ":visible:not(.info)").length) $(this).nextAll("." + $(this).attr("class") + ".info").hide();
        }
      }
    });
  });
