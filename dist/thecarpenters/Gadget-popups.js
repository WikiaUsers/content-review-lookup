var popupVersion="DJ Fork: Wed Oct 22 13:15:00 UTC 2008";
...
Title.prototype.anchorFromUtf=function(str) {
	this.anchor=encodeURIComponent(str)
	.split('.').join('%2E').split("'").join('%27').split('%').join('.');
};
...
Previewmaker.prototype.editSummaryPreview=function() {
	...
	// Turn [[Image:]] links into [[:Image:]] links so that they are not included in the preview
...
function loadQueryPreview(queryType, article, navpop) {
	...
	case 'imagepagepreview':
		...
		if (pg.wiki.commons && !getValueOf('popupSharedBug')) { trail += '&iishared'; }
...
function loadAPIPreview(queryType, article, navpop) {
	...
	// converted...
		// redundant case
		url += 'list=imageusage&iutitle=' + art ;
		htmlGenerator=APIimagelinksPreviewHTML;
		break;
	case 'category':
	// converted...
	...
	case 'contribs':
	// converted...
	    var usernameart = escape( new Title( article ).userName() );
	...
	case 'imagepagepreview':
	// converted...
	// TODO - support disable of popupImageLink
	//      - removed imageinfo since we don't detect commons/local conflicts anymore
		var trail='&prop=revisions|imageinfo&list=imageusage&rvprop=content&iilimit=1&iiprop=';
		//if (!getValueOf('popupImageLinks')) { trail = '&prop=imageinfo|content'; }
		url += 'titles=' + art + '&iutitle=' + art + trail;
		htmlGenerator=APIimagepagePreviewHTML;
		break;
	case 'sharedimagepage':
	// removed proxylang=pg.wiki.commonslang
	// TODO
	log( "shareimagepage was requested" );
		//url = pg.wiki.commonsbase + '/api.php?format=json&action=query&titles=' + art + '&prop=revisions&rvprop=content';
		//htmlGenerator=APIsharedImagePagePreviewHTML;
		break;
	case 'backlinks':
	// DJ: converted... TODO? bllimit
...
function sharedImagePagePreviewHTML(article, download, navpop) {
	...
	try {
	  ...
	  return 'sharedimagepage preview failed :( ' +
	    'is the query.php extension installed?';
	}
	var ret='';
	if (content) {
...
function imagepagePreviewHTML(article, download, navpop) {
	...
	try {
	  ...
	  return 'imagepage preview failed :( ' +
	    'is the query.php extension installed?<p>' +
	    'Alternatively, the query.php config may have broken. ' +
	    'As a workaround, try setting popupSharedBug=true ' +
	    'although this will disable information previews for commons images.';
	}
	var ret='';
	if (content) {
		var p=prepPreviewmaker(content, article, navpop);
...
function editPreviewTable(article, h, reallyContribs, timeOffset) {
	...
	for (var i=0; i<h.length; ++i) {
		if (reallyContribs && getValueOf('popupUseAPI') ) { 
		    var page=h[i]['title']; curart = new Title(page);
		} else if (reallyContribs) { var page=h[i]['*']; curart = new Title(page); }
...
function APIsharedImagePagePreviewHTML(article, download, navpop) {
    // DJ: Disabled: retrieving commons page for preview.
    return "APIsharedImagePagePreviewHTML was called";
	try {
	    var jsObj=getJsObj(download.data);
	  ...
	  return 'sharedimagepage preview failed :( ' +
	    'is the api.php extension installed?';
	}
	var ret='';
	if (content) {
	  var p=prepPreviewmaker(content, article, navpop);
...
function APIimagepagePreviewHTML(article, download, navpop) {
	...
	else ret += '<hr> No local imagepage content';
 
	var sharingInfo = '';
	if (typeof( page.missing ) != "undefined" && !page.imagerepository )
	    sharingInfo = popupString('No image found');
	else if (page.imagerepository == "local" && getValueOf('popupShowNonCommonsImages' ))
		sharingInfo = popupString('not commons');
    else if (page.imagerepository == "shared" )
        sharingInfo = popupString('commons only');
	// TODO add checks for commons dupe and commons conflict
	if (sharingInfo ) ret += '<hr>' + sharingInfo;
	...
	if (typeof( page.missing ) != "undefined" && page.imagerepository == "shared" ) {
	   // TODO
	   log( "we should retrieve the commmons page" );
	   // loadAPIPreview('sharedimagepage', article, navpop);
	}
...
function APIimagelinksPreviewHTML(article, download) {
	try {
		...
		if (!list) { return popupString('No image links found'); }
	} catch(someError) { return 'Image links preview generation failed :( Is the api.php extension installed?'; }
	var ret=[];
	for (var i in list) { ret.push(list[i]['title']); }
	if (ret.length === 0) { return popupString('No image links found'); }
	return '<h2>' + popupString('File links') + '</h2>' + linkList(ret);
}
...
function APIcategoryPreviewHTML(article, download) {
    ...
    } catch(someError) { return 'Category preview failed :( Is the api.php extension installed?'; }
	var ret=[];
...
function APIhistoryPreviewHTML(article, download, navpop, reallyContribs) {
    try {
		...
		return 'History preview failed :-( Is the api.php extension installed?';
	}
	var timeOffset = getTimeOffset(tz);
	Cookie.create('popTz', timeOffset, 1);
...
navlinkTag.prototype.getPrintFunction=function() { //think about this some more
	...
	case 'contribs': case 'history': case 'whatLinksHere':
		if (!getValueOf('popupUseQueryInterface') && !getValueOf('popupsUseAPI' )) { break; }
...
function loadDiff(article, oldid, diff, navpop) {
	...
	var go = function() {
		...
		if (getValueOf('popupUseAPI') && getValueOf('popupAdjustDiffDates') && tz===null) {
		    pendingNavpopTask(navpop);
		...
		} else if (getValueOf('popupUseQueryInterface') && getValueOf('popupAdjustDiffDates') && tz===null) {
		    pendingNavpopTask(navpop);
...
function setTimecorrectionCookie(d) {
    if( getValueOf('popupUseAPI') )
        Cookie.create( 'popTz', getTimeOffset(getJsObj(d.data).query.userinfo.options.timecorrection), 1);
    else
...
function diffDownloadsComplete(navpop) {
	if ((getValueOf('popupUseAPI') || getValueOf('popupUseQueryInterface')) && Cookie.read('popTz')===null) { return false; }
...
function revertSummary(oldid, diff) {
	if (getValueOf('popupUseAPI') || getValueOf('popupUseQueryInterface')) { return revertSummaryQueried(oldid, diff); }
...
function kateLink(l) {
	...
	var url;
	var defaultToolUrl='http://toolserver.org/~$3/cgi-bin/Tool1/wannabe_kate?username=$1&site=en.wikipedia.org&$2';
...
function contribsTreeLink(l) {
	...
	var url='http://toolserver.org/~interiot/cgi-bin/contribution_tree?dbname=';
...
function getHistory(wikipage, onComplete) {
	...
	var url;
	if (getValueOf('popupUseAPI')) {
		url = pg.wiki.wikibase + '/api.php?format=json&action=query&prop=revisions&titles=' +
...
function processHistory(download) {
	if (getValueOf('popupUseAPI') || getValueOf('popupUseQueryInterface')) {
...
pg.string = {
	...
	'katelinkHint': 'Count the contributions made by %s',
	'contribs': 'contribs',
