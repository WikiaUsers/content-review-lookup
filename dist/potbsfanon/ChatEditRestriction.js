...
// Disable message (1 = enabled, 0 = disabled)
    ...
    window.chatRestr.undereditMessage = "Hi "+ wgUserName+"! Thanks for trying to use the PotBS Fanon Wiki chat room. However, the chat room is only for active editors of the PotBS Fanon Wiki, and you haven't yet made the minimum number of edits required to use it. Our chat policy (which can be found at http://potbsfanon.wikia.com/wiki/Project:Chat) requires that you make at least 75 edits, with at least 35 of these being in our mainspace (that is, on the wiki articles that make up the main content of the wiki). We apologize for this inconvenience and hope to see you in the chat soon.";
...
if (exceptionList.indexOf(userObject.name) == -1 && userObject.groups.indexOf("vstf") == -1 && userObject.groups.indexOf("staff") == -1){
  if (typeof alertPopped === "undefined"){
    $.get("/wiki/Special:Editcount/"+userObject.name, function(result){
      ...
      var mainCountRestriction = 35;
      var totalCountRestriction = 75;
