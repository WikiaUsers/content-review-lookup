	...
	RequestTooltip: function(source)
	{
		...
		if (objInfo && objInfo.Net)
		{
			...
			if (data[objInfo.Type][objInfo.Id][objInfo.Lang] == null)
			{
				...
				YG.Common.ajaxishRequest('http://' + objInfo.Server + '/' + (objInfo.Lang != 'en' ? objInfo.Lang  + '/': '')  + 'obj_' + objInfo.Type + '_' + objInfo.Id + '_ajx.js');
...
YG.Network =
{
	...
	__matchToYg: function(url, network)
	{
		var regExp = /http:\/\/(([a-z]+)\d*\.yg\.com)\/([a-z]{2}\/)?(\w+)\/.+?\?(id=(\d+)|(.*?id=\d+.*?))$/i;
		var matches = url.match(regExp);
		var typeId;
		...
		if (matches && (matches.length > 4) && matches[2] && (matches[2].indexOf(network) >= 0))
		{
			...
			if (matches[6])
				return {Net: this[network].Id, Type: typeId, Id: parseInt(matches[6], 10), Lang: matches[3], Server: matches[1], NetStr: network};
			else
			{
				if (matches[5])
				{
...
YG.Common =
{
	...
	ajaxishRequest: function(url)
	{
		...
		request.src = url + (YG.Versioning ? '?' + YG.Versioning : '');
...
YG.Syndication =
{
	...
	Init: function()
	{
		var anchors = document.links, linkType, addName;
		...
		if (header)
		{
			...
			request.media = 'all';
			request.href = 'http://static.yg.com/css/ex_tooltip.css'+ (YG.Versioning ? '?' + YG.Versioning : '');
			header.appendChild(request);
		}
				...
				if (objInfo && objInfo.Net)
				{
					...
					if (!(data && data[objInfo.Type] && data[objInfo.Type][objInfo.Id] && data[objInfo.Type][objInfo.Id][objInfo.Lang]))
					{
						...
						if (!this.Requests[objInfo.Net])
							this.Requests[objInfo.Net] = { server: 'http://' + objInfo.Server + '/synd_req_ajx.js?', request: []};
					...
					//try to update links style and content
					//if (!this.__applyAutostyles(anchors[i], objInfo, data[objInfo.Type][objInfo.Id]))
					//	this.ProcessingLinks[this.ProcessingLinks.length] = anchors[i];
				}
			}
		}
