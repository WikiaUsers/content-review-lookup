...
if (wgCanonicalSpecialPageName == 'Chat') {
    function ChatTopic() {
        ...
        api.send(new api.Query(api, 'GET', {
            action: 'query',
            prop: 'revisions',
            rvprop: 'content',
            titles: 'Project:Chat/Topic',
            indexpageids: '1'
        }, function (result) {
...
function updatetimer(i) {
    ...
    if (diff > 0) left = (diff % 7) + ' days ' + left;
    diff = Math.floor(diff / 7);
    if (diff > 0) left = (diff % 52) + ' weeks ' + left;
...
window.UserTagsJS = {
    ...
    tags: {
        ...
        bureaucrat: 'Hermione Chase',
        sysop: 'Hermione Chase',
        chatmoderator: 'Hermione Chase'
    }
};