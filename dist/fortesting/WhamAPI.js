...
function WhamUI() {
	...
	for(var i in wgUserGroups) {
		if(wgUserGroups[i] == 'sysop' || wgUserGroups[i] == 'bureaucrat' || wgUserGroups[i] == 'vstf' || wgUserGroups[i] == 'helper' || wgUserGroups[i] == 'staff') {run = true; break;}
	}
	if(!run) {return;}
...
if(ajaxCallAgain) {ajaxCallAgain.push(WhamUI);}
	...
	api.send(new api.Query(api, 'GET', 'action=query&list=usercontribs&ucuser=' + user + '&ucend=' + time + '&uclimit=' + limit, function(result) {
		for(var i in result.query.usercontribs) {
			if(result.query.usercontribs[i].new == '') {
				$('a[title="' + result.query.usercontribs[i].title + '"] ~ abbr.newpage').html('<img src="http://images2.wikia.nocookie.net/dev/images/8/82/Facebook_throbber.gif" style="vertical-align: baseline;" border="0" alt="Whamming...">');
				...
				$('a[title="' + result.query.usercontribs[i].title + '"] + span + span.mw-uctop').html('<img src="http://images2.wikia.nocookie.net/dev/images/8/82/Facebook_throbber.gif" style="vertical-align: baseline;" border="0" alt="Whamming...">');
...
function timestr(time) {
	try { //Some of this may break if the date is a fail
		...
		if(time.split(',').length == 3) { //Contains two commas, assumed to be full string
			var hours = time.split(',')[0].removeTrailing(' ').split(':')[0].removeTrailing(' '); if(hours < 10) {hours = '0' + hours;}
			var minutes = time.split(',')[0].removeTrailing(' ').split(':')[1].removeTrailing(' '); if(minutes < 10) {minutes = '0' + minutes;}
			...
			if(time.indexOf(':') != -1) { //Time
				time = time.replace(/ /g, '');
				var hours = time.split(':')[0]; if(hours < 10) {hours = '0' + hours;}
				var minutes = time.split(':')[1]; if(minutes < 10) {minutes = '0' + minutes;}
