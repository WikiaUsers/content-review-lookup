...
function getData(c,isSmall,avginput) {
	...
	if (volumes.length >= 10&&!isSmall) {
		...
		yAxis = {
			title: {
				text: isSmall?null:'Price history',
				offset: 60,
				style: {
					color: 'black',
					fontSize: '12px',
...
var enlarge = '<a href="javascript:popupChart('+c+');" style="text-decoration:underline;">Enlarge chart</a>';
...
_GEC['chart'+c] = new Highcharts.StockChart({
	...
	subtitle: {
		text: isSmall?(isInfobox?null:enlarge):(itemName.toLowerCase()=='blank'?'Historical chart':itemName),
		y: 35,
		style: {
			color: '#666',
