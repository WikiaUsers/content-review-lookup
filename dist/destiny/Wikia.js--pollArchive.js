...
if (wgUserGroups.indexOf("sysop") > -1 && wgPageName == "Destiny_Wiki") { 
...
function pollDate() {	
	...
	var statementDate = pollStatement.split('on ')[1].split(',')[0] + pollStatement.split('on ')[1].split(',')[1];
	var dateArray = statementDate.split(" ")
 
	//Saves day and month as integer through matching index on monthArray
	var statementDay = dateArray[1];
	var statementMonth = monthArray.indexOf(dateArray[0]) + 1;
...
function archivePoll() {	
	if (jQuery.inArray("sysop", wgUserGroups) > -1) {
		...
		$.get( "http://destiny.wikia.com/api.php", {'action':'query', 'prop':'revisions', 'titles':pollPage, 'rvprop':'timestamp|content', 'format':'json'}, 
			function( data ) { 
				...
				$.get( "http://destiny.wikia.com/api.php", {'action':'query', 'prop':'info', 'titles':'Main Page', 'intoken':'edit|delete|protect|move|block|unblock|email|import', 'format':'json'}, function( data ) {
					...
					$.post( "http://destiny.wikia.com/api.php", {'action':'edit', 'title':pollPage, 'summary':'Automated archive of main page poll using archive add-on.', 'text':reconstructedPollPage, 'token':editToken, 'format':'json'}, function( data ) {
						//Feeds back success data to original user
						if (data["edit"]["result"] == "Success") {
							alert("Poll successfully archived!");
						} else {
							alert("Error: DW004 - Edit request unsuccessful. Please report to T3CHNOCIDE.");
						}
					}).fail(
						function() { 
							alert( "Error: DW003 - Could not post collated poll data to server. Please report to T3CHNOCIDE." );
						});
				}).fail(
					function() { 
						alert( "Error: DW002 - Could not retrieve edit token from API. Please report to T3CHNOCIDE." );
					});
 
			} , "json").fail(
				function() { 
					alert( "Error: DW001 - Cannot fetch poll page contents. Please report to T3CHNOCIDE." );
				});
	} else {
