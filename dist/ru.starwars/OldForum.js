...
function ShowPreview()
{
	...
	$.post("https://starwars.fandom.com/ru/api.php", {action: "parse", format: "json", formatversion: "2", prop: "text|indicators|displaytitle|modules|jsconfigvars|categorieshtml|templates|langlinks|limitreporthtml", text: $('#wpTextbox1').val(), pst: "true", preview: "true", sectionpreview: "true", disableeditsection: "true", useskin: "oasis", uselang: "ru"})
	.done(function(data)
	{
    ...
    $.get( wgScript, { title: sTemplateSource, action: 'raw', ctype: 'text/plain' } ).then( 
        function( data )
        {
            ...
            if (sSection === 'new')
            {
				...
				//.done(function(data)
				//{
				//	sMessage= (data.parse.text).replace(/\<!--(.*\n)*.*/, '').replace(/<div class="mw-parser-output">/, '').replace(/btn_ForumReply/g, 'btn_ForumReply wds-button-dissabled');
				//	$('div.mw-parser-output').append(sMessage);
				//});
			...
			else
			{
				...
				//.done(function(data)
				//{
				//	sMessage= (data.parse.text).replace(/\<!--(.*\n)*.*/, '').replace(/<div class="mw-parser-output">/, '').replace(/BlockForumMessage/g, 'BlockForumReply').replace(/wds-button/g, 'wds-button-dissabled');
				//	oLast.after(sMessage);
				//});
			...
			//window.location.href = 'https://starwars.fandom.com/ru/wiki/'+ wgPageName + '#'+ sMessageID;
			//	location.reload(); 
