...
function Wikify()
{
    ...
    $.get( mw.config.values.wgScript, { title: 'Шаблон:Wikificator-source', action: 'raw', ctype: 'text/plain' } ).done( function( data )
    {
        ...
        $.get( mw.config.values.wgScript, { title: 'Шаблон:Wikificator-source-categ', action: 'raw', ctype: 'text/plain' } ).done( function( data )
        {
            ...
            {
                 wmEnNS[i] = $(this).text()
                	.replace(/\]\]/g, "\\]\\]") // двойное экранирование экраннированной скобок ]]
                	.replace(/\|/g, "\\|") // двойное экранирование вертикальной черты
                	.replace(/ИЛИ/g, "|") // конвертация слова ИЛИ в логический оператор |
                	.replace(/\n\*/g, "|") //конвертация звездочки в начале строки (в списках) в логический оператор |
                	.replace(/^\|/g, "") // удаление вертикальной черты в самом начале искомого текста
                	; 
                i = i+1; 
            });
