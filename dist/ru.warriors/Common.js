...
/******************************/
/* Simple sliders by Wildream */
/******************************/
var NumberOfSlides = $('.displaytemplate').length;
var IDNum = '';
var OldID = '#1';
var tmpNumber = 1;
$('#1').show();
...
$('.switch-next').click(function () {
    $(OldID).hide();
    if (tmpNumber != parseInt(NumberOfSlides, 10)) {
        tmpNumber++;
        IDNum = '#' + tmpNumber;
    } else {
        tmpNumber = 1;
...
$(function() {
    if (mw.config.get('wgNamespaceNumber') !== -1) {
        $('.wikia-gallery').each(function() {
            if ($(this).find(".wikia-gallery-item").size() > showCount) {
                var CurrentGallery = $(this);
                $(this).children('.wikia-gallery-item').slice(showCount).hide();
                $(this).children('.wikia-gallery-item').last().after($('<center><button class="wikia-button">Показать еще/скрыть</button><br></center>').click(function() {
...
$(function () {
    $('.wikia-gallery-item').each(function () {
        if ($(this).find('.video').length) {
            $(this).find('.lightbox-caption').remove();
        }
    });
});