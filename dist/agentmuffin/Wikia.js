if(document.URL.indexOf(":") !> 0) {
    document.getElementById("WikiaSiteWrapper").style.background="#0064de";
};

if(document.URL.indexOf("Talk") > 0) {
    document.getElementById("WikiaSiteWrapper").style.background="#0064de";
};

if(document.URL.indexOf("Template") > 0) {
...
window.snowStorm = {
	...
	flakesMax: 32,			// Limit total amount of snow made (falling + sticking)
	flakesMaxActive: 32,		// Limit amount of snow falling at once (less = lower CPU use)
	...
	snowColor: '#002040',		// Don't eat (or use?) yellow snow.
	snowCharacter: 'lel',	// &bull; = bullet, &middot; is square on some systems etc.
	...
	useMeltEffect: false,		// When recycling fallen snow (or rarely, when falling), have it "melt" and fade out if browser supports it
	useTwinkleEffect: true,	// Allow snow to randomly "flicker" in and out of view while falling
	...
	flakeWidth: 64,			// Max pixel width reserved for snow element
	flakeHeight: 64,			// Max pixel height reserved for snow element
	vMaxX: 4,			// Maximum X velocity range for snow
