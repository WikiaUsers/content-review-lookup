...
function checkCSSIntegrity() {
	...
	if (!c) {
		var testcontainer = $('<div id="testcontainer" style="position:absolute;top:-1000;left:-1000;"></div>').appendTo(document.body);
		c = wgStyleVersion+';';
		// Testing
		var tester = $('<span class="wikia-gallery-item"></span>').appendTo(testcontainer);
		var propval = tester.css('display');
		tester.remove();
		if (propval == 'inline') {
			c += 'wikia/shared.css;';
		}
		tester = $('<span class="mw-hidden-cats-hidden"></span>').appendTo(testcontainer);
		propval = tester.css('display');
		tester.remove();
		if (propval == 'inline') {
			c += 'common/shared.css;';
		}
		tester = $('<span class="hiddenStructure"></span>').appendTo(testcontainer);
		propval = tester.css('display');
		tester.remove();
		if (propval == 'inline') {
			c += 'monobook/main.css;';
		}
		tester = $('<span class="yui-ac-container"></span>').appendTo(testcontainer);
		propval = tester.css('position');
		tester.remove();
		testcontainer.remove();
		if (propval != 'absolute') {
			c += 'wikia/css/Monobook.css;';
		}
		var d = new Date();
		d.setTime(d.getTime()*1000*60*30); //30 min.
		$UT.cookie('CSSCheck', c, d);
		// Si alguien le interesa, con esto se alerta si algo falla
		if (c.length != wgStyleVersion.length+1 && window.trackCSSIntegrityFailure) {
			trackCSSIntegrityFailure(c);
		}
	}
	var cel = c.split(';');
	if (cel.length > 0 && cel[0] == wgStyleVersion) {
		for (var i = 1; i < cel.length-1; i++) {
			//importStylesheetURI(stylepath+'/'+cel[i]+'?'+wgStyleVersion+'.0');
			// Para que quede encima del Common.js, etc
			$('head').children('link[rel="stylesheet"]').eq(0).append('<link rel="stylesheet" href="'+stylepath+'/'+cel[i]+'?'+wgStyleVersion+'.0"/>')
		}
	} else {
		// Outdated: recheck
		$UT.cookie('CSSCheck', null);
		checkCSSIntegrity();
	}
}
$(function() {
