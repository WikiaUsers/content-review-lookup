...
function makeMap (isTrusted) {
    ...
    var editToolSaveButton = L.Control.extend({
        ...
        onAdd: function (map) {
            ...
            container.onclick = function () {
                mapData = 'Informasi untuk pin dan spidol yang digunakan pada [[User:KettleMeetPot/WorldMap|peta halaman utama]] disimpan di sini.\n<!-- Demarkasi konten halaman untuk parser -->\n' + mapData + '\n<!-- Demarkasi konten halaman untuk parser -->'; 
                var edittoken = mw.user.tokens.get( 'editToken' );
                $('.EditToolCommit').prop("disabled", true);
                $.ajax({
                    type: "POS",
                    url: "https://avatar.fandom.com/api.php",
                    data: { action:'edit', title:'Avatar_Wiki:MainpageMapData', text: mapData, token: edittoken, summary:'Alat pengeditan visual: melakukan perubahan.' },
                    success: function () {
...
function addMarker(event, buttonID, general, manmade, capital, air, aang, water, natural, addMarkerVar, editToolSaveButton) {
    ...
    Marker.addTo(map).bindPopup('<input name="title" placeholder="title" autocomplete="off" id="titleEditPopUp"><textarea id="descriptionEditPopUp" tabindex="5" rows="4" style="resize:none; width:100%;margin-top: 5px" placeholder="Enter description"></textarea><button id="pin-submit" class="set-pin" disabled="disabled">Kirim pin</button>');
...
function editToolPopUps (Marker, buttonID) {
    ...
    Marker.on('click', function () {
        ...
        $("#pin-submit").click(function (){
            ...
            Marker._popup.setContent('Pin disimpan. Silakan tekan tombol "simpan perubahan" untuk melakukan semua pin yang ditambahkan/diubah/dihapus.');
    ...
    eMarker.on('click', function () {
        ...
        $("#editLink").click(function () {
            ...
            $("#pin-submit").click(function (){
                ...
                eMarker._popup.setContent('Pin disimpan. Silakan tekan tombol "simpan perubahan" untuk melakukan semua pin yang ditambahkan/diubah/dihapus.');
                if ( $('.EditToolCommit').prop('disabled') == true ) {
                    $('.EditToolCommit').prop("disabled", false);
                }
            });
...
function markerListControl (markerIcons) {
    var clicked = false;
    var iconNameOrder = ['Umum', 'Buatan manusia', 'Kapital', 'Udara', 'Aang', 'Air', 'Alam'];
    $('.marker-list').append('<div class="listButton" id="intListButton"><div class="topFilterListText">Daftar Filter</div><div class="filterListBody"></div></div>');
    for ( i = 0; i < 7; i++ ) {
...
$(document).ready(function() {
    if (wgPageName == "Avatar_Wiki" || wgPageName == "User:KettleMeetPot/WorldMap")  {
        ...
        $.ajax({
            type: "POST",
            url: "https://avatar.fandom.com/api.php",
            data: { action:'query', prop:'revisions', titles:'Avatar_Indonesia_Wiki:MainpageMapData', rvprop:'content' },
        ...
        $.ajax({
            type: "POST",
            url: "https://avatar.fandom.com/api.php",
            data: { action:'query', prop:'revisions', titles:'Avatar_Indonesia_Wiki:MainpageMapData/CheckPage', rvprop:'content' },
            success: function ( data ) {
