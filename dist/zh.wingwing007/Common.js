{{Template:null}}<br />{{Template:null}}<br />$.multimap = function(f){
...
function staminaSlider(){
    ...
    var t = null, r = function(){
        clearTimeout(t);
        var now = new Date(), eta = new Date(now.getTime() + ($('#staminaSlider input[type=hidden]').eq(1).val()-$('#staminaSlider input[type=hidden]').eq(0).val())*600000), d = Math.max(0,(Date.UTC(eta.getFullYear(),eta.getMonth(),eta.getDate()) - Date.UTC(now.getFullYear(),now.getMonth(),now.getDate()))/86400000);
...
$(function() {
    ...
    $("#WikiaArticleComments").before("<div style='font-weight:bold; font-size:12pt; background: rgba(0,0,0,0.5); border-radius: 5px; padding: 5px; text-shadow: 1px 1px 0px rgba(155,155,155,0.5);'><img style='vertical-align:middle' src='https://images.wikia.nocookie.net/__cb20131122002606/tos/zh/images/thumb/c/c7/Attention.png/45px-Attention.png'> 嚴禁於評論區發佈<span style='color: yellow'>徵友</span>及<span style='color: yellow'>公會招生</span>訊息。違反<a href='http://zh.tos.wikia.com/wiki/Tower_of_Saviors_%E7%BB%B4%E5%9F%BA:%E7%94%A8%E6%88%B6%E5%AE%88%E5%89%87#.E8.A8.8E.E8.AB.96.E5.AE.88.E5.89.87'>用戶守則</a>之用戶將會受到封禁帳號處分。</div>");
    ...
    if ($.isArray(wgUserGroups) && !(0 > $.inArray("autoconfirmed", wgUserGroups))) {
        var a = $('<input type="button" id="ReportButton" style="vertical-align:top;margin-top:3px" value="\u56de\u5831\u9801\u9762\u932f\u8aa4" /><input type="button" id="PurgeButton" style="vertical-align:top;margin-top:3px" value="\u986f\u793a\u6700\u65b0\u9801\u9762" /><span style="float:right;font-weight:bold;">Copyright © 2015 神魔之塔繁中維基 All rights reserved</span>');
...
//Adds the button
if($('.UserProfileActionButton .WikiaMenuElement').exists()) {
    $('.UserProfileActionButton .WikiaMenuElement').after('<a id="Temp" class="wikia-menu-button" onclick="this.innerHTML = \'Added\'; addTemplates();">Add Template</a>');
} else {
    $('.page-header__contribution-buttons').append('<a id="Temp" class="wds-button wds-is-squished wds-is-secondary" onclick="this.innerHTML = \'Added\'; addTemplates();">Add Template</a>');
}


var gnntopnewsnow=1, gnntopnewstimer, gnntopnewstno=0;
function gnnShowtopNews(pagectr){
var topnewsno = 5;
var topimgWidth = egg('.GN-topimg').get(0).clientWidth;
egg('#gnntopnews').css('width', (topimgWidth*topnewsno)+'px');
egg('#topnewstitle').css('width', (topimgWidth*topnewsno)+'px');
pagectr = pagectr || 'N';
if( 'P' == pagectr ){
gnntopnewsnow = (1==gnntopnewsnow) ? topnewsno : gnntopnewsnow-1;
}else{
gnntopnewsnow = (topnewsno==gnntopnewsnow) ? 1 : gnntopnewsnow+1;
}
var leftpos = topimgWidth*(gnntopnewsnow-1);
egg('#gnntopnews').css({'left':-leftpos+'px','WebkitTransition':'1s','transition':'1s'});
egg('#topnewstitle').css({'left':-leftpos+'px','WebkitTransition':'1s','transition':'1s'});
}
function gnnShowtopNewsbar(){
if( 700 > gnntopnewstno ){
gnntopnewstno += 1;
}else{
egg('.GN-topimgBar').html('<i></i><i></i><i></i><i></i><i></i>');
gnntopnewstno = 1;
gnnShowtopNews();
}
egg(egg('.GN-topimgBar i').get(gnntopnewsnow-1)).html('<span style="width:'+Math.ceil(gnntopnewstno/7)+'%"></span>');
gnntopnewstimer = setTimeout("gnnShowtopNewsbar()", 5);
}
function gnnShowtopNewsstop(pagectr){
clearTimeout(gnntopnewstimer);
gnntopnewstno = 0;
egg('.GN-topimgBar').html('<i></i><i></i><i></i><i></i><i></i>');
gnnShowtopNews(pagectr);
gnnShowtopNewsbar();
}
...
wgPageName === '首页' && importScript('MediaWiki:CalendarWidgetNew.js');
/*wgPageName === 'Tt' && importScript('MediaWiki:Test9999.js');*/
