...
// Скопировано с Genshin Impact Wiki [[w:c:genshin-impact:MediaWiki:Common.js]]
...
window.dev.BetterUpload = {
	'default': '==Лицензия==\n{{Fairuse}}',
	redirectFormat: '#redirect [[Файл:%TARGET%]]\n'
};
...
mw.loader.using('oojs-ui-widgets').then(() => { // make sure the PopupWidget library is loaded
	mw.hook('wikipage.content').add((contents)=>{ // hydrate any content inserted
		...
		.each((_, wrapper) => {
			...
			const isEE = $wrapper.hasClass('wuwa-extra-effect-wrapper');
			...
			const $toggle = $('<span>', {
				'class': 'custom-tt toggle-tooltip'+(isEE ? ' wuwa-extra-effect' : ''),
			...
			if (isEE) { popup.$element.find('.oo-ui-popupWidget-popup').toggleClass('wuwa-extra-effect', true); }
