...
/*Troops Sort DIV event define here*/
$('.div_troops>div').click(function(e){ 
 
 $('.div_troops>>div').css({"filter":"grayscale(1)", "opacity":0.65}); 
 $('.div_troops P').css({"opacity":0.65, "color":"black"});

 $($(this).children()[0]).css({"filter":"grayscale(0)", "opacity":1});
 $($(this).children()[1]).css({"opacity":1, "color":"#1342cc"});
 
$('.troop_heroes_list').css('display', 'none');
$('.troop_weaponry_list').css('display', 'none');

switch($(this).attr('class')){
  case 'troops_heroes_tab':
       $('.troop_heroes_list').css("display", "inline-block");
  break;

 case 'troops_weaponry_tab':
       $('.troop_weaponry_list').css("display", "inline-block");
  break;

  case 'troops_helmets_tab':
       //$('.troops_heroes_tab>div').css({"filter":"grayscale(0)", "opacity":1});
  break;

  case 'troops_bodyarmor_tab':
       //$('.troops_helmets_tab>div').css({"filter":"grayscale(0)", "opacity":1});
  break;

  default:
       //$('.troops_accessories_tab>div').css({"filter":"grayscale(0)", "opacity":1});
  break;
}

});

/*arms div toggle action*/
$('.arms_tabs').click(function(e){
 //---- toggle tabs ---
 $('.arms_tabs').css('background-color', '#408dbf');
 $(this).css('background-color', '#fd0000d4');

 $('#troops_gal>div').css('display', 'none');
 
 //---- show/hide galleries ---
switch($(this).text()){
  case 'All':
       $('.troop_div_all').css("display", "inline-block");
  break;

 case 'Infantry':
       $('.troop_div_Sword_Soldier').css("display", "inline-block");
  break;

  case 'Lancers':
       $('.troop_div_gunman').css("display", "inline-block");
  break;

  case 'Cavalry':
       $('.troop_div_cavalry').css("display", "inline-block");
  break;
  
  case 'Fliers':
       $('.troop_div_flying_soldier').css("display", "inline-block");
  break;

  case 'Assassins':
       $('.troop_div_assassin').css("display", "inline-block");
  break;

  case 'Archers':
       $('.troop_div_bow_soldier').css("display", "inline-block");
  break;

  case 'Demons':
       $('.troop_div_monster').css("display", "inline-block");
  break;

  case 'Holy & Mage':
       $('.troop_div_sergeant').css("display", "inline-block");
  break;

  default:
       $('.troop_div_sailor').css("display", "inline-block");
  break;
} 
}); // <<- end function
