keys = [
	['prop', '主屬性', 'select', 'prop'],
	['prop2', '複屬性', 'select', 'prop2'],
...
options = {
	...
	senzai: [
		...
		"複屬性HP上升Ⅰ", "複屬性HP上升Ⅱ", "複屬性HP上升Ⅲ", "複屬性HP上升Ⅳ", "複屬性HP上升Ⅴ", "複屬性HP上升Ⅹ",
		"複屬性攻擊力上升Ⅰ", "複屬性攻擊力上升Ⅱ", "複屬性攻擊力上升Ⅲ", "複屬性攻擊力上升Ⅳ", "複屬性攻擊力上升Ⅴ", "複屬性攻擊力上升Ⅹ",
		"elmts", "breeds",
		"減輕火屬性傷害Ⅰ", "減輕火屬性傷害Ⅱ",
		"減輕水屬性傷害Ⅰ", "減輕水屬性傷害Ⅱ",
		"減輕雷屬性傷害Ⅰ", "減輕雷屬性傷害Ⅱ",
		...
		"快速技能Ⅰ", "快速技能Ⅱ", "快速技能Ⅲ", "快速技能Ⅳ", "快速技能Ⅴ", "快速技能Ⅹ",
		...
		"心眼", "心眼・破", "心眼・絕" , "心眼・怒" , "心眼・逝" ,
		"更換精靈" , "答題重置" ,
		"掉寶率上升Ⅰ" , "減少難易度Ⅰ" , "防禦連鎖Ⅰ" , "提升連鎖Ⅰ" ,
...
evo_material = {
	...
	"闇": [
		{ "id": "disabled", "name": "────────────────────" },
		{ "id": 14774, "name": "黑曜石幻色龍 (SS)" },
	],
	"光": [
		{ "id": "disabled", "name": "────────────────────" },
		{ "id": 14774, "name": "白金剛幻色龍 (SS)" },
	]
};
...
function getCardDataDone(data) {
	...
	lastBreed = (undefined === card_data.breed) ? "" : card_data.breed.toString();
	// For updating evo materials dynamically.
	for (var i = 1; i <= 8; i++) {
	...
	$("#data").html(table_elem).show(500, "swing", function () {
		...
		if($("#rank").val().indexOf("L") == -1)$("#as2, #ss2, #senzaiL_1, #senzaiL_2, #senzaiL_3, #senzaiL_4").closest('tr').hide();
	});
	...
	$("#confirm_btn").on("click", function () {
		...
		// 如果不是L卡，迴避跟L卡相關的輸入欄位
		var key_l = ["as2", "ss2", "senzaiL_1", "senzaiL_2", "senzaiL_3", "senzaiL_4"];

		// 產生該張卡片的資料表單
		var card_text = "{{ Card/Data/{{{data}}}\n|id=" + card_id.toString() + "\n";
		for (var key_index in keys) {
			var key = keys[key_index];
			if((key_l.indexOf(key) != -1) && ($('#rank').val() != "L")) continue;
...
function getCardImage() {
	...
	$.get(mw.util.wikiScript('api'), {
		format: 'json',
		action: 'parse',
		text: decodeURI('%5B%5Bfile:' + fileName + '%7C60px%7Clink= %5D%5D')
	}, function (data) {
...
$(document).ready(function () {
	if (mw.config.values.wgUserName === null) {
		alert('請先登入Wikia');
		location.replace("https://nekowiz.fandom.com/zh/wiki/");
		return false;
	}
