(function($, mw) {
    ...
    function addOasisPurgeButton() {
        $('#DisplayClockJS').before('<a id="purge" class="wikia-button" href="javascript:void(0)" title="Purge page">Purge</a>');
    }
    function addPurgeButton() {
        switch (mw.config.get('skin')) {
            case 'monobook':
                $('#p-cactions > .pBody > ul').append('<li id="ca-purge"><a id="purge" href="javascript:void(0)" title="Purge server cache & refresh page">Purge cache</a></li>');
                break;
            case 'oasis':
            case 'wikia':
                addOasisPurgeButton();
