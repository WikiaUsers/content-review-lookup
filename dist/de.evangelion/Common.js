...
var messageWallUserTags = {
    ...
    'Amextro': 'Administrator',
    'Nekky-chan': 'Administrator',
    // erst Username  :  Text hinter Username
};
 
(function($) {
    for (var name in messageWallUserTags) {
        $('a.subtle[href$="Nachrichtenseite:' + name + '"]').after('<span style="color: white; background: #470000; border-radius: 10px; padding: 2px 5px; font-size: 9pt; font-weight: bold; vertical-align: top;">' + messageWallUserTags[name] + '</span>');
    }
}(jQuery));
...
function createToggle()
{
	...
	for(var i=0; i < document.getElementsByTagName("span").length; i++)
	{
		...
		if(elemClasses[0] == 'Toggler')
		{
			var SpoilerLink= document.createElement("a");
			var attrVar;
			attrVar = elemClasses[0] + elemClasses[1];
			SpoilerLink.setAttribute('id', attrVar);
			SpoilerLink.setAttribute('href', "javascript:clickToggler('" + attrVar + "');");
...
function clickToggler(hideID)
{
	var SpoilerLink= document.getElementById(hideID);
	if (!SpoilerLink) {
		return false;
	}
	
	ToggleClasses('span', hideID);
	ToggleClasses('div', hideID);
}
function ToggleClasses(tag, hideID)
{
	var obj;
	var myFirst;
	myFirst = '1';
	for(var i=0; i < document.getElementsByTagName(tag).length; i++)
	{
		obj= document.getElementsByTagName(tag)[i];
		var elemClasses = obj.className.split(' ');
		if(hideID == 'TogglerMaster')
		{
			if(elemClasses[0] == 'toToggle' && myFirst == '1')
				myFirst = 'unToggle '
			else if(elemClasses[0] == 'unToggle' && myFirst == '1')
				myFirst = 'toToggle '
			if (elemClasses[0] == 'toToggle' || elemClasses[0] == 'unToggle')
				obj.className = myFirst + elemClasses[1];
		}
		else
		{
			if(elemClasses[1] == hideID)
			{
				if(elemClasses[0] == 'toToggle')
					obj.className = 'unToggle ' + elemClasses[1];
				else if(elemClasses[0] == 'unToggle')
					obj.className = 'toToggle ' + elemClasses[1];
			}
			else if(elemClasses[0] == 'unToggle' && elemClasses[1] != hideID)
			{
				obj.className = 'toToggle ' + elemClasses[1];
			}
		}
...
function changeStart()
{
	...
	foo.style.background="url('https://images.wikia.nocookie.net/denaruto3/de/images/d/d5/PediaLogo.png') no-repeat left top";
