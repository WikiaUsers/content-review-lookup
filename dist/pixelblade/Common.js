...
// Add this to MediaWiki:Common.js if you have admin rights
function createDragonRingCalculator() {
    var html = `
        <div class="dragon-ring-calculator" style="border: 2px solid #4CAF50; padding: 15px; margin: 10px 0; background-color: #f9f9f9; border-radius: 8px;">
            <h3>Dragon Ring Calculator</h3>
            <p>
                <label>Quality Level: </label>
                <input type="number" id="dragonQuality" min="1" max="118" value="1" style="padding: 5px;">
                <button onclick="calculateDragonRing()" style="background-color: #4CAF50; color: white; padding: 5px 15px; border: none; border-radius: 4px;">Calculate</button>
            </p>
            <p id="dragonResult" style="font-size: 18px; font-weight: bold;"></p>
        </div>
    `;
    
    document.getElementById('dragon-calculator-container').innerHTML = html;
}

function calculateDragonRing() {
    var quality = document.getElementById('dragonQuality').value;
    var result = -12.100 - (quality * 0.179);
    document.getElementById('dragonResult').innerHTML = 
        'Quality ' + quality + ' → Result: <span style="color: #4CAF50;">' + result.toFixed(3) + '</span>';
}

// Initialize when page loads
$(document).ready(function() {
    if (document.getElementById('dragon-calculator-container')) {
        createDragonRingCalculator();
        calculateDragonRing(); // Show initial result
    }
});
