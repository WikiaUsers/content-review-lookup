...
// Start debugging block
    console.log('L4DWiki_PageName:', wgPageName)
// End debugging block
 ...
 */
/* customized for Left 4 Dead wiki */
...
function fetchUsernameElements () {
  const Elements = document.getElementsByTagName("span")
  const array = []
  for (var i = 0; i < Elements.length; i++) {
    if (hasClass(Elements[i], "current-user-name")) {
      array.push([Elements[i], "name", "User"])
    } else if (hasClass(Elements[i], "current-user-gender")) {
      array.push([Elements[i], "gender", "Unknown gender"])
    } else if (hasClass(Elements[i], "current-user-date")) {
      array.push([Elements[i], "registration", "Anon"])
    }
  }
  return array
}

function renderUserInfo (elements) {
  return function (data) {
    const user = data.query.users[0]
    elements.forEach(function (element) {
      element[0].innerHTML = user[element[1]] || element[2]
    })
  }
}

function fetchUser () {
  const elements = fetchUsernameElements()
  ...
  if (loggedIn(username)) {
    return $.ajax({
      crossDomain: true,
      url: 'https://community.wikia.com/api.php',
      type: 'POST',
      data: {
        action: 'query',
        list: 'users',
      ...
      dataType: 'JSONP',
      success: renderUserInfo(elements),
