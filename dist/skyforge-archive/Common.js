...
// License: CC-BY 3.0
// Version: beta
// Modified By: Sean Weyant (User:XSean)
// Modified Date: 2016-Jul-13
...
var wst = wst || {
    ...
    getTitleFromHref : function( href ) {
        ...
        } else if ( href.indexOf( wst.articlePathSecure ) === 0 ) {
            end = href.substring( wst.articlePathSecure.length );
            result = ( new RegExp( '([^?]*)' ) ).exec( end );
        }
    ...
    getTitlesAndContentLinks : function() {
        var links = $( '#mw-content-text .hoverTool a[href]' );
        var uniqueTitles = {};
        var title;
    ...
    beginLinkSetup : function() {
        wst.articlePath = "https:" + mw.config.get( 'wgServer' ) + mw.config.get( 'wgArticlePath' ).replace( /\$1/, '' );
        wst.articlePathSecure = "https:" + mw.config.get( 'wgServer' ) + mw.config.get( 'wgArticlePath' ).replace( /\$1/, '' );
        wst.fullScriptPath = "https:" + mw.config.get( 'wgServer' ) + mw.config.get( 'wgScript' );
