 ...
 */
/* Mejoras y solución de errores de Jesús Martínez Novo (Ciencia Al Poder) a partir de la versión del 31/03/2007 <pre> */
...
function sortables_init() {
	...
	for (ti=0;ti<tbls.length;ti++) {
		thisTbl = tbls[ti];
		// CAP: modificado. Antes: if ( (' '+thisTbl.className+' ').indexOf("sortable") != -1 ) {
		var cn = ' ' + thisTbl.className + ' ';
		if ( cn.indexOf(' sortable ') != -1 || cn.indexOf(' movnivel ') != -1 || cn.indexOf(' movmtmo ') != -1 || cn.indexOf(' movtutor ') != -1 || cn.indexOf(' movhuevo ') != -1 ) {
			if (!thisTbl.id) {
...
/* CAP: Función mejorada: Si hay imágenes obtiene su 'alt' */
function ts_getInnerText(el){
  if (typeof el == "string") return el;
  if (typeof el == "undefined") return "";
  if (el.nodeType == 1 && el.getElementsByTagName('img').lenght == 0)
...
function ts_resortTable(lnk) {
	...
	// CAP: Agregada condición de salida de seguridad (para columnas vacías)
	for( var i = 1, itm = ""; itm.match(/^([\s]|\n|\&nbsp;|<!--[^-]+-->)*$/); i++) {
		if (i == table.rows.length) break;
		var itm = trim(ts_getInnerText(table.tBodies[0].rows[i].cells[column]));
	}
