// WMU - Wikia Mini Upload
// patched script by User:Ciencia_Al_Poder from Wikia http://images.wikia.com/common/extensions/wikia/WikiaMiniUpload/js/WMU.js
// to support MonoBook. Wikia just boycott this extension to prevent proper function in MonoBook
...
if(mwCustomEditButtons) {
	...
	/*mwInsertEditButton(document.getElementById('toolbar'), {
		"imageFile": wgExtensionsPath + '/wikia/WikiaMiniUpload/images/button_wmu.png',
		"speedTip": wmu_imagebutton,
		"tagOpen": "",
		"tagClose": "",
...
function WMU_show( e, gallery, box, align, thumb, size, caption, link ) {
	...
	// Load Slider if not already loaded (monobook)
	var loader = new YAHOO.util.YUILoader();
	loader.require("slider");
	loader.insert();
...
//                $G( 'ImageSizeRow' ).style.display = 'none';
	...
	$( '#ImageColumnRow' ).css('display', 'none');
//	if( -1 != WMU_gallery ) {
				...
				case 'summary':
					...
					if((WMU_refid == null) || (wgAction == "view") || (wgAction == "purge") ){ // not FCK
						if( -2 == WMU_gallery) {
							...
							// MW1.19 needs textarea focused for insertTags to actually work
							$('#wpTextbox1').focus();
							insertTags($G('ImageUploadTag').value.replace(/_/g, ' '), '', '');
						}
					} else { // FCK
