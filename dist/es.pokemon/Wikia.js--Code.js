...
try {
	if (window.CreaEnlacesDex) {
		CreaEnlacesDex.registerRenderFn(function() {
			...
			$('#LatestPhotosModule').add('#WikiaRail > .loading').eq(0).before('<section class="CreaEnlacesDexModule rail-module" id="CreaEnlacesDexModule"><h2>Enlaces a otras Pokédex</h2><ul></ul></section>');
		});
		CreaEnlacesDex.registerLinkFn(function(url, text, caption) {
			$('#CreaEnlacesDexModule').find('ul').eq(0).append($('<li>').append($('<a class="external">').attr({href:url, title:caption}).text(text)));
		});
