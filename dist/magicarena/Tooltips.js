// This is a copy of
// https://dev.wikia.com/wiki/Tooltips
// with one minor modification, import css from local wiki, not dev
// to make it independant of the dev wiki.

// The tooltip functionality is absolutely _critical_ to this wiki, so just
// in case it gets broken on the dev wiki this local copy provides an
// immediate fallback until the dev wiki code is fixed.
...
var tooltips = {
    ...
    init: function() {
        ...
        if(!tooltips.noCSS) {
            $(importArticle({
                type: 'style',
                article: 'MediaWiki:Tooltips.css'
            })).prependTo('head');
        }
