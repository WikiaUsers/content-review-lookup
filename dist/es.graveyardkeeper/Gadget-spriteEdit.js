...
var i18n = {
	...
	toolbarHelp: 'Help',
	toolbarHelpPage: 'gphelp:User:Majr/Sprite editor',
	...
	toolbarToolNolink: 'Nolink',
	toolbarToolNolinkTip: 'Mark names to always not have a link when used',
	toolbarToolBlack: 'Black',
	toolbarToolBlackTip: 'Mark logos to be inverted to white in night mode',
	toolbarToolDark: 'Dark',
	toolbarToolDarkTip: 'Mark logos to have a white glow in night mode',
	toolbarTools: 'Tools',
	toolbarUndo: 'Undo',
	toolbarDataDeprecate: 'deprecate',
	toolbarDataNolink: 'nolink',
	toolbarDataBlack: 'black',
	toolbarDataDark: 'dark'
};

var DROPDOWN_ACTIONS = ['Deprecate', 'Nolink', 'Black', 'Dark'];
...
var create = function( state ) {
	...
	var saveModules = mw.loader.using( [ 'jquery.byteLimit', 'mediawiki.diff.styles' ] ).fail( console.warn );	
	...
	var enable = function() {
		...
		var nolinkOption = new OO.ui.MenuOptionWidget( {
			data: 'nolink',
			label: i18n.toolbarToolNolink,
			icon: 'flag',
		} );
		nolinkOption.$element.prop( 'title', i18n.toolbarToolNolinkTip );
		
		var blackOption = new OO.ui.MenuOptionWidget( {
			data: 'black',
			label: i18n.toolbarToolBlack,
			icon: 'flag',
		} );
		blackOption.$element.prop( 'title', i18n.toolbarToolBlackTip );
		
		var darkOption = new OO.ui.MenuOptionWidget( {
			data: 'dark',
			label: i18n.toolbarToolDark,
			icon: 'flag',
		} );
		darkOption.$element.prop( 'title', i18n.toolbarToolDarkTip );
		
		toolboxSelect.getMenu().addItems( [ deprecateOption, nolinkOption, blackOption, darkOption ] );
		...
		toolboxSelect.getMenu().on( 'choose', function( item ) {
			...
			switch ( tool ) {
				...
				case 'nolink':
					$doc.on( 'click' + toolNamespace + 'Nolink', '.spritedoc-name > code', function() {
						change( 'toggle nolink', { $elem: $( this ) } );
					} );
				break;
				
				case 'black':
					$doc.on( 'click' + toolNamespace + 'Black', '.spritedoc-name > code', function() {
						change( 'toggle black', { $elem: $( this ) } );
					} );
				break;
				
				case 'dark':
					$doc.on( 'click' + toolNamespace + 'Darl', '.spritedoc-name > code', function() {
						change( 'toggle dark', { $elem: $( this ) } );
					} );
				break;
			}
		} );
			...
			getObject: function() {
				...
				sheet.updatePositions().done( function() {
						...
						$section.find( '.spritedoc-box' ).each( function() {
							...
							$box.find( '.spritedoc-name' ).find( 'code' ).each( function() {
								...
								ids.push( {
									...
									nolink: $this.hasClass( 'spritedoc-nolink'),
									black: $this.hasClass('spritedoc-black'),
									dark: $this.hasClass('spritedoc-dark')
								} );
							} );
						} );
					} );
					...
					$.each( ids, function() {
						...
						if (this.nolink) {
							idData[i18n.luaKeyNolink] = this.nolink;
						}
						if (this.black) {
							idData[i18n.luaKeyBlack] = this.black;
						}
						if (this.dark) {
							idData[i18n.luaKeyDark] = this.dark;
						}
	...
	var change = ( function() {
		...
		var func = function( action, content, queueChange, oldChange ) {
			...
			switch ( action ) {
				...
				case 'toggle nolink':
					content.$elem.toggleClass( 'spritedoc-nolink' );
					
					names.invalidate( true );
				break;
				
				case 'toggle black':
					content.$elem.toggleClass( 'spritedoc-black' );
					
					names.invalidate( true );
				break;
				
				case 'toggle dark':
					content.$elem.toggleClass( 'spritedoc-dark' );
	...
	var revert = function( hist ) {
		...
		while ( i-- ) {
			...
			switch( histChange.action ) {
				...
				case 'toggle nolink':
					change( 'toggle nolink', content, false, true );
				break;
				
				case 'toggle black':
					change( 'toggle black', content, false, true );
				break;
				
				case 'toggle dark':
					change( 'toggle dark', content, false, true );
				break;
			}
		}
