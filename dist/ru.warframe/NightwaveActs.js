 ...
 */
const WIKI_IMG_URL = "https://vignette.wikia.nocookie.net/warframe/ru/images/";
...
const API_URL = "https://api.warframestat.us/pc/nightwave?language=" + "ru"; // + config.wgUserLanguage;
// Scaled down reputation icon
const REP_IMG_URL = WIKI_IMG_URL + "e/ed/Очки_Синдиката_иконка_вики.png";
...
if (WHITELIST_PAGES.includes(NW_PAGE_NAME)) {
	...
	mw.hook("dev.i18n").add(function (i18n) {
		...
		// Loading messages
		i18n.loadMessages(I18N_MESSAGES,{noCache:true}).done(function (i18n) {
		...
		actDataJSON.activeChallenges.forEach(function (actJSON) {
			...
			switch (actType) {
				case ActTypeEnum.DAILY:
					...
					actImgURL = WIKI_IMG_URL + jsonImgMap.daily[camelCase(actJSON.title)];
					break;

				case ActTypeEnum.WEEKLY:
					tabIDAttr = "nightwave_weekly";
					rowIDAttr = "weekly_acts";
					actImgURL = WIKI_IMG_URL + jsonImgMap.weekly[camelCase(actJSON.title)];
					break;

				case ActTypeEnum.ELITE:
					tabIDAttr = "nightwave_elite";
					rowIDAttr = "elite_acts";
					actImgURL = WIKI_IMG_URL + jsonImgMap.elite[camelCase(actJSON.title)];
					break;
			}
...
//заменяет первые буквы строки на заглавные, взято с просторов интернета
function camelCase(str) {
	return str.replace(/(^|\s)\S/g, function(a) {return a.toUpperCase()});
}

/*возвращает перевод названия или описания деяния

если передано название деяния title и описание деяния desc то возваращает текст
с файла i18n.json, с поля "Название Деяния/Описание",

если же передано только название деяния title то возваращает текст
с файла i18n.json, с поля "Название Деяния"
*/
function translate(desc, title, i18n) {
	var res;
	if ( (desc) && (desc.search(/^\w/) > -1) ) 
		 res = i18n.msg(camelCase(title)+'/Описание').plain();
	else if (!desc) {
		if (title.search(/^\w/) > -1)
			res = i18n.msg(camelCase(title)).plain();
		else
			res = camelCase(title);
		}
	else
		...
		.find(".emodtable").append($("<tr>", {
			append: [
				$("<td>", {
					append: [
						$("<div>", {
							class: "center floatnone invertableIcon",
							append: [
								$("<a>", {
									href: actImgURL,
				...
				$("<td>", {
					append: [
						$("<b>").text(translate(null, actJSON.title, i18n)),
						$("<br>"),
						$("<i>").text(translate(actJSON.desc,actJSON.title, i18n))
					]
				}),
				$("<td>", {
					append: [
						...
						$("<span>", {
							...
							class: "invertableIcon base-tooltip tooltips-init-complete",
							append: [
							$("<a>", {
						...
						}).attr("data-param", "Очки Синдиката"),
