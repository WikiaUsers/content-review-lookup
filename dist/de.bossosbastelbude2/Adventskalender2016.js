/* Adventskalender */
if (mediaWiki.config.get('wgPageName') === "Bossos_Bastelbude_Wiki:Adventskalender_2016") {
	...
	$(document).ready(function() {
		...
		var cookie = jQuery.parseJSON(getCookie("advent"));
			...
			$('.advent .item').each(function(index) {
				...
				if (((currentmonth == 10 && currentday >= day) ||  currentyear > 2016) && opened == 0) {
		...
		$('.advent .item').on('click',function() {
			...
			if ((currentmonth == 10 && currentday >= day) ||  currentyear > 2016) {
				$('img',this).data('src',image);
				$('img',this).attr('src',image);
				$('img',this).wrap('<a href="' + link + '">');
				$(this).data('opened','1');
