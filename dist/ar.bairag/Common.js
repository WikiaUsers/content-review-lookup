  ...
  function subNavLevel(strNav, strReturnLink) {
    ...
    switch (strReturnLink) {
      case 'findlink': {
        ...
        return 'a[href="' + check_url($.escapeSelector(strNav[0])) + '"] + .subnav-' + (idx + 1);
      }
 
      case 'fulllink':
        return '<a href="' + check_url(mw.html.escape(strNav[0])) + '">' + mw.html.escape(strNav[1]) + '</a>';
 
      case 'fullsublink':
        return '<a href="' + check_url(mw.html.escape(strNav[0])) + '" class="subnav-' + idx + 'a">' +
...
function generate_card_price_calculator() {
    ...
    html += '<select>';
        html += '<option value="' + prices.common + '">عادية</option>';
        html += '<option value="' + prices.rare + '">نادرة</option>';
        html += '<option value="' + prices.epic + '">رهيبة</option>';
        html += '<option value="' + prices.legendary + '">أسطورية</option>';
    html += '</select>';
...
function check_url(input_url) {
    // if (input_url.indexOf('ar.bairag.wikia.com') != -1 || input_url.indexOf('/') == 0) {
    //     if (input_url.indexOf('?') == -1) {
    //         url = input_url + '?uselang=ar';
    //     } else {
    //         var query_string = input_url.split('?')[1];
 
    //         if (query_string == '') {
    //             url = input_url + 'uselang=ar';
    //         } else {
    //             var elements = query_string.split('&');
 
    //             if (elements.indexOf('uselang=ar') == -1) {
    //                 url = input_url + '&uselang=ar';
    //             }
    //         }
    //     }
