...
function copyCite(ID) {
    var msg = getByXPath("//div[@id='cite-button-"+ ID +"']/preceding-sibling::div[@class='forum-message-text']")[0].innerHTML; //gets parameter {{{сообщение}}} from Шаблон:Форум/Ответ
    msg = cleanUpHTML(msg); //it was in HTML, but we prefer wikitext
 
    var author = getByXPath("//div[@id='cite-button-"+ ID +"']/preceding-sibling::div[@class='forum-theme-author-nickname']")[0].innerText; //gets parameter {{{автор}}} from Шаблон:Форум/Ответ
...
function addOnclickСitation() {
    if (wgNamespaceNumber == 110) { //Т. е. если пространство имён Форум: / for Forum: namespace only
        citeButtons = getByXPath("//div[@class='cite-button']"); //this class is used only in Шаблон:Форум/Ответ
