...
Description: A lightweight plugin to support highlighting in imagemap extension
Taken from: https://he.wikipedia.org/wiki/מדיה_ויקי:Imagemap-Highlight.js

Cedido por Fire Emblem Wiki en inglés
*/
$(document).ready(function() {
    ...
    /*	add this class to all elements created by the script. the reason is that we call the script again on
        ...
        hilightDivMarker = '.mapHilight'; //CLASS TO HIGHLIGHT IMAGEMAP
    ...
    function mouseAction(e) {
        ...
        if (li && activate) {
            ...
            if (top < 0 || top + height > ol.height())
                ol.animate({
                    scrollTop: ol.scrollTop() + top - ol.height() / 2
                });
        }
    ...
    function pageOfHref(href, cssClass) {
        ...
        page = page.replace(/#(.*)/, function(toReplace) {
            return toReplace.replace(/\.([\dA-F]{2})/g, '%$1');
        });
    ...
    function init() {
        ...
        $(hilightDivMarker + ' img').each(function() {
            ...
            var context = $.extend(jcanvas[0].getContext("2d"), highlightStyle);
            ...
            $('area', map).each(function() {
                ...
                if (!li) { //no? create a new one.
                    ...
                    lis[text] = li = $('<li>', {
                        ...
                        .append($('<a>', {
                            href: href,
                            title: pageOfHref(href, cssClass),
                            text: text,
                            'class': cssClass + ' internal'
                        }))
    ...
    //has at least one "hilight" div, and canvas-capable browser:
