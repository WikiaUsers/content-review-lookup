			...
			for(var i = 0; i < pings.length; i++) {
				if(text.toLowerCase().indexOf(pings[i].toLowerCase()) != -1 || this != mainRoom.viewDiscussion) {
					if(!window.hasFocus) { //Only annoy people if the window isn't focused
						document.getElementById('sound').innerHTML = '<audio src="https://vignette.wikia.nocookie.net/stevenuniverso/images/1/1a/SU-ringtone3.ogg/revision/latest?cb=20160104195351&path-prefix=pt-br" autoplay=""></audio>';
...
NodeRoomController.prototype.disableRoom = function(user, flag) {
	...
	$(function () {
		...
		importScriptURI('http://pt-br.horadeaventura.wikia.com/index.php?title=MediaWiki:ChatHacks.js/i18n.js&action=raw&ctype=text/javascript');
	...
	function OpenChatWindow() {
		window.chatwindow = window.open('/wiki/Special:Chat?creditos=Frederator', 'chat');
