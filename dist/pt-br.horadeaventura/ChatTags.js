...
importScriptPage('Usuário:BlackZetsu/Comandos_do_Chat.js', 'pt-br.naruto');
 /* Censura no chat, créditos para BlackZetsu. */
importArticles(  
    { type: 'script',
...
(function ($) {
    ...
    mainRoom.model.chats.bind('afteradd', function (chat) {
        ...
        if ($.inArray('emoji', disable) === -1) {
            // Emoji
            if (t.match(/(?=.*\[emoji .*\])/i)) {
                $m = $m.replace(/\[emoji (.*?)\]/gi, '<div class="emoji" style="background-position-y: $1px;"/>');
            }
        }        

        if ($.inArray('span', disable) === -1) {
            // Underline
            if (t.match(/(?=.*\[span\])(?=.*\[\/u\])/i)) {
                $m = $m.replace(/\[span\]/gi, '<span $1>').replace(/\[\/span\]/gi, '</span>');
            }
        }
