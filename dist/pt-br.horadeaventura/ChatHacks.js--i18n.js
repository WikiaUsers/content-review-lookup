...
switch(wgUserLanguage) {
	...
	default:
		var i18n = {
			...
			'afk': 'Ausente',
			'away': 'Você está ausente',
			'back': 'Você está On-line',
			'clear': 'Limpar',
			'cleared': 'Chat foi limpo com sucesso',
			'commands': 'Commands: $1',
			...
			'id': 'id',
			'init': 'Chat hacks initialized.\nPara reportar erros ou bugs, ou para sugestões  de recursos outraduções, envie uma mensagem para o usuário <a href="http://community.wikia.com/wiki/User_talk:Monchoman45">Monchoman45</a>.',
			'nobodycares': 'Nobody cares',
			'nohelp': 'No help data found for /$1',
			'notbanned': '$1 is not banned.',
			'notmod': '$1 is not a mod.',
			'pingphrases': 'PING',
			'pingphrasestooltip': 'De acordo com o nome que você colocar na caixa, qualquer um pode ativar o "bip".',
			'subcoms': 'Subcommands',
...
$('#Write').append('<a class="wikia-button awaybutton" href="javascript:toggleAway()">' + i18n['afk'] + '</a><a class="wikia-button limparjanelabutton" href="javascript:active().clearWindow()">' + i18n['clear'] + '</a>');
$('#ChatHeader .wordmark').append('<span id="pingspan"><div class="cping">' + i18n['pingphrases'] + ' <span title="' + i18n['pingphrasestooltip'] + '">(?)</span></div><textarea id="pings" style="display:none;">' + ((readCookie('pingphrases')) ? readCookie('pingphrases').split('\\n').join('\n') : wgUserName) + '</textarea></span>');
