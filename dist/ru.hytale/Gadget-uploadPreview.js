...
'<div style="border:1.3px solid red;padding:.5em">' +
'<div class="error">' +
'Вы замените уже существующий файл.' +
'Если вы продолжите, информация в этой форме не будет отображаться на странице описания.' +
'</div>' +
'</div>'
);
...
'<h2 id="mw-previewheader">Предпросмотр</h2>\n' +
'<div class="previewnote">\n' +
'{{int:previewnote}}\n' +
'<hr>\n' +
'</div>\n' +
...
action: 'parse',
text: text,
title: ( destFilenameText ? 'Файл:' + destFilenameText : '' ),
...
'<a href="/Служебная:Категории" title="Служебная:Категории">Категории</a>: <ul>';

for (var i = 0; i < categories.length; i++) {
var catName = categories[i]['*'].replace(/_/g, ' ')
  , catNameEncoded = encodeURIComponent(categories[i]['*'].replace(/ /g, '_'));
catBar += '<li><a href="/Категория:' + catNameEncoded + '" title="Категория:' + catNameEncoded + '">' + catName + '</a></li>';
}
...
if ( $('#wpDestFile-warning > p').text().startsWith('Файл с таким именем уже существует') )
return true;
...
preview.innerHTML = '<div class="error">Ошибка. Не удалось включить предпросмотр.</div>';
}
...
var previewButton = document.createElement('input[type="button"]');
...
previewButton.style     = 'margin-left:.5em';
