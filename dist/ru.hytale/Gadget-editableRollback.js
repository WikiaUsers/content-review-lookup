...
$( '.mw-rollback-link > a' ).after( $( '<span>' ).addClass( 'edit-rollback' ).prop( 'title', 'Описание отката правки' ) );
	...
	if ( $( this ).parent().is( $rollback.parent() ) ) {
		...
		$rollback = $( '<div id="rollback-summary">' ).append(
			$( '<input type="text">' ).addClass( 'mw-ui-input rollback-text' ).prop( { maxlength: 250, spellcheck: true } ).val(
				'Откат правок [[Special:Contribs/' + name + '|' + name + ']] ([[User talk:' + name + '|обс.]])'
			),
			$( '<input type="button">' ).addClass( 'mw-ui-button mw-ui-constructive rollback-submit-button' ).val( 'Откат' )
		).insertAfter( this );
	}
