function testtesttest() {
   var nocountdowns = getElementsByClassName(document, 'span', 'test1');
   for(var i in nocountdowns){ nocountdowns[i].style.background = 'white';
   nocountdowns[i].style.color = 'black';}
}

function updatetimerq(i) {
   var now = new Date();
   ...
   timeouts[i] = setTimeout('updatetimerq(' + i + ')', 1000 * 60 * 60 * 24);
 }
 
 function checktimersq() {
   //Скрыть 'nocountdown' и показать 'countdown'
   var nocountdowns = getElementsByClassName(document, 'span', 'nocountdownONIq');
   for(var i in nocountdowns) nocountdowns[i].style.display = 'none'
   var countdowns = getElementsByClassName(document, 'span', 'countdownONIq');
   for(var i in countdowns) countdowns[i].style.display = 'inline'
 
   timers = getElementsByClassName(document, 'span', 'countdowndateONIq');  
   timeouts = new Array();
