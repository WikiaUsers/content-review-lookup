/* Dowolny kod JavaScript z tej strony będzie załadowany przez wszystkich użytkowników. */
...
$(function() { // wait for content load (DOMContentLoaded)
  $('.copy-clipboard').each(function () {
    var $this = $(this);
    var $button = $('<button title="Skopiuj do schowka">&#xf0ea;</button>');
    $this.append($button);
    $button.click(function () {
      ...
      try {
        if (!document.execCommand('copy'))
          throw 42;
        mw.notify('Pomyślne skopiowano do schowka.');
      } catch (err) {
        mw.notify('Nie udało się skopiować do schowka. Zrób to sam.', {type:'error'});
      }
    });
  });
});
...
mw.loader.using( ['mediawiki.util', 'jquery.client'], function () {
  ...
  gridFilters = {
	...
	'map': [ '- Mapa -',
		['Wyspa','Wyspa'],
		['Spalona Ziemia','Spalona Ziemia'],
		['The Center','The Center'],
		['Ragnarok','Ragnarok'],
		['Aberracja','Aberracja'],
		['Wyginięcie','Wyginięcie'],
		['Brakujące','Brakujące'],
		['Niewydane','Niewydane'],
	],
	'group': [ '- Grupa -',
		['Drapieżnicy Alfa','Drapieżnicy Alfa'],
	...
	'diet': [ '- Dieta -',
		['Mięsożercze','Mięsożercy'],
		['Rybożercze','Rybożercy'],
		['Roślinożercze','Roślinożercy'],
		['Wszystkożercze','Wszystkożercy'],
		['Padlinożercze','Padlinożercy'],
