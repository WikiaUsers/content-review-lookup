//Автор: GoodLuck
var Just_Cause_4_вики = (function(window) {
	'use strict';
	if (window === null) {
	...
	return This = {
		...
		Mainpage: {
			cache: {},
			selected: null,
			ajaxThrobberURL: 'https://vignette.wikia.nocookie.net/just-cause-4/images/6/62/%D0%98%D0%B4%D1%91%D1%82_%D0%B7%D0%B0%D0%B3%D1%80%D1%83%D0%B7%D0%BA%D0%B0.gif/revision/latest?cb=20190212081151&path-prefix=ru',
			...
			Avatar: function($elem) {
				...
				$.get('/ru/api/v1/User/Details', {
					...
					$elem.append('<img alt="avatar" class="Avatar" width="' + avatarsize + '" height="' + avatarsize + '" src="' + (function() {
						...
						return 'https://vignette.wikia.nocookie.net/just-cause-4/images/1/19/Avatar.jpg/revision/latest?cb=20190212182732&path-prefix=ru';
					})() + '"/>');
				});
			},
			Calcs: function($elem) {
			...
			MobileExport: function($elem) {
				...
				$.get('/ru/index.php', {
					...
					setTimeout(function append() {
						$elem.html('<div style="clear: both;"><img src="//content.just-cause-4.com/MobileExport' + data[i++].replace(/\\/g, '/') + '" style="max-height: 250px; width: auto;" /></div>');
						if (i < data.length) {
		...
		Just_Cause_4_WikiBuilder: {
			init: function() {
				if (wgPageName !== 'Служебная:Just_Cause_4_WikiBuilder') {
					return;
				}
			}
		},
		RelicsDatabase: {
			...
			DBMainpage: 'Участник:Shadow of the Corporation eDEN/DB',
			init: function($init) {
				var that = this;
