/* original: https://dev.fandom.com/wiki/MediaWiki:ChatLinkPreview.js
modified to allow embedding videos from sites not supported by Fandom's OpenGraph endpoint
*/
...
(function() {
    ...
    window.ChatLinkPreview = $.extend({
        ...
        videoRegex: {
        	vlive: /^https?:\/\/(?:www|m)\.vlive\.tv\/video\/(\d{0,})/i,
        	bitchute: /^https?:\/\/www\.bitchute\.com\/video\/([^"&?\/ ]{10,13})/i
        },
        ...
        buildMedia: function(og, thumb) {
            if (og.videoSecureUrl && og.videoType == 'text/html') {
            	return this.buildVideo(og);
            } else {
            	// support video URLs not supported by FANDOM's OpenGraph endpoint
            	if (og.url.match(this.videoRegex.vlive)) { // vlive.tv
            		var match = og.url.match(this.videoRegex.vlive),
            			videoId = match[1],
            			videoURL = 'https://www.vlive.tv/embed/'+ videoId +'?autoPlay=true';
            		og.videoSecureUrl = videoURL;
            		return this.buildVideo(og);
            	} else if (og.url.match(this.videoRegex.bitchute)) { // bitchute.com
            		var match = og.url.match(this.videoRegex.bitchute),
            			videoId = match[1],
            			videoURL = 'https://www.bitchute.com/embed/' + videoId;
            		og.videoSecureUrl = videoURL;
            		return this.buildVideo(og);
            	}
            }
