...
window.HotCat = {
  ...
  messages :
    { cat_removed  : 'removed [[Category:$1]]'
     ...
     ,cat_added    : 'agregar [[Categoría:$1]]'
     ,cat_keychange: 'nueva clave para [[Categoría:$1]]: "$2"' // $2 is the new key
     ,cat_notFound : 'La categoría "$1" no se encuentra'
     ,cat_exists   : 'La categoría "$1" ya existe; no se agregó.'
     ,cat_resolved : ' (redirección [[Categoría:$1]] resuelto)'
     ,uncat_removed: '{{Categorízame}} removido'
     ,separator    : '; '
     ,prefix       : ""
        // Some text to prefix to the edit summary.
     ,using        : ' usando [[Ayuda:Gadget-HotCat|HotCat]]'
        // Some text to append to the edit summary. Named 'using' for historical reasons. If you prefer
        // to have a marker at the front, use prefix and set this to the empty string.
     ,multi_change : '$1 categorías'
     ...
     ,cancel       : 'Cancelar'
        // Button text. Localize to wgContentLanguage here; localize to wgUserLanguage in a subpage,
        // see localization hook below.
     ,multi_error  : 'No se pudo recuperar el texto de la página del servidor. Por lo tanto, los cambios '
                    +'no se pudieron guardar. Pedimos disculpas por los inconvenientes.'
 ...
 ,tooltips : {
    change:  'Modificar'
   ,remove:  'Remover'
   ,add:     'Agregar una nueva categoría'
   ,restore: 'Rehacer cambios'
   ,undo:    'Deshacer cambios'
   ,down:    'Abierto para la modificación y visualización de subcategorías'
   ,up:      'Abierto para modificar y visualizar categorias principales'
  }
   // The tooltips for the above links
 ,addmulti           : '<span>+<sup>+</sup></span>'
   // The HTML content of the "enter multi-mode" link at the front.
 ,multi_tooltip      : 'Modificación de categorías'
...
(function () { // Local scope to avoid polluting the global namespace with declarations
  ...
  function resolveOne (page, toResolve) {
    ...
    for (var j = 0; j < toResolve.length; j++) {
      if (toResolve[j].dabInputCleaned != page.title.substring (page.title.indexOf (':') + 1)) continue;
      toResolve[j].currentHidden = is_hidden;
      toResolve[j].inputExists = !is_missing;
      toResolve[j].icon.src = armorUri(is_missing ? HotCat.existsNo : HotCat.existsYes);
    }
    ...
    for (var j = 0; j < toResolve.length; j++) {
      if (toResolve[j].dabInputCleaned != page.title.substring (page.title.indexOf (':') + 1)) continue;
      toResolve[j].inputExists = true; // Might actually be wrong if it's a redirect pointing to a non-existing category
