...
(function (ns) {
    ...
    function api() {
        ...
        console.log("fire()                            // fires Railgun *without* modules package");
        console.log("getComputedStyle(jQuery, style)   // retrieves computed style of an element");
        console.log("getItem(key)                      // returns a value from storage");
        console.log("info()                            // prints framework, config and storage data");
        console.log("init([anything])                  // fires Railgun *with* modules package");
    ...
    /**
     * Retrieves the computed style of a DOM element, cross-browser support.
     * @methodOf Railgun
     * @param {jQuery} jQueryElem a jQuery object referencing an element in the DOM
     * @param {String} style the style name to be retrieved
     * @returns the requested style, or all CSS declarations if style is undefined
     */
    function getComputedStyle(jQueryElem, style) {
        var computedStyle = {};
        var elem = jQueryElem[0];
        if ('function' == typeof window.getComputedStyle) {
            // chrome, firefox
            computedStyle = window.getComputedStyle(elem);
        } else if ('undefined' != typeof elem.currentStyle) {
            // internet explorer
            computedStyle = elem.currentStyle;
        } else if ('undefined' != typeof document.defaultView
                && 'function' == typeof document.defaultView.getComputedStyle){
            // fallback
    ...
    /**
     * Fires Railgun's main initialization sequence (after the modules package has loaded).
     * Attaches stylesheets to the document and calls Railgun.Storage.init().
     * @methodOf Railgun
     * @see Railgun.Storage.init()
     */
    function fire() {
        try {
            // check if this page has #WikiaRail
            if (0 === $('#WikiaRail').length) {
     ...
     */
    function init() {
        // load module package and then initialize Railgun on pageload
    ...
    ns.api = api;
    ns.info = info;
    ns.getComputedStyle = getComputedStyle;
...
(function (ns) {
    ...
    function init() {
        ...
        $('.WikiaFooter .toolbar').prepend(leftArrow + rightArrow);
...
(function (ns) {
    ...
    function anchor_clickHandlerAnchor() {
        ...
        if ("null" !== typeof anchor_moduleId) {
    ...
    function anchor_clickHandlerDetach() {
        ...
        $(this).parent().parent().removeClass('railgun-module-anchor-topright').removeAttr('style');
