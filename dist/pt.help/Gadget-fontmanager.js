			...
			for (f in fonts){
				...
				if (font.main != undefined) {
					...
					if ($m.find("div.hiddenFamily").length==0) {
						...
						$m.children('.hiddenFamily').wrapAll('<div class="mw-collapsible-content" />');
					}
					...
					$f.children('.font_preview,.font_path,.font_css,.font_controls,.toggleInstructions').wrapAll('<div class="mw-collapsible-content" />');
