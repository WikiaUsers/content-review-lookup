/* 
 ****
 * Forked from ChatHacks.js, by Monchoman45.
 * Version 7.0
 ****
 * Fork by Scarfacial
 * - Changed chat sound
 * - Ping phrases are now Regex (Maybe)
 ****
 * Features:
...
if(wgCanonicalSpecialPageName == 'Chat') {
	...
	//Function to match for a ping
    function MatchPingPhrase( patternText , text ) {
		var match;
		try { 
			match = new RegExp("(\\W|^)"+ patternText +"(\\W|$)", 'i').test(text);
		} catch(e) {
			console.log("Invalid RegEx!", e.message)
			match = text.toLowerCase().indexOf(patternText.toLowerCase()) != -1;
		}
		return match;
	}
	...
	NodeChatDiscussion.prototype.iconPing = function (chat) {
		...
		for(var i in this.model.users.models) {
			if(this.model.users.models[i].attributes.name == chat.attributes.name) {
				if(this.model.users.models[i].attributes.isStaff) { // Wikia staff
					icon = ' <img class="stafficon" src="https://images.wikia.nocookie.net/monchbox/images/f/f3/Icon-staff.png">';
				}
				else if(this.model.users.models[i].attributes.isCanGiveChatMod) { // Admin
					icon = ' <img class="modicon" src="https://images.wikia.nocookie.net/survival303/images/7/7d/AdminIcon.png">';
				}
				else if(this.model.users.models[i].attributes.isModerator) { // Mod
					icon = ' <img class="modicon" src="https://images.wikia.nocookie.net/survival303/images/8/89/ModIcon.png">';
				}
				break;
			}
		}
		if(icon) {this.chatUL.children().last().children('.username').html(icon + this.chatUL.children().last().children('.username').html());}
...
//		if(mainRoom.isInitialized && chat.attributes.name != wgUserName && !chat.attributes.isInlineAlert) {
		if(mainRoom.isInitialized && !chat.attributes.isInlineAlert) {
			...
			for(var i = 0; i < pings.length; i++) {
				if(MatchPingPhrase(pings[i], text) || this != mainRoom.viewDiscussion) {
					if(!window.hasFocus) { //Only annoy people if the window isn't focused
						document.getElementById('sound').innerHTML = '<audio src="' + document.getElementById('pingsound').value +'" autoplay=""></audio>';
	...
	$(function () {
		...
		active().viewDiscussion.chatUL.html('');
		active().inlineAlert('Welcome to the Survival 303 Wiki Chat');

		$('#ChatHeader .wordmark').append('<span id="pingsoundspan"><div>Ping sound</div><input type="text" id="pingsound" style="display:none;"></input></span>');

		var pingSoundInput = document.getElementById('pingsound');
		var pingSoundSpam = document.getElementById('pingsoundspan')

		pingSoundInput.value = ((readCookie('pingsound')) ? readCookie('pingsound') : 'https://images.wikia.nocookie.net/survival303/images/b/bc/Chat.ogg')
		pingSoundSpam.onmouseover = function(event) {pingSoundInput.style.display = 'block';}
		pingSoundSpam.onmouseout = function(event) {pingSoundInput.style.display = 'none'; createCookie('pingsound', pingSoundInput.value, 99999);}
	});
}
else {
	$(function () {
