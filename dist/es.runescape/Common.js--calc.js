/* <pre> */
importScriptPage('User:Stewbasic/calc.js', 'runescape');
...
$('.jcConfig').each(function() {
	...
	$form = $('<form />').attr({'action': '#', 'id': 'jcForm' + formId}).submit(function() {
		function dispResult(response) {
			$('#' + formId + ' .jcSubmit input').val('Subir').removeAttr('disabled');
		...
		function loadTemplate() {
			code += '}}';
			$('#' + formId + ' .jcSubmit input').val('Cargando...').attr('disabled', 'disabled');
		...
		if (formError) {
			showError('Uno o más campos contienen valores inválidos.');
		} else {
			if (hs.length > 0) {
				i = 0;
					    	...
					    	if( data.results[0] ) {
					            ...
					            loadTemplate();
						   		...
						   		if (tplParams[i].range != 'yes') {
						   	    	showError("El jugador '" + name + "' no existe, está banneado o es no miembro. Por favor, introduzca la data manualmente.");
						   		} else {
						   			showError("El jugador '" + name + "' no existe, está banneado o es no miembro. Por favor, introduzca un nombre de cuenta válido.");
					    		}
					    	}
					    },
					    error: function(xhr, error) {
	...
	$td.append($('<input />').attr({type: 'submit', value: 'Subir'}));
	$tr.append($td);
	$table.append($tr);
