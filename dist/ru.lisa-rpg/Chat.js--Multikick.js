function showPopup() {
        $('body').append('<section style="left: 50%; top: 50px; width: 434px; z-index: 2000000002; margin-left: -222px;" class="modalWrapper" id="masskicker"><button class="close wikia-chiclet-button" onclick="cancelKick()"><img src="https://images.wikia.nocookie.net/__cb57523/common/skins/oasis/images/icon_close.png"></button><h1>М-м-мультикик!</h1><section class="modalContent"><div><form method="" name="" class="WikiaForm "><fieldset><p>Вставьте сюда никнеймы нескольких пользователей через запятые и пробелы.</p><input type="text" name="multikickinput" id="multikickinput" /></fieldset></form><div style="float:right;"><a onclick="kickUsers()" class="wikia-button">Кикнуть</a> <a onclick="cancelKick()" id="cancel" class="wikia-button secondary">Закрыть</a></div></section></section><div style="height: 100%; width: 100%; z-index: 2000000001; opacity: 0.65; display: block;" data-opacity="0.65" class="blackout"></div>');
}
 
function cancelKick() {
	$('#masskicker','.blackout').remove();
}
...
$('<a id= "multiKickerButton" class="wikia-button" href="javascript:showPopup()" style="position:absolute; right:55px; top:22px;">Мультикик</a>').appendTo('.Write');
