...
(function(window, $, mw, config) {
	...
	if ((namespace & 1) !== 1) {
		if (namespace !== mw.config.get('wgNamespaceIds').forum) {
			return;
		}
		haveForum = true;
	}
	...
	config = $.extend({
		...
		preamble: 'Es gibt ein paar Probleme mit deiner Bearbeitung:\n\n',
		epilogue: '\nBist du sicher, dass du die Seite trotzdem abspeichern möchtest?\n[Ein Nichtbefolgen der Regeln könnte die Administratoren auf dich aufmerksam machen]',
		noForumheader: '!! Du hast keine überschrift für dein Forums-Beitrag gewählt. Ohne Überschrift wird dein Beitrag womöglich nicht in der Liste angezeigt.\n',
		noSignature: '?? Sieht so aus, als hättest du vergessen, mit deiner Signatur zu unterschreiben. Dafür musst du einfach nur die vier Tilde (~) hinter deinen Beitrag setzen.\n',
