/*MODIFIED VERSION OF User:Monchoman45 's Chat Hacks. Ping Snooze Added.*/
...
if(wgCanonicalSpecialPageName == 'Chat') {
		...
		if(mainRoom.isInitialized && chat.attributes.name != wgUserName && !chat.attributes.isInlineAlert) {
			...
			for(var i = 0; i < pings.length; i++) {
				if(text.toLowerCase().indexOf(pings[i].toLowerCase()) != -1 || this != mainRoom.viewDiscussion) {
					if(!window.hasFocus && snoozed===false) { //Only annoy people if the window isn't focused
	...
	window.commands = {
		...
		'shrug': function(com, text) { //Shrug
			$('#Write [name="message"]').val(' ¯\\_(ツ)_/¯');
			return true;
		},
		'ooo': function(com, text) { //OOOOO
			$('#Write [name="message"]').val('OOOOO! https://www.youtube.com/watch?v=aZHvd0ks7Es');
			return true;
		},
		'kick': 'kickban',
		'ban': 'kickban',
