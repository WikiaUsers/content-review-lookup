...
$('.jcConfig').each(function () {
    ...
    $form = $('<form />').attr({'action': '#', 'id': 'jcForm' + formId}).submit(function () {
        ...
        function dispResult(response) {
            $('#' + formId + ' .jcSubmit input').val('Soumettre').removeAttr('disabled');
        ...
        function loadTemplate() {
            code += '}}';
            $('#' + formId + ' .jcSubmit input').val('Chargement...').attr('disabled', 'disabled');
        ...
        if (formError) {
            showError('Un champ ou plus a une valeur invalide.');
        } else {
            loadTemplate();
        }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.results[0]) {
                                ...
                                showError("The player '" + name + "' does not exist, is banned or unranked, or we couldn't fetch your hiscores. Please enter the data manually.");
                            }
    ...
    $td.append($('<input />').attr({type: 'submit', value: 'Soumettre'}));
    $tr.append($td);
    $table.append($tr);
