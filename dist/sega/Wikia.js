...
$.extend(true, window, {
	...
	batchDeleteDelay:   2500,
	batchUndeleteDelay: 2500,
	
	// ---------------------
	// DiscussionsActivity
	// ---------------------
	
	rdaRefreshInterval: 120000,
	...
	rwaOptions: {
	    ...
	    loadModule:        true,
	    customRendering:   {},
	    headerLink:        false, // Do not set to true in sitewide (ToU violation)
	    refresh:           true,
...
!function () {
  ...
  $('#mw-content-text').on('change', '#wpGroup-bureaucrat', function() {
    ...
    if (!!$('#wpGroup-bureaucrat').attr('checked') && !confirm(String(BuroWarning)))
