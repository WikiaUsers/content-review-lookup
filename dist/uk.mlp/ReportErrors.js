// ###################################################### //
// ### Повідомлення про помилки Дружба — це диво Вікі ### //
// ### Version 1.1                                    ### //
// ###################################################### //
...
function openReportForm() {
    $.showCustomModal('Повідомляйте про помилки!', 'Якщо ви помітили помилку на «Дружба — це диво Вікі», будь ласка, виправте її самостійно, «Дружба — це диво Вікі» використовує технологію вікі, яка дозволяє це зробити самостійно. Не дивуйтеся, одне з правил «Дружба — це диво Вікі» говорить: «Редагуйте сміливо»! Якщо ви не можете самостійно виправити помилку, повідомте про неї за допомогою даної форми.'+
...
'<ul style="list-style: square; font-size:14px; padding:0 10px; margin-left:10px;">' +
                '<li>' +
                    '<a href="/uk/wiki/Дружба_—_це_диво_Вікі:Запити до адміністраторів" title="Поточний список повідомлень про помилки.">Поточний список повідомлень про помилки.</a>' +
                '</li>' +
            '</ul>' +
...
console.log('Перевірка...');
    ...
    // Ajax URL
    var url = des.server + '/api.php?action=edit&title=Дружба_—_це_диво_Вікі:Повідомлення_про_помилки&section=new' + '&text=' + encodeURIComponent(page) + '&token=' + encodeURIComponent(des.edittoken);
...
console.log('Це повинно бути:',r);
    $("#requestWindow").closeModal();
window.location = des.server + '/wiki/' + 'Дружба_—_це_диво_Вікі:Повідомлення_про_помилки#' + encodeURIComponent(des.pagename);
    });
