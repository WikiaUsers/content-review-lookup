(function(window) {
  'use strict';
  //Load Protection
  if (window.Loopx264Loaded) return;
  window.Loopx264Loaded = true;

  const searchClass = 'loopx264';
  function init() {
    const HTML = mw.html;
    const all = document.getElementsByClassName(searchClass);
    const canPlayx264 = document.createElement('video').canPlayType('video/mp4') !== '';
    if (canPlayx264)
      while (0 < all.length) {
        // Ingest Values
