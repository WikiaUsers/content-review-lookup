 ...
 *   Short sound after a message was sent
 */
    ...
    if (wgCanonicalSpecialPageName == 'Chat') {
            ...
            for (var i in this.model.users.models) {
                if (this.model.users.models[i].attributes.name == chat.attributes.name) {
                    if (this.model.users.models[i].attributes.isStaff) {
                        icon = ' <img class="stafficon" src="https://vignette.wikia.nocookie.net/wikiaglobal/images/6/6c/Staff_icon.png/revision/latest?cb=20160714211241">';
                    } else if (this.model.users.models[i].attributes.isModerator) {
                        icon = ' <img class="modicon" src="https://vignette.wikia.nocookie.net/rio/images/a/a8/Twin_Feathers_small.png/revision/latest?cb=20160810111851">';
                    }
                    break;
                }
            }
                ...
                for (var i = 0; i < pings.length; i++) {
                    if (text.toLowerCase().indexOf(pings[i].toLowerCase()) != -1 || this != mainRoom.viewDiscussion) {
                        if (!window.hasFocus) { //Only annoy people if the window isn't focused
                            document.getElementById('sound').innerHTML = '<audio src="https://vignette.wikia.nocookie.net/rio/images/b/b0/Macaw_Messanger.ogg/revision/latest?cb=20160810114515" autoplay=""></audio>';
                            if (!window.dinged) {
            ...
            if (typeof connectedUser == "undefined") {
                ...
                if (!this.isPrivate) {
                    ...
                    var newChatEntry = new models.InlineAlert({
                        text: $.msg('chat-user-joined', [joinedUser.get('name')])
                    });
        ...
        NodeRoomController.prototype.onPart = function(message) {
            ...
            if (typeof connectedUser != "undefined") {
                ...
                if (!this.isPrivate) {
                    var newChatEntry = new models.InlineAlert({
                        text: $.msg('chat-user-parted', [connectedUser.get('name')])
                    });
                    this.model.chats.add(newChatEntry);
                }
        ...
        window.commands = {
            ...
            'coppa': function(com, text) { //Too young
                if (com) {
                    $('#Write [name="message"]').val('http://coppa.org ' + com);
                } else {
                    $('#Write [name="message"]').val('http://coppa.org ' + i18n['coppa']);
                }
                return true;
            },
            ...
            'kickban': function(com, text) { //Kickban a user
                if (com) {
                    var mod = new models.KickBanCommand({
                        userToBan: com
                    });
                    this.socket.send(mod.xport());
                } else {
                    this.inlineAlert(i18n['erruser'].replace(/\$1/g, text.split(' ')[0]));
                }
            },
            ...
            'help': function(com, text) {
                if (com) {
                    ...
                    var subcoms = [];
                    var subdirs = [];
                    while (typeof eval('window.commands.' + ref) == 'string') {
                        ref.substring(0, ref.lastIndexOf('.')) + '.' + eval('window.commands' + ref).toLowerCase();
                    }
                    var command = eval('window.commands.' + ref);
                    if (i18n['help-' + ref]) {
                        str += i18n['help'].replace(/\$1/g, ref.replace(/\./g, ' ')).replace(/\$2/g, i18n['help-' + ref].replace(/\$1/g, i18n['example']).replace(/\$1/g, i18n['exampleuser']));
                    }
                    if (typeof command == 'object') {
                        for (var i in command) {
                            if (typeof command[i] == 'function') {
                                subcoms.push(i);
                            }
                            if (typeof command[i] == 'object') {
                                subdirs.push(i);
                            }
                        }
                    }
                    ...
                    if (str !== '') {
                        this.inlineAlert(str);
                    } else {
                        this.inlineAlert(i18n['nohelp'].replace(/\$1/g, command));
                    }
                } else {
                    var str = [];
                    for (var i in window.commands) {
                        if (typeof window.commands[i] == 'function' || typeof window.commands[i] == 'object') {
                            str.push(i);
                        }
                    }
        ...
        function build(text, index, stop) {
            ...
            if (index == stop || index == text.length - 1) {
                return text[index];
            }
        ...
        $(function() {
            ...
            importScriptURI("/load.php?mode=articles&articles=u:dev:MediaWiki:ChatHacks.js/i18n.js|u:dev:MediaWiki:ChatHacks.js/APIQuery.js&only=scripts");
        ...
        function ChatCheck() {
            ...
            if ($('.start-a-chat-button').length != 0) {
                $('.start-a-chat-button').replaceWith('<a class="wds-is-secondary wds-button wds-is-squished" onclick="OpenChatWindow()">' + $('.start-a-chat-button').html() + '</a>');
                clearInterval(window.chatcheck);
            }
        }
