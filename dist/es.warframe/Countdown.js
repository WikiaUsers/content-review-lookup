...
function Countdown(seedDate, loopTime, loopUnit, loopLimit, delayTime, delayUnit, dateFormat, dateLabels, separators) {
     ...
     for (var k = 0; k < count.length; k++) {
         ...
         // convierte el bucle/retraso de tiempo que se haya introducido al período de tiempo especificado en milisegundos
         ...
         // representa el horario de verano entre ahora y la fecha dada a menos que se especifique lo contrario
         var dst_offset = 0,
             dst_offset2 = 0;
         ...
         // tiempo total en milisegundos entre ahora y la fecha objetivo, convertidos para cierto período de tiempo
         // (por ejemplo, for 120 minutes: years = 0; months = 0; days = 0;
         // hours = 2; minutes = 120; seconds = 7200)
         // la cadena de tiempo mostrará "00021207200" hasta ahora
         ...
         // encuentra los períodos de tiempo requeridos por el formato de fecha especificado
         var Y_Count = 0,
             M_Count = 0,
             D_Count = 0,
             h_Count = 0,
         ...
         // basándose en los períodos de tiempo especificados, establece los períodos de tiempo para que tengan en cuenta el resto de períodos de tiempo
         // (por ejemplo, for 120 minutes & "hh mm ss": years = 0; months = 0; days = 0;
         // hours = 2; minutes = 0; seconds = 0)
         // la cadena de tiempo mostrará "000200" hasta ahora
         if (Y_Count === 0) {
             yearsleft = 0;
             yearsleft2 = 0;
         }
         ...
         // basándose en el formato deseado de los períodos de tiempo especificados, da ceros iniciales a la cadena de tiempo
         // (por ejemplo, for 120 minutes & "hh mm ss": years = 0; months = 0; days = 0;
         // hours = 02; minutes = 00; seconds = 00)
         // la cadena de tiempo mostrará "000020000" hasta ahora
         var Y_zeros = '',
             M_zeros = '',
             D_zeros = '',
             h_zeros = '',
         ...
         // basándose en las unidades deseadas de los períodos de tiempo especificados, da a cada período de tiempo en la cadena ciertas unidades
         // (por ejemplo, for 120 minutes & "hh mm ss" & "single": years = 0Y; months = 0M;
         // days = 0D; hours = 02h; minutes = 00m; seconds = 00s)
         // la cadena de tiempo mostrará "0A0M0D02h00m00s" hasta ahora
         var yearunit = '',
             monthunit = '',
             dayunit = '';
         var hourunit = '',
         ...
         // separa mediante los separadores especificados cada período de tiempo en la cadena de tiempo
         // (por ejemplo, for 120 minutes & "hh mm ss" & "single" & " " or "&nbsp;": 
         // years = 0Y ; months = 0M ; days = 0D ; hours = 02h ;
         // minutes = 00m ; seconds = 00s)
         // la cadena de tiempo mostrará "0Y 0M 0D 02h 00m 00s" hasta ahora
         ...
         // cuando las iteraciones del ciclo alcancen el límite del mismo, oculta el texto normal, oculta el texto restante, oculta los períodos de tiempo normal/y restante, sólo mostrando el texto de fin de ciclo
         if ((i === (loopLimit1 + 1)) && (nextDate.getTime() <= now.getTime())) {
             ...
             // cuando el tiempo restante se haya consumido, muestra el texto restante, oculta el texto normalsólo mostrando los períodos de tiempo restante especificados por el formato de fecha
             ...
             // muestra texto normal mientras que el tiempo restante no se haya consumido, ocultando el texto restante, sólo mostrando los períodos de tiempo normales especificados por el formato de fecha
             // (por ejemplo, for 120 minutes & "hh mm ss" & "single" & " " or "&nbsp;": 
             // years = ; months = ; days = ; hours = 02h ;
             // minutes = 00m ; seconds = 00s)
             // la cadena de tiempo mostrará "02h 00m 00s" hasta ahora
             else {
                 document.getElementById('endText_' + k).setAttribute("style", "display:none");
                 document.getElementById('bDelayText_' + k).setAttribute("style", "display:none");
                 document.getElementById('aDelayText_' + k).setAttribute("style", "display:none");
 ...
 // da a cada instancia de elementos repetitivos el mismo identificador único de clase
...
function PCPlanetTracker(count) {
    if ((count + 3) % 4 === 0) {
        text = "Repetidor Orcus, <a href='https://warframe.fandom.com/es/wiki/Plutón'>Plutón</a> (PC)<br/>";
        return text;
    } else if ((count + 2) % 4 === 0) {
        text = "Repetidor Kronia, <a href='https://warframe.fandom.com/es/wiki/Saturno'>Saturno</a> (PC)<br/>";
        return text;
    } else if ((count + 1) % 4 === 0) {
        text = "Repetidor Larunda, <a href='https://warframe.fandom.com/es/wiki/Mercurio'>Mercurio</a> (PC)<br/>";
        return text;
    } else {
        text = "Repetidor Strata, <a href='https://warframe.fandom.com/es/wiki/Tierra'>La Tierra</a> (PC)<br/>";
        return text;
    }
}
function PS4PlanetTracker(count) {
    if ((count + 3) % 4 === 0) {
        text = "Repetidor Kuiper, <a href='https://warframe.fandom.com/es/wiki/Eris'>Eris</a> (PS4)<br/>";
        return text;
    } else if ((count + 2) % 4 === 0) {
        text = "Repetidor Larunda, <a href='https://warframe.fandom.com/es/wiki/Mercurio'>Mercurio</a> (PS4)<br/>";
        return text;
    } else if ((count + 1) % 4 === 0) {
        text = "Repetidor Kronia, <a href='https://warframe.fandom.com/es/wiki/Saturno'>Saturno</a> (PS4)<br/>";
        return text;
    } else {
        text = "Repetidor Strata, <a href='https://warframe.fandom.com/es/wiki/Tierra'>La Tierra</a> (PS4)<br/>";
        return text;
    }
}
function XB1PlanetTracker(count) {
    if ((count + 3) % 4 === 0) {
        text = "Repetidor Orcus, <a href='https://warframe.fandom.com/es/wiki/Plutón'>Plutón</a> (XB1)<br/>";
        return text;
    } else if ((count + 2) % 4 === 0) {
        text = "Repetidor Leonov, <a href='https://warframe.fandom.com/es/wiki/Europa'>Europa</a> (XB1)<br/>";
        return text;
    } else if ((count + 1) % 4 === 0) {
        text = "Repetidor Strata, <a href='https://warframe.fandom.com/es/wiki/Tierra'>La Tierra</a> (XB1)<br/>";
        return text;
    } else {
        text = "Repetidor Vesper, <a href='https://warframe.fandom.com/es/wiki/Venus'>Venus</a> (XB1)<br/>";
        return text;
    }
}
function NSWPlanetTracker(count) {
    if ((count + 3) % 4 === 0) {
        text = "Repetidor Kuiper, <a href='https://warframe.fandom.com/es/wiki/Eris'>Eris</a> (NSW)";
        return text;
    } else if ((count + 2) % 4 === 0) {
        text = "Repetidor Larunda, <a href='https://warframe.fandom.com/es/wiki/Mercurio'>Mercurio</a> (NSW)";
        return text;
    } else if ((count + 1) % 4 === 0) {
        text = "Repetidor Vesper, <a href='https://warframe.fandom.com/es/wiki/Venus'>Venus</a> (NSW)<br/>";
        return text;
    } else {
        text = "Repetidor Leonov, <a href='https://warframe.fandom.com/es/wiki/Europa'>Europa</a> (NSW)";
        return text;
    }
}