	...
	return This = {
			...
			init: function() {
				...
				if (wgUserName) {
					this.appendLIElement($('<li>', {
						html: '<span class="' + this.personalJSFAIcon + '"></span><a href="' + encodeURI('/wiki/User:' + wgUserName + '/wikia.js') + '">' + this.personalJSText + '</a><a href="' + encodeURI('https://community.wikia.com/wiki/User:' + wgUserName + '/global.js') + '">' + this.globalAutoText + '</a>'
					}));
					this.appendLIElement($('<li>', {
						html: '<span class="' + this.personalCSSFAIcon + '"></span><a href="' + encodeURI('/wiki/Участник:' + wgUserName + '/wikia.css') + '">' + this.personalCSSText + '</a><a href="' + encodeURI('https://community.wikia.com/wiki/User:' + wgUserName + '/global.css') + '">' + this.globalAutoText + '</a>'
					}));
				}
		...
		AjaxAutoRefresh: {
			...
			refreshPage: function() {
				...
				$mwContentText.load('/wiki/' + wgPageName + ' #mw-content-text>', function() {
		...
		Mainpage: {
			...
			load: function($button, pagename) {
				...
				if (id in this.cache) {
					...
					this.HTMLTempContainer.load('/wiki/' + pagename + ' ' + this.ajaxContentTargetSelector, function() {
			...
			getDiscussionsHTML: function() {
				return '<a target="' + this.targetId + '" href="/d/f">Обновить</a><a class="FixMainpageDiscussions">' + this.fixButtonText + '</a><a href="/d/f" target="_blank">Открыть в новом окне</a><iframe src="/d/f" class="MainpageDiscussionsIframe" name="' + this.targetId + '" style="height: ' + ($window.height() * this.sizeCoeff) + 'px;"></iframe>';
			},
			init: function() {
				var that = this;
			...
			init: function() {
				...
				this.Tooltip.prototype.loadTooltipHTML = function() {
					...
					if (!this.triggered && this.html === null) {
						this.triggered = true;
						$container = $('<div>').load('/wiki/' + pagename + ' ' + selector, function() {
