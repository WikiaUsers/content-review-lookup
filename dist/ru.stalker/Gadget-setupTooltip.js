// Source: https://nkch.fandom.com/wiki/MediaWiki:WdsTooltip.js
...
function showTooltip( tooltip, element ) {
	...
	switch (direction) {
		case 'right':
			element.style.left = position.right + 6 + 'px', 
			element.style.top = ( position.bottom - position.top ) / 2 + position.top + 'px';
			break;
		case "top":
			element.style.top = position.top - 6 + 'px', 
			element.style.left = ( position.right - position.left ) / 2 + position.left + 'px';
			break;
		case "bottom":
			element.style.top = position.bottom + 6 + 'px', 
			element.style.left = ( position.right - position.left ) / 2 + position.left + 'px';
			break;
		case "left":
			element.style.left = position.left - 6 + 'px', 
			element.style.top = ( position.bottom - position.top ) / 2 + position.top + 'px';
			break;
	}
...
function _setupTooltip( tooltip ) {
	if ( tooltip.dataset.tooltipAttached || !tooltip.dataset.wdsTooltip ) return;
...
window.setupTooltip = function ( tooltip ) {
