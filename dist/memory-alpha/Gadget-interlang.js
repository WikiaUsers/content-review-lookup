...
const DateFormatter = require('mediawiki.DateFormatter').forSiteZone();
const validGroups = [
	'bot',
	'bureaucrat',
	'sysop',
				...
				Promise.all(promises).then(results => {
					...
					results.forEach(apiOutput => {
						...
						tBody.append($('<tr>', {
							...
							$('<td>', {
								'data-sort-value': lastEdit.timestamp,
								class: status,
							}).append(link(
								`Special:Diff/${lastEdit.revid}`,
								DateFormatter.formatTimeAndDate(new Date(lastEdit.timestamp)),
								lang
							)),
