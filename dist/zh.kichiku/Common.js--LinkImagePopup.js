...
mw.loader.using('mediawiki.util').then(function() {
    ...
    if (typeof window.sessionStorage !== 'undefined' && cacheEnabled &&
      typeof window.sessionStorage.linkPopupStorage !== 'undefined') {
        ...
        $.each(categories, function(index, value) {
            pending += 1;
            $.getJSON('/zh/api.php?action=query&list=categorymembers&cmtitle=Category:' + enc(value) +
              '&cmprop=title&cmtype=page&cmlimit=max&format=json', function(data) {
                ...
                if (pending === 0) {
                    ...
                    $.each(lists, function(index2, value2) {
                        pending2 += 1;
                        $.getJSON('/zh/api.php?action=parse&page=List_of_' + enc(value2) +
    ...
    $('#mw-content-text a').on('mouseover', function(e) {
        ...
        if (! names[name]) {
            ...
            $.getJSON('/zh/api.php?action=parse&page=' + enc(name) + '&section=0&prop=wikitext&format=json',
              function(data) {
                ...
                $.getJSON('/zh/api.php?action=query&prop=imageinfo&titles=File:' + enc(result) +
            ...
            pendingPages[name] = true;
            $.getJSON('/zh/api.php?action=query&prop=imageinfo&titles=File:' + enc(names[name].file) +
