...
window.UserTagsJS = {
    tags: {
        bureaucrat: { link: 'Especial:ListUsers/bureaucrat' },
        bot: { link: 'Especial:Listusers/bot' },
        chatmoderator: { link: 'Especial:ListUsers/chatmoderator' },
        rollback: { link: 'Especial:ListUsers/rollback' },
        sysop: { link: 'Especial:ListUsers/sysop' },
        imagecontroller: { link: 'Especial:ListUsers/imagecontroller' }
    },
    modules: {
        autoconfirmed: true,
        inactive: {
...
window.UserTagsJS = {
	modules: {},
	tags: {
        vstf: {u:  'Vstf (Volunteer Spam Task Force)'}, 
        bot: {u: 'Bot'},
        founder: {u: 'Fundador del Wiki', f: 'Fundadora del Wiki'},
        bureaucrat: {u: 'Burócrata', f: 'Burócrata'},
    }
};
...
if (mw.config.get('mwCustomEditButtons')) {
    mw.config.get('mwCustomEditButtons').length = {
...
setInterval(function () {
    ...
    $('.comments li.message blockquote .MiniEditorWrapper .edited-by a:not(.subtle)').each(function () {
        var $user = $(this).text();
        if ($user.match(/SKYNX|MegaSonicX4|Pikoaztlan/g)) {
