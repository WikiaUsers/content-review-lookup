...
dev.StarRatings.ui.pecoesSvg = function($this, callbacks) {
	...
	if (!$.fn.rateable) {
		ready = dev.StarRatings.load('u:sonako:Mediawiki:StarRatings/ui.js');
	}
...
(function(module, window, $, mw) {
	...
	if (mw.config.get('wgNamespaceNumber') === -1 && mw.config.get('wgTitle') === 'StarRatings') {
		//return module.load('u:sonako:Mediawiki:StarRatings/special.js');
	}

	// Shows the in-page UI for viewing the stats for the clicked poll
	function showStats(poll) {
		if (!module.showStats) {
			module.load('u:sonako:Mediawiki:StarRatings/stats.js').done(function() {
				showStats(poll);
			});
			return;
		}
