...
$(function() {
	...
	catnav.data = {
		...
		selectedPage: null,
		storage: {
			url: "//sonako.fandom.com/vi", // url to the wiki that hosts your imports
	...
	catnav.data.markup.html = '<nav id="catnav">\n' +
		...
		'\t<div class="catnav-gui-group">\n' +
			'\t\t<div class="catnav-commoncats">\n' +
				...
				'\t\t\t</h3>\n' +
				'\t\t\t<h5>\n' +
					'\t\t\t\tNhập và Xuất dữ liệu thể loại yêu thích từ trang&nbsp;<a href="//sonako.fandom.com/vi/wiki/Special:MyPage/catnav.css">cài đặt</a>' +
				...
				'\t\t\t\t<span style="font-size: smaller;">Click chuột trái để chèn vào ô thể loại muốn tìm và Click chuột phải để thêm vào ô thể loại muốn loại trừ. Click vào biểu tượng xóa sẽ xóa tên những thể loại bạn đã thêm vào mục ưa thích của mình.</span>\n' +
			...
			'\t\t<table><tbody>\n' +
				'\t\t\t<tr>\n' +
					'\t\t\t\t<th>Tìm trang từ thể loại:</th>\n' +
					'\t\t\t\t<th>Loại trừ trang từ thể loại:</th>\n' +
				'\t\t\t</tr>\n' +
				'\t\t\t<tr>\n' +
		...
		'\t<div class="catnav-gui-group">\n' +
			'\t\t<p>\n' +
				'\t\t\t<input type="checkbox" id="catnav-filter-state" /><label for="catnav-filter-state">Lọc tiêu đề trang</label><br />\n' +
				'\t\t\t<input type="text" id="catnav-filter-text" placeholder="Mẫu lọc" /><br />\n' +
				'\t\t\t<input type="checkbox" id="catnav-filter-case" checked /><label for="catnav-filter-case">Không phân biệt chữ hoa-chữ thường</label><br />\n' +
				'\t\t\t<input type="checkbox" id="catnav-filter-regex" /><label for="catnav-filter-regex">Mẫu Regex</label>\n' +
			'\t\t</p>\n' +
		'\t</div>\n' +
		// settings
		'\t<div class="catnav-gui-group">\n' +
			'\t\t<p>\n' +
				...
				'\t\t\t<input type="radio" name="catnav-sort" id="catnav-sort-alphabet" value="alphabet" checked /><label for="catnav-sort-alphabet">Xếp theo bảng chữ cái</label> <span style="font-size: 66%;">(lưu ý chọn các tùy chọn sắp xếp khác có thể sẽ gia tăng đáng kể thời gian tải)</span><br />\n' +
				'\t\t\t<input type="radio" name="catnav-sort" id="catnav-sort-bypageid" value="bypageid" /><label for="catnav-sort-bypageid">Xếp theo page ID (Ngày tạo trang / Ngày khôi phục lại sau xóa)</label><br />\n' +
				'\t\t\t<!-- <input type="radio" name="catnav-sort" id="catnav-sort-creation" value="creation" /><label for="catnav-sort-creation">Xếp theo ngày tạo</label><br /> -->\n' +
				...
				'\t\t\t<input type="checkbox" id="catnav-dir" /><label for="catnav-dir">Theo thứ tự cao xuống thấp</label><br />\n' +
			'\t\t</p>\n' +
	...
	catnav.fn.addGlobalFavorites = function() {
		...
		$.ajax({
			url: catnav.data.storage.url + (catnav.data.storage.url === "//sonako.fandom.com/vi" ? "" : mw.config.get("wgScriptPath")) + "/api.php?action=query&format=json&prop=revisions&rvprop=content&titles=User:" + user + "/catnav.css&callback=catnavcb&" + new Date().getTime(),
			action: "GET",
			dataType: "jsonp",
			...
			success: function(data) {
				...
				for (pageid in pages) {
					if (pageid == -1) {
						alert("Không tìm thấy dữ liệu. Hãy bảo đảm rằng trang 'https://sonako.fandom.com/vi/wiki/User:" + user + "/catnav.css' tồn tại");
					} else {
							...
							switch (line.charAt(0)) {
								case "@":
									...
									try {
										...
										alert("Tên miền wiki không hợp lệ ở dòng " + (lineNumber + 1) + ": " + temp);
									}
									break;
								case "#":
									...
									if (currCity) {
										...
										alert("Lỗi ở dòng " + (lineNumber + 1) + ": thể loại được liệt kê mà không ghi rõ tên miền wiki ở trước");
									}
									break;
							}
						});
	...
	catnav.fn.init = function() {
		...
		$("#catnav-go").click(function() {
			...
			if (incCats.length === 0) {
				catnav.fn.unsealUI();
				$("#catnav-loading").hide();
				catnav.fn.error(1, "Hãy điền ít nhất tên 1 thể loại hợp lệ  vào mục \"Thêm Thể Loại from categories\".");
				return;
			}
    ...
    if (mw.config.get("wgNamespaceNumber") === -1 && mw.config.get("wgTitle") == "CatNav") {
		...
		// update titles
		$(".page-header__title").html("Tìm Kiếm Nâng Cao | Sonako Light Novel Wiki");
