...
function ajaxTasks(proyecto, registry, purge) {
    ...
    e.open("POST", "http://es.warhammer40k.wikia.com/wiki/Plantilla:PanelAjax/" + proyecto + "?action=" + action, true);
    e.send();
}
...
function notification(proyecto) {
    if (proyecto === 'Librarium') {
        setTimeout(function() {
            setNotification('Librarium');
        }, 1000);
    } else if (proyecto === 'Apothecarion') {
        setTimeout(function() {
            setNotification('Apothecarion');
        }, 1000);
    } else if (proyecto === 'Cruzada') {
        setTimeout(function() {
            setNotification('Cruzada');
        }, 1000);
    }
}
...
function setNotification(proyecto) {
    ...
    if (proyecto === 'Librarium') {
        a_message.setAttribute('href', '/wiki/Usuario_Blog:Aresius_King/Librarium_de_Traducción_-_Boletín_XXIV?action=purge');
        a_message.innerHTML = 'El proyecto Librarium tiene modificaciones.';
    } else if (proyecto === 'Apothecarion') {
        a_message.setAttribute('href', '/wiki/Sepe1978/VIII_Expediente_del_Apothecarion?action=purge');
        a_message.innerHTML = 'El proyecto Apothecarion tiene modificaciones.';
    } else if (proyecto === 'Cruzada') {
        a_message.setAttribute('href', '/wiki/Usuario_Blog:Aresius_King/Cruzada_de_Scuramaccus:_Risas_Enmascaradas?action=purge');
        a_message.innerHTML = 'El proyecto Cruzada tiene modificaciones.';
    } else {
