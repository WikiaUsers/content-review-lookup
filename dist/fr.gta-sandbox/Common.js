 ...
 * ScriptPage: https://gta.fandom.com/fr/wiki/MediaWiki:Common.js
 ...
 * @author: Thegamer1604 (https://gta.fandom.com/fr/wiki/user:Thegamer1604 [Adaptation de la version de Celdrøn])
...
(function () {
	...
	jQuery(function ($) {
		var FairUseUpload = {
			preload: function() {
				this.api = new mw.Api();
				this.getContent(26632).then(this.init.bind(this));//mw.config.get('wgArticleId')
			},
				...
				if (mw.config.get('wgCanonicalSpecialPageName') === 'Upload') {
					...
					if (!mw.util.getParamValue('wpForReUpload')) {
						if (formstyle == "guided" || (formstyle == "" && window.location.search.indexOf('basic=true') == -1)) { // Add link to basic form
							$("#uploadtext").prepend('<div style="float: right;" id="uploadBasicLinkJS"><a href="https://gta.fandom.com/fr/index.php?title=Spécial:Téléverser&basic=true" onclick="javascript:setCookie(\'uploadform\', \'basic\', 30)">Basculer vers le formulaire basique</a></div>'); // Stretch table to full width
