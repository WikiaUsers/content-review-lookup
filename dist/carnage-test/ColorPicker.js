 ...
 **/
(function($, mw){
    function ColorPicker($element, options, callback){
        mw.hook('dev.colors').add($.proxy(function(colors){
            this.process.call(this, colors, $element, callback, options);
        }, this));
    }
 
    ColorPicker.prototype.process = function(colors, $element, callback, options){
        // The main object for the Colors library
        this.colors = colors;
        // The configurations for the color picker
        ...
        this.$colorpicker.box.width = Math.max(this.options.width, 200);
        // The height of the color picker
        this.$colorpicker.box.height = Math.max(this.options.height, 200);
        // The color strip
        ...
        // The width of the color strip
        this.$colorpicker.strip.width = Math.max(this.options.width, 200);
        // The height of the color strip
        this.$colorpicker.strip.height = 10;
        // Checks whether the color strip is vertical
        this.isVertical = typeof this.options.vertical !== 'undefined' && this.options.vertical;
        if (this.isVertical){
            var height = this.$colorpicker.strip.height,
                width = this.$colorpicker.strip.width;
            this.$colorpicker.strip.width = height;
            this.$colorpicker.strip.height = width;
        }
    ...
    ColorPicker.prototype.createContext = function(){
        ...
        this.box.height = Math.max(this.box.elem.height || 0, 200);
        this.box.width = Math.max(this.box.elem.width || 0, 200);
        ...
        if (typeof this.isVertical === 'boolean' && this.isVertical){
            this.strip.height = Math.max(this.strip.elem.height || 0, 200);
            this.strip.width = Math.max(this.strip.elem.width || 0, 10);
        } else {
            this.strip.height = Math.max(this.strip.elem.height || 0, 10);
            this.strip.width = Math.max(this.strip.elem.width || 0, 200);
        }
