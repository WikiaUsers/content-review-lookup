//<nowiki>
if (QuickToolsAdvancedtop === undefined) var QuickToolsAdvancedtop = true;
$(function (window, $, mw) {
    "use strict";
    //Global Variables
    ...
    function userrightsform(user) {
        $.ajax({
            type: "GET",
            url: wgServer + wgArticlePath.slice(0, -2) + 'Special:UserRights/' + user,
    ...
    function mainMenu() {
        ...
        $.showCustomModal("Quick Tools (version 2)", '<form class="WikiaForm" method="" name=""><fieldset><div id="quicktoolsv2button" style="float:left; width:30%;"><div style="text-align: center;"><a class="wikia-button sysop" id="at-block">Block user</a><a class="wikia-button sysop" id="at-unblock">Unblock user</a><a class="wikia-button bureaucrat" id="at-userrights">User rights</a><a class="wikia-button" id="at-categories">Manage categories</a><a class="wikia-button sysop" id="at-delete">Delete page</a><a class="wikia-button" id="at-move">Move page</a><a class="wikia-button sysop" id="at-protect">(Un)Protect page</a><a class="wikia-button" id="at-purge">Purge page</a><a class="wikia-button" id="at-redirect">Redirect page</a><a class="wikia-button thispage" id="at-redlinks">Remove redlinks</a><a class="wikia-button" id="at-switch">Switch skin</a><a class="wikia-button thispage" id="at-template">Add template</a><a class="wikia-button thispage" id="at-unsigned">Add unsigned</a><a class="wikia-button sysop" id="at-batch">Batch delete</a><a class="wikia-button" id="at-massnulledit">MassNullEdit</a></div></div><div id="quicktoolsadvancedcontainer" style=" overflow:hidden; width:70%"></div><div style="clear:both;"><br /><h4> User rights : ' + usergroups.join() + '</h5><hr><h4 style="font-style: italic; float:left;color:black;">QuickTools 2 created by <a href="http://dev.wikia.com/wiki/User:Gguigui1" target="_blank">Gguigui1</a>, source code inspired and improved from QuickTools created by <a href="http://dev.wikia.com/wiki/User:Shining-Armor" target="_blank">Shining-Armor</a>.</h5><br><p>Page will be infer if you clicked on QuickTools on a page.</p><p>User IP or username will be infer if you clicked on QuickTools on a user profile, user contributions, user wall or user blog list.</p><p>Template will be infer if you are on a template page.</p></div></form>', {
            id: "quicktoolsv2",
            width: 650,
            buttons: [{
                id: "close-menu",
        ...
        // Styles of buttons which only have styles in oasis
        if (skin != 'oasis') {
        ...
        $("#at-switch").click(function () {
            $container.html('<strong>Skin to use:</strong><br /><select id="skin-switch-dropdown"></select>');
			for (var i in wgAvailableSkins) {
				if (i == skin) {
					$('#skin-switch-dropdown').append('<option selected value="' + i + '">' + wgAvailableSkins[i] + '</option>');
				} else {
					$('#skin-switch-dropdown').append('<option value="' + i + '">' + wgAvailableSkins[i] +'</option>');
				}
			}
			$('#skin-switch-dropdown').change(function() {
				window.location = wgServer + wgArticlePath.slice(0,-2) + wgPageName + "?useskin=" + $(this).val();
			});
        });
    ...
    function ajaxCategories(page, categories) {
		...
		$.getJSON(wgServer + '/api.php?action=query&format=json&titles=Category:' + $('#select-categories option:last').val(),function(data) {
			if (data.query.normalized) {
				categorie = decodeURI(data.query.normalized[0].to.split(':')[0]);
			}
		var content;
		var url = wgServer + '/api.php?action=query&prop=info|revisions&titles=' + page + '&format=json&rvprop=content&rvlimit=1&indexpageids=1';
        ...
        var url = wgServer + '/api.php?action=edit&title=' + encodeURIComponent(page) + '&text=' + content + '&format=json&token=' + encodeURIComponent(token);
        $.post(url, function (data) {
            document.location.reload(false);
        });
		});
