...
// SpoilerAlert
SpoilerAlert = {
    question: '이 문서는 스포일러를 포함하고 있습니다. 열람하시겠습니까?',
    yes: '네, 열람합니다',
    no: '아니오, 문서를 닫습니다',
    isSpoiler: function () {
...
importArticle({
    type: "script",
    article: "w:dev:SpoilerAlert/code.js"
});
///////////////////////////////////////////////////////////////////
// jQuery Toggle
$(function() {
...
function bindTT() {
    ...
    $p = $t.parent();
    if ($p.hasClass("selflink") == false) {
        $t.data("tt", $p.attr("title").replace(" (page does not exist)", "").replace("?", "%3F")).hover(showTip, hideTip);
    }
}
...
function ttMouseOver() {
    if (tooltipsOn) {
        ...
        $(article + " span.tttemplatelink").hover(showTemplateTip, hideTemplateTip);
    }
}
