	...
	if ( $( this ).parent().is( $rollback.parent() ) ) {
		...
		$rollback = $( '<div id="rollback-summary">' ).append(
			$( '<input type="text">' ).addClass( 'mw-ui-input rollback-text' ).prop( { maxlength: 250, spellcheck: true } ).val(
				'Відкидання редагувань [[Special:Contribs/' + name + '|' + name + ']] ([[User talk:' + name + '|обговорення]])'
			),
			$( '<input type="button">' ).addClass( 'mw-ui-button mw-ui-constructive rollback-submit-button' ).val( 'Відкинути' )
		).insertAfter( this );
	}
