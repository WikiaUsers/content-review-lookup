...
/*#############################################
### Ajax Sysop: SpecialDeleteHelpers
	...
	ajax_mw: {
		...
		*********/
		InitSpecialDeleteHelpers: function () {
			...
			** Fetch subpages
			*********/
			function _GetSubpages(ul, namespace, prefix, limit) {
							...
							for (var i = 0, len = pages.length; i < len; i++) {
								ul.append(
									...
									.append(
										$(document.createElement('a'))
										.attr({
											'href': mw.config.get( 'wgServer' ) + mw.config.get( 'wgArticlePath' ).replace('$1', encodeURIComponent(pages[i].title)) + "?action=delete",
											'alt': 'Delete ' + pages[i].title,
											'target': '_blank'
										})
										.text("удалить")
									)
			...
			_GetSubpages($('#ajax-mediawiki-subpages-main'), ns_main, mw.config.get( 'wgTitle' ) + '/', 500);
			_GetSubpages($('#ajax-mediawiki-subpages-talk'), ns_talk, mw.config.get( 'wgTitle' ) + '/', 500);
			_GetSubpages($('#ajax-mediawiki-talk'), ns_talk, mw.config.get( 'wgTitle' ), 1);
		...
		BuildFormattedBox: function () {
			...
			.append(
				...
				.text("Сторінки, просторово пов'язані з даною")
			);
		},
