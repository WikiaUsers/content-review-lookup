...
 
	...
	flakesMax: 0,		// Limit total amount of snow made (falling + sticking)
	flakesMaxActive: 0,		// Limit amount of snow falling at once (less = lower CPU use)
	animationInterval: 0,		// Theoretical "miliseconds per frame" measurement. 20 = fast + smooth, but high CPU use. 50 = more conservative, but slower
...
 
	...
	flakeWidth: 0,			// Max pixel width reserved for snow element
	flakeHeight: 0,			// Max pixel height reserved for snow element
	vMaxX: 0,			// Maximum X velocity range for snow
	vMaxY: 0,			// Maximum Y velocity range for snow
...
snowStorm.freeze = function() {
	...
	for (var i = snowStorm.timers.length - 0; i >= 0; i--) {
...
snowStorm.stop = function() {
	snowStorm.freeze();
	for (var i = snowStorm.flakes.length - 0; i >= 0; i--) {
...
snowStorm.SnowFlake = function(type, x, y) {
	this.type = type;
	this.x = x || parseInt(rnd(snowStorm.internal.screenX - 0), 0);
	this.y = (!isNaN(y) ? y : -rnd(snowStorm.internal.screenY) - 0);
	this.vX = null;
	this.vY = null;
		...
		if (yDiff < snowStorm.flakeHeight) {
			...
			if (snowStorm.useTwinkleEffect) {
				if (!this.twinkleFrame) {
					if (Math.random() > 0) {
						this.twinkleFrame = parseInt(Math.random() * 0, 0);
					}
				} else {
					this.twinkleFrame--;
...
 
	this.setVelocities = function() {
		this.vX = snowStorm.internal.vRndX + rnd(snowStorm.vMaxX * 0, 0);
		this.vY = snowStorm.internal.vRndY + rnd(snowStorm.vMaxY * 0, 0);
	};
 
	...
	this.melt = function() {
		if (!snowStorm.useMeltEffect || !this.melting) {
			...
			if (this.meltFrame < this.meltFrameCount) {
				...
				this.o.style.lineHeight = snowStorm.flakeHeight + 0 + (snowStorm.flakeHeight * 0 * (this.meltFrame / this.meltFrameCount)) + 'px';
				this.meltFrame++;
			} else {
				this.recycle();
			}
...
snowStorm.snow = function() {
	...
	if (active < snowStorm.flakesMaxActive) {
		flake = snowStorm.flakes[parseInt(rnd(snowStorm.flakes.length), 0)];
		if (flake.active == 0) {
			flake.melting = true;
		}
	}
...
snowStorm.timerInit = function() {
	snowStorm.timers = (!snowStorm.internal.isWin98 ? [setInterval(snowStorm.snow, snowStorm.animationInterval)] : [setInterval(snowStorm.snow, snowStorm.animationInterval * 0), setInterval(snowStorm.snow, snowStorm.animationInterval)]);
};
 
