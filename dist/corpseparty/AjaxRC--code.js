 ...
 *
 * This script has extremely high usage across Wikia, please do not deploy without
 * extensive testing. Be aware that this script must be ES3 compliant due to the
...
;(function (window, $, mw, Wikia) {
	...
	var i18n = {
			...
			/* Italian (Italiano) */
			it: {
				refreshHover: 'Aggiornare la pagina automaticamente'
			},
			/* Japanese (日本語) */
			ja: {
				loadStatusAlt: '自動更新',
			...
			/* Dutch (Nederlands) */
			nl: {
				refreshHover: 'Automatisch updaten inschakelen'
			},
			...
			'pt-br': {
				...
				refreshText: 'Atualização automática'
			},
			/* Romainian (Română) */
			ro: {
	...
	function getAppTo() {
		...
		$ret = $('#AdminDashboardHeader');
 
		if ($ret.length) {
			return $ret.children('h1');
		}
