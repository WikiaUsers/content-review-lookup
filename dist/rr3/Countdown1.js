 ...
 * <http://dev.wikia.com/wiki/Countdown1>
 */
...
;(function (module, mw, $, undefined) {
	...
	function output (i, diff) {
		...
		var delta, result, days, parts = [];
		delta = diff % 60;
		if (countdowns[i].opts & COMPACT)
		{
            if ( delta < 10 )
            {
                parts.unshift('0'+delta) ;
            }
            else
            {
                parts.unshift(delta);
		...
		{
            days=diff  + ' ' + i18n[diff  === 1 ? 'day'    : 'days'   ] ;
            parts.unshift(days);
		}
		result = parts.pop();
...
}(window.countdownTimer1 = window.countdownTimer1 || {}, mediaWiki, jQuery));