            ...
            while (element && element.tagName !== 'H2') {
                if (element.tagName === 'TABLE') {
                    if (!element.classList.contains('song-lyrics-legend')) {
                        ...
                        if (columnCount >= 2) {
                            tables.push({ heading: heading, table: element });
                        }
                    }
                }
                ...
                for (var j = 0; j < nestedTables.length; j++) {
                    ...
                    if (!nestedTable.classList.contains('song-lyrics-legend')) {
                        ...
                        if (nestedColumnCount >= 2) {
                            tables.push({ heading: heading, table: nestedTable });
                        }
                    }
                }
    ...
    try {
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(finalContent)
                .then(function() {
                    mw.notify('Текст скопирован', { type: 'success' });
                })
                .catch(function(err) {
        ...
        mw.notify('Ошибка при копировании', { type: 'error' });
    } finally {
...
function processLyricsTable(heading, table) {
    ...
    // Add translation toggle for 3-column tables
    var columnCount = getLogicalColumnCount(table);
    
    if (columnCount === 3) {
        var toggleButton = document.createElement('button');
        toggleButton.textContent = 'Скрыть перевод';
        toggleButton.className = 'mw-ui-button col-toggle';
        toggleButton.style.cssText = 'float:right; margin-left:8px;';
        ...
        toggleButton.onclick = function() {
            ...
            for (var i = 0; i < rows.length; i++) {
                var cell = getCellAtLogicalIndex(rows[i], 2);
                if (cell) {
                    cell.style.display = isHidden ? 'none' : '';
                }
            }
            
            toggleButton.textContent = isHidden ? 'Показать перевод' : 'Скрыть перевод';
        };
