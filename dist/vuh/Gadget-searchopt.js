...
(function () {
  ...
  var jq_go = function () {
    ...
    var
      ...
      go_button_arrow = 'https://images.wikia.nocookie.net/dev/images/6/65/GoArrow2.png',
    ...
    $go_button.add($options_button).find('img')
      ...
      .parent().css('margin', '0px 0px 0px 2px');
    ...
    $options_button.click(function () {
      ...
      if ($('#search_fix_options').size() < 1) {
        $wikia_search.after(
          $('<section />', {'id': 'search_fix_options', 'class': 'module'})
            .css({
              'display': 'none',
              'line-height': '1.5em',
              'font-size': '11px',
            ...
            .append(
              $('<label>Po naciśnięciu klawisza Enter:</label>'),
              $('<br />'),
              $('<input />', {'type': 'radio', 'id': 'search_fix_opt_search', 'value': '1', 'name': 'search_fix_default_button', 'checked': 'checked'}),
              $('<label>Szukaj</label>'),
              $('<br />'),
//            $('<input />', {'type': 'radio', 'id': 'search_fix_opt_go',  'value': '0', 'name': 'search_fix_default_button'}),
//            $('<label>Przejdź</label>'),
              $('<br />'),
              $('<input />', {'type': 'checkbox', 'value': '1', 'id': 'search_fix_opt_new_window', 'name': 'search_fix_new_window'}),
              $('<label>Otwórz wyszukiwane słowa w nowym oknie</label>'),
              $('<br />'),
              $('<input />', {'type': 'checkbox', 'value': '1', 'id': 'search_fix_opt_acomplete', 'name': 'search_fix_acomplete'}),
              $('<label>Wyłącz autouzupełnianie</label>'),
      ...
      settings = get_cookie();
      $('#' + (settings & 1 ? 'search_fix_opt_search' : 'search_fix_opt_go')).click();
