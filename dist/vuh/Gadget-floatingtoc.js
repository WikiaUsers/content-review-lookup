...
(function (module, mw, $, window) {
    ...
    var win, page, toc, dialog, i18n, tocLinks;
    ...
    function initStyles () {
        ...
        $('<a href="#toc" class="toc-link" title="Spis treści"></a>')
    ...
    function initDialog () {
        ...
        function adjustDialog () {
            ...
            core.dialog('option', 'height', Math.min(toc.height + 40, winHeight - 8));
            var panelOffset = panel.offset(),
                panelLeft = panelOffset.left,
                posLeft = false, posTop = false;
            if (panelLeft < 0) {
                posLeft = 'left';
            } else if (panelLeft + panel.width() < page.offset().left) {
                posLeft = Math.floor((page.offset().left - panel.width()) * 0.75);
            } else if (panelLeft + panel.width() > win.scrollLeft() + win.width()) {
                posLeft = 'right';
            }
            if (posLeft || posTop) {
                posLeft = posLeft === false ? panelLeft : posLeft;
                core.dialog('option', 'position', [posLeft, 'center']);
            }
            panel.slideDown(SPEED);
        }
        */
