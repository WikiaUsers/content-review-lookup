...
jQuery(document).ready( function ( $ ) {
	...
	function createUndoLink( diffUndoUrl ) {
		var $ajaxUndoLink = $( '<a />' ).text( 'Szybko cofnij' ).attr( 'href', '#' ).click( function () {
				...
				$.ajax( {
					...
					success: function ( data ) {
						if ( data.edit && data.edit.result === 'Success' ) {
							$ajaxUndoLinkob.text( '(Cofnięte)' );
						} else if ( data.error && data.error.code === 'undofailure' ) {
							$ajaxUndoLinkob.text( '(Błąd)' );
							alert( data.error.info );
						} else {
							$ajaxUndoLinkob.text( '(Błąd)' );
							alert( 'Błąd: Nie znaleziono rezultatu w API.' );
						}
					},
					error: function () {
