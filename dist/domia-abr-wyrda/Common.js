...
importScript( 'User:Cade Calrayn/MiniComplete.js' );
...
function verifySummary(){
	...
	if ( wpDestFile.value.match(/(JPG|PNG|GIF|SVG|jpg\.jpg|png\.png|gif\.gif|svg\.svg)$/)) {
		alert('Please do not use capitalized or duplication file extensions in the filename.');
		return false;
	}
...
function visualEditorWarning() {
	var elems = document.getElementsByTagName( "div" ), i;
	for ( i in elems ) {
		if ( ( ' ' + elems[i].className + ' ' ).indexOf( " oo-ui-processDialog-error " ) > -1) {
			elems[i].innerHTML = "Your edit has triggered one of our site's AbuseFilters. For more information, please try your edit again in another editor."
		}
	}
}
